...
struct ConfigComposerSpec {
    static func main() {
        ...
        run("composeAdditiveBaseConfig merges provider overrides by name", recorder: recorder) {
            let bundledRoot: [String: Any] = [
                "openai-compatibility": [
                    [
                        ...
                        "help-text": String(
                            localized: "verification.config-composer.bundled-help",
                            defaultValue: "Bundled help",
                            comment: "Verification spec label for bundled help source"
                        ),
                        "models": [
            ...
            expectEqual(nvidia?["help-text"] as? String, String(
                localized: "verification.config-composer.bundled-help",
                defaultValue: "Bundled help",
                comment: "Verification spec label for bundled help source"
            ), "bundled help-text should remain", recorder: recorder)
        ...
        run("composeAdditiveBaseConfig preserves bundled provider order and appends new overlays", recorder: recorder) {
            ...
            let userRoot: [String: Any] = [
                "openai-compatibility": [
                    ["name": "beta", "display-name": String(
                        localized: "verification.config-composer.beta-override",
                        defaultValue: "Beta Override",
                        comment: "Verification spec label for beta override source"
                    )],
        ...
        run("parseCustomProviders ignores reserved providers and keeps UI metadata", recorder: recorder) {
            let root: [String: Any] = [
                "openai-compatibility": [
                    ...
                    [
                        ...
                        "display-name": "NVIDIA",
                        "help-text": String(
                            localized: "verification.config-composer.openai-compatible-nvidia-endpoint",
                            defaultValue: "OpenAI-compatible NVIDIA endpoint",
                            comment: "Verification spec label for OpenAI-compatible NVIDIA endpoint source"
                        ),
            ...
            expectEqual(providers.first?.helpText, String(
                localized: "verification.config-composer.openai-compatible-nvidia-endpoint",
                defaultValue: "OpenAI-compatible NVIDIA endpoint",
                comment: "Verification spec label for OpenAI-compatible NVIDIA endpoint source"
            ), "help text should be preserved for UI", recorder: recorder)
