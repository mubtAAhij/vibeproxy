...
final class CustomProviderCredentialStore {
    ...
    private func loadRecord(at filePath: URL) throws -> CustomProviderCredentialRecord {
        ...
        do {
            ...
            throw CustomProviderCredentialStoreError.invalidCredentialJSON(
                String(format: String(localized: "custom-provider-credential-store.error.credential-file.invalid-json", defaultValue: "Credential file at %@ contains invalid JSON: %@", comment: "Error when credential file JSON is invalid"), "\(filePath.path)", "\(error.localizedDescription)")
            )
        }
    ...
    private func record(from json: [String: Any], filePath: URL) throws -> CustomProviderCredentialRecord {
        guard (json["type"] as? String) == Self.authType else {
            throw CustomProviderCredentialStoreError.malformedCredential(
                String(format: String(localized: "custom-provider-credential-store.error.credential-file.unexpected-type", defaultValue: "Credential file at %@ has an unexpected type.", comment: "Error when credential file type field has an unexpected value"), "\(filePath.path)")
            )
        }
        guard let providerID = json["provider"] as? String, !providerID.isEmpty else {
            throw CustomProviderCredentialStoreError.malformedCredential(
                String(format: String(localized: "custom-provider-credential-store.error.credential-file.missing-provider", defaultValue: "Credential file at %@ is missing a provider.", comment: "Error when credential file is missing provider field"), "\(filePath.path)")
            )
        }
        guard let apiKey = json["api_key"] as? String, !apiKey.isEmpty else {
            throw CustomProviderCredentialStoreError.malformedCredential(
                String(format: String(localized: "custom-provider-credential-store.error.credential-file.missing-api-key", defaultValue: "Credential file at %@ is missing an api_key.", comment: "Error when credential file is missing api_key field"), "\(filePath.path)")
            )
        }
    ...
    private func ensureDirectoryExists() throws {
        do {
            ...
            throw CustomProviderCredentialStoreError.failedToCreateDirectory(
                String(format: String(localized: "custom-provider-credential-store.error.create-auth-directory.failed", defaultValue: "Failed to create auth directory at %@: %@", comment: "Error when creating auth directory for credential files fails"), "\(directoryURL.path)", "\(error.localizedDescription)")
            )
        }
    }
        ...
        for file in files
            ...
            do {
                ...
                issues.append(
                    CustomProviderCredentialLoadIssue(
                        filePath: file,
                        message: String(format: String(localized: "custom-provider-credential-store.error.load-file.unexpected", defaultValue: "Unexpected error while loading %@: %@", comment: "Error when loading a credential file fails unexpectedly"), "\(file.path)", "\(error.localizedDescription)")
                    )
                )
            }
        }
    ...
    private func setDisabledUnlocked(
        ...
        for credentialRecord in matchingRecords {
            ...
            do {
                ...
                throw CustomProviderCredentialStoreError.failedToReadCredential(
                    String(format: String(localized: "custom-provider-credential-store.error.read-credential-record-file.failed", defaultValue: "Failed to read credential file at %@: %@", comment: "Error when reading credential file for a provider record fails"), "\(credentialRecord.filePath.path)", "\(error.localizedDescription)")
                )
            }
            ...
            do {
                ...
                throw CustomProviderCredentialStoreError.invalidCredentialJSON(
                    String(format: String(localized: "custom-provider-credential-store.error.credential-record-file.invalid-json", defaultValue: "Credential file at %@ contains invalid JSON: %@", comment: "Error when credential record file JSON is invalid"), "\(credentialRecord.filePath.path)", "\(error.localizedDescription)")
                )
            }

            guard var json = ConfigComposer.stringKeyedDictionary(jsonObject) else {
                throw CustomProviderCredentialStoreError.malformedCredential(
                    String(format: String(localized: "custom-provider-credential-store.error.credential-record-file.must-contain-json-object", defaultValue: "Credential file at %@ must contain a JSON object.", comment: "Error when credential record file root JSON value is not an object"), "\(credentialRecord.filePath.path)")
                )
            }
    ...
    private func writeCredentialJSONUnlocked(_ json: [String: Any], to filePath: URL, providerID: String) throws {
        ...
        do {
            ...
            throw CustomProviderCredentialStoreError.failedToSerializeCredential(
                String(format: String(localized: "custom-provider-credential-store.error.serialize-credential.failed", defaultValue: "Failed to serialize credential for %@: %@", comment: "Error when serializing provider credential data fails"), "\(providerID)", "\(error.localizedDescription)")
            )
        }
