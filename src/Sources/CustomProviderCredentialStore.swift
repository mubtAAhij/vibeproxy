...
final class CustomProviderCredentialStore {
        ...
        try queue.sync {
            ...
            for record in matchingRecords {
                do {
                    ...
                    throw CustomProviderCredentialStoreError.failedToDeleteCredential(
                        String(format: String(localized: "custom-provider-credential-store.error.delete-credential-file-failed", defaultValue: "Failed to delete credential file at %@: %@", comment: "Error when deleting credential file from disk fails"), "\(record.filePath.path)", "\(error.localizedDescription)")
                    )
                }
            }
    ...
    private func loadRecord(at filePath: URL) throws -> CustomProviderCredentialRecord {
        ...
        do {
            ...
            throw CustomProviderCredentialStoreError.failedToReadCredential(
                String(format: String(localized: "custom-provider-credential-store.error.read-credential-file-failed", defaultValue: "Failed to read credential file at %@: %@", comment: "Error when reading credential file from disk fails"), "\(filePath.path)", "\(error.localizedDescription)")
            )
        }
        ...
        do {
            ...
            throw CustomProviderCredentialStoreError.invalidCredentialJSON(
                String(format: String(localized: "custom-provider-credential-store.error.invalid-json-in-credential-file", defaultValue: "Credential file at %@ contains invalid JSON: %@", comment: "Error when credential file contains invalid JSON"), "\(filePath.path)", "\(error.localizedDescription)")
            )
        }
        ...
        guard let json = ConfigComposer.stringKeyedDictionary(jsonObject) else {
            throw CustomProviderCredentialStoreError.malformedCredential(
                String(format: String(localized: "custom-provider-credential-store.error.credential-file-must-contain-json-object", defaultValue: "Credential file at %@ must contain a JSON object.", comment: "Error when credential file root value is not a JSON object"), "\(filePath.path)")
            )
        }
    ...
    private func record(from json: [String: Any], filePath: URL) throws -> CustomProviderCredentialRecord {
        guard (json["type"] as? String) == Self.authType else {
            throw CustomProviderCredentialStoreError.malformedCredential(
                String(format: String(localized: "custom-provider-credential-store.error.unexpected-credential-type", defaultValue: "Credential file at %@ has an unexpected type.", comment: "Error when credential payload type in file is unexpected"), "\(filePath.path)")
            )
        }
        guard let providerID = json["provider"] as? String, !providerID.isEmpty else {
            throw CustomProviderCredentialStoreError.malformedCredential(
                String(format: String(localized: "custom-provider-credential-store.error.missing-provider-in-credential-file", defaultValue: "Credential file at %@ is missing a provider.", comment: "Error when credential file is missing provider field"), "\(filePath.path)")
            )
        }
        guard let apiKey = json["api_key"] as? String, !apiKey.isEmpty else {
            throw CustomProviderCredentialStoreError.malformedCredential(
                String(format: String(localized: "custom-provider-credential-store.error.missing-api-key-in-credential-file", defaultValue: "Credential file at %@ is missing an api_key.", comment: "Error when credential file is missing api_key field"), "\(filePath.path)")
            )
        }
    ...
    private func ensureDirectoryExists() throws {
        do {
            ...
            throw CustomProviderCredentialStoreError.failedToCreateDirectory(
                String(format: String(localized: "custom-provider-credential-store.error.create-auth-directory-failed", defaultValue: "Failed to create auth directory at %@: %@", comment: "Error when creating auth directory for credential storage fails"), "\(directoryURL.path)", "\(error.localizedDescription)")
            )
        }
    }
        ...
        for file in files
            ...
            do {
                ...
                issues.append(
                    CustomProviderCredentialLoadIssue(
                        filePath: file,
                        message: String(format: String(localized: "custom-provider-credential-store.error.unexpected-load-error", defaultValue: "Unexpected error while loading %@: %@", comment: "Unexpected error while loading credential file from disk"), "\(file.path)", "\(error.localizedDescription)")
                    )
                )
            }
        }
    ...
    private func setDisabledUnlocked(
        ...
        for credentialRecord in matchingRecords {
            ...
            do {
                ...
                throw CustomProviderCredentialStoreError.failedToReadCredential(
                    String(format: String(localized: "custom-provider-credential-store.error.read-credential-record-file-failed", defaultValue: "Failed to read credential file at %@: %@", comment: "Error when reading a selected credential file record fails"), "\(credentialRecord.filePath.path)", "\(error.localizedDescription)")
                )
            }
            ...
            do {
                ...
                throw CustomProviderCredentialStoreError.invalidCredentialJSON(
                    String(format: String(localized: "custom-provider-credential-store.error.invalid-json-in-selected-credential-file", defaultValue: "Credential file at %@ contains invalid JSON: %@", comment: "Error when selected credential file contains invalid JSON"), "\(credentialRecord.filePath.path)", "\(error.localizedDescription)")
                )
            }

            guard var json = ConfigComposer.stringKeyedDictionary(jsonObject) else {
                throw CustomProviderCredentialStoreError.malformedCredential(
                    String(format: String(localized: "custom-provider-credential-store.error.selected-credential-file-must-contain-json-object", defaultValue: "Credential file at %@ must contain a JSON object.", comment: "Error when selected credential file root value is not a JSON object"), "\(credentialRecord.filePath.path)")
                )
            }
    ...
    private func writeCredentialJSONUnlocked(_ json: [String: Any], to filePath: URL, providerID: String) throws {
        ...
        do {
            ...
            throw CustomProviderCredentialStoreError.failedToSerializeCredential(
                String(format: String(localized: "custom-provider-credential-store.error.serialize-credential-failed", defaultValue: "Failed to serialize credential for %@: %@", comment: "Error when serializing provider credential payload fails"), "\(providerID)", "\(error.localizedDescription)")
            )
        }
        ...
        do {
            ...
            throw CustomProviderCredentialStoreError.failedToWriteCredential(
                String(format: String(localized: "custom-provider-credential-store.error.write-credential-file-failed", defaultValue: "Failed to write credential file at %@: %@", comment: "Error when writing credential file to disk fails"), "\(filePath.path)", "\(error.localizedDescription)")
            )
        }
    }
