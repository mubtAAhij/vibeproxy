...
final class CustomProviderCredentialStore {
        ...
        try queue.sync {
            ...
            guard !matchingRecords.isEmpty else {
                throw CustomProviderCredentialStoreError.failedToDeleteCredential(
                    String(format: String(localized: "custom-provider-credential-store.error.no-stored-credential-for-provider", defaultValue: "No stored credential was found for provider %@.", comment: "Error when no stored credential exists for the requested provider"), "\(providerID)")
                )
            }
            ...
            for record in matchingRecords {
                do {
                    ...
                    throw CustomProviderCredentialStoreError.failedToDeleteCredential(
                        String(format: String(localized: "custom-provider-credential-store.error.delete-credential-file-failed", defaultValue: "Failed to delete credential file at %@: %@", comment: "Error when deleting a credential file fails"), "\(record.filePath.path)", "\(error.localizedDescription)")
                    )
                }
            }
    ...
    private func loadRecord(at filePath: URL) throws -> CustomProviderCredentialRecord {
        ...
        do {
            ...
            throw CustomProviderCredentialStoreError.failedToReadCredential(
                String(format: String(localized: "custom-provider-credential-store.error.read-credential-file-failed", defaultValue: "Failed to read credential file at %@: %@", comment: "Error when reading a credential file fails"), "\(filePath.path)", "\(error.localizedDescription)")
            )
        }
        ...
        do {
            ...
            throw CustomProviderCredentialStoreError.invalidCredentialJSON(
                String(format: String(localized: "custom-provider-credential-store.error.credential-file-invalid-json", defaultValue: "Credential file at %@ contains invalid JSON: %@", comment: "Error when a credential file contains invalid JSON"), "\(filePath.path)", "\(error.localizedDescription)")
            )
        }
        ...
        guard let json = ConfigComposer.stringKeyedDictionary(jsonObject) else {
            throw CustomProviderCredentialStoreError.malformedCredential(
                String(format: String(localized: "custom-provider-credential-store.error.credential-file-must-be-json-object", defaultValue: "Credential file at %@ must contain a JSON object.", comment: "Error when a credential file JSON root is not an object"), "\(filePath.path)")
            )
        }
    ...
    private func record(from json: [String: Any], filePath: URL) throws -> CustomProviderCredentialRecord {
        guard (json["type"] as? String) == Self.authType else {
            throw CustomProviderCredentialStoreError.malformedCredential(
                String(format: String(localized: "custom-provider-credential-store.error.credential-file-unexpected-type", defaultValue: "Credential file at %@ has an unexpected type.", comment: "Error when a credential file value has an unexpected type"), "\(filePath.path)")
            )
        }
        guard let providerID = json["provider"] as? String, !providerID.isEmpty else {
            throw CustomProviderCredentialStoreError.malformedCredential(
                String(format: String(localized: "custom-provider-credential-store.error.credential-file-missing-provider", defaultValue: "Credential file at %@ is missing a provider.", comment: "Error when a credential file is missing provider field"), "\(filePath.path)")
            )
        }
        guard let apiKey = json["api_key"] as? String, !apiKey.isEmpty else {
            throw CustomProviderCredentialStoreError.malformedCredential(
                String(format: String(localized: "custom-provider-credential-store.error.credential-file-missing-api-key", defaultValue: "Credential file at %@ is missing an api_key.", comment: "Error when a credential file is missing api_key field"), "\(filePath.path)")
            )
        }
        ...
        for file in files
            ...
            do {
                ...
                issues.append(
                    CustomProviderCredentialLoadIssue(
                        filePath: file,
                        message: String(format: String(localized: "custom-provider-credential-store.error.unexpected-load-error", defaultValue: "Unexpected error while loading %@: %@", comment: "Error when an unexpected failure occurs while loading credential files"), "\(file.path)", "\(error.localizedDescription)")
                    )
                )
            }
        }
    ...
    private func setDisabledUnlocked(
        ...
        for credentialRecord in matchingRecords {
            ...
            do {
                ...
                throw CustomProviderCredentialStoreError.failedToReadCredential(
                    String(format: String(localized: "custom-provider-credential-store.error.read-credential-record-file-failed", defaultValue: "Failed to read credential file at %@: %@", comment: "Error when reading provider credential record file fails"), "\(credentialRecord.filePath.path)", "\(error.localizedDescription)")
                )
            }
            ...
            do {
                ...
                throw CustomProviderCredentialStoreError.invalidCredentialJSON(
                    String(format: String(localized: "custom-provider-credential-store.error.credential-record-file-invalid-json", defaultValue: "Credential file at %@ contains invalid JSON: %@", comment: "Error when provider credential record file contains invalid JSON"), "\(credentialRecord.filePath.path)", "\(error.localizedDescription)")
                )
            }

            guard var json = ConfigComposer.stringKeyedDictionary(jsonObject) else {
                throw CustomProviderCredentialStoreError.malformedCredential(
                    String(format: String(localized: "custom-provider-credential-store.error.credential-record-file-must-be-json-object", defaultValue: "Credential file at %@ must contain a JSON object.", comment: "Error when provider credential record JSON root is not an object"), "\(credentialRecord.filePath.path)")
                )
            }
    ...
    private func writeCredentialJSONUnlocked(_ json: [String: Any], to filePath: URL, providerID: String) throws {
        ...
        do {
            ...
            throw CustomProviderCredentialStoreError.failedToSerializeCredential(
                String(format: String(localized: "custom-provider-credential-store.error.serialize-credential-failed", defaultValue: "Failed to serialize credential for %@: %@", comment: "Error when serializing a provider credential fails"), "\(providerID)", "\(error.localizedDescription)")
            )
        }
        ...
        do {
            ...
            throw CustomProviderCredentialStoreError.failedToWriteCredential(
                String(format: String(localized: "custom-provider-credential-store.error.write-credential-file-failed", defaultValue: "Failed to write credential file at %@: %@", comment: "Error when writing a credential file fails"), "\(filePath.path)", "\(error.localizedDescription)")
            )
        }
    }
