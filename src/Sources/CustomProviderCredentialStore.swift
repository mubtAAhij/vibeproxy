...
final class CustomProviderCredentialStore {
        ...
        try queue.sync {
            ...
            guard !matchingRecords.isEmpty else {
                throw CustomProviderCredentialStoreError.failedToDeleteCredential(
                    String(format: String(localized: "custom-provider-credential-store.error.no-stored-credential-for-provider", defaultValue: "No stored credential was found for provider %@.", comment: "Error when no stored credential exists for requested provider"), "\(providerID)")
                )
            }
            ...
            for record in matchingRecords {
                do {
                    ...
                    throw CustomProviderCredentialStoreError.failedToDeleteCredential(
                        String(format: String(localized: "custom-provider-credential-store.error.failed-delete-credential-file", defaultValue: "Failed to delete credential file at %@: %@", comment: "Error when deleting credential file from disk fails"), "\(record.filePath.path)", "\(error.localizedDescription)")
                    )
                }
            }
    ...
    private func loadRecord(at filePath: URL) throws -> CustomProviderCredentialRecord {
        ...
        do {
            ...
            throw CustomProviderCredentialStoreError.failedToReadCredential(
                String(format: String(localized: "custom-provider-credential-store.error.failed-read-credential-file", defaultValue: "Failed to read credential file at %@: %@", comment: "Error when reading credential file from disk fails"), "\(filePath.path)", "\(error.localizedDescription)")
            )
        }
        ...
        do {
            ...
            throw CustomProviderCredentialStoreError.invalidCredentialJSON(
                String(format: String(localized: "custom-provider-credential-store.error.credential-file-invalid-json", defaultValue: "Credential file at %@ contains invalid JSON: %@", comment: "Error when credential file JSON parsing fails"), "\(filePath.path)", "\(error.localizedDescription)")
            )
        }
        ...
        guard let json = ConfigComposer.stringKeyedDictionary(jsonObject) else {
            throw CustomProviderCredentialStoreError.malformedCredential(
                String(format: String(localized: "custom-provider-credential-store.error.credential-file-must-contain-json-object", defaultValue: "Credential file at %@ must contain a JSON object.", comment: "Error when credential file root JSON value is not an object"), "\(filePath.path)")
            )
        }
    ...
    private func record(from json: [String: Any], filePath: URL) throws -> CustomProviderCredentialRecord {
        guard (json["type"] as? String) == Self.authType else {
            throw CustomProviderCredentialStoreError.malformedCredential(
                String(format: String(localized: "custom-provider-credential-store.error.credential-file-unexpected-type", defaultValue: "Credential file at %@ has an unexpected type.", comment: "Error when credential file fields have unexpected types"), "\(filePath.path)")
            )
        }
        guard let providerID = json["provider"] as? String, !providerID.isEmpty else {
            throw CustomProviderCredentialStoreError.malformedCredential(
                String(format: String(localized: "custom-provider-credential-store.error.credential-file-missing-provider", defaultValue: "Credential file at %@ is missing a provider.", comment: "Error when credential file does not include provider field"), "\(filePath.path)")
            )
        }
        guard let apiKey = json["api_key"] as? String, !apiKey.isEmpty else {
            throw CustomProviderCredentialStoreError.malformedCredential(
                String(format: String(localized: "custom-provider-credential-store.error.credential-file-missing-api-key", defaultValue: "Credential file at %@ is missing an api_key.", comment: "Error when credential file does not include api_key field"), "\(filePath.path)")
            )
        }
    ...
    private func ensureDirectoryExists() throws {
        do {
            ...
            throw CustomProviderCredentialStoreError.failedToCreateDirectory(
                String(format: String(localized: "custom-provider-credential-store.error.failed-create-auth-directory", defaultValue: "Failed to create auth directory at %@: %@", comment: "Error when auth directory creation fails"), "\(directoryURL.path)", "\(error.localizedDescription)")
            )
        }
    }
        ...
        for file in files
            ...
            do {
                ...
                issues.append(
                    CustomProviderCredentialLoadIssue(
                        filePath: file,
                        message: String(format: String(localized: "custom-provider-credential-store.error.unexpected-error-while-loading-file", defaultValue: "Unexpected error while loading %@: %@", comment: "Unexpected error while loading credential file during discovery"), "\(file.path)", "\(error.localizedDescription)")
                    )
                )
            }
        }
    ...
    private func setDisabledUnlocked(
        ...
        for credentialRecord in matchingRecords {
            ...
            do {
                ...
                throw CustomProviderCredentialStoreError.failedToReadCredential(
                    String(format: String(localized: "custom-provider-credential-store.error.failed-read-credential-record-file", defaultValue: "Failed to read credential file at %@: %@", comment: "Error when reading selected credential record file fails"), "\(credentialRecord.filePath.path)", "\(error.localizedDescription)")
                )
            }
            ...
            do {
                ...
                throw CustomProviderCredentialStoreError.invalidCredentialJSON(
                    String(format: String(localized: "custom-provider-credential-store.error.credential-record-file-invalid-json", defaultValue: "Credential file at %@ contains invalid JSON: %@", comment: "Error when selected credential record file contains invalid JSON"), "\(credentialRecord.filePath.path)", "\(error.localizedDescription)")
                )
            }

            guard var json = ConfigComposer.stringKeyedDictionary(jsonObject) else {
                throw CustomProviderCredentialStoreError.malformedCredential(
                    String(format: String(localized: "custom-provider-credential-store.error.credential-record-file-must-contain-json-object", defaultValue: "Credential file at %@ must contain a JSON object.", comment: "Error when selected credential record file root JSON value is not an object"), "\(credentialRecord.filePath.path)")
                )
            }
    ...
    private func writeCredentialJSONUnlocked(_ json: [String: Any], to filePath: URL, providerID: String) throws {
        ...
        do {
            ...
            throw CustomProviderCredentialStoreError.failedToSerializeCredential(
                String(format: String(localized: "custom-provider-credential-store.error.failed-serialize-credential-for-provider", defaultValue: "Failed to serialize credential for %@: %@", comment: "Error when serializing credential payload for provider fails"), "\(providerID)", "\(error.localizedDescription)")
            )
        }
        ...
        do {
            ...
            throw CustomProviderCredentialStoreError.failedToWriteCredential(
                String(format: String(localized: "custom-provider-credential-store.error.failed-write-credential-file", defaultValue: "Failed to write credential file at %@: %@", comment: "Error when writing credential file to disk fails"), "\(filePath.path)", "\(error.localizedDescription)")
            )
        }
    }
