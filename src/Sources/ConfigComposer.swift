...
enum ConfigComposer {
    ...
    static func validateCustomProviders(
        ...
        guard let entries = rawOpenAICompatibility as? [Any] else {
            return [String(localized: "config-composer.validation.openai-compatibility-array-required", defaultValue: "openai-compatibility must be an array of provider mappings.", comment: "Validation error when openai-compatibility is not an array of provider mappings")]
        }
        ...
        for (index, rawEntry) in entries.enumerated() {
            ...
            guard let entry = stringKeyedDictionary(rawEntry) else {
                errors.append(String(format: String(localized: "config-composer.validation.path-must-be-mapping", defaultValue: "%@ must be a mapping.", comment: "Validation error when a config path is not a mapping"), "\(path)"))
                continue
            }

            guard let rawProviderName = entry["name"] as? String else {
                errors.append(String(format: String(localized: "config-composer.validation.path-must-define-string-name", defaultValue: "%@ must define a string name.", comment: "Validation error when a config path does not define a string name"), "\(path)"))
                continue
            }

            guard let providerID = normalizedString(rawProviderName) else {
                errors.append(String(format: String(localized: "config-composer.validation.path-must-define-non-empty-name", defaultValue: "%@ must define a non-empty name.", comment: "Validation error when a config path does not define a non-empty name"), "\(path)"))
                continue
            }

            guard rawProviderName == providerID else {
                errors.append(String(format: String(localized: "config-composer.validation.provider-name-no-edge-whitespace", defaultValue: "Provider name '%@' must not include leading or trailing whitespace.", comment: "Validation error when provider name includes leading or trailing whitespace"), "\(rawProviderName)"))
                continue
            }
            ...
            if reservedProviderIDs.contains(providerID), providerID != ProviderCatalog.managedZAIProviderName {
                errors.append(String(format: String(localized: "config-composer.validation.provider-reserved-under-openai-compatibility", defaultValue: "Provider '%@' is reserved and cannot be declared under openai-compatibility.", comment: "Validation error when reserved provider is declared under openai-compatibility"), "\(providerID)"))
                continue
            }
                ...
                if let apiKeyEntries = apiKeyEntriesValue as? [Any] {
                    for (apiKeyIndex, rawAPIKeyEntry) in apiKeyEntries.enumerated() {
                        ...
                        guard let apiKeyEntry = stringKeyedDictionary(rawAPIKeyEntry) else {
                            errors.append(String(format: String(localized: "config-composer.validation.api-key-path-must-be-mapping", defaultValue: "%@ must be a mapping.", comment: "Validation error when api-key path is not a mapping"), "\(apiKeyPath)"))
                            continue
                        }
                    ...
                    errors.append(String(format: String(localized: "config-composer.validation.path-api-key-entries-array-required", defaultValue: "%@.api-key-entries must be an array of mappings.", comment: "Validation error when api-key-entries is not an array of mappings"), "\(path)"))
                }
            }
            ...
            guard normalizedString(entry["base-url"]) != nil else {
                errors.append(String(format: String(localized: "config-composer.validation.custom-provider-must-define-non-empty-base-url", defaultValue: "Custom provider '%@' must define a non-empty base-url.", comment: "Validation error when custom provider does not define non-empty base-url"), "\(providerID)"))
                continue
            }
        }
    ...
    private static func validateMappingArray(_ value: Any, path: String) -> [String] {
        guard let array = value as? [Any] else {
            return [String(format: String(localized: "config-composer.validation.path-must-be-array-of-mappings", defaultValue: "%@ must be an array of mappings.", comment: "Validation error when path is not an array of mappings"), "\(path)")]
        }
