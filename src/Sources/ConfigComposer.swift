...
enum ConfigComposer {
    ...
    static func validateCustomProviders(
        ...
        guard let entries = rawOpenAICompatibility as? [Any] else {
            return [String(
                localized: "config-composer.validation.openai-compatibility-array-required",
        ...
        for (index, rawEntry) in entries.enumerated() {
            ...
            guard let entry = stringKeyedDictionary(rawEntry) else {
                errors.append(String(format: String(
                    ...
                    comment: "Validation error when a config path is not a mapping"
                ), "\(path)"))
                continue
            }
            ...
            guard let rawProviderName = entry["name"] as? String else {
                errors.append(String(format: String(
                    ...
                    comment: "Validation error when a config path does not define a string name"
                ), "\(path)"))
                continue
            }
            ...
            guard let providerID = normalizedString(rawProviderName) else {
                errors.append(String(format: String(
                    ...
                    comment: "Validation error when a config path does not define a non-empty name"
                ), "\(path)"))
                continue
            }
            ...
            if reservedProviderIDs.contains(providerID), providerID != ProviderCatalog.managedZAIProviderName {
                errors.append(String(format: String(
                    localized: "config-composer.validation.provider-reserved-under-openai-compatibility",
                    defaultValue: "Provider '%@' is reserved and cannot be declared under openai-compatibility.",
                    comment: "Validation error when reserved provider is declared under openai-compatibility"
                ), "\(providerID)"))
                continue
            }
            ...
            if let apiKeyEntriesValue = entry["api-key-entries"] {
                if let apiKeyEntries = apiKeyEntriesValue as? [Any] {
                    for (apiKeyIndex, rawAPIKeyEntry) in apiKeyEntries.enumerated() {
                        ...
                        guard let apiKeyEntry = stringKeyedDictionary(rawAPIKeyEntry) else {
                            errors.append(String(format: String(
                                ...
                                defaultValue: "%@ must be a mapping.",
                                comment: "Validation error when api-key path is not a mapping"
                            ), "\(apiKeyPath)"))
                            continue
                        }
                    ...
                    errors.append(String(format: String(
                        localized: "config-composer.validation.path-api-key-entries-array-required",
            ...
            guard normalizedString(entry["base-url"]) != nil else {
                errors.append(String(format: String(
                    ...
                    comment: "Validation error when custom provider does not define non-empty base-url"
                ), "\(providerID)"))
                continue
            }
        }
    ...
    private static func validateMappingArray(_ value: Any, path: String) -> [String] {
        guard let array = value as? [Any] else {
            return [String(format: String(
                ...
                comment: "Validation error when path is not an array of mappings"
            ), "\(path)")]
        }
