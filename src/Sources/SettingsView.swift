...
struct VercelGatewayControls: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 6) {
            ...
            .help(String(localized: "settings.vercel.help-text", defaultValue: "Route Claude requests through Vercel AI Gateway for safer access to your Claude Max subscription", comment: "Help text explaining Vercel AI Gateway"))
...
struct ServiceRow<ExtraContent: View>: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 4) {
            ...
            if isEnabled {
                if !accounts.isEmpty {
                    // Collapsible summary
                    HStack(spacing: 4) {
                        Text(String(localized: "settings.provider.account-count", defaultValue: "\(accounts.count) connected account\(accounts.count == 1 ? "" : "s")", comment: "Summary showing number of connected accounts"))
                            .font(.caption)
                            .foregroundColor(.green)
        ...
        .alert(String(localized: "settings.account.remove-title", defaultValue: "Remove Account", comment: "Alert title for removing account"), isPresented: $showingRemoveConfirmation) {
            Button(String(localized: "settings.account.cancel", defaultValue: "Cancel", comment: "Button to cancel account removal"), role: .cancel) {
                accountToRemove = nil
            }
            Button(String(localized: "settings.account.remove-confirm", defaultValue: "Remove", comment: "Button to confirm account removal"), role: .destructive) {
...
struct SettingsView: View {
    ...
    var body: some View {
            ...
            Form {
                ...
                Section(String(localized: "settings.services", defaultValue: "Services", comment: "Section header for services")) {
                    ...
                    ServiceRow(
                        ...
                        customTitle: serverManager.vercelGatewayEnabled && !serverManager.vercelApiKey.isEmpty ? String(localized: "settings.provider.claude-via-vercel", defaultValue: "Claude Code (via Vercel)", comment: "Custom title for Claude when using Vercel") : nil,
        ...
        .sheet(isPresented: $showingQwenEmailPrompt) {
            VStack(spacing: 16) {
                ...
                TextField(String(localized: "settings.qwen.email-placeholder", defaultValue: "your.email@example.com", comment: "Placeholder for Qwen email field"), text: $qwenEmail)
                    ...
                    .frame(width: 250)
                HStack(spacing: 12) {
                    Button(String(localized: "settings.qwen.cancel", defaultValue: "Cancel", comment: "Button to cancel Qwen email prompt")) {
        ...
        .sheet(isPresented: $showingZaiApiKeyPrompt) {
            VStack(spacing: 16) {
                Text(String(localized: "settings.zai.api-key-prompt-title", defaultValue: "Z.AI API Key", comment: "Title for Z.AI API key prompt dialog"))
                    .font(.headline)
                Text(String(localized: "settings.zai.api-key-prompt-message", defaultValue: "Enter your Z.AI API key from https://z.ai/manage-apikey/apikey-list", comment: "Message for Z.AI API key prompt"))
                    .font(.caption)
                    .foregroundColor(.secondary)
                TextField("", text: $zaiApiKey)
                    ...
                    .frame(width: 300)
                HStack(spacing: 12) {
                    Button(String(localized: "settings.zai.cancel", defaultValue: "Cancel", comment: "Button to cancel Z.AI API key prompt")) {
        ...
        .alert(String(localized: "settings.auth.result-title", defaultValue: "Authentication Result", comment: "Title for authentication result alert"), isPresented: $showingAuthResult) {
            Button(String(localized: "settings.auth.ok", defaultValue: "OK", comment: "Button to dismiss authentication result"), role: .cancel) { }
        } message: {
        ...
        serverManager.runAuthCommand(command) { success, output in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    self.authResultMessage = String(format: NSLocalizedString("settings.auth.failed-message", comment: "Authentication failure message"), details)
                    self.showingAuthResult = true
                }
            }
        }
        ...
        serverManager.runAuthCommand(.qwenLogin(email: email)) { success, output in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    self.authResultMessage = String(format: NSLocalizedString("settings.auth.qwen-failed-message", comment: "Qwen authentication failure message"), details)
                    self.showingAuthResult = true
                }
            }
        }
        ...
        serverManager.saveZaiApiKey(apiKey) { success, output in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    self.authResultMessage = String(format: NSLocalizedString("settings.auth.zai-failed-message", comment: "Z.AI API key save failure message"), details)
                    self.showingAuthResult = true
                }
            }
        }
