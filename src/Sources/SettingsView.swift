    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 6) {
            ...
            if serverManager.vercelGatewayEnabled {
                HStack(spacing: 8) {
                    Text(String(localized: "settings.vercel.api-key", defaultValue: "Vercel API key", comment: "Label for Vercel API key input"))
                        .font(.caption)
                    ...
                    if showingSaved {
                        Text(String(localized: "settings.vercel.saved", defaultValue: "Saved", comment: "Status shown when Vercel API key is saved"))
                            .font(.caption)
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 4) {
            ...
            HStack {
                ...
                .controlSize(.mini)
                .labelsHidden()
                .help(isEnabled ? String(localized: "settings.provider.disable", defaultValue: "Disable this provider", comment: "Help text for disabling provider") : String(localized: "settings.provider.enable", defaultValue: "Enable this provider", comment: "Help text for enabling provider"))
            ...
            if isEnabled {
                if !accounts.isEmpty {
                    // Collapsible summary
                    HStack(spacing: 4) {
                        Text(String(localized: "settings.provider.account-count", defaultValue: accounts.count == 1 ? "1 connected account" : "\(accounts.count) connected accounts", comment: "Number of connected accounts for provider"))
                            .font(.caption)
                            .foregroundColor(.green)
        ...
        .alert(String(localized: "settings.account.remove-alert-title", defaultValue: "Remove Account", comment: "Alert title for removing account"), isPresented: $showingRemoveConfirmation) {
            Button(String(localized: "settings.account.cancel", defaultValue: "Cancel", comment: "Cancel button in remove account alert"), role: .cancel) {
                accountToRemove = nil
            }
    ...
    var body: some View {
        VStack(spacing: 0) {
            Form {
                ...
                Section {
                    Toggle(String(localized: "settings.launch-at-login", defaultValue: "Launch at login", comment: "Toggle for launch at login preference"), isOn: $launchAtLogin)
                ...
                Section(String(localized: "settings.services", defaultValue: "Services", comment: "Section header for services")) {
                    ...
                    ServiceRow(
                        ...
                        customTitle: serverManager.vercelGatewayEnabled && !serverManager.vercelApiKey.isEmpty ? String(localized: "settings.claude.via-vercel", defaultValue: "Claude Code (via Vercel)", comment: "Custom title when using Vercel gateway") : nil,
                    ...
                    ServiceRow(
                        ...
                        helpText: String(localized: "settings.gemini.help", defaultValue: "⚠️ Note: If you're an existing Gemini user with multiple projects, authentication will use your default project. Set your desired project as default in Google AI Studio before connecting.", comment: "Help text warning about Gemini projects"),
        ...
        .sheet(isPresented: $showingQwenEmailPrompt) {
            VStack(spacing: 16) {
                Text(String(localized: "settings.qwen.prompt-title", defaultValue: "Qwen Account Email", comment: "Title for Qwen email prompt sheet"))
                    .font(.headline)
                Text(String(localized: "settings.qwen.prompt-message", defaultValue: "Enter your Qwen account email address", comment: "Instructions for Qwen email prompt"))
                    .font(.caption)
                    .foregroundColor(.secondary)
                TextField(String(localized: "settings.qwen.email-placeholder", defaultValue: "your.email@example.com", comment: "Placeholder for Qwen email field"), text: $qwenEmail)
                    ...
                    .frame(width: 250)
                HStack(spacing: 12) {
                    Button(String(localized: "settings.account.cancel", defaultValue: "Cancel", comment: "Cancel button in remove account alert")) {
                        showingQwenEmailPrompt = false
                        qwenEmail = ""
                    }
                    Button(String(localized: "settings.qwen.continue", defaultValue: "Continue", comment: "Continue button in Qwen prompt")) {
                        showingQwenEmailPrompt = false
        ...
        .sheet(isPresented: $showingZaiApiKeyPrompt) {
            VStack(spacing: 16) {
                Text(String(localized: "settings.zai.prompt-title", defaultValue: "Z.AI API Key", comment: "Title for Z.AI API key prompt sheet"))
                    .font(.headline)
                Text(String(localized: "settings.zai.prompt-message", defaultValue: "Enter your Z.AI API key from https://z.ai/manage-apikey/apikey-list", comment: "Instructions for Z.AI API key prompt"))
                    .font(.caption)
                    .foregroundColor(.secondary)
        ...
        serverManager.runAuthCommand(command) { success, output in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    let fallback = String(localized: "settings.auth-result.no-output", defaultValue: "No output from authentication process", comment: "Fallback message when auth process provides no output")
                    self.authResultMessage = String(format: NSLocalizedString("settings.auth-result.failed", comment: "Message when authentication fails"), output.isEmpty ? fallback : output)
                    self.showingAuthResult = true
                }
            }
        }
        ...
        serverManager.runAuthCommand(.qwenLogin(email: email)) { success, output in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    self.authResultSuccess = false
                    let fallback = String(localized: "settings.auth-result.qwen-no-output", defaultValue: "No output", comment: "Fallback message when Qwen auth provides no output")
                    self.authResultMessage = String(format: NSLocalizedString("settings.auth-result.qwen-failed", comment: "Message when Qwen authentication fails"), output.isEmpty ? fallback : output)
                    self.showingAuthResult = true
                }
            }
        }
        ...
        serverManager.saveZaiApiKey(apiKey) { success, output in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    self.authResultSuccess = false
                    let fallback = String(localized: "settings.auth-result.zai-unknown-error", defaultValue: "Unknown error", comment: "Fallback message when Z.AI save fails without details")
                    self.authResultMessage = String(format: NSLocalizedString("settings.auth-result.zai-save-failed", comment: "Message when Z.AI API key save fails"), output.isEmpty ? fallback : output)
                    self.showingAuthResult = true
                }
            }
        }
