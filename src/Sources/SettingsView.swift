...
struct VercelGatewayControls: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 6) {
            ...
            .help(String(localized: "settings.vercel.help", defaultValue: "Route Claude requests through Vercel AI Gateway for safer access to your Claude Max subscription", comment: "Help text for Vercel AI Gateway feature"))
            ...
            if serverManager.vercelGatewayEnabled {
                HStack(spacing: 8) {
                    ...
                    if showingSaved {
                        Text(String(localized: "settings.vercel.saved", defaultValue: "Saved", comment: "Confirmation message after saving Vercel API key"))
                            .font(.caption)
...
struct ServiceRow<ExtraContent: View>: View {
    ...
    var body: some View {
            ...
            if isEnabled {
                if !accounts.isEmpty {
                    ...
                    HStack(spacing: 4) {
                        ...
                        if accounts.count > 1 {
                            Text(String(localized: "settings.provider.round-robin", defaultValue: "• Round-robin w/ auto-failover", comment: "Indicator that multiple accounts use round-robin strategy"))
                                .font(.caption)
                                .foregroundColor(.secondary)
                        }
        ...
        .alert(String(localized: "settings.remove-account.title", defaultValue: "Remove Account", comment: "Alert title when removing an account"), isPresented: $showingRemoveConfirmation) {
            Button(String(localized: "settings.remove-account.cancel", defaultValue: "Cancel", comment: "Cancel button in remove account alert"), role: .cancel) {
                accountToRemove = nil
            }
            Button(String(localized: "settings.account.remove", defaultValue: "Remove", comment: "Button to remove an account"), role: .destructive) {
            ...
            if let account = accountToRemove {
                Text(String(format: String(localized: "settings.remove-account.message", defaultValue: "Are you sure you want to remove %@ from %@?", comment: "Confirmation message when removing an account"), account.displayName, serviceType.displayName))
            }
        }
    }
}
    ...
    var body: some View {
        VStack(spacing: 0) {
            Form {
                ...
                Section(String(localized: "settings.services.section", defaultValue: "Services", comment: "Services section header")) {
                    ServiceRow(
                    ...
                    ServiceRow(
                        ...
                        customTitle: serverManager.vercelGatewayEnabled && !serverManager.vercelApiKey.isEmpty ? String(localized: "settings.provider.claude.custom-title", defaultValue: "Claude Code (via Vercel)", comment: "Custom title when Claude is routed via Vercel") : nil,
            ...
            VStack(spacing: 4) {
                HStack(spacing: 4) {
                    Text(String(format: String(localized: "settings.footer.attribution", defaultValue: "VibeProxy %@ was made possible thanks to", comment: "Footer attribution text"), appVersion))
                        .font(.caption)
                        .foregroundColor(.secondary)
                ...
                HStack(spacing: 4) {
                    ...
                    Text(String(localized: "settings.footer.rights-reserved", defaultValue: "All rights reserved.", comment: "Copyright footer text"))
                        .font(.caption)
                        .foregroundColor(.secondary)
                }
        ...
        .sheet(isPresented: $showingQwenEmailPrompt) {
            VStack(spacing: 16) {
                Text(String(localized: "settings.qwen-prompt.title", defaultValue: "Qwen Account Email", comment: "Title for Qwen email prompt dialog"))
                    .font(.headline)
                Text(String(localized: "settings.qwen-prompt.message", defaultValue: "Enter your Qwen account email address", comment: "Message in Qwen email prompt dialog"))
                    .font(.caption)
                    .foregroundColor(.secondary)
                TextField(String(localized: "settings.qwen-prompt.placeholder", defaultValue: "your.email@example.com", comment: "Placeholder for Qwen email input"), text: $qwenEmail)
                    ...
                    .frame(width: 250)
                HStack(spacing: 12) {
                    Button(String(localized: "settings.remove-account.cancel", defaultValue: "Cancel", comment: "Cancel button in remove account alert")) {
                        showingQwenEmailPrompt = false
                        qwenEmail = ""
                    }
                    Button(String(localized: "settings.qwen-prompt.continue", defaultValue: "Continue", comment: "Continue button in Qwen email prompt")) {
                        showingQwenEmailPrompt = false
        ...
        .sheet(isPresented: $showingZaiApiKeyPrompt) {
            VStack(spacing: 16) {
                Text(String(localized: "settings.zai-prompt.title", defaultValue: "Z.AI API Key", comment: "Title for Z.AI API key prompt dialog"))
                    .font(.headline)
                Text(String(localized: "settings.zai-prompt.message", defaultValue: "Enter your Z.AI API key from https://z.ai/manage-apikey/apikey-list", comment: "Message in Z.AI API key prompt dialog"))
                    .font(.caption)
                    .foregroundColor(.secondary)
        ...
        serverManager.runAuthCommand(command) { success, output in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    let details = output.isEmpty ? String(localized: "settings.auth-result.no-output", defaultValue: "No output from authentication process", comment: "Fallback message when auth fails with no output") : output
        ...
        serverManager.runAuthCommand(.qwenLogin(email: email)) { success, output in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    self.authResultSuccess = false
                    let details = output.isEmpty ? String(localized: "settings.auth-result.qwen-no-output", defaultValue: "No output", comment: "Fallback when Qwen auth fails with no output") : output
        ...
        serverManager.saveZaiApiKey(apiKey) { success, output in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    self.authResultSuccess = false
                    let details = output.isEmpty ? String(localized: "settings.auth-result.zai-unknown-error", defaultValue: "Unknown error", comment: "Fallback when Z.AI key save fails with no details") : output
                    self.authResultMessage = String(format: String(localized: "settings.auth-result.zai-failed", defaultValue: "Failed to save API key.\n\nDetails: %@", comment: "Error message when Z.AI key save fails"), details)
                    self.showingAuthResult = true
                }
            }
        }
    ...
    private func disconnectAccount(_ account: AuthAccount) {
        ...
        let cleanup = {
            if self.authManager.deleteAccount(account) {
                self.authResultSuccess = true
                self.authResultMessage = String(format: String(localized: "settings.disconnect.success", defaultValue: "✓ Removed %@ from %@", comment: "Success message when account is removed"), account.displayName, account.type.displayName)
            } else {
                self.authResultSuccess = false
                self.authResultMessage = String(localized: "settings.disconnect.failed", defaultValue: "Failed to remove account", comment: "Error message when account removal fails")
            }
            self.showingAuthResult = true
