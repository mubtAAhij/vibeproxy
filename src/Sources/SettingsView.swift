            ...
            Button(action: onRemove) {
                HStack(spacing: 2) {
                    Image(systemName: "minus.circle.fill")
                        .font(.caption)
                    Text(String(localized: "settings.remove", defaultValue: "Remove", comment: "Button to remove an account"))
                        .font(.caption)
                }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 6) {
            Toggle(isOn: $serverManager.vercelGatewayEnabled) {
                Text(String(localized: "settings.vercel-gateway", defaultValue: "Use Vercel AI Gateway", comment: "Toggle to enable Vercel AI Gateway routing"))
                    .font(.caption)
            }
            ...
            if serverManager.vercelGatewayEnabled {
                HStack(spacing: 8) {
                    ...
                    if showingSaved {
                        Text(String(localized: "settings.saved", defaultValue: "Saved", comment: "Confirmation message after saving Vercel API key"))
                            .font(.caption)
                            .foregroundColor(.green)
                    } else {
                        Button(String(localized: "settings.save", defaultValue: "Save", comment: "Button to save Vercel API key")) {
                            showingSaved = true
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 4) {
            ...
            HStack {
                ...
                .controlSize(.mini)
                .labelsHidden()
                .help(isEnabled ? String(localized: "settings.disable-provider", defaultValue: "Disable this provider", comment: "Help text for disabling a provider") : String(localized: "settings.enable-provider", defaultValue: "Enable this provider", comment: "Help text for enabling a provider"))
                ...
                if isAuthenticating {
                    ...
                    Button(String(localized: "settings.add-account", defaultValue: "Add Account", comment: "Button to add a new account for a service")) {
                        onConnect()
                    }
                    .controlSize(.small)
                }
            ...
            if isEnabled {
                if !accounts.isEmpty {
                    // Collapsible summary
                    HStack(spacing: 4) {
                        Text(String(format: String(localized: "settings.connected-accounts", defaultValue: "%d connected account", comment: "Number of connected accounts for a service"), accounts.count))
                            .font(.caption)
                            .foregroundColor(.green)

                        if accounts.count > 1 {
                            Text(String(localized: "settings.round-robin", defaultValue: "• Round-robin w/ auto-failover", comment: "Load balancing strategy indicator"))
                                .font(.caption)
                                .foregroundColor(.secondary)
                        }
                    ...
                    Text(String(localized: "settings.no-accounts", defaultValue: "No connected accounts", comment: "Message when no accounts are connected for a service"))
                        .font(.caption)
                        .foregroundColor(.secondary)
                        .padding(.leading, 28)
                }
        ...
        .alert(String(localized: "settings.remove-account-title", defaultValue: "Remove Account", comment: "Alert title for account removal confirmation"), isPresented: $showingRemoveConfirmation) {
            ...
            Button(String(localized: "settings.remove", defaultValue: "Remove", comment: "Button to remove an account"), role: .destructive) {
            ...
            if let account = accountToRemove {
                Text(String(format: String(localized: "settings.remove-account-confirmation", defaultValue: "Are you sure you want to remove %@ from %@?", comment: "Confirmation message for removing an account"), account.displayName, serviceType.displayName))
            }
        }
    }
}
...
struct SettingsView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            Form {
                ...
                Section(String(localized: "settings.services", defaultValue: "Services", comment: "Section header for services list")) {
                    ...
                    ServiceRow(
                        ...
                        customTitle: serverManager.vercelGatewayEnabled && !serverManager.vercelApiKey.isEmpty ? String(localized: "settings.claude-via-vercel", defaultValue: "Claude Code (via Vercel)", comment: "Custom title when Claude is routed through Vercel") : nil,
            ...
            VStack(spacing: 4) {
                HStack(spacing: 4) {
                    Text(String(format: String(localized: "settings.credits", defaultValue: "VibeProxy %@ was made possible thanks to", comment: "Credits footer with app version"), appVersion))
                        .font(.caption)
                        .foregroundColor(.secondary)
                ...
                HStack(spacing: 4) {
                    ...
                    Text(String(localized: "settings.all-rights-reserved", defaultValue: "All rights reserved.", comment: "Copyright footer text"))
                        .font(.caption)
                        .foregroundColor(.secondary)
                }

                Link(String(localized: "settings.report-issue", defaultValue: "Report an issue", comment: "Link to report an issue on GitHub"), destination: URL(string: "https://github.com/automazeio/vibeproxy/issues")!)
                    .font(.caption)
                    .padding(.top, 6)
                    .onHover { inside in
                        if inside { NSCursor.pointingHand.push() } else { NSCursor.pop() }
        ...
        .sheet(isPresented: $showingQwenEmailPrompt) {
            VStack(spacing: 16) {
                Text(String(localized: "settings.qwen-email-title", defaultValue: "Qwen Account Email", comment: "Title for Qwen email prompt sheet"))
                    .font(.headline)
                Text(String(localized: "settings.qwen-email-prompt", defaultValue: "Enter your Qwen account email address", comment: "Instruction text for Qwen email prompt"))
                    .font(.caption)
                    .foregroundColor(.secondary)
                TextField(String(localized: "settings.qwen-email-placeholder", defaultValue: "your.email@example.com", comment: "Placeholder for Qwen email input field"), text: $qwenEmail)
                    .textFieldStyle(.roundedBorder)
                    .frame(width: 250)
                HStack(spacing: 12) {
                    ...
                    Button(String(localized: "settings.continue", defaultValue: "Continue", comment: "Continue button")) {
                        showingQwenEmailPrompt = false
        ...
        .sheet(isPresented: $showingZaiApiKeyPrompt) {
            VStack(spacing: 16) {
                Text(String(localized: "settings.zai-api-key-title", defaultValue: "Z.AI API Key", comment: "Title for Z.AI API key prompt sheet"))
                    .font(.headline)
                Text(String(localized: "settings.zai-api-key-prompt", defaultValue: "Enter your Z.AI API key from https://z.ai/manage-apikey/apikey-list", comment: "Instruction text for Z.AI API key prompt"))
                    .font(.caption)
                    .foregroundColor(.secondary)
        ...
        serverManager.runAuthCommand(command) { success, output in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    let details = output.isEmpty ? String(localized: "error.auth-no-output", defaultValue: "No output from authentication process", comment: "Fallback when auth process produces no output") : output
                    self.authResultMessage = String(format: String(localized: "error.auth-failed-details", defaultValue: "Authentication failed. Please check if the browser opened and try again.\n\nDetails: %@", comment: "Error message when authentication fails"), details)
                    self.showingAuthResult = true
                }
            }
        }
        ...
        serverManager.runAuthCommand(.qwenLogin(email: email)) { success, output in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    self.authResultSuccess = false
                    let details = output.isEmpty ? String(localized: "error.no-output", defaultValue: "No output", comment: "Fallback when process produces no output") : output
                    self.authResultMessage = String(format: String(localized: "error.qwen-auth-failed", defaultValue: "Authentication failed.\n\nDetails: %@", comment: "Error message when Qwen authentication fails"), details)
                    self.showingAuthResult = true
                }
            }
        }
        ...
        serverManager.saveZaiApiKey(apiKey) { success, output in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    self.authResultSuccess = false
                    let details = output.isEmpty ? String(localized: "error.unknown", defaultValue: "Unknown error", comment: "Fallback for unknown errors") : output
                    self.authResultMessage = String(format: String(localized: "error.zai-key-save-failed", defaultValue: "Failed to save API key.\n\nDetails: %@", comment: "Error message when Z.AI API key save fails"), details)
                    self.showingAuthResult = true
                }
            }
        }
    ...
    private func disconnectAccount(_ account: AuthAccount) {
        ...
        let cleanup = {
            if self.authManager.deleteAccount(account) {
                ...
                self.authResultSuccess = false
                self.authResultMessage = String(localized: "error.account-remove-failed", defaultValue: "Failed to remove account", comment: "Error message when account removal fails")
            }
            self.showingAuthResult = true
