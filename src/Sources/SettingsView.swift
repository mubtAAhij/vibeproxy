    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            if account.isExpired {
                Text(String(localized: "settings.account.expired", defaultValue: "(expired)", comment: "Label shown when an account has expired"))
                    .font(.caption2)
...
struct VercelGatewayControls: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 6) {
            ...
            .help(String(localized: "settings.vercel.gateway-help", defaultValue: "Route Claude requests through Vercel AI Gateway for safer access to your Claude Max subscription", comment: "Help text for Vercel AI Gateway option"))
...
struct ServiceRow<ExtraContent: View>: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 4) {
            ...
            if isEnabled {
                if !accounts.isEmpty {
                    // Collapsible summary
                    HStack(spacing: 4) {
                        Text(String(format: String(localized: "settings.provider.connected-accounts", defaultValue: "%d connected account", comment: "Count of connected accounts for a provider"), accounts.count))
                            .font(.caption)
                            .foregroundColor(.green)

                        if accounts.count > 1 {
                            Text(String(localized: "settings.provider.round-robin", defaultValue: "• Round-robin w/ auto-failover", comment: "Info text about round-robin behavior when multiple accounts are connected"))
                                .font(.caption)
                                .foregroundColor(.secondary)
                        }
                    ...
                    Text(String(localized: "settings.provider.no-accounts", defaultValue: "No connected accounts", comment: "Message shown when no accounts are connected"))
                        .font(.caption)
                        .foregroundColor(.secondary)
                        .padding(.leading, 28)
                }
        ...
        .alert(String(localized: "settings.remove-account.title", defaultValue: "Remove Account", comment: "Title for remove account confirmation dialog"), isPresented: $showingRemoveConfirmation) {
            ...
            if let account = accountToRemove {
                Text(String(format: String(localized: "settings.remove-account.message", defaultValue: "Are you sure you want to remove %@ from %@?", comment: "Confirmation message for removing an account"), account.displayName, serviceType.displayName))
            }
        }
    }
}
...
struct SettingsView: View {
    ...
    var body: some View {
            ...
            Form {
                ...
                Section(String(localized: "settings.services.title", defaultValue: "Services", comment: "Section title for services")) {
                    ...
                    ServiceRow(
                        ...
                        customTitle: serverManager.vercelGatewayEnabled && !serverManager.vercelApiKey.isEmpty ? String(localized: "settings.provider.claude.via-vercel", defaultValue: "Claude Code (via Vercel)", comment: "Claude provider title when using Vercel gateway") : nil,
                    ...
                    ServiceRow(
                        ...
                        helpText: String(localized: "settings.provider.gemini.help", defaultValue: "⚠️ Note: If you're an existing Gemini user with multiple projects, authentication will use your default project. Set your desired project as default in Google AI Studio before connecting.", comment: "Help text for Gemini provider"),
        ...
        .sheet(isPresented: $showingQwenEmailPrompt) {
            VStack(spacing: 16) {
                Text(String(localized: "settings.qwen.email-prompt.title", defaultValue: "Qwen Account Email", comment: "Title for Qwen email prompt"))
                    .font(.headline)
                Text(String(localized: "settings.qwen.email-prompt.message", defaultValue: "Enter your Qwen account email address", comment: "Message for Qwen email prompt"))
                    .font(.caption)
                    .foregroundColor(.secondary)
                TextField(String(localized: "settings.qwen.email-prompt.placeholder", defaultValue: "your.email@example.com", comment: "Placeholder for Qwen email field"), text: $qwenEmail)
                    .textFieldStyle(.roundedBorder)
                    .frame(width: 250)
                HStack(spacing: 12) {
                    ...
                    Button(String(localized: "settings.qwen.email-prompt.continue", defaultValue: "Continue", comment: "Button to continue with Qwen auth")) {
                        showingQwenEmailPrompt = false
        ...
        .sheet(isPresented: $showingZaiApiKeyPrompt) {
            VStack(spacing: 16) {
                Text(String(localized: "settings.zai.key-prompt.title", defaultValue: "Z.AI API Key", comment: "Title for Z.AI API key prompt"))
                    .font(.headline)
                Text(String(localized: "settings.zai.key-prompt.message", defaultValue: "Enter your Z.AI API key from https://z.ai/manage-apikey/apikey-list", comment: "Message for Z.AI API key prompt"))
                    .font(.caption)
                    .foregroundColor(.secondary)
                ...
                HStack(spacing: 12) {
                    ...
                    Button(String(localized: "settings.zai.key-prompt.add-key", defaultValue: "Add Key", comment: "Button to add Z.AI API key")) {
                        showingZaiApiKeyPrompt = false
        ...
        .alert(String(localized: "settings.auth-result.title", defaultValue: "Authentication Result", comment: "Title for authentication result dialog"), isPresented: $showingAuthResult) {
        ...
        serverManager.runAuthCommand(command) { success, output in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    let fallbackMessage = output.isEmpty ? String(localized: "auth.error.no-output", defaultValue: "No output from authentication process", comment: "Fallback message when auth process has no output") : output
                    self.authResultMessage = String(format: String(localized: "auth.error.failed-check-browser", defaultValue: "Authentication failed. Please check if the browser opened and try again.\n\nDetails: %@", comment: "Error message when authentication fails"), fallbackMessage)
                    self.showingAuthResult = true
                }
            }
        }
        ...
        serverManager.runAuthCommand(.qwenLogin(email: email)) { success, output in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    self.authResultSuccess = false
                    let fallbackOutput = output.isEmpty ? String(localized: "auth.error.no-output-short", defaultValue: "No output", comment: "Short fallback when auth has no output") : output
                    self.authResultMessage = String(format: String(localized: "auth.error.qwen-failed", defaultValue: "Authentication failed.\n\nDetails: %@", comment: "Error message when Qwen authentication fails"), fallbackOutput)
                    self.showingAuthResult = true
                }
            }
        }
        ...
        serverManager.saveZaiApiKey(apiKey) { success, output in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    self.authResultSuccess = false
                    let errorDetails = output.isEmpty ? String(localized: "auth.error.unknown", defaultValue: "Unknown error", comment: "Fallback for unknown errors") : output
                    self.authResultMessage = String(format: String(localized: "auth.error.zai-save-failed", defaultValue: "Failed to save API key.\n\nDetails: %@", comment: "Error message when saving Z.AI API key fails"), errorDetails)
                    self.showingAuthResult = true
                }
            }
        }
    ...
    private func disconnectAccount(_ account: AuthAccount) {
        ...
        let cleanup = {
            if self.authManager.deleteAccount(account) {
                ...
                self.authResultSuccess = false
                self.authResultMessage = String(localized: "settings.account.remove-failed", defaultValue: "Failed to remove account", comment: "Error when removing account fails")
            }
            self.showingAuthResult = true
