    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            if account.isExpired {
                Text(String(localized: "settings.account.expired", defaultValue: "(expired)", comment: "Badge indicating account is expired"))
                    .font(.caption2)
...
struct ServiceRow<ExtraContent: View>: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 4) {
            ...
            HStack {
                ...
                if isAuthenticating {
                    ...
                    Button(String(localized: "settings.provider.add-account", defaultValue: "Add Account", comment: "Button to add a new account to a provider")) {
                        onConnect()
                    }
                    .controlSize(.small)
                }
            ...
            if isEnabled {
                if !accounts.isEmpty {
                    // Collapsible summary
                    HStack(spacing: 4) {
                        let suffix = accounts.count == 1 ? "" : "s"
                        Text(String(format: NSLocalizedString("settings.provider.account-count", comment: "Number of connected accounts"), accounts.count, suffix))
                            .font(.caption)
                            .foregroundColor(.green)
        ...
        .alert(String(localized: "settings.provider.remove-account-title", defaultValue: "Remove Account", comment: "Alert title when removing an account"), isPresented: $showingRemoveConfirmation) {
            Button(String(localized: "settings.provider.remove-account-cancel", defaultValue: "Cancel", comment: "Button to cancel account removal"), role: .cancel) {
                accountToRemove = nil
            }
            Button(String(localized: "settings.provider.remove-account-confirm", defaultValue: "Remove", comment: "Button to confirm account removal"), role: .destructive) {
            ...
            if let account = accountToRemove {
                Text(String(format: NSLocalizedString("settings.provider.remove-account-message", comment: "Confirmation message when removing an account"), account.displayName, serviceType.displayName))
            }
        }
    }
}
    ...
    var body: some View {
        VStack(spacing: 0) {
            Form {
                ...
                Section(String(localized: "settings.services-section", defaultValue: "Services", comment: "Section header for provider services")) {
                    ServiceRow(
                    ...
                    ServiceRow(
                        ...
                        helpText: String(localized: "settings.gemini.help", defaultValue: "⚠️ Note: If you're an existing Gemini user with multiple projects, authentication will use your default project. Set your desired project as default in Google AI Studio before connecting.", comment: "Help text explaining Gemini multi-project setup"),
                    ...
                    ServiceRow(
                        ...
                        helpText: String(localized: "settings.zai.help", defaultValue: "Z.AI GLM provides access to GLM-4.7 and other models via API key. Get your key at https://z.ai/manage-apikey/apikey-list", comment: "Help text explaining Z.AI service"),
        ...
        .sheet(isPresented: $showingQwenEmailPrompt) {
            VStack(spacing: 16) {
                Text(String(localized: "settings.qwen.email-prompt-title", defaultValue: "Qwen Account Email", comment: "Title for Qwen email input dialog"))
                    .font(.headline)
                ...
                TextField(String(localized: "settings.qwen.email-placeholder", defaultValue: "your.email@example.com", comment: "Placeholder for Qwen email field"), text: $qwenEmail)
                    ...
                    .frame(width: 250)
                HStack(spacing: 12) {
                    Button(String(localized: "settings.qwen.email-cancel", defaultValue: "Cancel", comment: "Button to cancel Qwen email input")) {
                        showingQwenEmailPrompt = false
                        qwenEmail = ""
                    }
                    Button(String(localized: "settings.qwen.email-continue", defaultValue: "Continue", comment: "Button to proceed with Qwen authentication")) {
                        showingQwenEmailPrompt = false
        ...
        .sheet(isPresented: $showingZaiApiKeyPrompt) {
            VStack(spacing: 16) {
                Text(String(localized: "settings.zai.api-key-prompt-title", defaultValue: "Z.AI API Key", comment: "Title for Z.AI API key input dialog"))
                    .font(.headline)
                Text(String(localized: "settings.zai.api-key-prompt-message", defaultValue: "Enter your Z.AI API key from https://z.ai/manage-apikey/apikey-list", comment: "Instructions for Z.AI API key input"))
                    .font(.caption)
                    .foregroundColor(.secondary)
                TextField("", text: $zaiApiKey)
                    ...
                    .frame(width: 300)
                HStack(spacing: 12) {
                    Button(String(localized: "settings.zai.api-key-cancel", defaultValue: "Cancel", comment: "Button to cancel Z.AI API key input")) {
                        showingZaiApiKeyPrompt = false
                        zaiApiKey = ""
                    }
                    Button(String(localized: "settings.zai.api-key-add", defaultValue: "Add Key", comment: "Button to add Z.AI API key")) {
                        showingZaiApiKeyPrompt = false
        ...
        serverManager.runAuthCommand(command) { success, output in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    let details = output.isEmpty ? String(localized: "settings.auth-result.no-output", defaultValue: "No output from authentication process", comment: "Message when auth process returns no output") : output
                    self.authResultMessage = String(format: NSLocalizedString("settings.auth-result.failed", comment: "Authentication failure message with details"), details)
                    self.showingAuthResult = true
                }
            }
        }
        ...
        serverManager.runAuthCommand(.qwenLogin(email: email)) { success, output in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    self.authResultSuccess = false
                    let details = output.isEmpty ? String(localized: "settings.auth-result.qwen-no-output", defaultValue: "No output", comment: "Message when Qwen auth returns no output") : output
                    self.authResultMessage = String(format: NSLocalizedString("settings.auth-result.qwen-failed", comment: "Qwen authentication failure message"), details)
                    self.showingAuthResult = true
                }
            }
        }
        ...
        serverManager.saveZaiApiKey(apiKey) { success, output in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    self.authResultSuccess = false
                    let details = output.isEmpty ? String(localized: "settings.auth-result.zai-unknown-error", defaultValue: "Unknown error", comment: "Message when Z.AI API key save fails with no error details") : output
                    self.authResultMessage = String(format: NSLocalizedString("settings.auth-result.zai-failed", comment: "Z.AI API key save failure message"), details)
                    self.showingAuthResult = true
                }
            }
        }
    ...
    private func disconnectAccount(_ account: AuthAccount) {
        ...
        let cleanup = {
            if self.authManager.deleteAccount(account) {
                self.authResultSuccess = true
                self.authResultMessage = String(format: NSLocalizedString("settings.account-removal.success", comment: "Success message when account is removed"), account.displayName, account.type.displayName)
            } else {
                self.authResultSuccess = false
