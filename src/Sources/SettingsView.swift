    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 6) {
            Toggle(isOn: $serverManager.vercelGatewayEnabled) {
                Text(String(localized: "settings.vercel.enable", defaultValue: "Use Vercel AI Gateway", comment: "Toggle to enable Vercel AI Gateway"))
                    .font(.caption)
            }
...
struct ServiceRow<ExtraContent: View>: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 4) {
            ...
            if isEnabled {
                if !accounts.isEmpty {
                    // Collapsible summary
                    HStack(spacing: 4) {
                        Text(String(localized: "settings.provider.accounts-count", defaultValue: "%d connected account%@", comment: "Count of connected accounts"))
                            .font(.caption)
                            .foregroundColor(.green)

                        if accounts.count > 1 {
                            Text(String(localized: "settings.provider.round-robin", defaultValue: "• Round-robin w/ auto-failover", comment: "Indicator that multiple accounts use round-robin"))
                                .font(.caption)
                                .foregroundColor(.secondary)
                        }
        ...
        .alert(String(localized: "settings.remove-account.title", defaultValue: "Remove Account", comment: "Title of alert for removing account"), isPresented: $showingRemoveConfirmation) {
            Button(String(localized: "settings.remove-account.cancel", defaultValue: "Cancel", comment: "Button to cancel removing account"), role: .cancel) {
                accountToRemove = nil
            }
            Button(String(localized: "settings.remove-account.confirm", defaultValue: "Remove", comment: "Button to confirm removing account"), role: .destructive) {
    ...
    var body: some View {
        VStack(spacing: 0) {
            Form {
                ...
                Section(String(localized: "settings.services.title", defaultValue: "Services", comment: "Section title for AI services")) {
                    ServiceRow(
                    ...
                    ServiceRow(
                        ...
                        customTitle: serverManager.vercelGatewayEnabled && !serverManager.vercelApiKey.isEmpty ? String(localized: "settings.services.claude.custom-title", defaultValue: "Claude Code (via Vercel)", comment: "Custom title for Claude when using Vercel Gateway") : nil,
                    ...
                    ServiceRow(
                        ...
                        helpText: String(localized: "settings.services.gemini.help", defaultValue: "⚠️ Note: If you're an existing Gemini user with multiple projects, authentication will use your default project. Set your desired project as default in Google AI Studio before connecting.", comment: "Help text warning about Gemini default project"),
            ...
            VStack(spacing: 4) {
                HStack(spacing: 4) {
                    Text(String(format: NSLocalizedString("settings.footer.made-possible", comment: "Footer text about CLIProxyAPIPlus"), appVersion))
                        .font(.caption)
                        .foregroundColor(.secondary)
                ...
                HStack(spacing: 4) {
                    ...
                    Text(String(localized: "settings.footer.rights-reserved", defaultValue: "All rights reserved.", comment: "Copyright notice"))
                        .font(.caption)
                        .foregroundColor(.secondary)
                }
        ...
        serverManager.runAuthCommand(command) { success, output in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    let details = output.isEmpty ? String(localized: "settings.auth-result.no-output", defaultValue: "No output from authentication process", comment: "Default message when auth process has no output") : output
                    self.authResultMessage = String(format: NSLocalizedString("settings.auth-result.auth-failed", comment: "Message when authentication fails"), details)
                    self.showingAuthResult = true
                }
            }
        }
        ...
        serverManager.runAuthCommand(.qwenLogin(email: email)) { success, output in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    self.authResultSuccess = false
                    let details = output.isEmpty ? String(localized: "settings.auth-result.no-output-short", defaultValue: "No output", comment: "Short message when process has no output") : output
                    self.authResultMessage = String(format: NSLocalizedString("settings.auth-result.qwen-failed", comment: "Message when Qwen authentication fails"), details)
                    self.showingAuthResult = true
                }
            }
        }
        ...
        serverManager.saveZaiApiKey(apiKey) { success, output in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    self.authResultSuccess = false
                    let details = output.isEmpty ? String(localized: "settings.auth-result.unknown-error", defaultValue: "Unknown error", comment: "Default error when no details available") : output
                    self.authResultMessage = String(format: NSLocalizedString("settings.auth-result.zai-save-failed", comment: "Message when Z.AI API key save fails"), details)
                    self.showingAuthResult = true
                }
            }
        }
