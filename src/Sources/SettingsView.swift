        ...
        HStack(spacing: 8) {
            ...
            if account.isExpired && !account.isDisabled {
                Text(String(
                    ...
                    comment: "Status suffix for an expired account"
                ))
                    .font(.caption2)
            ...
            if account.isDisabled {
                Text(String(
                    ...
                    comment: "Status suffix for a disabled account"
                ))
                    .font(.caption2)
            ...
            if showDisableToggle {
                ...
                Button(action: onToggleDisabled) {
                    Text(account.isDisabled ? String(
                        localized: "settings.accounts.action.enable",
                        defaultValue: "Enable",
                        comment: "Button title to enable an account"
                    ) : String(
                        localized: "settings.accounts.action.disable",
                        defaultValue: "Disable",
                        comment: "Button title to disable an account"
                    ))
                        .font(.caption)
                ...
                .help(!canDisable ? String(
                    localized: "settings.accounts.error.at-least-one-enabled-required",
                    defaultValue: "At least one account must remain enabled",
                    comment: "Alert message shown when disabling the last enabled account is blocked"
                ) : "")
                .onHover { inside in
                    if canDisable {
            ...
            Button(action: onRemove) {
                HStack(spacing: 2) {
                    Image(systemName: "minus.circle.fill")
                        .font(.caption)
                    Text(String(
                        localized: "settings.accounts.action.remove",
                        defaultValue: "Remove",
                        comment: "Button title to remove an account"
                    ))
                        .font(.caption)
                }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 6) {
            Toggle(isOn: $serverManager.vercelGatewayEnabled) {
                Text(String(
                    localized: "settings.vercel-gateway.toggle.use-vercel-ai-gateway",
                    defaultValue: "Use Vercel AI Gateway",
                    comment: "Toggle label to enable routing through Vercel AI Gateway"
                ))
                    .font(.caption)
            }
            .toggleStyle(.checkbox)
            .help(String(
                localized: "settings.vercel-gateway.description.route-claude-through-gateway",
            ...
            if serverManager.vercelGatewayEnabled {
                HStack(spacing: 8) {
                    Text(String(
                        localized: "settings.vercel-gateway.field.vercel-api-key",
                        defaultValue: "Vercel API key",
                        comment: "Field label for entering Vercel API key"
                    ))
                        .font(.caption)
                        .foregroundColor(.secondary)
                    ...
                    if showingSaved {
                        Text(String(
                            localized: "settings.vercel-gateway.action.saved",
                            defaultValue: "Saved",
                            comment: "Button label indicating Vercel API key has been saved"
                        ))
                            .font(.caption)
                            .foregroundColor(.green)
                    } else {
                        Button(String(
                            localized: "settings.vercel-gateway.action.save",
                            defaultValue: "Save",
                            comment: "Button label to save Vercel API key"
                        )) {
                            showingSaved = true
...
struct ServiceRow<ExtraContent: View>: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 4) {
            ...
            HStack {
                ...
                .help(toggleHelpText ?? (isEnabled ? String(
                    localized: "settings.provider.action.disable-this-provider",
                    defaultValue: "Disable this provider",
                    comment: "Context menu action to disable a provider"
                ) : String(
                    localized: "settings.provider.action.enable-this-provider",
                    defaultValue: "Enable this provider",
                    comment: "Context menu action to enable a provider"
                )))
                ...
                if isAuthenticating {
                    ...
                    Button(String(
                        localized: "settings.accounts.action.add-account",
                        defaultValue: "Add Account",
                        comment: "Button title to add an account"
                    )) {
                        onConnect()
                    }
                    .controlSize(.small)
            ...
            if isEnabled {
                ...
                if !accounts.isEmpty {
                    ...
                    HStack(spacing: 4) {
                        ...
                        if enabledCount > 1 {
                            Text(String(
                                localized: "settings.accounts.feature.round-robin-auto-failover",
                                defaultValue: "• Round-robin w/ auto-failover",
                                comment: "Feature bullet describing round-robin with automatic failover"
                            ))
                                .font(.caption)
                                .foregroundColor(.secondary)
                        }
                    ...
                    Text(String(
                        localized: "settings.accounts.empty.no-connected-accounts",
                        defaultValue: "No connected accounts",
                        comment: "Empty state label when no accounts are connected"
                    ))
                        .font(.caption)
                        .foregroundColor(.secondary)
                        .padding(.leading, 28)
        ...
        .alert(String(
            localized: "settings.accounts.confirm-remove.title",
            defaultValue: "Remove Account",
            comment: "Confirmation dialog title for removing an account"
        ), isPresented: $showingRemoveConfirmation) {
            Button(String(
                localized: "settings.accounts.confirm-remove.action.cancel",
                defaultValue: "Cancel",
                comment: "Cancel button title in remove account confirmation dialog"
            ), role: .cancel) {
                accountToRemove = nil
            }
            Button(String(
                localized: "settings.accounts.confirm-remove.action.remove",
                defaultValue: "Remove",
                comment: "Destructive button title in remove account confirmation dialog"
            ), role: .destructive) {
            ...
            if let account = accountToRemove {
                Text(String(format: String(
                    localized: "settings.accounts.confirm-remove.message",
        ...
        HStack(spacing: 8) {
            ...
            if credential.isDisabled {
                Text(String(
                    ...
                    defaultValue: "(disabled)",
                    comment: "Status suffix for a disabled API key"
                ))
                    .font(.caption2)
                    .foregroundColor(.secondary)
            }
            if showDisableToggle {
                ...
                Button(action: onToggleDisabled) {
                    Text(credential.isDisabled ? String(
                        localized: "settings.api-keys.action.enable",
                        defaultValue: "Enable",
                        comment: "Button title to enable an API key"
                    ) : String(
                        localized: "settings.api-keys.action.disable",
                        defaultValue: "Disable",
                        comment: "Button title to disable an API key"
                    ))
                        .font(.caption)
                ...
                .disabled(!canDisable)
                .help(!canDisable ? String(
                    localized: "settings.api-keys.error.at-least-one-enabled-required",
                    defaultValue: "At least one API key must remain enabled",
                    comment: "Alert message shown when disabling the last enabled API key is blocked"
                ) : "")
                .onHover { inside in
                    if canDisable {
                        if inside { NSCursor.pointingHand.push() } else { NSCursor.pop() }
            ...
            Button(action: onRemove) {
                HStack(spacing: 2) {
                    Image(systemName: "minus.circle.fill")
                        .font(.caption)
                    Text(String(
                        localized: "settings.api-keys.action.remove",
                        defaultValue: "Remove",
                        comment: "Button title to remove an API key"
                    ))
                        .font(.caption)
                }
                .foregroundColor(removeColor)
...
struct CustomProviderRow: View {
    ...
    private var summaryText: String {
        if totalConfiguredKeyCount == 0 {
            return String(
                localized: "settings.api-keys.empty.no-configured-api-keys",
                defaultValue: "No configured API keys",
                comment: "Empty state label when no API keys are configured"
            )
        }
        if provider.inlineKeyCount > 0 && !credentials.isEmpty {
            return String(format: String(
                localized: "settings.api-keys.summary.total-config-added",
                defaultValue: "%d API keys • %d in config • %d added here",
                comment: "Summary showing total API keys, keys in config, and keys added in settings"
    ...
    private var poolingStatusText: String? {
        ...
        return String(
            localized: "settings.api-keys.feature.pooled-across-available-keys",
            defaultValue: "• Pooled across available keys",
            comment: "Feature bullet describing key pooling behavior"
        )
    }

    private var endpointSummaryText: String {
        String(format: String(
            localized: "settings.api-keys.endpoint",
    ...
    private var modelSummaryText: String? {
        ...
        return String(format: String(
            localized: "settings.api-keys.models",
            defaultValue: "Models: %@",
            comment: "Label showing provider model aliases"
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 4) {
            HStack {
                ...
                .help(isEnabled ? String(
                    localized: "settings.api-keys.provider.action.disable-this-provider",
                    defaultValue: "Disable this provider",
                    comment: "Context menu action to disable a custom provider"
                ) : String(
                    localized: "settings.api-keys.provider.action.enable-this-provider",
                    defaultValue: "Enable this provider",
                    comment: "Context menu action to enable a custom provider"
                ))
                ...
                if isAuthenticating {
                    ...
                    Button(String(
                        localized: "settings.api-keys.action.add-api-key",
        ...
        .alert(String(
            localized: "settings.api-keys.confirm-remove.title",
            defaultValue: "Remove API Key",
            comment: "Confirmation dialog title for removing an API key"
        ), isPresented: $showingRemoveConfirmation) {
            Button(String(
                localized: "settings.api-keys.confirm-remove.action.cancel",
                defaultValue: "Cancel",
                comment: "Cancel button title in remove API key confirmation dialog"
            ), role: .cancel) {
                credentialToRemove = nil
            }
            Button(String(
                localized: "settings.api-keys.confirm-remove.action.remove",
                defaultValue: "Remove",
                comment: "Destructive button title in remove API key confirmation dialog"
            ), role: .destructive) {
            ...
            if let credential = credentialToRemove {
                Text(String(format: String(
                    localized: "settings.api-keys.confirm-remove.message",
                    defaultValue: "Are you sure you want to remove %@ from %@?",
                    comment: "Confirmation message asking whether to remove an API key from a provider"
...
struct SettingsView: View {
    ...
    private var appVersion: String {
        if let version = Bundle.main.infoDictionary?["CFBundleShortVersionString"] as? String {
            return String(format: String(
                localized: "settings.app.version.prefix-v",
                defaultValue: "v%@",
                comment: "App version label prefixed with v"
            ), "\(version)")
        }
        return ""
    }
    ...
    var body: some View {
        VStack(spacing: 0) {
            Form {
                Section {
                    HStack {
                        Text(String(
                            ...
                            comment: "Section title for server status"
                        ))
                        Spacer()
                        Button(action: {
                            ...
                            HStack(spacing: 6) {
                                ...
                                Text(serverManager.isRunning ? String(
                                    ...
                                    comment: "Server status label when server is running"
                                ) : String(
                                    localized: "settings.server.status.stopped",
                                    defaultValue: "Stopped",
                                    comment: "Server status label when server is stopped"
                                ))
                            }
                        }
                        .buttonStyle(.plain)
                ...
                if let configErrorMessage = serverManager.configErrorMessage {
                    Section(String(
                        ...
                        comment: "Server status label when configuration has an error"
                    )) {
                ...
                Section {
                    Toggle(String(
                        localized: "settings.launch-at-login.toggle",
                    ...
                    HStack {
                        Text(String(
                            ...
                            comment: "Section title for authentication files controls"
                        ))
                        Spacer()
                        Button(String(
                            localized: "settings.auth-files.action.open-folder",
                ...
                Section(String(
                    ...
                    comment: "Section title for connected services"
                )) {
                    ServiceRow(
                        ...
                        helpText: String(
                            ...
                            comment: "Description text for Antigravity service"
                        ),
                    ...
                    ServiceRow(
                        ...
                        customTitle: serverManager.vercelGatewayEnabled && !serverManager.vercelApiKey.isEmpty ? String(
                            localized: "settings.services.claude-via-vercel.title",
                            defaultValue: "Claude Code (via Vercel)",
                            comment: "Service title for Claude Code routed via Vercel"
                        ) : nil,
                    ...
                    ServiceRow(
                        ...
                        helpText: String(
                            ...
                            comment: "Description text for Kimi service"
                        ),
                    ...
                    ServiceRow(
                        ...
                        helpText: String(
                            ...
                            comment: "Description text for GitHub Copilot service"
                        ),
                    ...
                    ServiceRow(
                        ...
                        helpText: String(
                            ...
                            comment: "Description text for Z.AI GLM service including key management URL"
                        ),
                ...
                if !serverManager.customProviders.isEmpty {
                    Section(String(
                        ...
                        comment: "Section title for custom providers"
                    )) {
            ...
            VStack(spacing: 4) {
                HStack(spacing: 4) {
                    Text(String(format: String(
                        localized: "settings.about.credits.vibeproxy-made-possible-thanks-to",
                        defaultValue: "VibeProxy %@ was made possible thanks to",
                        comment: "Credits heading mentioning app version"
                    ), "\(appVersion)"))
                        .font(.caption)
                        .foregroundColor(.secondary)
                ...
                HStack(spacing: 4) {
                    Text(String(
                        localized: "settings.about.copyright.year-2026",
                        defaultValue: "© 2026",
                        comment: "Copyright year label in about section"
                    ))
                        .font(.caption)
                        .foregroundColor(.secondary)
                    ...
                    Text(String(
                        localized: "settings.about.copyright.all-rights-reserved",
                ...
                Link(String(
                    localized: "settings.about.action.report-issue",
        ...
        .sheet(isPresented: $showingQwenEmailPrompt) {
            VStack(spacing: 16) {
                ...
                Text(String(
                    ...
                    comment: "Prompt text for Qwen account email input"
                ))
                    .font(.caption)
                    .foregroundColor(.secondary)
                TextField(String(
                    localized: "settings.qwen-account-email.placeholder.email-address",
                    defaultValue: "your.email@example.com",
                    comment: "Placeholder text for Qwen account email input"
                ), text: $qwenEmail)
                    .textFieldStyle(.roundedBorder)
                    .frame(width: 250)
                HStack(spacing: 12) {
                    Button(String(
                        localized: "settings.qwen-account-email.action.cancel",
                        defaultValue: "Cancel",
                        comment: "Cancel button title in Qwen account email dialog"
                    )) {
                        showingQwenEmailPrompt = false
                        qwenEmail = ""
                    }
                    Button(String(
                        localized: "settings.qwen-account-email.action.continue",
                        defaultValue: "Continue",
                        comment: "Continue button title in Qwen account email dialog"
                    )) {
                        showingQwenEmailPrompt = false
        ...
        .sheet(isPresented: $showingZaiApiKeyPrompt) {
            VStack(spacing: 16) {
                ...
                HStack(spacing: 12) {
                    Button(String(
                        localized: "settings.zai-api-key.action.cancel",
                        defaultValue: "Cancel",
                        comment: "Cancel button title in Z.AI API key dialog"
                    )) {
                        showingZaiApiKeyPrompt = false
                        zaiApiKey = ""
                    }
                    Button(String(
                        localized: "settings.zai-api-key.action.add-key",
                        defaultValue: "Add Key",
                        comment: "Add key button title in Z.AI API key dialog"
                    )) {
                        showingZaiApiKeyPrompt = false
        ...
        .sheet(item: $selectedCustomProvider, onDismiss: {
            ...
            VStack(spacing: 16) {
                Text(String(format: String(
                    ...
                    comment: "Dialog title for entering custom provider API key"
                ), "\(provider.title)"))
                    .font(.headline)
                Text(String(format: String(
                    localized: "settings.custom-provider-api-key.prompt",
                    defaultValue: "Enter an API key for %@",
                    comment: "Prompt text for custom provider API key input"
                ), "\(provider.title)"))
                    .font(.caption)
                    .foregroundColor(.secondary)
                ...
                HStack(spacing: 12) {
                    Button(String(
                        localized: "settings.custom-provider-api-key.action.cancel",
                        defaultValue: "Cancel",
                        comment: "Cancel button title in custom provider API key dialog"
                    )) {
                        selectedCustomProvider = nil
                        customProviderApiKey = ""
                    }
                    Button(String(
                        localized: "settings.custom-provider-api-key.action.add-key",
                        defaultValue: "Add Key",
                        comment: "Add key button title in custom provider API key dialog"
                    )) {
        ...
        .alert(String(
            ...
            comment: "Title for authentication result dialog"
    ...
    private func toggleAccountDisabled(_ account: AuthAccount) {
        if authManager.toggleAccountDisabled(account) {
            ...
            authResultMessage = account.isDisabled
                ? String(format: String(
                    localized: "settings.authentication-result.enabled-account",
                    defaultValue: "✓ Enabled %@",
                    comment: "Message indicating an account was enabled after authentication"
                ), "\(account.displayName)")
                : String(format: String(
                    localized: "settings.authentication-result.disabled-account",
                    defaultValue: "✓ Disabled %@",
                    comment: "Message indicating an account was disabled"
                ), "\(account.displayName)")
            ...
            authResultMessage = String(format: String(
                ...
                comment: "Error message when updating account enabled state fails"
            ), "\(account.displayName)")
            showingAuthResult = true
        }
    }
        ...
        serverManager.runAuthCommand(command) { success, output in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    if serviceType == .copilot && (output.contains(String(
                        localized: "settings.authentication.code-copied",
                        defaultValue: "Code copied",
                        comment: "Toast message confirming authentication code was copied"
                    ...
                    self.authResultMessage = String(format: String(
                        localized: "settings.authentication.error.failed-browser-check-details",
                        defaultValue: "Authentication failed. Please check if the browser opened and try again.\n\nDetails: %@",
                        comment: "Authentication error with detailed process output"
                    ), "\(output.isEmpty ? String(
                        localized: "settings.authentication.error.no-output-from-process",
                        defaultValue: "No output from authentication process",
                        comment: "Fallback detail text when authentication process has no output"
                    ) : output)")
                    self.showingAuthResult = true
                }
            }
    ...
    private func successMessage(for serviceType: ServiceType) -> String {
        ...
        case .claude:
            return String(
                localized: "settings.authentication.status.browser-opened.claude",
                defaultValue: "🌐 Browser opened for Claude Code authentication.\n\nPlease complete the login in your browser.\n\nThe app will automatically detect your credentials.",
                comment: "Status message shown when Claude authentication browser flow starts"
            )
        case .codex:
            return String(
                localized: "settings.authentication.status.browser-opened.codex",
                defaultValue: "🌐 Browser opened for Codex authentication.\n\nPlease complete the login in your browser.\n\nThe app will automatically detect your credentials.",
                comment: "Status message shown when Codex authentication browser flow starts"
            )
        case .copilot:
            return String(
                localized: "settings.authentication.status.github-copilot-device-flow-started",
        ...
        case .gemini:
            return String(
                localized: "settings.authentication.status.browser-opened.gemini",
                ...
                comment: "Status message shown when Gemini authentication browser flow starts"
            )
        case .kimi:
            return String(
                localized: "settings.authentication.status.browser-opened.kimi",
                defaultValue: "🌐 Browser opened for Kimi authentication.\n\nPlease complete the login in your browser.\n\nThe app will automatically detect your Kimi account.",
                comment: "Status message shown when Kimi authentication browser flow starts"
            )
        case .qwen:
            return String(
                localized: "settings.authentication.status.browser-opened.qwen",
                defaultValue: "🌐 Browser opened for Qwen authentication.\n\nPlease complete the login in your browser.",
                comment: "Status message shown when Qwen authentication browser flow starts"
            )
        case .antigravity:
            return String(
                localized: "settings.authentication.status.browser-opened.antigravity",
                defaultValue: "🌐 Browser opened for Antigravity authentication.\n\nPlease complete the login in your browser.",
                comment: "Status message shown when Antigravity authentication browser flow starts"
            )
        case .zai:
            return String(
                localized: "settings.authentication.status.zai-api-key-added-successfully",
                defaultValue: "✓ Z.AI API key added successfully.\n\nYou can now use GLM models through the proxy.",
                comment: "Success message shown when Z.AI API key is added"
            )
        }
    }
        ...
        serverManager.runAuthCommand(.qwenLogin(email: email)) { success, output in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    self.authResultSuccess = false
                    self.authResultMessage = String(format: String(
                        localized: "settings.authentication.error.failed-details",
                        defaultValue: "Authentication failed.\n\nDetails: %@",
                        comment: "Authentication failure message with details"
                    ), "\(output.isEmpty ? String(
                        localized: "settings.authentication.error.no-output",
        ...
        serverManager.saveZaiApiKey(apiKey) { success, output in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    self.authResultSuccess = false
                    self.authResultMessage = String(format: String(
                        localized: "settings.api-key.error.failed-to-save-details",
                        defaultValue: "Failed to save API key.\n\nDetails: %@",
                        comment: "API key save error message with details"
                    ), "\(output.isEmpty ? String(
                        localized: "settings.api-key.error.unknown-error",
                        defaultValue: "Unknown error",
                        comment: "Fallback detail text when API key save error has no output"
                    ) : output)")
                    self.showingAuthResult = true
                }
            }
        ...
        serverManager.saveCustomProviderAPIKey(providerID: provider.id, apiKey: apiKey) { success, output in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    switch output {
                    case String(
                        localized: "settings.api-key.status.saved-successfully",
                        defaultValue: "API key saved successfully",
                        comment: "Status message shown when API key is saved successfully"
                    ):
                        self.authResultMessage = String(format: String(
                            localized: "settings.api-key.status.provider-key-added-successfully",
                            defaultValue: "✓ %@ API key added successfully.\n\nYou can now use this provider through the proxy.",
                            comment: "Success message shown when a provider API key is added"
                        ), "\(provider.title)")
                    case String(
                        localized: "settings.api-key.error.already-exists-in-config",
                        defaultValue: "API key already exists in config",
                        comment: "Error message shown when API key already exists in config"
                    ):
                        self.authResultMessage = String(format: String(
                            localized: "settings.api-key.status.provider-already-has-key-in-config-path",
                            defaultValue: "✓ %@ already has this API key in ~/.cli-proxy-api/config.yaml.",
                            comment: "Status message when provider already has API key in config file"
                        ), "\(provider.title)")
                    case String(
                        localized: "settings.api-key.error.already-exists",
                        defaultValue: "API key already exists",
                        comment: "Error title when API key already exists"
                    ):
                        self.authResultMessage = String(format: String(
                            localized: "settings.api-key.status.provider-already-has-key-stored",
                            defaultValue: "✓ %@ already has this API key stored.",
                            comment: "Status message when provider already has API key stored"
                        ), "\(provider.title)")
                    case String(
                        localized: "settings.api-key.status.already-stored-reenabled",
                        defaultValue: "API key was already stored and has been re-enabled",
                        comment: "Status message when stored API key is re-enabled"
                    ):
                        self.authResultMessage = String(format: String(
                            localized: "settings.api-key.status.provider-already-had-key-reenabled",
                            defaultValue: "✓ %@ already had this API key stored, and it has been re-enabled.",
                            comment: "Status message when provider key was already stored and re-enabled"
                        ), "\(provider.title)")
                    default:
                        self.authResultMessage = output
                    }
                    ...
                    self.authResultSuccess = false
                    self.authResultMessage = String(format: String(
                        localized: "settings.api-key.error.failed-to-save-for-provider-details",
                        defaultValue: "Failed to save API key for %@.\n\nDetails: %@",
                        comment: "Error message when saving API key for provider fails with details"
                    ), "\(provider.title)", "\(output.isEmpty ? String(
                        localized: "settings.api-key.error.unknown-error",
                        defaultValue: "Unknown error",
                        comment: "Fallback detail text when API key save fails without output"
                    ) : output)")
                    self.showingAuthResult = true
                }
            }
    ...
    private func toggleCustomProviderCredential(provider: CustomProviderDefinition, credential: CustomProviderCredential) {
        if serverManager.toggleCustomProviderCredentialDisabled(credential) {
            ...
            authResultMessage = credential.isDisabled
                ? String(format: String(
                    localized: "settings.api-key.status.enabled-credential-for-provider",
                    defaultValue: "✓ Enabled %@ for %@",
                    comment: "Status message when credential is enabled for provider"
                ), "\(credential.label)", "\(provider.title)")
                : String(format: String(
                    localized: "settings.api-key.status.disabled-credential-for-provider",
                    defaultValue: "✓ Disabled %@ for %@",
                    comment: "Status message when credential is disabled for provider"
                ), "\(credential.label)", "\(provider.title)")
        } else {
            authResultSuccess = false
            authResultMessage = String(format: String(
                localized: "settings.api-key.error.failed-to-update-credential-for-provider",
                defaultValue: "Failed to update %@ for %@. Please try again.",
                comment: "Error message when updating credential for provider fails"
            ), "\(credential.label)", "\(provider.title)")
        }
        showingAuthResult = true
    }
    ...
    private func disconnectCustomProviderCredential(provider: CustomProviderDefinition, credential: CustomProviderCredential) {
        if serverManager.deleteCustomProviderCredential(credential) {
            authResultSuccess = true
            authResultMessage = String(format: String(
                localized: "settings.api-key.status.removed-credential-from-provider",
                defaultValue: "✓ Removed %@ from %@",
                comment: "Status message when credential is removed from provider"
            ), "\(credential.label)", "\(provider.title)")
        } else {
            authResultSuccess = false
            authResultMessage = String(format: String(
                localized: "settings.api-key.error.failed-to-remove-credential-from-provider",
                defaultValue: "Failed to remove %@ from %@",
                comment: "Error message when removing credential from provider fails"
            ), "\(credential.label)", "\(provider.title)")
        }
        showingAuthResult = true
    }
    ...
    private func disconnectAccount(_ account: AuthAccount) {
        ...
        let cleanup = {
            if self.authManager.deleteAccount(account) {
                self.authResultSuccess = true
                self.authResultMessage = String(format: String(
                    localized: "settings.accounts.status.removed-account-from-service",
                    defaultValue: "✓ Removed %@ from %@",
                    comment: "Status message when account is removed from service"
                ), "\(account.displayName)", "\(account.type.displayName)")
            } else {
                self.authResultSuccess = false
                self.authResultMessage = String(
                    localized: "settings.accounts.error.failed-to-remove-account",
                    defaultValue: "Failed to remove account",
                    comment: "Error message when removing account fails"
                )
            }
            self.showingAuthResult = true
