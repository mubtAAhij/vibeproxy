    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            if account.isExpired {
                Text(String(localized: "settings.account.expired", defaultValue: "(expired)", comment: "Label indicating an account has expired"))
                    .font(.caption2)
        ...
        VStack(alignment: .leading, spacing: 6) {
            ...
            if serverManager.vercelGatewayEnabled {
                HStack(spacing: 8) {
                    ...
                    if showingSaved {
                        Text(String(localized: "settings.vercel.saved", defaultValue: "Saved", comment: "Confirmation message after saving API key"))
                            .font(.caption)
        ...
        VStack(alignment: .leading, spacing: 4) {
            ...
            if isEnabled {
                if !accounts.isEmpty {
                    // Collapsible summary
                    HStack(spacing: 4) {
                        Text(String(localized: "settings.provider.account-count", defaultValue: "\(accounts.count) connected account\(accounts.count == 1 ? "" : "s")", comment: "Number of connected accounts"))
                            .font(.caption)
                            .foregroundColor(.green)
        ...
        VStack(spacing: 0) {
            Form {
                ...
                Section {
                    ...
                    HStack {
                        Text(String(localized: "settings.auth-files", defaultValue: "Auth files", comment: "Label for authentication files"))
                        Spacer()
                        Button(String(localized: "settings.open-folder", defaultValue: "Open Folder", comment: "Button to open authentication folder")) {
                            openAuthFolder()
                        }
                    }
                }
                ...
                Section(String(localized: "settings.services", defaultValue: "Services", comment: "Section header for services")) {
                    ...
                    ServiceRow(
                        ...
                        customTitle: serverManager.vercelGatewayEnabled && !serverManager.vercelApiKey.isEmpty ? String(localized: "settings.service.claude-via-vercel", defaultValue: "Claude Code (via Vercel)", comment: "Custom title for Claude service when using Vercel") : nil,
        ...
        .sheet(isPresented: $showingQwenEmailPrompt) {
            VStack(spacing: 16) {
                Text(String(localized: "settings.qwen.email-title", defaultValue: "Qwen Account Email", comment: "Title for Qwen email prompt"))
                    .font(.headline)
                Text(String(localized: "settings.qwen.email-prompt", defaultValue: "Enter your Qwen account email address", comment: "Prompt to enter Qwen email"))
                    .font(.caption)
                    .foregroundColor(.secondary)
                TextField(String(localized: "settings.qwen.email-placeholder", defaultValue: "your.email@example.com", comment: "Placeholder for email field"), text: $qwenEmail)
                    .textFieldStyle(.roundedBorder)
                    .frame(width: 250)
                HStack(spacing: 12) {
                    Button(String(localized: "settings.qwen.cancel", defaultValue: "Cancel", comment: "Cancel button for Qwen prompt")) {
                        showingQwenEmailPrompt = false
                        qwenEmail = ""
                    }
                    Button(String(localized: "settings.qwen.continue", defaultValue: "Continue", comment: "Continue button for Qwen prompt")) {
                        showingQwenEmailPrompt = false
        ...
        .sheet(isPresented: $showingZaiApiKeyPrompt) {
            VStack(spacing: 16) {
                Text(String(localized: "settings.zai.api-key-title", defaultValue: "Z.AI API Key", comment: "Title for Z.AI API key prompt"))
                    .font(.headline)
                Text(String(localized: "settings.zai.api-key-prompt", defaultValue: "Enter your Z.AI API key from https://z.ai/manage-apikey/apikey-list", comment: "Prompt to enter Z.AI API key"))
                    .font(.caption)
                    .foregroundColor(.secondary)
                TextField("", text: $zaiApiKey)
                    ...
                    .frame(width: 300)
                HStack(spacing: 12) {
                    Button(String(localized: "settings.zai.cancel", defaultValue: "Cancel", comment: "Cancel button for Z.AI prompt")) {
        ...
        serverManager.runAuthCommand(command) { success, output in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    let details = output.isEmpty ? String(localized: "settings.auth.no-output", defaultValue: "No output from authentication process", comment: "Error when auth process produces no output") : output
                    self.authResultMessage = String(format: NSLocalizedString("settings.auth.failed-details", comment: "Authentication failure message with details"), details)
                    self.showingAuthResult = true
                }
            }
        }
        ...
        serverManager.runAuthCommand(.qwenLogin(email: email)) { success, output in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    self.authResultSuccess = false
                    let details = output.isEmpty ? String(localized: "settings.auth.qwen-no-output", defaultValue: "No output", comment: "Error when Qwen auth produces no output") : output
        ...
        serverManager.saveZaiApiKey(apiKey) { success, output in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    self.authResultSuccess = false
                    let details = output.isEmpty ? String(localized: "settings.auth.zai-unknown-error", defaultValue: "Unknown error", comment: "Error when Z.AI key save fails without output") : output
