        ...
        VStack(alignment: .leading, spacing: 4) {
            ...
            if isEnabled {
                if !accounts.isEmpty {
                    // Collapsible summary
                    HStack(spacing: 4) {
                        Text(String(localized: "settings.provider.connected-accounts", defaultValue: "\(accounts.count) connected account\(accounts.count == 1 ? "" : "s")", comment: "Number of connected accounts for provider"))
                            .font(.caption)
                            .foregroundColor(.green)

                        if accounts.count > 1 {
                            Text(String(localized: "settings.provider.round-robin", defaultValue: "• Round-robin w/ auto-failover", comment: "Indicator that multiple accounts use round-robin with failover"))
                                .font(.caption)
                                .foregroundColor(.secondary)
                        }
        ...
        VStack(spacing: 0) {
            Form {
                ...
                Section(String(localized: "settings.services", defaultValue: "Services", comment: "Section header for AI services")) {
                    ...
                    ServiceRow(
                        ...
                        customTitle: serverManager.vercelGatewayEnabled && !serverManager.vercelApiKey.isEmpty ? String(localized: "settings.claude.via-vercel", defaultValue: "Claude Code (via Vercel)", comment: "Claude provider name when using Vercel gateway") : nil,
            ...
            VStack(spacing: 4) {
                ...
                HStack(spacing: 4) {
                    ...
                    Text(String(localized: "settings.footer.rights", defaultValue: "All rights reserved.", comment: "Copyright rights reserved text"))
                        .font(.caption)
                        .foregroundColor(.secondary)
                }
        ...
        .sheet(isPresented: $showingQwenEmailPrompt) {
            VStack(spacing: 16) {
                Text(String(localized: "settings.qwen.email-prompt.title", defaultValue: "Qwen Account Email", comment: "Title for Qwen email prompt dialog"))
                    .font(.headline)
                Text(String(localized: "settings.qwen.email-prompt.message", defaultValue: "Enter your Qwen account email address", comment: "Instructions for entering Qwen email"))
                    .font(.caption)
                    .foregroundColor(.secondary)
                TextField(String(localized: "settings.qwen.email-prompt.placeholder", defaultValue: "your.email@example.com", comment: "Placeholder for email field"), text: $qwenEmail)
                    ...
                    .frame(width: 250)
                HStack(spacing: 12) {
                    Button(String(localized: "settings.qwen.email-prompt.cancel", defaultValue: "Cancel", comment: "Cancel button in Qwen email prompt")) {
                        showingQwenEmailPrompt = false
                        qwenEmail = ""
                    }
                    Button(String(localized: "settings.qwen.email-prompt.continue", defaultValue: "Continue", comment: "Continue button in Qwen email prompt")) {
                        showingQwenEmailPrompt = false
        ...
        .sheet(isPresented: $showingZaiApiKeyPrompt) {
            VStack(spacing: 16) {
                Text(String(localized: "settings.zai.api-key-prompt.title", defaultValue: "Z.AI API Key", comment: "Title for Z.AI API key prompt dialog"))
                    .font(.headline)
                Text(String(localized: "settings.zai.api-key-prompt.message", defaultValue: "Enter your Z.AI API key from https://z.ai/manage-apikey/apikey-list", comment: "Instructions for entering Z.AI API key"))
                    .font(.caption)
                    .foregroundColor(.secondary)
                TextField("", text: $zaiApiKey)
                    ...
                    .frame(width: 300)
                HStack(spacing: 12) {
                    Button(String(localized: "settings.zai.api-key-prompt.cancel", defaultValue: "Cancel", comment: "Cancel button in Z.AI API key prompt")) {
        ...
        serverManager.runAuthCommand(command) { success, output in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    let details = output.isEmpty ? String(localized: "settings.auth.no-output", defaultValue: "No output from authentication process", comment: "Error detail when auth process has no output") : output
        ...
        serverManager.runAuthCommand(.qwenLogin(email: email)) { success, output in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    self.authResultSuccess = false
                    let details = output.isEmpty ? String(localized: "settings.auth.qwen-no-output", defaultValue: "No output", comment: "Error detail when Qwen auth has no output") : output
        ...
        serverManager.saveZaiApiKey(apiKey) { success, output in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    self.authResultSuccess = false
                    let details = output.isEmpty ? String(localized: "settings.auth.zai-unknown-error", defaultValue: "Unknown error", comment: "Error detail when Z.AI save fails with no output") : output
