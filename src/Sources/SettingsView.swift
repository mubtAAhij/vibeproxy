        ...
        VStack(alignment: .leading, spacing: 6) {
            ...
            if serverManager.vercelGatewayEnabled {
                HStack(spacing: 8) {
                    ...
                    if showingSaved {
                        Text(String(localized: "settings.vercel.saved", defaultValue: "Saved", comment: "Confirmation that settings were saved"))
                            .font(.caption)
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 4) {
            ...
            HStack {
                ...
                .controlSize(.mini)
                .labelsHidden()
                .help(isEnabled ? String(localized: "settings.provider.disable", defaultValue: "Disable this provider", comment: "Tooltip to disable provider") : String(localized: "settings.provider.enable", defaultValue: "Enable this provider", comment: "Tooltip to enable provider"))
                ...
                if isAuthenticating {
                    ...
                    Button(String(localized: "settings.provider.add-account", defaultValue: "Add Account", comment: "Button to add new account")) {
                        onConnect()
                    }
                    .controlSize(.small)
                }
        ...
        .alert(String(localized: "settings.account.remove-title", defaultValue: "Remove Account", comment: "Alert title for account removal"), isPresented: $showingRemoveConfirmation) {
            Button(String(localized: "settings.account.cancel", defaultValue: "Cancel", comment: "Cancel button"), role: .cancel) {
                accountToRemove = nil
            }
            ...
            if let account = accountToRemove {
                Text(String(format: String(localized: "settings.account.remove-confirm", defaultValue: "Are you sure you want to remove %@ from %@?", comment: "Confirmation message for account removal"), account.displayName, serviceType.displayName))
            }
        }
    }
}
    ...
    var body: some View {
        VStack(spacing: 0) {
            Form {
                    ...
                    HStack {
                        ...
                        Button(action: {
                            ...
                            HStack(spacing: 6) {
                                ...
                                Text(serverManager.isRunning ? String(localized: "settings.server.running", defaultValue: "Running", comment: "Server status: running") : String(localized: "settings.server.stopped", defaultValue: "Stopped", comment: "Server status: stopped"))
                            }
                        }
                        .buttonStyle(.plain)
                    }
                ...
                Section(String(localized: "settings.services-section", defaultValue: "Services", comment: "Section title for AI services")) {
                    ServiceRow(
                        ...
                        helpText: String(localized: "settings.antigravity.help", defaultValue: "Antigravity provides OAuth-based access to various AI models including Gemini and Claude. One login gives you access to multiple AI services.", comment: "Help text for Antigravity provider"),
                    ...
                    ServiceRow(
                        ...
                        customTitle: serverManager.vercelGatewayEnabled && !serverManager.vercelApiKey.isEmpty ? String(localized: "settings.claude.title-vercel", defaultValue: "Claude Code (via Vercel)", comment: "Claude provider title when using Vercel gateway") : nil,
                    ...
                    ServiceRow(
                        ...
                        helpText: String(localized: "settings.copilot.help", defaultValue: "GitHub Copilot provides access to Claude, GPT, Gemini and other models via your Copilot subscription.", comment: "Help text for GitHub Copilot provider"),
                    ...
                    ServiceRow(
                        ...
                        helpText: String(localized: "settings.zai.help", defaultValue: "Z.AI GLM provides access to GLM-4.7 and other models via API key. Get your key at https://z.ai/manage-apikey/apikey-list", comment: "Help text for Z.AI provider"),
            ...
            VStack(spacing: 4) {
                HStack(spacing: 4) {
                    Text(String(format: String(localized: "settings.footer.thanks", defaultValue: "VibeProxy %@ was made possible thanks to", comment: "Footer text thanking contributors"), appVersion))
                        .font(.caption)
                        .foregroundColor(.secondary)
        ...
        .sheet(isPresented: $showingQwenEmailPrompt) {
            VStack(spacing: 16) {
                Text(String(localized: "settings.qwen.email-title", defaultValue: "Qwen Account Email", comment: "Title for Qwen email prompt"))
                    .font(.headline)
                Text(String(localized: "settings.qwen.email-prompt", defaultValue: "Enter your Qwen account email address", comment: "Prompt for Qwen email input"))
                    .font(.caption)
                    .foregroundColor(.secondary)
                TextField(String(localized: "settings.qwen.email-placeholder", defaultValue: "your.email@example.com", comment: "Placeholder for Qwen email field"), text: $qwenEmail)
                    .textFieldStyle(.roundedBorder)
                    .frame(width: 250)
                HStack(spacing: 12) {
                    Button(String(localized: "settings.account.cancel", defaultValue: "Cancel", comment: "Cancel button")) {
                        showingQwenEmailPrompt = false
                        qwenEmail = ""
                    }
                    Button(String(localized: "settings.qwen.continue", defaultValue: "Continue", comment: "Button to continue with Qwen auth")) {
                        showingQwenEmailPrompt = false
        ...
        .sheet(isPresented: $showingZaiApiKeyPrompt) {
            VStack(spacing: 16) {
                Text(String(localized: "settings.zai.api-key-title", defaultValue: "Z.AI API Key", comment: "Title for Z.AI API key prompt"))
                    .font(.headline)
                Text(String(localized: "settings.zai.api-key-prompt", defaultValue: "Enter your Z.AI API key from https://z.ai/manage-apikey/apikey-list", comment: "Prompt for Z.AI API key input"))
                    .font(.caption)
                    .foregroundColor(.secondary)
        ...
        .alert(String(localized: "settings.auth.result-title", defaultValue: "Authentication Result", comment: "Alert title for authentication result"), isPresented: $showingAuthResult) {
            Button(String(localized: "settings.auth.ok", defaultValue: "OK", comment: "OK button"), role: .cancel) { }
        } message: {
        ...
        serverManager.saveZaiApiKey(apiKey) { success, output in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    self.authResultSuccess = false
                    let details = output.isEmpty ? String(localized: "settings.auth.zai-unknown-error", defaultValue: "Unknown error", comment: "Message for unknown Z.AI error") : output
                    self.authResultMessage = String(format: String(localized: "settings.auth.zai-save-failed", defaultValue: "Failed to save API key.\n\nDetails: %@", comment: "Z.AI save failure message"), details)
                    self.showingAuthResult = true
                }
            }
        }
    ...
    private func disconnectAccount(_ account: AuthAccount) {
        ...
        let cleanup = {
            if self.authManager.deleteAccount(account) {
                self.authResultSuccess = true
                self.authResultMessage = String(format: String(localized: "settings.account.remove-success", defaultValue: "✓ Removed %@ from %@", comment: "Success message for account removal"), account.displayName, account.type.displayName)
            } else {
                self.authResultSuccess = false
