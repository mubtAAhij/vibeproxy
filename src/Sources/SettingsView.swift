    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            if account.isExpired {
                Text(String(localized: "settings.account.expired", defaultValue: "(expired)", comment: "Label shown for expired account credentials"))
                    .font(.caption2)
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 6) {
            Toggle(isOn: $serverManager.vercelGatewayEnabled) {
                Text(String(localized: "settings.vercel.use-gateway", defaultValue: "Use Vercel AI Gateway", comment: "Toggle to enable Vercel AI Gateway for Claude"))
                    .font(.caption)
            }
            .toggleStyle(.checkbox)
            .help(String(localized: "settings.vercel.help", defaultValue: "Route Claude requests through Vercel AI Gateway for safer access to your Claude Max subscription", comment: "Help tooltip for Vercel AI Gateway toggle"))
            ...
            if isEnabled {
                if !accounts.isEmpty {
                    ...
                    HStack(spacing: 4) {
                        ...
                        Text(count == 1
                            ? String(localized: "settings.accounts.count-singular", defaultValue: "1 connected account", comment: "Label for one connected account")
                            : String(format: NSLocalizedString("settings.accounts.count-plural", comment: "Label for multiple connected accounts"), count))
                            .font(.caption)
                            .foregroundColor(.green)

                        if accounts.count > 1 {
                            Text(String(localized: "settings.accounts.round-robin", defaultValue: "• Round-robin w/ auto-failover", comment: "Description of multi-account load balancing"))
                                .font(.caption)
                                .foregroundColor(.secondary)
                        }
                    ...
                    Text(String(localized: "settings.accounts.none", defaultValue: "No connected accounts", comment: "Label when no accounts are connected"))
                        .font(.caption)
                        .foregroundColor(.secondary)
                        .padding(.leading, 28)
                }
        ...
        VStack(spacing: 0) {
            Form {
                ...
                Section(String(localized: "settings.services", defaultValue: "Services", comment: "Section header for service providers")) {
                    ...
                    ServiceRow(
                        ...
                        customTitle: serverManager.vercelGatewayEnabled && !serverManager.vercelApiKey.isEmpty ? String(localized: "settings.service.claude.via-vercel", defaultValue: "Claude Code (via Vercel)", comment: "Custom title when using Vercel gateway for Claude") : nil,
            ...
            VStack(spacing: 4) {
                ...
                HStack(spacing: 4) {
                    ...
                    Text(String(localized: "settings.footer.rights", defaultValue: "All rights reserved.", comment: "Footer copyright text"))
                        .font(.caption)
                        .foregroundColor(.secondary)
                }
        ...
        .sheet(isPresented: $showingQwenEmailPrompt) {
            VStack(spacing: 16) {
                ...
                TextField(String(localized: "settings.qwen.email-prompt.placeholder", defaultValue: "your.email@example.com", comment: "Placeholder for Qwen email input field"), text: $qwenEmail)
                    ...
                    .frame(width: 250)
                HStack(spacing: 12) {
                    Button(String(localized: "settings.qwen.email-prompt.cancel", defaultValue: "Cancel", comment: "Button to cancel Qwen email prompt")) {
        ...
        .sheet(isPresented: $showingZaiApiKeyPrompt) {
            VStack(spacing: 16) {
                ...
                TextField("", text: $zaiApiKey)
                    ...
                    .frame(width: 300)
                HStack(spacing: 12) {
                    Button(String(localized: "settings.zai.api-key-prompt.cancel", defaultValue: "Cancel", comment: "Button to cancel Z.AI API key prompt")) {
        ...
        serverManager.runAuthCommand(command) { success, output in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    let details = output.isEmpty ? String(localized: "settings.auth-result.no-output", defaultValue: "No output from authentication process", comment: "Fallback when auth process returns no output") : output
                    self.authResultMessage = String(format: NSLocalizedString("settings.auth-result.failed", comment: "Error message when authentication fails"), details)
                    self.showingAuthResult = true
                }
            }
        }
        ...
        serverManager.runAuthCommand(.qwenLogin(email: email)) { success, output in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    self.authResultSuccess = false
                    let details = output.isEmpty ? String(localized: "settings.auth-result.qwen-no-output", defaultValue: "No output", comment: "Fallback when Qwen auth returns no output") : output
                    self.authResultMessage = String(format: NSLocalizedString("settings.auth-result.qwen-failed", comment: "Error message when Qwen authentication fails"), details)
                    self.showingAuthResult = true
                }
            }
        }
        ...
        serverManager.saveZaiApiKey(apiKey) { success, output in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    self.authResultSuccess = false
                    let details = output.isEmpty ? String(localized: "settings.auth-result.zai-unknown-error", defaultValue: "Unknown error", comment: "Fallback when Z.AI save fails with no details") : output
                    self.authResultMessage = String(format: NSLocalizedString("settings.auth-result.zai-save-failed", comment: "Error message when Z.AI API key save fails"), details)
                    self.showingAuthResult = true
                }
            }
        }
