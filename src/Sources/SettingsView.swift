...
struct ServiceRow<ExtraContent: View>: View {
    ...
    var body: some View {
            ...
            if isEnabled {
                if !accounts.isEmpty {
                    ...
                    HStack(spacing: 4) {
                        ...
                        if accounts.count > 1 {
                            Text(String(localized: "settings.provider.round-robin", defaultValue: "• Round-robin w/ auto-failover", comment: "Label indicating round-robin load balancing with failover"))
                                .font(.caption)
                                .foregroundColor(.secondary)
                        }
        ...
        .alert(String(localized: "settings.remove-account.title", defaultValue: "Remove Account", comment: "Alert title for removing account"), isPresented: $showingRemoveConfirmation) {
            ...
            if let account = accountToRemove {
                Text(String(format: String(localized: "settings.remove-account.confirmation", defaultValue: "Are you sure you want to remove %@ from %@?", comment: "Confirmation message for removing account with account name and service name"), account.displayName, serviceType.displayName))
            }
        }
    }
}
        ...
        VStack(spacing: 0) {
            Form {
                ...
                Section {
                    ...
                    HStack {
                        Text(String(localized: "settings.auth-files", defaultValue: "Auth files", comment: "Label for auth files row"))
                        Spacer()
                ...
                Section(String(localized: "settings.services", defaultValue: "Services", comment: "Section header for services list")) {
                    ...
                    ServiceRow(
                        ...
                        customTitle: serverManager.vercelGatewayEnabled && !serverManager.vercelApiKey.isEmpty ? String(localized: "settings.claude.title-vercel", defaultValue: "Claude Code (via Vercel)", comment: "Custom title when Claude uses Vercel gateway") : nil,
            ...
            VStack(spacing: 4) {
                HStack(spacing: 4) {
                    Text(String(format: String(localized: "settings.footer.made-possible", defaultValue: "VibeProxy %@ was made possible thanks to", comment: "Footer text with version number"), appVersion))
                        .font(.caption)
                        .foregroundColor(.secondary)
                ...
                HStack(spacing: 4) {
                    ...
                    Text(String(localized: "settings.footer.all-rights-reserved", defaultValue: "All rights reserved.", comment: "Copyright footer text"))
                        .font(.caption)
                        .foregroundColor(.secondary)
                }
        ...
        .sheet(isPresented: $showingQwenEmailPrompt) {
            VStack(spacing: 16) {
                ...
                Text(String(localized: "settings.qwen.email-prompt", defaultValue: "Enter your Qwen account email address", comment: "Prompt for Qwen email input"))
                    .font(.caption)
                    .foregroundColor(.secondary)
                TextField(String(localized: "settings.qwen.email-placeholder", defaultValue: "your.email@example.com", comment: "Placeholder for email input field"), text: $qwenEmail)
                    .textFieldStyle(.roundedBorder)
                    .frame(width: 250)
                HStack(spacing: 12) {
        ...
        serverManager.runAuthCommand(command) { success, output in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    let details = output.isEmpty ? String(localized: "auth.error.no-output", defaultValue: "No output from authentication process", comment: "Fallback when auth process produces no output") : output
        ...
        serverManager.runAuthCommand(.qwenLogin(email: email)) { success, output in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    self.authResultSuccess = false
                    let details = output.isEmpty ? String(localized: "auth.error.qwen-no-output", defaultValue: "No output", comment: "Fallback when Qwen auth produces no output") : output
        ...
        serverManager.saveZaiApiKey(apiKey) { success, output in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    self.authResultSuccess = false
                    let details = output.isEmpty ? String(localized: "auth.error.zai-unknown", defaultValue: "Unknown error", comment: "Fallback when Z.AI key save fails without details") : output
    ...
    private func disconnectAccount(_ account: AuthAccount) {
        ...
        let cleanup = {
            if self.authManager.deleteAccount(account) {
                self.authResultSuccess = true
                self.authResultMessage = String(format: String(localized: "settings.account.removed-success", defaultValue: "✓ Removed %@ from %@", comment: "Success message when account is removed with account name and service name"), account.displayName, account.type.displayName)
            } else {
                self.authResultSuccess = false
