...
struct ServiceRow<ExtraContent: View>: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 4) {
            ...
            if isEnabled {
                if !accounts.isEmpty {
                    // Collapsible summary
                    HStack(spacing: 4) {
                        Text(String(localized: "settings.provider.account-count", defaultValue: "%d connected account", comment: "Number of connected accounts for a provider"))
                            .font(.caption)
                            .foregroundColor(.green)
        ...
        .alert(String(localized: "settings.account.remove-title", defaultValue: "Remove Account", comment: "Title for remove account confirmation dialog"), isPresented: $showingRemoveConfirmation) {
            ...
            if let account = accountToRemove {
                Text(String(format: NSLocalizedString("settings.account.remove-confirmation", comment: "Confirmation message for removing an account"), account.displayName, serviceType.displayName))
            }
        }
    }
}
...
struct SettingsView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            Form {
                    ...
                    HStack {
                        ...
                        Button(action: {
                            ...
                            HStack(spacing: 6) {
                                ...
                                Text(serverManager.isRunning ? String(localized: "settings.server-status.running", defaultValue: "Running", comment: "Server running status") : String(localized: "settings.server-status.stopped", defaultValue: "Stopped", comment: "Server stopped status"))
                            }
                        }
                        .buttonStyle(.plain)
                    }
                ...
                Section(String(localized: "settings.services", defaultValue: "Services", comment: "Section header for services")) {
                    ...
                    ServiceRow(
                        ...
                        customTitle: serverManager.vercelGatewayEnabled && !serverManager.vercelApiKey.isEmpty ? String(localized: "settings.provider.claude.vercel-title", defaultValue: "Claude Code (via Vercel)", comment: "Custom title for Claude when using Vercel gateway") : nil,
            ...
            VStack(spacing: 4) {
                HStack(spacing: 4) {
                    Text(String(format: NSLocalizedString("settings.footer.thanks", comment: "Footer attribution text"), appVersion))
                        .font(.caption)
                        .foregroundColor(.secondary)
                ...
                HStack(spacing: 4) {
                    ...
                    Text(String(localized: "settings.footer.rights", defaultValue: "All rights reserved.", comment: "Copyright all rights reserved text"))
                        .font(.caption)
                        .foregroundColor(.secondary)
                }
        ...
        serverManager.runAuthCommand(command) { success, output in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    let fallback = String(localized: "settings.auth.error.no-output", defaultValue: "No output from authentication process", comment: "Fallback message when auth process produces no output")
                    self.authResultMessage = String(format: NSLocalizedString("settings.auth.error.failed", comment: "Generic auth failure message with details"), output.isEmpty ? fallback : output)
                    self.showingAuthResult = true
                }
            }
        }
        ...
        serverManager.runAuthCommand(.qwenLogin(email: email)) { success, output in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    self.authResultSuccess = false
                    let fallback = String(localized: "settings.auth.error.qwen-no-output", defaultValue: "No output", comment: "Fallback when Qwen auth produces no output")
                    self.authResultMessage = String(format: NSLocalizedString("settings.auth.error.qwen-failed", comment: "Qwen auth failure message with details"), output.isEmpty ? fallback : output)
                    self.showingAuthResult = true
                }
            }
        }
        ...
        serverManager.saveZaiApiKey(apiKey) { success, output in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    self.authResultSuccess = false
                    let fallback = String(localized: "settings.zai.error.unknown", defaultValue: "Unknown error", comment: "Fallback for unknown Z.AI error")
                    self.authResultMessage = String(format: NSLocalizedString("settings.zai.error.save-failed", comment: "Z.AI API key save failure message"), output.isEmpty ? fallback : output)
                    self.showingAuthResult = true
                }
            }
        }
