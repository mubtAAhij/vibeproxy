        ...
        VStack(alignment: .leading, spacing: 4) {
            ...
            if isEnabled {
                if !accounts.isEmpty {
                    // Collapsible summary
                    HStack(spacing: 4) {
                        Text(String(localized: "settings.provider.account-count", defaultValue: "\(accounts.count) connected account\(accounts.count == 1 ? "" : "s")", comment: "Number of connected accounts for a provider"))
                            .font(.caption)
                            .foregroundColor(.green)

                        if accounts.count > 1 {
                            Text(String(localized: "settings.provider.round-robin", defaultValue: "• Round-robin w/ auto-failover", comment: "Description of account rotation strategy"))
                                .font(.caption)
                                .foregroundColor(.secondary)
                        }
        ...
        VStack(spacing: 0) {
            Form {
                ...
                Section(String(localized: "settings.services", defaultValue: "Services", comment: "Section header for services")) {
                    ...
                    ServiceRow(
                        ...
                        customTitle: serverManager.vercelGatewayEnabled && !serverManager.vercelApiKey.isEmpty ? String(localized: "settings.claude.via-vercel", defaultValue: "Claude Code (via Vercel)", comment: "Custom title when Claude is using Vercel gateway") : nil,
                    ...
                    ServiceRow(
                        ...
                        helpText: String(localized: "settings.zai.help", defaultValue: "Z.AI GLM provides access to GLM-4.7 and other models via API key. Get your key at https://z.ai/manage-apikey/apikey-list", comment: "Help text describing Z.AI service and where to get API key"),
            ...
            VStack(spacing: 4) {
                ...
                HStack(spacing: 4) {
                    ...
                    Text(String(localized: "settings.footer.all-rights-reserved", defaultValue: "All rights reserved.", comment: "Copyright all rights reserved text"))
                        .font(.caption)
                        .foregroundColor(.secondary)
                }
        ...
        .sheet(isPresented: $showingQwenEmailPrompt) {
            VStack(spacing: 16) {
                ...
                HStack(spacing: 12) {
                    ...
                    Button(String(localized: "settings.qwen.continue", defaultValue: "Continue", comment: "Continue button for Qwen authentication")) {
                        showingQwenEmailPrompt = false
        ...
        .sheet(isPresented: $showingZaiApiKeyPrompt) {
            VStack(spacing: 16) {
                Text(String(localized: "settings.zai.api-key-title", defaultValue: "Z.AI API Key", comment: "Title for Z.AI API key prompt sheet"))
                    .font(.headline)
                Text(String(localized: "settings.zai.api-key-prompt", defaultValue: "Enter your Z.AI API key from https://z.ai/manage-apikey/apikey-list", comment: "Prompt for Z.AI API key input with URL"))
                    .font(.caption)
                    .foregroundColor(.secondary)
        ...
        serverManager.runAuthCommand(command) { success, output in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    let details = output.isEmpty ? String(localized: "settings.auth-result.no-output", defaultValue: "No output from authentication process", comment: "Default message when auth process returns no output") : output
        ...
        serverManager.runAuthCommand(.qwenLogin(email: email)) { success, output in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    self.authResultSuccess = false
                    let details = output.isEmpty ? String(localized: "settings.auth-result.no-output-short", defaultValue: "No output", comment: "Short message when process returns no output") : output
        ...
        serverManager.saveZaiApiKey(apiKey) { success, output in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    self.authResultSuccess = false
                    let details = output.isEmpty ? String(localized: "settings.auth-result.unknown-error", defaultValue: "Unknown error", comment: "Generic unknown error message") : output
                    self.authResultMessage = String(format: NSLocalizedString("settings.auth-result.zai-failed", comment: "Error message when saving Z.AI API key fails"), details)
                    self.showingAuthResult = true
                }
            }
        }
