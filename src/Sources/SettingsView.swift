...
struct VercelGatewayControls: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 6) {
            ...
            .help(String(localized: "settings.vercel.help", defaultValue: "Route Claude requests through Vercel AI Gateway for safer access to your Claude Max subscription", comment: "Help text for Vercel Gateway option"))
            ...
            if isEnabled {
                if !accounts.isEmpty {
                    ...
                    HStack(spacing: 4) {
                        ...
                        if accounts.count > 1 {
                            Text(String(localized: "settings.provider.round-robin", defaultValue: "• Round-robin w/ auto-failover", comment: "Indicator for round-robin load balancing"))
                                .font(.caption)
                                .foregroundColor(.secondary)
                        }
    ...
    var body: some View {
        VStack(spacing: 0) {
            Form {
                    ...
                    HStack {
                        ...
                        Button(action: {
                            ...
                            HStack(spacing: 6) {
                                ...
                                Text(serverManager.isRunning ? String(localized: "settings.server-status.running", defaultValue: "Running", comment: "Server status: running") : String(localized: "settings.server-status.stopped", defaultValue: "Stopped", comment: "Server status: stopped"))
                            }
                        }
                        .buttonStyle(.plain)
                    }
                ...
                Section(String(localized: "settings.services.section-title", defaultValue: "Services", comment: "Section title for services list")) {
                    ServiceRow(
                    ...
                    ServiceRow(
                        ...
                        customTitle: serverManager.vercelGatewayEnabled && !serverManager.vercelApiKey.isEmpty ? String(localized: "settings.claude.title-vercel", defaultValue: "Claude Code (via Vercel)", comment: "Title when Claude is using Vercel gateway") : nil,
        ...
        .sheet(isPresented: $showingQwenEmailPrompt) {
            VStack(spacing: 16) {
                ...
                TextField(String(localized: "settings.qwen.email-prompt.placeholder", defaultValue: "your.email@example.com", comment: "Placeholder for Qwen email field"), text: $qwenEmail)
                    ...
                    .frame(width: 250)
                HStack(spacing: 12) {
                    Button(String(localized: "settings.remove-account.cancel", defaultValue: "Cancel", comment: "Cancel button in remove account dialog")) {
        ...
        serverManager.saveZaiApiKey(apiKey) { success, output in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    self.authResultSuccess = false
                    let details = output.isEmpty ? String(localized: "settings.auth-result.zai-unknown-error", defaultValue: "Unknown error", comment: "Message when Z.AI key save fails with no details") : output
                    self.authResultMessage = String(format: String(localized: "settings.auth-result.zai-save-failed", defaultValue: "Failed to save API key.\n\nDetails: %@", comment: "Message when Z.AI API key save fails"), details)
                    self.showingAuthResult = true
                }
            }
        }
    ...
    private func disconnectAccount(_ account: AuthAccount) {
        ...
        let cleanup = {
            if self.authManager.deleteAccount(account) {
                self.authResultSuccess = true
                self.authResultMessage = String(format: String(localized: "settings.remove-account.success", defaultValue: "✓ Removed %@ from %@", comment: "Success message when account is removed"), account.displayName, account.type.displayName)
            } else {
                self.authResultSuccess = false
