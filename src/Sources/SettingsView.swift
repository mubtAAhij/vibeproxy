...
struct SettingsView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            VStack(spacing: 4) {
                HStack(spacing: 4) {
                    Text(String(format: String(localized: "settings.footer.thanks", defaultValue: "VibeProxy %@ was made possible thanks to", comment: "Footer text crediting CLIProxyAPIPlus"), appVersion))
                        .font(.caption)
                        .foregroundColor(.secondary)
                ...
                HStack(spacing: 4) {
                    ...
                    Text(String(localized: "settings.footer.rights", defaultValue: "All rights reserved.", comment: "Copyright all rights reserved"))
                        .font(.caption)
                        .foregroundColor(.secondary)
                }
        ...
        .sheet(isPresented: $showingQwenEmailPrompt) {
            VStack(spacing: 16) {
                ...
                HStack(spacing: 12) {
                    ...
                    Button(String(localized: "settings.qwen.email-prompt.continue", defaultValue: "Continue", comment: "Continue button for Qwen authentication")) {
                        showingQwenEmailPrompt = false
        ...
        serverManager.runAuthCommand(command) { success, output in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    self.authResultMessage = String(format: String(localized: "settings.auth.failed", defaultValue: "Authentication failed. Please check if the browser opened and try again.\n\nDetails: %@", comment: "Error message when authentication fails"), output.isEmpty ? String(localized: "settings.auth.no-output", defaultValue: "No output from authentication process", comment: "Placeholder when authentication has no output") : output)
                    self.showingAuthResult = true
                }
            }
        }
        ...
        serverManager.runAuthCommand(.qwenLogin(email: email)) { success, output in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    self.authResultSuccess = false
                    self.authResultMessage = String(format: String(localized: "settings.auth.qwen-failed", defaultValue: "Authentication failed.\n\nDetails: %@", comment: "Error message when Qwen authentication fails"), output.isEmpty ? String(localized: "settings.auth.qwen-no-output", defaultValue: "No output", comment: "Placeholder when Qwen authentication has no output") : output)
                    self.showingAuthResult = true
                }
            }
        }
        ...
        serverManager.saveZaiApiKey(apiKey) { success, output in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    self.authResultSuccess = false
                    self.authResultMessage = String(format: String(localized: "settings.zai.save-failed", defaultValue: "Failed to save API key.\n\nDetails: %@", comment: "Error message when Z.AI API key save fails"), output.isEmpty ? String(localized: "settings.zai.unknown-error", defaultValue: "Unknown error", comment: "Placeholder for unknown error") : output)
                    self.showingAuthResult = true
                }
            }
        }
