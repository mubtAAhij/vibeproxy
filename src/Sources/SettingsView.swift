...
struct ServiceRow<ExtraContent: View>: View {
    ...
    var body: some View {
            ...
            if isEnabled {
                if !accounts.isEmpty {
                    ...
                    HStack(spacing: 4) {
                        ...
                        if accounts.count > 1 {
                            Text(String(localized: "settings.provider.round-robin", defaultValue: "• Round-robin w/ auto-failover", comment: "Description of multi-account failover strategy"))
                                .font(.caption)
                                .foregroundColor(.secondary)
                        }
        ...
        .alert(String(localized: "settings.alert.remove-account.title", defaultValue: "Remove Account", comment: "Alert title for removing account"), isPresented: $showingRemoveConfirmation) {
...
struct SettingsView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            Form {
                ...
                Section(String(localized: "settings.services.title", defaultValue: "Services", comment: "Section title for services")) {
                    ...
                    ServiceRow(
                        ...
                        customTitle: serverManager.vercelGatewayEnabled && !serverManager.vercelApiKey.isEmpty ? String(localized: "settings.services.claude.vercel-title", defaultValue: "Claude Code (via Vercel)", comment: "Title when Claude is routed via Vercel") : nil,
            ...
            VStack(spacing: 4) {
                HStack(spacing: 4) {
                    Text(String(format: String(localized: "settings.footer.attribution", defaultValue: "VibeProxy %@ was made possible thanks to", comment: "Attribution message in footer"), appVersion))
                        .font(.caption)
                        .foregroundColor(.secondary)
        ...
        .sheet(isPresented: $showingQwenEmailPrompt) {
            VStack(spacing: 16) {
                ...
                HStack(spacing: 12) {
                    ...
                    Button(String(localized: "settings.qwen.email-prompt.continue", defaultValue: "Continue", comment: "Continue button")) {
                        showingQwenEmailPrompt = false
        ...
        .sheet(isPresented: $showingZaiApiKeyPrompt) {
            VStack(spacing: 16) {
                Text(String(localized: "settings.zai.apikey-prompt.title", defaultValue: "Z.AI API Key", comment: "Title for Z.AI API key prompt"))
                    .font(.headline)
                Text(String(localized: "settings.zai.apikey-prompt.message", defaultValue: "Enter your Z.AI API key from https://z.ai/manage-apikey/apikey-list", comment: "Message for Z.AI API key prompt"))
                    .font(.caption)
                    .foregroundColor(.secondary)
                ...
                HStack(spacing: 12) {
                    ...
                    Button(String(localized: "settings.zai.apikey-prompt.add-key", defaultValue: "Add Key", comment: "Button to add Z.AI API key")) {
                        showingZaiApiKeyPrompt = false
        ...
        serverManager.runAuthCommand(.qwenLogin(email: email)) { success, output in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    self.authResultSuccess = false
                    let details = output.isEmpty ? String(localized: "auth.error.qwen-no-output", defaultValue: "No output", comment: "Error message when Qwen auth produces no output") : output
        ...
        serverManager.saveZaiApiKey(apiKey) { success, output in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    self.authResultSuccess = false
                    let details = output.isEmpty ? String(localized: "auth.error.zai-unknown", defaultValue: "Unknown error", comment: "Error message when Z.AI save fails with no output") : output
                    self.authResultMessage = String(format: String(localized: "auth.error.zai-failed", defaultValue: "Failed to save API key.\n\nDetails: %@", comment: "Error message when Z.AI API key save fails"), details)
                    self.showingAuthResult = true
                }
            }
        }
