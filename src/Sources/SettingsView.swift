    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            if account.isExpired {
                Text(String(localized: "settings.account.expired", defaultValue: "(expired)", comment: "Label indicating account credentials have expired"))
                    .font(.caption2)
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 4) {
            ...
            HStack {
                ...
                .controlSize(.mini)
                .labelsHidden()
                .help(isEnabled ? String(localized: "settings.provider.disable", defaultValue: "Disable this provider", comment: "Help text for toggle to disable a provider") : String(localized: "settings.provider.enable", defaultValue: "Enable this provider", comment: "Help text for toggle to enable a provider"))
            ...
            if isEnabled {
                if !accounts.isEmpty {
                    ...
                    HStack(spacing: 4) {
                        ...
                        if accounts.count > 1 {
                            Text(String(localized: "settings.provider.load-balancing", defaultValue: "• Round-robin w/ auto-failover", comment: "Description of load balancing strategy"))
                                .font(.caption)
                                .foregroundColor(.secondary)
                        }
        ...
        .alert(String(localized: "settings.remove-account.title", defaultValue: "Remove Account", comment: "Alert title for removing account"), isPresented: $showingRemoveConfirmation) {
            ...
            if let account = accountToRemove {
                Text(String(format: String(localized: "settings.remove-account.confirmation", defaultValue: "Are you sure you want to remove %@ from %@?", comment: "Confirmation message for removing account"), account.displayName, serviceType.displayName))
            }
        }
    }
}
        ...
        VStack(spacing: 0) {
            Form {
                ...
                Section {
                    ...
                    HStack {
                        ...
                        Spacer()
                        Button(String(localized: "settings.open-folder", defaultValue: "Open Folder", comment: "Button to open auth folder in Finder")) {
                            openAuthFolder()
                        }
                    }
                }

                Section(String(localized: "settings.section.services", defaultValue: "Services", comment: "Section header for provider services")) {
                    ServiceRow(
                        ...
                        helpText: String(localized: "settings.provider.antigravity.help", defaultValue: "Antigravity provides OAuth-based access to various AI models including Gemini and Claude. One login gives you access to multiple AI services.", comment: "Help text describing Antigravity provider"),
                    ...
                    ServiceRow(
                        ...
                        customTitle: serverManager.vercelGatewayEnabled && !serverManager.vercelApiKey.isEmpty ? String(localized: "settings.provider.claude.custom-title", defaultValue: "Claude Code (via Vercel)", comment: "Custom title for Claude when using Vercel gateway") : nil,
                    ...
                    ServiceRow(
                        ...
                        helpText: String(localized: "settings.provider.copilot.help", defaultValue: "GitHub Copilot provides access to Claude, GPT, Gemini and other models via your Copilot subscription.", comment: "Help text describing GitHub Copilot provider"),
                    ...
                    ServiceRow(
                        ...
                        helpText: String(localized: "settings.provider.zai.help", defaultValue: "Z.AI GLM provides access to GLM-4.7 and other models via API key. Get your key at https://z.ai/manage-apikey/apikey-list", comment: "Help text describing Z.AI provider"),
            ...
            VStack(spacing: 4) {
                ...
                HStack(spacing: 4) {
                    ...
                    Text(String(localized: "settings.footer.rights-reserved", defaultValue: "All rights reserved.", comment: "Copyright rights reserved text"))
                        .font(.caption)
                        .foregroundColor(.secondary)
                }
        ...
        .sheet(isPresented: $showingQwenEmailPrompt) {
            VStack(spacing: 16) {
                Text(String(localized: "settings.qwen.email-prompt.title", defaultValue: "Qwen Account Email", comment: "Title for Qwen email prompt dialog"))
                    .font(.headline)
                Text(String(localized: "settings.qwen.email-prompt.message", defaultValue: "Enter your Qwen account email address", comment: "Message for Qwen email prompt dialog"))
                    .font(.caption)
                    .foregroundColor(.secondary)
                TextField(String(localized: "settings.qwen.email-prompt.placeholder", defaultValue: "your.email@example.com", comment: "Placeholder for Qwen email input field"), text: $qwenEmail)
                    .textFieldStyle(.roundedBorder)
                    .frame(width: 250)
                HStack(spacing: 12) {
        ...
        .sheet(isPresented: $showingZaiApiKeyPrompt) {
            VStack(spacing: 16) {
                Text(String(localized: "settings.zai.api-key-prompt.title", defaultValue: "Z.AI API Key", comment: "Title for Z.AI API key prompt dialog"))
                    .font(.headline)
                Text(String(localized: "settings.zai.api-key-prompt.message", defaultValue: "Enter your Z.AI API key from https://z.ai/manage-apikey/apikey-list", comment: "Message for Z.AI API key prompt dialog"))
                    .font(.caption)
                    .foregroundColor(.secondary)
        ...
        serverManager.runAuthCommand(command) { success, output in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    let details = output.isEmpty ? String(localized: "settings.auth-result.no-output", defaultValue: "No output from authentication process", comment: "Error detail when auth process produces no output") : output
        ...
        serverManager.runAuthCommand(.qwenLogin(email: email)) { success, output in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    self.authResultSuccess = false
                    let details = output.isEmpty ? String(localized: "settings.auth-result.no-output-short", defaultValue: "No output", comment: "Short error detail when auth process produces no output") : output
                    self.authResultMessage = String(format: String(localized: "settings.auth-result.failed-short", defaultValue: "Authentication failed.\n\nDetails: %@", comment: "Short error message when authentication fails"), details)
                    self.showingAuthResult = true
                }
            }
        }
        ...
        serverManager.saveZaiApiKey(apiKey) { success, output in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    self.authResultSuccess = false
                    let details = output.isEmpty ? String(localized: "settings.auth-result.unknown-error", defaultValue: "Unknown error", comment: "Error detail when no error information is available") : output
                    self.authResultMessage = String(format: String(localized: "settings.auth-result.api-key-save-failed", defaultValue: "Failed to save API key.\n\nDetails: %@", comment: "Error message when API key save fails"), details)
                    self.showingAuthResult = true
                }
            }
        }
    ...
    private func disconnectAccount(_ account: AuthAccount) {
        ...
        let cleanup = {
            if self.authManager.deleteAccount(account) {
                self.authResultSuccess = true
                self.authResultMessage = String(format: String(localized: "settings.account-removal.success", defaultValue: "✓ Removed %@ from %@", comment: "Success message when account is removed"), account.displayName, account.type.displayName)
            } else {
                self.authResultSuccess = false
                self.authResultMessage = String(localized: "settings.account-removal.failed", defaultValue: "Failed to remove account", comment: "Error message when account removal fails")
            }
            self.showingAuthResult = true
