...
struct VercelGatewayControls: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 6) {
            ...
            .help(String(localized: "settings.vercel.help-text", defaultValue: "Route Claude requests through Vercel AI Gateway for safer access to your Claude Max subscription", comment: "Help text for Vercel AI Gateway feature"))
            ...
            if serverManager.vercelGatewayEnabled {
                HStack(spacing: 8) {
                    ...
                    if showingSaved {
                        Text(String(localized: "settings.vercel.saved", defaultValue: "Saved", comment: "Status message when API key is saved"))
                            .font(.caption)
...
struct ServiceRow<ExtraContent: View>: View {
    ...
    var body: some View {
        ...
        .alert(String(localized: "settings.alert.remove-account-title", defaultValue: "Remove Account", comment: "Alert title for removing an account"), isPresented: $showingRemoveConfirmation) {
            ...
            if let account = accountToRemove {
                Text(String(format: String(localized: "settings.alert.remove-account-message", defaultValue: "Are you sure you want to remove %@ from %@?", comment: "Confirmation message for removing account"), account.displayName, serviceType.displayName))
            }
        }
    }
}
    ...
    var body: some View {
        VStack(spacing: 0) {
            Form {
                ...
                Section {
                    Toggle(String(localized: "settings.launch-at-login", defaultValue: "Launch at login", comment: "Toggle to start app at login"), isOn: $launchAtLogin)
                ...
                Section(String(localized: "settings.services-section", defaultValue: "Services", comment: "Section header for services")) {
                    ...
                    ServiceRow(
                        ...
                        customTitle: serverManager.vercelGatewayEnabled && !serverManager.vercelApiKey.isEmpty ? String(localized: "settings.service.claude.via-vercel", defaultValue: "Claude Code (via Vercel)", comment: "Title for Claude service when using Vercel gateway") : nil,
            ...
            VStack(spacing: 4) {
                HStack(spacing: 4) {
                    Text(String(format: String(localized: "settings.footer.credits", defaultValue: "VibeProxy %@ was made possible thanks to", comment: "Footer credits text with version number"), appVersion))
                        .font(.caption)
                        .foregroundColor(.secondary)
        ...
        serverManager.runAuthCommand(.qwenLogin(email: email)) { success, output in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    self.authResultSuccess = false
                    let details = output.isEmpty ? String(localized: "auth.error.no-output-short", defaultValue: "No output", comment: "Error when auth process produces no output (short version)") : output
                    self.authResultMessage = String(format: String(localized: "auth.error.failed-with-details", defaultValue: "Authentication failed.\n\nDetails: %@", comment: "Generic authentication failure message with details"), details)
                    self.showingAuthResult = true
                }
            }
        }
        ...
        serverManager.saveZaiApiKey(apiKey) { success, output in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    self.authResultSuccess = false
                    let details = output.isEmpty ? String(localized: "auth.error.unknown-error", defaultValue: "Unknown error", comment: "Generic unknown error message") : output
                    self.authResultMessage = String(format: String(localized: "auth.error.failed-to-save-api-key-with-details", defaultValue: "Failed to save API key.\n\nDetails: %@", comment: "Error when saving API key fails"), details)
                    self.showingAuthResult = true
                }
            }
        }
