...
struct VercelGatewayControls: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 6) {
            ...
            .help(String(localized: "settings.vercel.gateway-help", defaultValue: "Route Claude requests through Vercel AI Gateway for safer access to your Claude Max subscription", comment: "Help text for Vercel AI Gateway toggle"))
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 4) {
            ...
            HStack {
                ...
                .controlSize(.mini)
                .labelsHidden()
                .help(isEnabled ? String(localized: "settings.provider.disable", defaultValue: "Disable this provider", comment: "Help text for disabling a provider") : String(localized: "settings.provider.enable", defaultValue: "Enable this provider", comment: "Help text for enabling a provider"))
            ...
            if isEnabled {
                if !accounts.isEmpty {
                    ...
                    Text(String(localized: "settings.provider.no-accounts", defaultValue: "No connected accounts", comment: "Message when provider has no accounts"))
                        .font(.caption)
                        .foregroundColor(.secondary)
                        .padding(.leading, 28)
                }
        ...
        .alert(String(localized: "settings.account.remove-alert-title", defaultValue: "Remove Account", comment: "Title for account removal confirmation alert"), isPresented: $showingRemoveConfirmation) {
            Button(String(localized: "settings.account.cancel", defaultValue: "Cancel", comment: "Cancel button in account removal alert"), role: .cancel) {
                accountToRemove = nil
            }
            ...
            if let account = accountToRemove {
                Text(String(format: String(localized: "settings.account.remove-confirmation", defaultValue: "Are you sure you want to remove %@ from %@?", comment: "Confirmation message for removing account"), account.displayName, serviceType.displayName))
            }
        }
    }
}
    ...
    var body: some View {
        VStack(spacing: 0) {
            Form {
                ...
                Section {
                    Toggle(String(localized: "settings.general.launch-at-login", defaultValue: "Launch at login", comment: "Toggle to launch app at login"), isOn: $launchAtLogin)
                ...
                Section(String(localized: "settings.services.section-title", defaultValue: "Services", comment: "Section header for services")) {
                    ServiceRow(
                        ...
                        helpText: String(localized: "settings.services.antigravity-help", defaultValue: "Antigravity provides OAuth-based access to various AI models including Gemini and Claude. One login gives you access to multiple AI services.", comment: "Help text for Antigravity provider"),
                    ...
                    ServiceRow(
                        ...
                        customTitle: serverManager.vercelGatewayEnabled && !serverManager.vercelApiKey.isEmpty ? String(localized: "settings.services.claude-via-vercel", defaultValue: "Claude Code (via Vercel)", comment: "Title for Claude when using Vercel gateway") : nil,
                    ...
                    ServiceRow(
                        ...
                        helpText: String(localized: "settings.services.gemini-help", defaultValue: "⚠️ Note: If you're an existing Gemini user with multiple projects, authentication will use your default project. Set your desired project as default in Google AI Studio before connecting.", comment: "Help text for Gemini provider"),
                    ...
                    ServiceRow(
                        ...
                        helpText: String(localized: "settings.services.copilot-help", defaultValue: "GitHub Copilot provides access to Claude, GPT, Gemini and other models via your Copilot subscription.", comment: "Help text for GitHub Copilot provider"),
                    ...
                    ServiceRow(
                        ...
                        helpText: String(localized: "settings.services.zai-help", defaultValue: "Z.AI GLM provides access to GLM-4.7 and other models via API key. Get your key at https://z.ai/manage-apikey/apikey-list", comment: "Help text for Z.AI provider"),
            ...
            VStack(spacing: 4) {
                ...
                HStack(spacing: 4) {
                    ...
                    Text(String(localized: "settings.footer.all-rights-reserved", defaultValue: "All rights reserved.", comment: "Copyright notice in footer"))
                        .font(.caption)
                        .foregroundColor(.secondary)
                }

                Link(String(localized: "settings.footer.report-issue", defaultValue: "Report an issue", comment: "Link text for issue reporting"), destination: URL(string: "https://github.com/automazeio/vibeproxy/issues")!)
                    .font(.caption)
                    .padding(.top, 6)
                    .onHover { inside in
                        if inside { NSCursor.pointingHand.push() } else { NSCursor.pop() }
        ...
        .sheet(isPresented: $showingQwenEmailPrompt) {
            VStack(spacing: 16) {
                Text(String(localized: "settings.qwen.email-prompt-title", defaultValue: "Qwen Account Email", comment: "Dialog title for Qwen email prompt"))
                    .font(.headline)
                Text(String(localized: "settings.qwen.email-prompt-message", defaultValue: "Enter your Qwen account email address", comment: "Dialog instruction for Qwen email"))
                    .font(.caption)
                    .foregroundColor(.secondary)
                TextField(String(localized: "settings.qwen.email-placeholder", defaultValue: "your.email@example.com", comment: "Placeholder text for email field"), text: $qwenEmail)
                    .textFieldStyle(.roundedBorder)
                    .frame(width: 250)
                HStack(spacing: 12) {
                    Button(String(localized: "settings.account.cancel", defaultValue: "Cancel", comment: "Cancel button in account removal alert")) {
                        showingQwenEmailPrompt = false
                        qwenEmail = ""
                    }
                    Button(String(localized: "settings.qwen.continue", defaultValue: "Continue", comment: "Continue button in Qwen dialog")) {
                        showingQwenEmailPrompt = false
        ...
        .sheet(isPresented: $showingZaiApiKeyPrompt) {
            VStack(spacing: 16) {
                Text(String(localized: "settings.zai.api-key-prompt-title", defaultValue: "Z.AI API Key", comment: "Dialog title for Z.AI API key prompt"))
                    .font(.headline)
                Text(String(localized: "settings.zai.api-key-prompt-message", defaultValue: "Enter your Z.AI API key from https://z.ai/manage-apikey/apikey-list", comment: "Dialog instruction for Z.AI API key"))
                    .font(.caption)
                    .foregroundColor(.secondary)
        ...
        .alert(String(localized: "settings.auth.result-title", defaultValue: "Authentication Result", comment: "Alert title for authentication result"), isPresented: $showingAuthResult) {
            Button(String(localized: "settings.auth.ok", defaultValue: "OK", comment: "OK button in authentication result alert"), role: .cancel) { }
        } message: {
        ...
        serverManager.runAuthCommand(command) { success, output in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    self.authResultMessage = String(format: String(localized: "settings.auth.failed-message", defaultValue: "Authentication failed. Please check if the browser opened and try again.\n\nDetails: %@", comment: "Error message when authentication fails"), output.isEmpty ? String(localized: "settings.auth.no-output", defaultValue: "No output from authentication process", comment: "Fallback text when auth process has no output") : output)
                    self.showingAuthResult = true
                }
            }
        }
        ...
        serverManager.runAuthCommand(.qwenLogin(email: email)) { success, output in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    self.authResultSuccess = false
                    self.authResultMessage = String(format: String(localized: "settings.auth.qwen-failed-message", defaultValue: "Authentication failed.\n\nDetails: %@", comment: "Error message when Qwen authentication fails"), output.isEmpty ? String(localized: "settings.auth.qwen-no-output", defaultValue: "No output", comment: "Fallback text when Qwen auth has no output") : output)
                    self.showingAuthResult = true
                }
            }
        }
        ...
        serverManager.saveZaiApiKey(apiKey) { success, output in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    self.authResultSuccess = false
                    self.authResultMessage = String(format: String(localized: "settings.auth.zai-failed-message", defaultValue: "Failed to save API key.\n\nDetails: %@", comment: "Error message when Z.AI API key save fails"), output.isEmpty ? String(localized: "settings.auth.zai-unknown-error", defaultValue: "Unknown error", comment: "Fallback text when Z.AI save has unknown error") : output)
                    self.showingAuthResult = true
                }
            }
        }
