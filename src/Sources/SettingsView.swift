    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            if account.isExpired {
                Text(String(localized: "settings.account.expired", defaultValue: "(expired)", comment: "Label indicating an account credential has expired"))
                    .font(.caption2)
...
struct ServiceRow<ExtraContent: View>: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 4) {
            ...
            if isEnabled {
                if !accounts.isEmpty {
                    // Collapsible summary
                    HStack(spacing: 4) {
                        Text(String(localized: "settings.provider.connected-accounts", defaultValue: "\(accounts.count) connected account\(accounts.count == 1 ? "" : "s")", comment: "Label showing number of connected accounts"))
                            .font(.caption)
                            .foregroundColor(.green)

                        if accounts.count > 1 {
                            Text(String(localized: "settings.provider.round-robin", defaultValue: "• Round-robin w/ auto-failover", comment: "Label explaining round-robin load balancing with failover"))
                                .font(.caption)
                                .foregroundColor(.secondary)
                        }
                    ...
                    Text(String(localized: "settings.provider.no-accounts", defaultValue: "No connected accounts", comment: "Message when no accounts are connected to a provider"))
                        .font(.caption)
                        .foregroundColor(.secondary)
                        .padding(.leading, 28)
                }
        ...
        .alert(String(localized: "settings.remove-account.title", defaultValue: "Remove Account", comment: "Title of alert when removing an account"), isPresented: $showingRemoveConfirmation) {
            ...
            Button(String(localized: "settings.remove-account.remove", defaultValue: "Remove", comment: "Button to confirm account removal"), role: .destructive) {
            ...
            if let account = accountToRemove {
                Text(String(format: NSLocalizedString("settings.remove-account.confirm", comment: "Confirmation message for removing an account"), account.displayName, serviceType.displayName))
            }
        }
    }
}
    ...
    var body: some View {
        VStack(spacing: 0) {
            Form {
                Section {
                    HStack {
                        Text(String(localized: "settings.server.status-label", defaultValue: "Server status", comment: "Label for server status indicator"))
                        Spacer()
                        Button(action: {
                            ...
                            HStack(spacing: 6) {
                                ...
                                Text(serverManager.isRunning ? String(localized: "settings.server.running", defaultValue: "Running", comment: "Status when server is running") : String(localized: "settings.server.stopped", defaultValue: "Stopped", comment: "Status when server is stopped"))
                            }
                        }
                        .buttonStyle(.plain)
                    }
                ...
                Section {
                    ...
                    HStack {
                        Text(String(localized: "settings.auth-files", defaultValue: "Auth files", comment: "Label for auth files location"))
                        Spacer()
                ...
                Section(String(localized: "settings.services-section", defaultValue: "Services", comment: "Section header for AI services list")) {
                    ServiceRow(
                        ...
                        helpText: String(localized: "settings.service.antigravity.help", defaultValue: "Antigravity provides OAuth-based access to various AI models including Gemini and Claude. One login gives you access to multiple AI services.", comment: "Help text explaining Antigravity service"),
                    ...
                    ServiceRow(
                        ...
                        customTitle: serverManager.vercelGatewayEnabled && !serverManager.vercelApiKey.isEmpty ? String(localized: "settings.service.claude.vercel-title", defaultValue: "Claude Code (via Vercel)", comment: "Service title when using Vercel AI Gateway") : nil,
                    ...
                    ServiceRow(
                        ...
                        helpText: String(localized: "settings.service.gemini.help", defaultValue: "⚠️ Note: If you're an existing Gemini user with multiple projects, authentication will use your default project. Set your desired project as default in Google AI Studio before connecting.", comment: "Help text explaining Gemini authentication for multi-project users"),
                    ...
                    ServiceRow(
                        ...
                        helpText: String(localized: "settings.service.copilot.help", defaultValue: "GitHub Copilot provides access to Claude, GPT, Gemini and other models via your Copilot subscription.", comment: "Help text explaining GitHub Copilot service"),
            ...
            VStack(spacing: 4) {
                HStack(spacing: 4) {
                    Text(String(format: NSLocalizedString("settings.footer.thanks", comment: "Footer text thanking CLIProxyAPIPlus"), appVersion))
                        .font(.caption)
                        .foregroundColor(.secondary)
                ...
                Link(String(localized: "settings.footer.report-issue", defaultValue: "Report an issue", comment: "Link to report an issue on GitHub"), destination: URL(string: "https://github.com/automazeio/vibeproxy/issues")!)
                    .font(.caption)
                    .padding(.top, 6)
                    .onHover { inside in
                        if inside { NSCursor.pointingHand.push() } else { NSCursor.pop() }
        ...
        .sheet(isPresented: $showingQwenEmailPrompt) {
            VStack(spacing: 16) {
                Text(String(localized: "settings.qwen.email-prompt.title", defaultValue: "Qwen Account Email", comment: "Title of Qwen email input dialog"))
                    .font(.headline)
                Text(String(localized: "settings.qwen.email-prompt.message", defaultValue: "Enter your Qwen account email address", comment: "Message in Qwen email input dialog"))
                    .font(.caption)
                    .foregroundColor(.secondary)
                TextField(String(localized: "settings.qwen.email-prompt.placeholder", defaultValue: "your.email@example.com", comment: "Placeholder text for Qwen email input"), text: $qwenEmail)
                    .textFieldStyle(.roundedBorder)
                    .frame(width: 250)
                HStack(spacing: 12) {
                    Button(String(localized: "settings.qwen.email-prompt.cancel", defaultValue: "Cancel", comment: "Button to cancel Qwen email input")) {
                        showingQwenEmailPrompt = false
                        qwenEmail = ""
                    }
                    Button(String(localized: "settings.qwen.email-prompt.continue", defaultValue: "Continue", comment: "Button to continue with Qwen authentication")) {
                        showingQwenEmailPrompt = false
        ...
        .sheet(isPresented: $showingZaiApiKeyPrompt) {
            VStack(spacing: 16) {
                Text(String(localized: "settings.zai.api-key-prompt.title", defaultValue: "Z.AI API Key", comment: "Title of Z.AI API key input dialog"))
                    .font(.headline)
                Text(String(localized: "settings.zai.api-key-prompt.message", defaultValue: "Enter your Z.AI API key from https://z.ai/manage-apikey/apikey-list", comment: "Message in Z.AI API key input dialog"))
                    .font(.caption)
                    .foregroundColor(.secondary)
                TextField("", text: $zaiApiKey)
                    ...
                    .frame(width: 300)
                HStack(spacing: 12) {
                    Button(String(localized: "settings.zai.api-key-prompt.cancel", defaultValue: "Cancel", comment: "Button to cancel Z.AI API key input")) {
        ...
        .alert(String(localized: "settings.auth-result.title", defaultValue: "Authentication Result", comment: "Title of authentication result alert"), isPresented: $showingAuthResult) {
            Button(String(localized: "settings.auth-result.ok", defaultValue: "OK", comment: "Button to dismiss authentication result"), role: .cancel) { }
        } message: {
        ...
        serverManager.runAuthCommand(command) { success, output in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    let details = output.isEmpty ? String(localized: "settings.auth.no-output", defaultValue: "No output from authentication process", comment: "Placeholder when auth process produces no output") : output
                    self.authResultMessage = String(format: NSLocalizedString("settings.auth.failed", comment: "Error message when authentication fails"), details)
                    self.showingAuthResult = true
                }
            }
        }
        ...
        serverManager.runAuthCommand(.qwenLogin(email: email)) { success, output in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    self.authResultSuccess = false
                    let details = output.isEmpty ? String(localized: "settings.auth.qwen-no-output", defaultValue: "No output", comment: "Placeholder when Qwen auth produces no output") : output
                    self.authResultMessage = String(format: NSLocalizedString("settings.auth.qwen-failed", comment: "Error message when Qwen authentication fails"), details)
                    self.showingAuthResult = true
                }
            }
        }
        ...
        serverManager.saveZaiApiKey(apiKey) { success, output in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    self.authResultSuccess = false
                    let details = output.isEmpty ? String(localized: "settings.auth.zai-unknown-error", defaultValue: "Unknown error", comment: "Placeholder when Z.AI key save fails with no details") : output
                    self.authResultMessage = String(format: NSLocalizedString("settings.auth.zai-save-failed", comment: "Error message when Z.AI API key save fails"), details)
                    self.showingAuthResult = true
                }
            }
        }
