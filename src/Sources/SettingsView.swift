    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            if account.isExpired {
                Text(String(localized: "account.expired", defaultValue: "(expired)", comment: "Account expired status label"))
                    .font(.caption2)
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 6) {
            Toggle(isOn: $serverManager.vercelGatewayEnabled) {
                Text(String(localized: "settings.use-vercel-gateway", defaultValue: "Use Vercel AI Gateway", comment: "Toggle to enable Vercel AI Gateway"))
                    .font(.caption)
            }
            .toggleStyle(.checkbox)
            .help(String(localized: "settings.vercel-gateway-help", defaultValue: "Route Claude requests through Vercel AI Gateway for safer access to your Claude Max subscription", comment: "Help text for Vercel gateway setting"))
            ...
            if serverManager.vercelGatewayEnabled {
                HStack(spacing: 8) {
                    ...
                    if showingSaved {
                        Text(String(localized: "settings.saved", defaultValue: "Saved", comment: "Confirmation when Vercel API key is saved"))
                            .font(.caption)
                            .foregroundColor(.green)
                    } else {
                        Button(String(localized: "settings.save", defaultValue: "Save", comment: "Save button for Vercel API key")) {
                            showingSaved = true
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 4) {
            ...
            HStack {
                ...
                .controlSize(.mini)
                .labelsHidden()
                .help(isEnabled ? String(localized: "settings.disable-provider", defaultValue: "Disable this provider", comment: "Help text to disable provider") : String(localized: "settings.enable-provider", defaultValue: "Enable this provider", comment: "Help text to enable provider"))
                ...
                if isAuthenticating {
                    ...
                    Button(String(localized: "settings.add-account", defaultValue: "Add Account", comment: "Button to add a new account")) {
                        onConnect()
                    }
                    .controlSize(.small)
                }
            ...
            if isEnabled {
                if !accounts.isEmpty {
                    // Collapsible summary
                    HStack(spacing: 4) {
                        Text(String(localized: "settings.connected-accounts", defaultValue: "\(accounts.count) connected account\(accounts.count == 1 ? "" : "s")", comment: "Number of connected accounts"))
                            .font(.caption)
                            .foregroundColor(.green)

                        if accounts.count > 1 {
                            Text(String(localized: "settings.round-robin", defaultValue: "• Round-robin w/ auto-failover", comment: "Load balancing strategy description"))
                                .font(.caption)
                                .foregroundColor(.secondary)
                        }
                    ...
                    Text(String(localized: "settings.no-accounts", defaultValue: "No connected accounts", comment: "Message when no accounts are connected"))
                        .font(.caption)
                        .foregroundColor(.secondary)
                        .padding(.leading, 28)
                }
        ...
        .alert(String(localized: "alert.remove-account", defaultValue: "Remove Account", comment: "Alert title for removing account"), isPresented: $showingRemoveConfirmation) {
            ...
            if let account = accountToRemove {
                Text(String(format: NSLocalizedString("alert.remove-account-message", comment: "Confirmation message for removing account"), account.displayName, serviceType.displayName))
            }
        }
    }
}
...
struct SettingsView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            Form {
                ...
                Section(String(localized: "settings.services", defaultValue: "Services", comment: "Services section header")) {
                    ...
                    ServiceRow(
                        ...
                        customTitle: serverManager.vercelGatewayEnabled && !serverManager.vercelApiKey.isEmpty ? String(localized: "settings.claude-via-vercel", defaultValue: "Claude Code (via Vercel)", comment: "Claude service title when using Vercel gateway") : nil,
                    ...
                    ServiceRow(
                        ...
                        helpText: String(localized: "settings.gemini-help", defaultValue: "⚠️ Note: If you're an existing Gemini user with multiple projects, authentication will use your default project. Set your desired project as default in Google AI Studio before connecting.", comment: "Help text for Gemini service with project warning"),
            ...
            VStack(spacing: 4) {
                HStack(spacing: 4) {
                    Text(String(format: NSLocalizedString("settings.footer-thanks", comment: "Footer text thanking contributors"), appVersion))
                        .font(.caption)
                        .foregroundColor(.secondary)
                ...
                Link(String(localized: "settings.report-issue", defaultValue: "Report an issue", comment: "Link to report an issue"), destination: URL(string: "https://github.com/automazeio/vibeproxy/issues")!)
                    .font(.caption)
                    .padding(.top, 6)
                    .onHover { inside in
                        if inside { NSCursor.pointingHand.push() } else { NSCursor.pop() }
        ...
        .sheet(isPresented: $showingQwenEmailPrompt) {
            VStack(spacing: 16) {
                Text(String(localized: "settings.qwen-email-title", defaultValue: "Qwen Account Email", comment: "Title for Qwen email prompt"))
                    .font(.headline)
                Text(String(localized: "settings.qwen-email-description", defaultValue: "Enter your Qwen account email address", comment: "Description for Qwen email prompt"))
                    .font(.caption)
                    .foregroundColor(.secondary)
                ...
                HStack(spacing: 12) {
                    ...
                    Button(String(localized: "settings.continue", defaultValue: "Continue", comment: "Continue button for Qwen auth")) {
                        showingQwenEmailPrompt = false
        ...
        .sheet(isPresented: $showingZaiApiKeyPrompt) {
            VStack(spacing: 16) {
                Text(String(localized: "settings.zai-api-key-title", defaultValue: "Z.AI API Key", comment: "Title for Z.AI API key prompt"))
                    .font(.headline)
                Text(String(localized: "settings.zai-api-key-description", defaultValue: "Enter your Z.AI API key from https://z.ai/manage-apikey/apikey-list", comment: "Description for Z.AI API key prompt"))
                    .font(.caption)
                    .foregroundColor(.secondary)
                ...
                HStack(spacing: 12) {
                    ...
                    Button(String(localized: "settings.add-key", defaultValue: "Add Key", comment: "Button to add Z.AI API key")) {
                        showingZaiApiKeyPrompt = false
        ...
        .alert(String(localized: "alert.auth-result", defaultValue: "Authentication Result", comment: "Alert title for authentication result"), isPresented: $showingAuthResult) {
        ...
        serverManager.runAuthCommand(command) { success, output in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    self.authResultMessage = String(format: NSLocalizedString("error.auth-failed-details", comment: "Authentication failed with details"), details)
                    self.showingAuthResult = true
                }
            }
        }
        ...
        serverManager.runAuthCommand(.qwenLogin(email: email)) { success, output in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    self.authResultSuccess = false
                    let details = output.isEmpty ? String(localized: "error.no-output", defaultValue: "No output", comment: "Error when process produces no output") : output
                    self.authResultMessage = String(format: NSLocalizedString("error.qwen-auth-failed", comment: "Qwen authentication failed with details"), details)
                    self.showingAuthResult = true
                }
            }
        }
        ...
        serverManager.saveZaiApiKey(apiKey) { success, output in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    self.authResultSuccess = false
                    let details = output.isEmpty ? String(localized: "error.unknown", defaultValue: "Unknown error", comment: "Unknown error message") : output
                    self.authResultMessage = String(format: NSLocalizedString("error.zai-key-save-failed", comment: "Z.AI API key save failed with details"), details)
                    self.showingAuthResult = true
                }
            }
        }
    ...
    private func disconnectAccount(_ account: AuthAccount) {
        ...
        let cleanup = {
            if self.authManager.deleteAccount(account) {
                self.authResultSuccess = true
                self.authResultMessage = String(format: NSLocalizedString("account.removed-success", comment: "Account removed successfully"), account.displayName, account.type.displayName)
            } else {
                self.authResultSuccess = false
                self.authResultMessage = String(localized: "error.remove-account-failed", defaultValue: "Failed to remove account", comment: "Error when account removal fails")
            }
            self.showingAuthResult = true
