    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            if account.isExpired {
                Text(String(localized: "settings.account.expired", defaultValue: "(expired)", comment: "Label indicating expired account"))
                    .font(.caption2)
        ...
        VStack(alignment: .leading, spacing: 6) {
            ...
            if serverManager.vercelGatewayEnabled {
                HStack(spacing: 8) {
                    ...
                    if showingSaved {
                        Text(String(localized: "settings.vercel.saved", defaultValue: "Saved", comment: "Status indicating API key was saved"))
                            .font(.caption)
...
struct ServiceRow<ExtraContent: View>: View {
    ...
    var body: some View {
        ...
        .alert(String(localized: "settings.remove-account.title", defaultValue: "Remove Account", comment: "Title for remove account confirmation alert"), isPresented: $showingRemoveConfirmation) {
            ...
            if let account = accountToRemove {
                Text(String(format: String(localized: "settings.remove-account.confirm", defaultValue: "Are you sure you want to remove %@ from %@?", comment: "Confirmation message for removing account"), account.displayName, serviceType.displayName))
            }
        }
    }
}
    ...
    var body: some View {
        VStack(spacing: 0) {
            Form {
                ...
                Section {
                    Toggle(String(localized: "settings.launch-at-login", defaultValue: "Launch at login", comment: "Toggle for launch at login setting"), isOn: $launchAtLogin)
                ...
                Section(String(localized: "settings.services", defaultValue: "Services", comment: "Section header for services")) {
                    ...
                    ServiceRow(
                        ...
                        helpText: String(localized: "settings.zai.help", defaultValue: "Z.AI GLM provides access to GLM-4.7 and other models via API key. Get your key at https://z.ai/manage-apikey/apikey-list", comment: "Help text for Z.AI service with API key URL"),
            ...
            VStack(spacing: 4) {
                HStack(spacing: 4) {
                    Text(String(format: String(localized: "settings.footer.credits", defaultValue: "VibeProxy %@ was made possible thanks to", comment: "Footer credits text with version"), appVersion))
                        .font(.caption)
                        .foregroundColor(.secondary)
        ...
        serverManager.runAuthCommand(command) { success, output in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    let details = output.isEmpty ? String(localized: "settings.auth-result.no-output", defaultValue: "No output from authentication process", comment: "Default message when auth process produces no output") : output
                    self.authResultMessage = String(format: String(localized: "settings.auth-result.failed", defaultValue: "Authentication failed. Please check if the browser opened and try again.\n\nDetails: %@", comment: "Error message for authentication failure"), details)
                    self.showingAuthResult = true
                }
            }
        }
        ...
        serverManager.runAuthCommand(.qwenLogin(email: email)) { success, output in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    self.authResultSuccess = false
                    let details = output.isEmpty ? String(localized: "settings.auth-result.qwen-no-output", defaultValue: "No output", comment: "Message when Qwen auth produces no output") : output
                    self.authResultMessage = String(format: String(localized: "settings.auth-result.qwen-failed", defaultValue: "Authentication failed.\n\nDetails: %@", comment: "Error message for Qwen authentication failure"), details)
                    self.showingAuthResult = true
                }
            }
        }
        ...
        serverManager.saveZaiApiKey(apiKey) { success, output in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    self.authResultSuccess = false
                    let details = output.isEmpty ? String(localized: "settings.auth-result.unknown-error", defaultValue: "Unknown error", comment: "Message when error details are unknown") : output
                    self.authResultMessage = String(format: String(localized: "settings.auth-result.save-key-failed", defaultValue: "Failed to save API key.\n\nDetails: %@", comment: "Error message for API key save failure"), details)
                    self.showingAuthResult = true
                }
            }
        }
