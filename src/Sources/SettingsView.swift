        ...
        HStack(spacing: 8) {
            ...
            if account.isExpired && !account.isDisabled {
                Text(String(
                    ...
                    comment: "Status suffix shown when provider credential is expired"
                ))
                    .font(.caption2)
            ...
            if account.isDisabled {
                Text(String(
                    ...
                    comment: "Status suffix shown when provider is disabled"
                ))
                    .font(.caption2)
            ...
            if showDisableToggle {
                ...
                .help(!canDisable ? String(
                    localized: "settings.provider-error.at-least-one-account-enabled",
                    defaultValue: "At least one account must remain enabled",
                    comment: "Validation message when disabling the last enabled account"
                ) : "")
                .onHover { inside in
                    if canDisable {
            ...
            Button(action: onRemove) {
                HStack(spacing: 2) {
                    Image(systemName: "minus.circle.fill")
                        .font(.caption)
                    Text(String(
                        localized: "settings.common.remove",
                        defaultValue: "Remove",
                        comment: "Destructive action title to remove an item"
                    ))
                        .font(.caption)
                }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 6) {
            Toggle(isOn: $serverManager.vercelGatewayEnabled) {
                Text(String(
                    localized: "settings.vercel-gateway.use-toggle-title",
                    defaultValue: "Use Vercel AI Gateway",
                    comment: "Toggle label to route requests through Vercel AI Gateway"
                ))
                    .font(.caption)
            }
            ...
            .help(String(
                ...
                comment: "Description text for Vercel AI Gateway option"
            ))
            ...
            if serverManager.vercelGatewayEnabled {
                HStack(spacing: 8) {
                    ...
                    if showingSaved {
                        Text(String(
                            localized: "settings.vercel-gateway.save-state.saved",
                            defaultValue: "Saved",
                            comment: "Status label shown after successfully saving Vercel API key"
                        ))
                            .font(.caption)
                            .foregroundColor(.green)
                    } else {
                        Button(String(
                            localized: "settings.vercel-gateway.save-action",
                            defaultValue: "Save",
...
struct ServiceRow<ExtraContent: View>: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 4) {
            ...
            HStack {
                ...
                .help(toggleHelpText ?? (isEnabled ? String(
                    ...
                    comment: "Help text for provider disable toggle action"
                ) : String(
                    localized: "settings.provider-toggle.enable-help",
                    defaultValue: "Enable this provider",
                    comment: "Help text for provider enable toggle action"
                )))
                ...
                if isAuthenticating {
                    ...
                    Button(String(
                        ...
                        comment: "Button title to add a connected account"
                    )) {
                        onConnect()
                    }
                    .controlSize(.small)
                ...
                if !accounts.isEmpty {
                    ...
                    HStack(spacing: 4) {
                        Text(String(format: String(
                            localized: "settings.accounts.connected-account-count",
                            defaultValue: "%d connected account%@",
                            comment: "Summary showing number of connected accounts"
                        ), accounts.count, (accounts.count == 1 ? "" : "s")))
                            .font(.caption)
                            .foregroundColor(.green)
                        ...
                        if enabledCount > 1 {
                            Text(String(
                                ...
                                comment: "Bullet description of account routing behavior"
                            ))
                                .font(.caption)
                                .foregroundColor(.secondary)
                        }
                    ...
                    Text(String(
                        ...
                        comment: "Empty state message when no accounts are connected"
                    ))
                        .font(.caption)
                        .foregroundColor(.secondary)
                        .padding(.leading, 28)
        ...
        .alert(String(
            localized: "settings.accounts.remove-account-title",
            defaultValue: "Remove Account",
            comment: "Confirmation title for removing an account"
            ...
            if let account = accountToRemove {
                Text(String(format: String(
                    ...
                    comment: "Confirmation message for removing an account from a service type"
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            if credential.isDisabled {
                Text(String(
                    localized: "settings.api-key-status.disabled",
                    defaultValue: "(disabled)",
                    comment: "Status suffix shown when API key is disabled"
                ))
                    .font(.caption2)
                    .foregroundColor(.secondary)
            }
            if showDisableToggle {
                ...
                Button(action: onToggleDisabled) {
                    Text(credential.isDisabled ? String(
                        localized: "settings.api-key-action.enable",
                        defaultValue: "Enable",
                        comment: "Action title to enable an API key"
                    ) : String(
                        localized: "settings.api-key-action.disable",
                        defaultValue: "Disable",
                ...
                .disabled(!canDisable)
                .help(!canDisable ? String(
                    localized: "settings.api-key-error.at-least-one-key-enabled",
                    defaultValue: "At least one API key must remain enabled",
                    comment: "Validation message when trying to disable the last enabled API key"
                ) : "")
                .onHover { inside in
                    if canDisable {
                        if inside { NSCursor.pointingHand.push() } else { NSCursor.pop() }
...
struct CustomProviderRow: View {
    ...
    private var summaryText: String {
        ...
        if provider.inlineKeyCount > 0 && !credentials.isEmpty {
            return String(format: String(
                localized: "settings.api-keys.summary.total-config-and-added",
                defaultValue: "%d API keys • %d in config • %d added here",
                comment: "Summary showing total API keys split between config and keys added in the app"
        ...
        if provider.inlineKeyCount > 0 {
            return String(format: String(
                ...
                defaultValue: "%d API key%@ from config",
                comment: "Summary showing configured API key count from config file"
            ), totalConfiguredKeyCount, (totalConfiguredKeyCount == 1 ? "" : "s"))
        }
        return String(format: String(
            localized: "settings.api-keys.summary.added-here",
            defaultValue: "%d API key%@ added here",
            comment: "Summary showing API key count added in the app"
        ), totalConfiguredKeyCount, (totalConfiguredKeyCount == 1 ? "" : "s"))
    }
    ...
    private var poolingStatusText: String? {
        ...
        return String(
            ...
            comment: "Bullet description of API key pooling behavior"
        )
    }
    ...
    private var modelSummaryText: String? {
        ...
        return String(format: String(
            ...
            defaultValue: "Models: %@",
            comment: "Label listing provider model aliases"
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 4) {
            HStack {
                ...
                if isAuthenticating {
                    ...
                    Button(String(
                        localized: "settings.api-keys.add-api-key",
                        ...
                        VStack(alignment: .leading, spacing: 6) {
                            ...
                            if provider.inlineKeyCount > 0 {
                                Text(String(format: String(
                                    ...
                                    defaultValue: "Using %d API key%@ from ~/.cli-proxy-api/config.yaml",
                                    comment: "Message showing number of API keys loaded from config file path"
                                ), provider.inlineKeyCount, (provider.inlineKeyCount == 1 ? "" : "s")))
                                    .font(.caption)
                                    .foregroundColor(.secondary)
                                    .padding(.leading, 28)
                            }
        ...
        .alert(String(
            localized: "settings.api-keys.remove-api-key-title",
            defaultValue: "Remove API Key",
            comment: "Confirmation title for removing an API key"
        ), isPresented: $showingRemoveConfirmation) {
            Button(String(
                localized: "common.action.cancel",
                defaultValue: "Cancel",
...
struct SettingsView: View {
    ...
    private var appVersion: String {
        if let version = Bundle.main.infoDictionary?["CFBundleShortVersionString"] as? String {
            return String(format: String(
                ...
                defaultValue: "v%@",
                comment: "Label showing app version string"
            ), "\(version)")
        }
        return ""
    }
    ...
    var body: some View {
            ...
            Form {
                Section {
                    HStack {
                        Text(String(
                            ...
                            comment: "Section title for server status controls"
                        ))
                        Spacer()
                        Button(action: {
                ...
                if let configErrorMessage = serverManager.configErrorMessage {
                    Section(String(
                        ...
                        comment: "Label shown when server has a configuration error"
                    )) {
                ...
                Section {
                    Toggle(String(
                        localized: "settings.launch-at-login.toggle-title",
                    ...
                    HStack {
                        Text(String(
                            ...
                            comment: "Section title for auth file controls"
                        ))
                        Spacer()
                        Button(String(
                ...
                Section(String(
                    ...
                    comment: "Section title for service providers list"
                )) {
                    ServiceRow(
                        ...
                        helpText: String(
                            ...
                            comment: "Description of Antigravity service and authentication model"
                        ),
                    ...
                    ServiceRow(
                        ...
                        helpText: String(
                            ...
                            comment: "Warning note for Gemini users with multiple projects during authentication"
                        ),
                    ...
                    ServiceRow(
                        ...
                        helpText: String(
                            ...
                            comment: "Description of Kimi browser-based authentication behavior"
                        ),
                    ...
                    ServiceRow(
                        ...
                        helpText: String(
                            ...
                            comment: "Description of models available through GitHub Copilot subscription"
                        ),
                    ...
                    ServiceRow(
                        ...
                        helpText: String(
                            ...
                            comment: "Description of Z.AI GLM access and API key link"
                        ),
                ...
                if !serverManager.customProviders.isEmpty {
                    Section(String(
                        localized: "settings.services.custom-providers.section-title",
                        defaultValue: "Custom Providers",
                        comment: "Section title for custom providers list"
                    )) {
            ...
            VStack(spacing: 4) {
                HStack(spacing: 4) {
                    Text(String(format: String(
                        ...
                        comment: "Intro line for acknowledgements with app version"
                    ), "\(appVersion)"))
                        .font(.caption)
                        .foregroundColor(.secondary)
                    ...
                    Text(String(
                        ...
                        comment: "License label shown in about section"
                    ))
                        .font(.caption)
                        .foregroundColor(.secondary)
                }
                ...
                HStack(spacing: 4) {
                    ...
                    Text(String(
                        ...
                        comment: "Footer copyright statement in about section"
                    ))
                        .font(.caption)
                        .foregroundColor(.secondary)
                }
        ...
        .sheet(isPresented: $showingQwenEmailPrompt) {
            VStack(spacing: 16) {
                ...
                Text(String(
                    ...
                    comment: "Prompt text for Qwen account email input dialog"
                ))
                    .font(.caption)
                    .foregroundColor(.secondary)
                ...
                HStack(spacing: 12) {
                    ...
                    Button(String(
                        localized: "settings.common.continue",
                        defaultValue: "Continue",
                        comment: "Action button title to continue dialog flow"
                    )) {
                        showingQwenEmailPrompt = false
        ...
        .sheet(isPresented: $showingZaiApiKeyPrompt) {
            VStack(spacing: 16) {
                ...
                Text(String(
                    ...
                    comment: "Prompt text for Z.AI API key input dialog with source URL"
                ))
                    .font(.caption)
                    .foregroundColor(.secondary)
                ...
                HStack(spacing: 12) {
                    ...
                    Button(String(
                        localized: "settings.api-keys.add-key",
                        defaultValue: "Add Key",
                        comment: "Button title to add an API key in dialog"
                    )) {
                        showingZaiApiKeyPrompt = false
        ...
        .sheet(item: $selectedCustomProvider, onDismiss: {
            ...
            VStack(spacing: 16) {
                Text(String(format: String(
                    localized: "settings.provider-api-key.title",
                    defaultValue: "%@ API Key",
                    comment: "Dialog title for entering API key for a specific provider"
                ), "\(provider.title)"))
                    .font(.headline)
                Text(String(format: String(
                    localized: "settings.provider-api-key.prompt",
                    defaultValue: "Enter an API key for %@",
                    comment: "Prompt text for provider API key input dialog"
                ), "\(provider.title)"))
                    .font(.caption)
                    .foregroundColor(.secondary)
        ...
        .alert(String(
            ...
            comment: "Alert title for authentication result dialog"
    ...
    private func toggleAccountDisabled(_ account: AuthAccount) {
        if authManager.toggleAccountDisabled(account) {
            ...
            authResultMessage = account.isDisabled
                ? String(format: String(
                    localized: "settings.account-update.enabled",
                    defaultValue: "✓ Enabled %@",
                    comment: "Message shown when an account is enabled"
                ), "\(account.displayName)")
                : String(format: String(
                    localized: "settings.account-update.disabled",
                    defaultValue: "✓ Disabled %@",
                    comment: "Message shown when an account is disabled"
                ), "\(account.displayName)")
            showingAuthResult = true
        } else {
            authResultSuccess = false
            authResultMessage = String(format: String(
                localized: "settings.account-update.failed",
                defaultValue: "Failed to update %@. Please try again.",
                comment: "Error message when updating account status fails"
            ), "\(account.displayName)")
            showingAuthResult = true
        }
    }
    ...
    private func successMessage(for serviceType: ServiceType) -> String {
        ...
        case .claude:
            return String(
                localized: "settings.authentication.browser-opened.claude-code",
                defaultValue: "🌐 Browser opened for Claude Code authentication.\n\nPlease complete the login in your browser.\n\nThe app will automatically detect your credentials.",
                comment: "Instruction message shown after opening browser for Claude Code authentication"
            )
        case .codex:
            return String(
                localized: "settings.authentication.browser-opened.codex",
                defaultValue: "🌐 Browser opened for Codex authentication.\n\nPlease complete the login in your browser.\n\nThe app will automatically detect your credentials.",
                comment: "Instruction message shown after opening browser for Codex authentication"
            )
        case .copilot:
            return String(
                localized: "settings.authentication.browser-opened.github-copilot-device-flow",
                defaultValue: "🌐 GitHub Copilot authentication started!\n\nPlease visit github.com/login/device and enter the code shown.\n\nThe app will automatically detect your credentials.",
                comment: "Instruction message for GitHub Copilot device authentication flow"
            )
        case .gemini:
            return String(
                localized: "settings.authentication.browser-opened.gemini",
                ...
                comment: "Instruction message shown after opening browser for Gemini authentication with project note"
            )
        case .kimi:
            return String(
                localized: "settings.authentication.browser-opened.kimi",
                defaultValue: "🌐 Browser opened for Kimi authentication.\n\nPlease complete the login in your browser.\n\nThe app will automatically detect your Kimi account.",
                comment: "Instruction message shown after opening browser for Kimi authentication"
            )
        case .qwen:
            return String(
                localized: "settings.authentication.browser-opened.qwen",
                defaultValue: "🌐 Browser opened for Qwen authentication.\n\nPlease complete the login in your browser.",
                comment: "Instruction message shown after opening browser for Qwen authentication"
            )
        case .antigravity:
            return String(
                localized: "settings.authentication.browser-opened.antigravity",
                defaultValue: "🌐 Browser opened for Antigravity authentication.\n\nPlease complete the login in your browser.",
                comment: "Instruction message shown after opening browser for Antigravity authentication"
            )
        case .zai:
            return String(
        ...
        serverManager.saveCustomProviderAPIKey(providerID: provider.id, apiKey: apiKey) { success, output in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    switch output {
                    case String(
                        localized: "settings.api-key.saved-successfully",
                        defaultValue: "API key saved successfully",
                        comment: "Success message title after saving API key"
                    ):
                        self.authResultMessage = String(format: String(
                            localized: "settings.provider-api-key.added-successfully",
                            defaultValue: "✓ %@ API key added successfully.\n\nYou can now use this provider through the proxy.",
                            comment: "Success message after adding API key for a provider"
                        ), "\(provider.title)")
                    case String(
                        ...
                        self.authResultMessage = String(format: String(
                            localized: "settings.provider-api-key.already-in-config",
                    ...
                    case String(
                        localized: "settings.api-key.already-exists",
                        defaultValue: "API key already exists",
                        comment: "Message title shown when attempting to add a duplicate API key"
                    ):
                        self.authResultMessage = String(format: String(
                            localized: "settings.provider-api-key.already-stored",
                            defaultValue: "✓ %@ already has this API key stored.",
                            comment: "Message shown when provider API key is already stored"
                        ), "\(provider.title)")
                    case String(
                        localized: "settings.api-key.re-enabled",
                        defaultValue: "API key was already stored and has been re-enabled",
                        comment: "Message shown when existing API key was re-enabled"
                    ):
                        self.authResultMessage = String(format: String(
                            localized: "settings.provider-api-key.re-enabled",
                            defaultValue: "✓ %@ already had this API key stored, and it has been re-enabled.",
                            comment: "Message shown when provider API key was previously stored and has been re-enabled"
                        ), "\(provider.title)")
                    default:
                        self.authResultMessage = output
                    }
    ...
    private func toggleCustomProviderCredential(provider: CustomProviderDefinition, credential: CustomProviderCredential) {
        if serverManager.toggleCustomProviderCredentialDisabled(credential) {
            ...
            authResultMessage = credential.isDisabled
                ? String(format: String(
                    localized: "settings.provider-api-key.enabled",
                    defaultValue: "✓ Enabled %@ for %@",
                    comment: "Success message when enabling a provider credential"
                ), "\(credential.label)", "\(provider.title)")
                : String(format: String(
                    localized: "settings.provider-api-key.disabled",
            ...
            authResultSuccess = false
            authResultMessage = String(format: String(
                localized: "settings.provider-api-key.update-failed",
                defaultValue: "Failed to update %@ for %@. Please try again.",
                comment: "Failure message when updating a provider credential status"
            ), "\(credential.label)", "\(provider.title)")
        }
        showingAuthResult = true
    }
    ...
    private func disconnectCustomProviderCredential(provider: CustomProviderDefinition, credential: CustomProviderCredential) {
        if serverManager.deleteCustomProviderCredential(credential) {
            authResultSuccess = true
            authResultMessage = String(format: String(
                localized: "settings.provider-api-key.removed",
            ...
            authResultSuccess = false
            authResultMessage = String(format: String(
                localized: "settings.provider-api-key.remove-failed",
                defaultValue: "Failed to remove %@ from %@",
                comment: "Failure message when removing provider credential"
            ), "\(credential.label)", "\(provider.title)")
        }
        showingAuthResult = true
    }
    ...
    private func disconnectAccount(_ account: AuthAccount) {
        ...
        let cleanup = {
            if self.authManager.deleteAccount(account) {
                self.authResultSuccess = true
                self.authResultMessage = String(format: String(
                    localized: "settings.account.removed",
                    defaultValue: "✓ Removed %@ from %@",
                    comment: "Success message when removing connected account from service type"
                ...
                self.authResultMessage = String(
                    ...
                    comment: "Failure message when removing connected account"
                )
            }
            self.showingAuthResult = true
