...
struct VercelGatewayControls: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 6) {
            ...
            .help(String(localized: "settings.vercel.help", defaultValue: "Route Claude requests through Vercel AI Gateway for safer access to your Claude Max subscription", comment: "Help text for Vercel Gateway feature"))
...
struct ServiceRow<ExtraContent: View>: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 4) {
            ...
            if isEnabled {
                if !accounts.isEmpty {
                    // Collapsible summary
                    HStack(spacing: 4) {
                        Text(String(localized: "settings.provider.accounts-count", defaultValue: "\(accounts.count) connected account\(accounts.count == 1 ? "" : "s")", comment: "Shows number of connected accounts"))
                            .font(.caption)
                            .foregroundColor(.green)

                        if accounts.count > 1 {
                            Text(String(localized: "settings.provider.load-balancing", defaultValue: "• Round-robin w/ auto-failover", comment: "Describes load balancing behavior"))
                                .font(.caption)
                                .foregroundColor(.secondary)
                        }
        ...
        .alert(String(localized: "settings.account.remove-alert-title", defaultValue: "Remove Account", comment: "Alert title for removing account"), isPresented: $showingRemoveConfirmation) {
            ...
            if let account = accountToRemove {
                Text(String(format: NSLocalizedString("settings.account.remove-confirmation", comment: "Confirmation message to remove account"), account.displayName, serviceType.displayName))
            }
        }
    }
}
...
struct SettingsView: View {
    ...
    var body: some View {
            ...
            Form {
                ...
                Section(String(localized: "settings.services", defaultValue: "Services", comment: "Services section header")) {
                    ...
                    ServiceRow(
                        ...
                        customTitle: serverManager.vercelGatewayEnabled && !serverManager.vercelApiKey.isEmpty ? String(localized: "settings.claude.vercel-title", defaultValue: "Claude Code (via Vercel)", comment: "Title for Claude when using Vercel Gateway") : nil,
        ...
        .sheet(isPresented: $showingQwenEmailPrompt) {
            VStack(spacing: 16) {
                Text(String(localized: "settings.qwen.email-prompt.title", defaultValue: "Qwen Account Email", comment: "Title for Qwen email prompt sheet"))
                    .font(.headline)
        ...
        .sheet(isPresented: $showingZaiApiKeyPrompt) {
            VStack(spacing: 16) {
                Text(String(localized: "settings.zai.api-key-prompt.title", defaultValue: "Z.AI API Key", comment: "Title for Z.AI API key prompt sheet"))
                    .font(.headline)
        ...
        .alert(String(localized: "settings.auth-result.title", defaultValue: "Authentication Result", comment: "Alert title for authentication result"), isPresented: $showingAuthResult) {
            Button(String(localized: "settings.auth-result.ok", defaultValue: "OK", comment: "OK button in auth result alert"), role: .cancel) { }
        } message: {
        ...
        serverManager.runAuthCommand(command) { success, output in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    let noOutput = String(localized: "settings.auth-result.no-output", defaultValue: "No output from authentication process", comment: "Fallback when auth process has no output")
                    let details = output.isEmpty ? noOutput : output
                    self.authResultMessage = String(format: NSLocalizedString("settings.auth-result.failed", comment: "Authentication failure message"), details)
                    self.showingAuthResult = true
                }
            }
        }
        ...
        serverManager.runAuthCommand(.qwenLogin(email: email)) { success, output in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    self.authResultSuccess = false
                    let noOutput = String(localized: "settings.auth-result.qwen-no-output", defaultValue: "No output", comment: "Fallback when Qwen auth has no output")
                    let details = output.isEmpty ? noOutput : output
        ...
        serverManager.saveZaiApiKey(apiKey) { success, output in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    self.authResultSuccess = false
                    let unknownError = String(localized: "settings.auth-result.zai-unknown-error", defaultValue: "Unknown error", comment: "Fallback when Z.AI save has unknown error")
                    let details = output.isEmpty ? unknownError : output
                    self.authResultMessage = String(format: NSLocalizedString("settings.auth-result.zai-save-failed", comment: "Z.AI API key save failure message"), details)
                    self.showingAuthResult = true
                }
            }
        }
