            ...
            if isEnabled {
                if !accounts.isEmpty {
                    ...
                    HStack(spacing: 4) {
                        ...
                        if accounts.count > 1 {
                            Text(String(localized: "settings.provider.round-robin", defaultValue: "• Round-robin w/ auto-failover", comment: "Description of multi-account load balancing"))
                                .font(.caption)
                                .foregroundColor(.secondary)
                        }
        ...
        VStack(spacing: 0) {
            Form {
                ...
                Section(String(localized: "settings.services", defaultValue: "Services", comment: "Section header for services")) {
                    ...
                    ServiceRow(
                        ...
                        customTitle: serverManager.vercelGatewayEnabled && !serverManager.vercelApiKey.isEmpty ? String(localized: "settings.service.claude.vercel-title", defaultValue: "Claude Code (via Vercel)", comment: "Title for Claude when using Vercel gateway") : nil,
            ...
            VStack(spacing: 4) {
                ...
                HStack(spacing: 4) {
                    ...
                    Text(String(localized: "settings.footer.rights", defaultValue: "All rights reserved.", comment: "Footer rights text"))
                        .font(.caption)
                        .foregroundColor(.secondary)
                }
        ...
        .sheet(isPresented: $showingQwenEmailPrompt) {
            VStack(spacing: 16) {
                ...
                TextField(String(localized: "settings.qwen.prompt.placeholder", defaultValue: "your.email@example.com", comment: "Placeholder for Qwen email field"), text: $qwenEmail)
                    ...
                    .frame(width: 250)
                HStack(spacing: 12) {
                    Button(String(localized: "settings.remove-account.cancel", defaultValue: "Cancel", comment: "Cancel button for remove account alert")) {
        ...
        serverManager.saveZaiApiKey(apiKey) { success, output in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    self.authResultSuccess = false
                    let details = output.isEmpty ? String(localized: "settings.auth.error.zai-unknown", defaultValue: "Unknown error", comment: "Error when Z.AI API key save fails with no details") : output
