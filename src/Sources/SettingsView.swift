        ...
        VStack(alignment: .leading, spacing: 6) {
            ...
            if serverManager.vercelGatewayEnabled {
                HStack(spacing: 8) {
                    ...
                    if showingSaved {
                        Text(String(localized: "settings.vercel.saved", defaultValue: "Saved", comment: "Confirmation message that API key was saved"))
                            .font(.caption)
...
struct ServiceRow<ExtraContent: View>: View {
    ...
    var body: some View {
            ...
            if isEnabled {
                if !accounts.isEmpty {
                    ...
                    HStack(spacing: 4) {
                        ...
                        if accounts.count > 1 {
                            Text(String(localized: "settings.provider.round-robin", defaultValue: "• Round-robin w/ auto-failover", comment: "Label showing round-robin with auto-failover for multiple accounts"))
                                .font(.caption)
                                .foregroundColor(.secondary)
                        }
        ...
        .alert(String(localized: "settings.alert.remove-account-title", defaultValue: "Remove Account", comment: "Title for remove account confirmation alert"), isPresented: $showingRemoveConfirmation) {
            Button(String(localized: "settings.alert.cancel", defaultValue: "Cancel", comment: "Cancel button for remove account alert"), role: .cancel) {
                accountToRemove = nil
            }
            ...
            if let account = accountToRemove {
                Text(String(format: String(localized: "settings.alert.remove-account-message", defaultValue: "Are you sure you want to remove %@ from %@?", comment: "Confirmation message to remove account from provider"), account.displayName, serviceType.displayName))
            }
        }
    }
}
...
struct SettingsView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            Form {
                ...
                Section(String(localized: "settings.services", defaultValue: "Services", comment: "Section header for services list")) {
                    ...
                    ServiceRow(
                        ...
                        customTitle: serverManager.vercelGatewayEnabled && !serverManager.vercelApiKey.isEmpty ? String(localized: "settings.claude.via-vercel", defaultValue: "Claude Code (via Vercel)", comment: "Custom title when Claude is routed via Vercel") : nil,
            ...
            VStack(spacing: 4) {
                ...
                HStack(spacing: 4) {
                    ...
                    Text(String(localized: "settings.footer.rights", defaultValue: "All rights reserved.", comment: "Footer copyright rights text"))
                        .font(.caption)
                        .foregroundColor(.secondary)
                }
        ...
        .sheet(isPresented: $showingQwenEmailPrompt) {
            VStack(spacing: 16) {
                ...
                TextField(String(localized: "settings.qwen.email-prompt.placeholder", defaultValue: "your.email@example.com", comment: "Placeholder for Qwen email field"), text: $qwenEmail)
                    ...
                    .frame(width: 250)
                HStack(spacing: 12) {
                    Button(String(localized: "settings.alert.cancel", defaultValue: "Cancel", comment: "Cancel button for remove account alert")) {
        ...
        .alert(String(localized: "settings.alert.auth-result-title", defaultValue: "Authentication Result", comment: "Title for authentication result alert"), isPresented: $showingAuthResult) {
            Button(String(localized: "settings.alert.ok", defaultValue: "OK", comment: "OK button for alerts"), role: .cancel) { }
        } message: {
        ...
        serverManager.saveZaiApiKey(apiKey) { success, output in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    self.authResultSuccess = false
                    let details = output.isEmpty ? String(localized: "settings.auth.error.zai-unknown", defaultValue: "Unknown error", comment: "Error when Z.AI key save fails with no details") : output
