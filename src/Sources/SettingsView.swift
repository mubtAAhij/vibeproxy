...
struct VercelGatewayControls: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 6) {
            ...
            .help(String(localized: "settings.vercel.help", defaultValue: "Route Claude requests through Vercel AI Gateway for safer access to your Claude Max subscription", comment: "Help text explaining Vercel AI Gateway functionality"))
...
struct ServiceRow<ExtraContent: View>: View {
    ...
    var body: some View {
            ...
            if isEnabled {
                if !accounts.isEmpty {
                    ...
                    HStack(spacing: 4) {
                        ...
                        Text(String(localized: "settings.provider.connected-accounts", defaultValue: "\(accountCount) connected account\(accountCount == 1 ? "" : "s")", comment: "Count of connected accounts for a provider"))
                            .font(.caption)
                            .foregroundColor(.green)

                        if accounts.count > 1 {
                            Text(String(localized: "settings.provider.round-robin", defaultValue: "• Round-robin w/ auto-failover", comment: "Indicator that multiple accounts use round-robin load balancing"))
                                .font(.caption)
                                .foregroundColor(.secondary)
                        }
                    ...
                    Text(String(localized: "settings.provider.no-accounts", defaultValue: "No connected accounts", comment: "Message when no accounts are connected for a provider"))
                        .font(.caption)
                        .foregroundColor(.secondary)
                        .padding(.leading, 28)
                }
        ...
        .alert(String(localized: "settings.remove-account.title", defaultValue: "Remove Account", comment: "Title for remove account confirmation dialog"), isPresented: $showingRemoveConfirmation) {
            Button(String(localized: "settings.remove-account.cancel", defaultValue: "Cancel", comment: "Cancel button in remove account dialog"), role: .cancel) {
                accountToRemove = nil
            }
    ...
    var body: some View {
        VStack(spacing: 0) {
            Form {
                ...
                Section(String(localized: "settings.services", defaultValue: "Services", comment: "Section header for AI service providers")) {
                    ServiceRow(
                        ...
                        helpText: String(localized: "settings.antigravity.help", defaultValue: "Antigravity provides OAuth-based access to various AI models including Gemini and Claude. One login gives you access to multiple AI services.", comment: "Help text describing Antigravity service"),
                    ...
                    ServiceRow(
                        ...
                        customTitle: serverManager.vercelGatewayEnabled && !serverManager.vercelApiKey.isEmpty ? String(localized: "settings.claude.via-vercel", defaultValue: "Claude Code (via Vercel)", comment: "Title when Claude is routed via Vercel") : nil,
                    ...
                    ServiceRow(
                        ...
                        helpText: String(localized: "settings.gemini.help", defaultValue: "⚠️ Note: If you're an existing Gemini user with multiple projects, authentication will use your default project. Set your desired project as default in Google AI Studio before connecting.", comment: "Help text about Gemini project selection"),
                    ...
                    ServiceRow(
                        ...
                        helpText: String(localized: "settings.copilot.help", defaultValue: "GitHub Copilot provides access to Claude, GPT, Gemini and other models via your Copilot subscription.", comment: "Help text describing GitHub Copilot service"),
                    ...
                    ServiceRow(
                        ...
                        helpText: String(localized: "settings.zai.help", defaultValue: "Z.AI GLM provides access to GLM-4.7 and other models via API key. Get your key at https://z.ai/manage-apikey/apikey-list", comment: "Help text describing Z.AI service"),
            ...
            VStack(spacing: 4) {
                ...
                HStack(spacing: 4) {
                    ...
                    Text(String(localized: "settings.footer.rights-reserved", defaultValue: "All rights reserved.", comment: "Copyright text"))
                        .font(.caption)
                        .foregroundColor(.secondary)
                }
        ...
        .sheet(isPresented: $showingQwenEmailPrompt) {
            VStack(spacing: 16) {
                Text(String(localized: "settings.qwen.email-prompt.title", defaultValue: "Qwen Account Email", comment: "Title for Qwen email prompt dialog"))
                    .font(.headline)
                Text(String(localized: "settings.qwen.email-prompt.message", defaultValue: "Enter your Qwen account email address", comment: "Instruction for Qwen email input"))
                    .font(.caption)
                    .foregroundColor(.secondary)
                TextField(String(localized: "settings.qwen.email-prompt.placeholder", defaultValue: "your.email@example.com", comment: "Placeholder for Qwen email input"), text: $qwenEmail)
                    .textFieldStyle(.roundedBorder)
                    .frame(width: 250)
                HStack(spacing: 12) {
                    Button(String(localized: "settings.qwen.email-prompt.cancel", defaultValue: "Cancel", comment: "Cancel button in Qwen email dialog")) {
                        showingQwenEmailPrompt = false
                        qwenEmail = ""
                    }
                    Button(String(localized: "settings.qwen.email-prompt.continue", defaultValue: "Continue", comment: "Continue button in Qwen email dialog")) {
                        showingQwenEmailPrompt = false
        ...
        .sheet(isPresented: $showingZaiApiKeyPrompt) {
            VStack(spacing: 16) {
                Text(String(localized: "settings.zai.api-key-prompt.title", defaultValue: "Z.AI API Key", comment: "Title for Z.AI API key prompt dialog"))
                    .font(.headline)
                Text(String(localized: "settings.zai.api-key-prompt.message", defaultValue: "Enter your Z.AI API key from https://z.ai/manage-apikey/apikey-list", comment: "Instruction for Z.AI API key input"))
                    .font(.caption)
                    .foregroundColor(.secondary)
                TextField("", text: $zaiApiKey)
                    ...
                    .frame(width: 300)
                HStack(spacing: 12) {
                    Button(String(localized: "settings.zai.api-key-prompt.cancel", defaultValue: "Cancel", comment: "Cancel button in Z.AI API key dialog")) {
        ...
        .alert(String(localized: "settings.auth-result.title", defaultValue: "Authentication Result", comment: "Title for authentication result dialog"), isPresented: $showingAuthResult) {
            Button(String(localized: "settings.auth-result.ok", defaultValue: "OK", comment: "OK button in authentication result dialog"), role: .cancel) { }
        } message: {
        ...
        serverManager.runAuthCommand(command) { success, output in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    let noOutputMessage = String(localized: "settings.auth.no-output", defaultValue: "No output from authentication process", comment: "Message when auth process provides no output")
                    self.authResultMessage = String(format: NSLocalizedString("settings.auth.failed", comment: "Generic authentication failure message"), output.isEmpty ? noOutputMessage : output)
                    self.showingAuthResult = true
                }
            }
        }
        ...
        serverManager.runAuthCommand(.qwenLogin(email: email)) { success, output in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    self.authResultSuccess = false
                    let noOutputMessage = String(localized: "settings.auth.qwen.no-output", defaultValue: "No output", comment: "Message when Qwen auth provides no output")
                    self.authResultMessage = String(format: NSLocalizedString("settings.auth.qwen.failed", comment: "Qwen authentication failure message"), output.isEmpty ? noOutputMessage : output)
                    self.showingAuthResult = true
                }
            }
        }
        ...
        serverManager.saveZaiApiKey(apiKey) { success, output in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    self.authResultSuccess = false
                    let unknownErrorMessage = String(localized: "settings.auth.zai.unknown-error", defaultValue: "Unknown error", comment: "Message when Z.AI key save has unknown error")
                    self.authResultMessage = String(format: NSLocalizedString("settings.auth.zai.save-failed", comment: "Z.AI API key save failure message"), output.isEmpty ? unknownErrorMessage : output)
                    self.showingAuthResult = true
                }
            }
        }
