...
struct ServiceRow<ExtraContent: View>: View {
    ...
    var body: some View {
            ...
            if isEnabled {
                if !accounts.isEmpty {
                    ...
                    HStack(spacing: 4) {
                        ...
                        if accounts.count > 1 {
                            Text(String(localized: "settings.provider.round-robin", defaultValue: "• Round-robin w/ auto-failover", comment: "Label explaining load balancing behavior"))
                                .font(.caption)
                                .foregroundColor(.secondary)
                        }
        ...
        .alert(String(localized: "settings.alert.remove-account.title", defaultValue: "Remove Account", comment: "Alert title for removing an account"), isPresented: $showingRemoveConfirmation) {
            Button(String(localized: "settings.alert.remove-account.cancel", defaultValue: "Cancel", comment: "Cancel button in remove account alert"), role: .cancel) {
                accountToRemove = nil
            }
            Button(String(localized: "settings.alert.remove-account.confirm", defaultValue: "Remove", comment: "Confirm button to remove account"), role: .destructive) {
            ...
            if let account = accountToRemove {
                Text(String(format: NSLocalizedString("settings.alert.remove-account.message", comment: "Confirmation message asking if user wants to remove an account"), account.displayName, serviceType.displayName))
            }
        }
    }
}
...
struct SettingsView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            Form {
                ...
                Section(String(localized: "settings.services.section", defaultValue: "Services", comment: "Section header for services")) {
                    ServiceRow(
                        ...
                        helpText: String(localized: "settings.services.antigravity.help", defaultValue: "Antigravity provides OAuth-based access to various AI models including Gemini and Claude. One login gives you access to multiple AI services.", comment: "Help text describing Antigravity service"),
                    ...
                    ServiceRow(
                        ...
                        helpText: String(localized: "settings.services.gemini.help", defaultValue: "⚠️ Note: If you're an existing Gemini user with multiple projects, authentication will use your default project. Set your desired project as default in Google AI Studio before connecting.", comment: "Help text describing Gemini multi-project behavior"),
                    ...
                    ServiceRow(
                        ...
                        helpText: String(localized: "settings.services.copilot.help", defaultValue: "GitHub Copilot provides access to Claude, GPT, Gemini and other models via your Copilot subscription.", comment: "Help text describing GitHub Copilot service"),
                    ...
                    ServiceRow(
                        ...
                        helpText: String(localized: "settings.services.zai.help", defaultValue: "Z.AI GLM provides access to GLM-4.7 and other models via API key. Get your key at https://z.ai/manage-apikey/apikey-list", comment: "Help text describing Z.AI service"),
            ...
            VStack(spacing: 4) {
                ...
                HStack(spacing: 4) {
                    ...
                    Text(String(localized: "settings.footer.copyright", defaultValue: "All rights reserved.", comment: "Copyright footer text"))
                        .font(.caption)
                        .foregroundColor(.secondary)
                }
        ...
        .sheet(isPresented: $showingQwenEmailPrompt) {
            VStack(spacing: 16) {
                Text(String(localized: "settings.qwen.prompt.title", defaultValue: "Qwen Account Email", comment: "Title for Qwen email input prompt"))
                    .font(.headline)
                Text(String(localized: "settings.qwen.prompt.message", defaultValue: "Enter your Qwen account email address", comment: "Message prompting for Qwen email address"))
                    .font(.caption)
                    .foregroundColor(.secondary)
                TextField(String(localized: "settings.qwen.prompt.placeholder", defaultValue: "your.email@example.com", comment: "Placeholder for Qwen email input field"), text: $qwenEmail)
                    .textFieldStyle(.roundedBorder)
                    .frame(width: 250)
                HStack(spacing: 12) {
                    Button(String(localized: "settings.qwen.prompt.cancel", defaultValue: "Cancel", comment: "Cancel button in Qwen email prompt")) {
        ...
        .sheet(isPresented: $showingZaiApiKeyPrompt) {
            VStack(spacing: 16) {
                Text(String(localized: "settings.zai.prompt.title", defaultValue: "Z.AI API Key", comment: "Title for Z.AI API key input prompt"))
                    .font(.headline)
                Text(String(localized: "settings.zai.prompt.message", defaultValue: "Enter your Z.AI API key from https://z.ai/manage-apikey/apikey-list", comment: "Message prompting for Z.AI API key"))
                    .font(.caption)
                    .foregroundColor(.secondary)
                TextField("", text: $zaiApiKey)
                    ...
                    .frame(width: 300)
                HStack(spacing: 12) {
                    Button(String(localized: "settings.zai.prompt.cancel", defaultValue: "Cancel", comment: "Cancel button in Z.AI API key prompt")) {
        ...
        serverManager.runAuthCommand(command) { success, output in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    let details = output.isEmpty ? String(localized: "settings.auth.error.no-output", defaultValue: "No output from authentication process", comment: "Fallback message when authentication fails with no output") : output
                    self.authResultMessage = String(format: NSLocalizedString("settings.auth.error.failed-with-details", comment: "Error message when authentication fails with details"), details)
                    self.showingAuthResult = true
                }
            }
        }
        ...
        serverManager.runAuthCommand(.qwenLogin(email: email)) { success, output in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    self.authResultSuccess = false
                    let details = output.isEmpty ? String(localized: "settings.auth.error.qwen-no-output", defaultValue: "No output", comment: "Fallback message when Qwen authentication fails with no output") : output
                    self.authResultMessage = String(format: NSLocalizedString("settings.auth.error.qwen-failed", comment: "Error message when Qwen authentication fails"), details)
                    self.showingAuthResult = true
                }
            }
        }
        ...
        serverManager.saveZaiApiKey(apiKey) { success, output in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    self.authResultSuccess = false
                    let details = output.isEmpty ? String(localized: "settings.auth.error.zai-unknown", defaultValue: "Unknown error", comment: "Fallback message when Z.AI API key save fails with unknown error") : output
                    self.authResultMessage = String(format: NSLocalizedString("settings.auth.error.zai-failed", comment: "Error message when Z.AI API key save fails"), details)
                    self.showingAuthResult = true
                }
            }
        }
