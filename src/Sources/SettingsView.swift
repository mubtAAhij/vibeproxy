    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            if account.isExpired {
                Text(String(localized: "settings.account.expired", defaultValue: "(expired)", comment: "Label showing an account has expired"))
                    .font(.caption2)
            ...
            if serverManager.vercelGatewayEnabled {
                HStack(spacing: 8) {
                    ...
                    if showingSaved {
                        ...
                        Button(String(localized: "settings.vercel.save", defaultValue: "Save", comment: "Button to save Vercel settings")) {
                            showingSaved = true
...
struct ServiceRow<ExtraContent: View>: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 4) {
            ...
            HStack {
                ...
                if isAuthenticating {
                    ...
                    Button(String(localized: "settings.provider.add-account", defaultValue: "Add Account", comment: "Button to add an account for a provider")) {
                        onConnect()
                    }
                    .controlSize(.small)
                }
            ...
            if isEnabled {
                if !accounts.isEmpty {
                    ...
                    HStack(spacing: 4) {
                        ...
                        if accounts.count > 1 {
                            Text(String(localized: "settings.provider.round-robin", defaultValue: "• Round-robin w/ auto-failover", comment: "Indicates round-robin load balancing with failover"))
                                .font(.caption)
                                .foregroundColor(.secondary)
                        }
        ...
        .alert(String(localized: "settings.account.remove-title", defaultValue: "Remove Account", comment: "Alert title for removing an account"), isPresented: $showingRemoveConfirmation) {
            Button(String(localized: "settings.account.remove-cancel", defaultValue: "Cancel", comment: "Cancel removing account"), role: .cancel) {
                accountToRemove = nil
            }
            ...
            if let account = accountToRemove {
                Text(String(format: NSLocalizedString("settings.account.remove-confirmation", comment: "Confirmation message for removing account"), account.displayName, serviceType.displayName))
            }
        }
    }
}
...
struct SettingsView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            Form {
                ...
                Section(String(localized: "settings.services", defaultValue: "Services", comment: "Services section header")) {
                    ...
                    ServiceRow(
                        ...
                        customTitle: serverManager.vercelGatewayEnabled && !serverManager.vercelApiKey.isEmpty ? String(localized: "settings.service.claude.vercel-title", defaultValue: "Claude Code (via Vercel)", comment: "Title when Claude uses Vercel gateway") : nil,
            ...
            VStack(spacing: 4) {
                HStack(spacing: 4) {
                    Text(String(format: NSLocalizedString("settings.footer.attribution", comment: "Attribution text in footer"), appVersion))
                        .font(.caption)
                        .foregroundColor(.secondary)
        ...
        .sheet(isPresented: $showingQwenEmailPrompt) {
            VStack(spacing: 16) {
                Text(String(localized: "settings.qwen.title", defaultValue: "Qwen Account Email", comment: "Title for Qwen email prompt"))
                    .font(.headline)
                Text(String(localized: "settings.qwen.prompt", defaultValue: "Enter your Qwen account email address", comment: "Prompt for Qwen email input"))
                    .font(.caption)
                    .foregroundColor(.secondary)
                TextField(String(localized: "settings.qwen.placeholder", defaultValue: "your.email@example.com", comment: "Placeholder for Qwen email field"), text: $qwenEmail)
                    .textFieldStyle(.roundedBorder)
                    .frame(width: 250)
                HStack(spacing: 12) {
        ...
        .sheet(isPresented: $showingZaiApiKeyPrompt) {
            VStack(spacing: 16) {
                Text(String(localized: "settings.zai.title", defaultValue: "Z.AI API Key", comment: "Title for Z.AI API key prompt"))
                    .font(.headline)
                Text(String(localized: "settings.zai.prompt", defaultValue: "Enter your Z.AI API key from https://z.ai/manage-apikey/apikey-list", comment: "Prompt for Z.AI API key input"))
                    .font(.caption)
                    .foregroundColor(.secondary)
                TextField("", text: $zaiApiKey)
                    ...
                    .frame(width: 300)
                HStack(spacing: 12) {
                    Button(String(localized: "settings.zai.cancel", defaultValue: "Cancel", comment: "Cancel Z.AI API key prompt")) {
        ...
        serverManager.runAuthCommand(command) { success, output in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    self.authResultMessage = String(format: NSLocalizedString("settings.auth.failed-details", comment: "Auth failure message with details"), details)
                    self.showingAuthResult = true
                }
            }
        }
        ...
        serverManager.runAuthCommand(.qwenLogin(email: email)) { success, output in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    self.authResultMessage = String(format: NSLocalizedString("settings.auth.qwen-failed", comment: "Qwen auth failure message with details"), details)
                    self.showingAuthResult = true
                }
            }
        }
        ...
        serverManager.saveZaiApiKey(apiKey) { success, output in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    self.authResultSuccess = false
                    let details = output.isEmpty ? String(localized: "settings.auth.zai-unknown-error", defaultValue: "Unknown error", comment: "Message when Z.AI key save fails with unknown error") : output
                    self.authResultMessage = String(format: NSLocalizedString("settings.auth.zai-failed", comment: "Z.AI key save failure message with details"), details)
                    self.showingAuthResult = true
                }
            }
        }
    ...
    private func disconnectAccount(_ account: AuthAccount) {
        ...
        let cleanup = {
            if self.authManager.deleteAccount(account) {
                ...
                self.authResultSuccess = false
                self.authResultMessage = String(localized: "settings.account.removed-failed", defaultValue: "Failed to remove account", comment: "Error when account removal fails")
            }
            self.showingAuthResult = true
