...
struct ServiceRow<ExtraContent: View>: View {
    ...
    var body: some View {
            ...
            if isEnabled {
                if !accounts.isEmpty {
                    ...
                    HStack(spacing: 4) {
                        ...
                        if accounts.count > 1 {
                            Text(String(localized: "settings.provider.round-robin", defaultValue: "• Round-robin w/ auto-failover", comment: "Load balancing strategy description"))
                                .font(.caption)
                                .foregroundColor(.secondary)
                        }
        ...
        .alert(String(localized: "settings.provider.remove-account-title", defaultValue: "Remove Account", comment: "Alert title for account removal confirmation"), isPresented: $showingRemoveConfirmation) {
            ...
            if let account = accountToRemove {
                Text(String(format: NSLocalizedString("settings.provider.remove-account-message", comment: "Confirmation message for account removal"), account.displayName, serviceType.displayName))
            }
        }
    }
}
        ...
        VStack(spacing: 0) {
            Form {
                ...
                Section {
                    ...
                    HStack {
                        Text(String(localized: "settings.general.auth-files", defaultValue: "Auth files", comment: "Label for authentication files section"))
                        Spacer()
                        Button(String(localized: "settings.general.open-folder", defaultValue: "Open Folder", comment: "Button to open authentication folder")) {
                            openAuthFolder()
                        }
                    }
                }
                ...
                Section(String(localized: "settings.section.services", defaultValue: "Services", comment: "Section header for services")) {
                    ...
                    ServiceRow(
                        ...
                        customTitle: serverManager.vercelGatewayEnabled && !serverManager.vercelApiKey.isEmpty ? String(localized: "settings.provider.claude.vercel-title", defaultValue: "Claude Code (via Vercel)", comment: "Custom title for Claude when using Vercel") : nil,
        ...
        serverManager.runAuthCommand(command) { success, output in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    let details = output.isEmpty ? String(localized: "settings.auth.error.no-output", defaultValue: "No output from authentication process", comment: "Error message when no output from authentication process") : output
        ...
        serverManager.runAuthCommand(.qwenLogin(email: email)) { success, output in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    self.authResultSuccess = false
                    let details = output.isEmpty ? String(localized: "settings.auth.error.qwen-no-output", defaultValue: "No output", comment: "Error message when no output from Qwen authentication") : output
        ...
        serverManager.saveZaiApiKey(apiKey) { success, output in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    self.authResultSuccess = false
                    let details = output.isEmpty ? String(localized: "settings.auth.error.zai-unknown", defaultValue: "Unknown error", comment: "Error message when Z.AI save fails with unknown error") : output
    ...
    private func disconnectAccount(_ account: AuthAccount) {
        ...
        let cleanup = {
            if self.authManager.deleteAccount(account) {
                self.authResultSuccess = true
                self.authResultMessage = String(format: NSLocalizedString("settings.account.remove-success", comment: "Success message for account removal"), account.displayName, account.type.displayName)
            } else {
                self.authResultSuccess = false
