        ...
        HStack(spacing: 8) {
            ...
            if account.isExpired && !account.isDisabled {
                Text(String(
                    ...
                    comment: "Status suffix shown for an expired account"
                ))
                    .font(.caption2)
            ...
            if account.isDisabled {
                Text(String(
                    ...
                    comment: "Status suffix shown for a disabled account"
                ))
                    .font(.caption2)
            ...
            if showDisableToggle {
                ...
                .help(!canDisable ? String(
                    ...
                    comment: "Validation message shown when disabling the last enabled account"
                ) : "")
                .onHover { inside in
                    if canDisable {
        ...
        VStack(alignment: .leading, spacing: 6) {
            Toggle(isOn: $serverManager.vercelGatewayEnabled) {
                Text(String(
                    ...
                    comment: "Toggle label for enabling Vercel AI Gateway routing"
                ))
                    .font(.caption)
            }
            ...
            .help(String(
                ...
                comment: "Description text for Vercel AI Gateway routing setting"
            ))
            
            if serverManager.vercelGatewayEnabled {
                HStack(spacing: 8) {
                    Text(String(
                        localized: "settings.vercel-gateway.api-key-field.title",
                    ...
                    if showingSaved {
                        Text(String(
                            ...
                            defaultValue: "Saved",
                            comment: "Status text indicating API key was saved"
                        ))
                            .font(.caption)
                            .foregroundColor(.green)
                    } else {
                        Button(String(
                            localized: "settings.vercel-gateway.api-key-action.save",
                            defaultValue: "Save",
...
struct ServiceRow<ExtraContent: View>: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 4) {
            ...
            HStack {
                ...
                .help(toggleHelpText ?? (isEnabled ? String(
                    localized: "settings.providers.action.disable-provider",
                    defaultValue: "Disable this provider",
                    comment: "Action title to disable a provider"
                ) : String(
                    localized: "settings.providers.action.enable-provider",
                    defaultValue: "Enable this provider",
                    comment: "Action title to enable a provider"
                )))
                ...
                if !accounts.isEmpty {
                    ...
                    HStack(spacing: 4) {
                        Text(String(format: String(
                            ...
                            comment: "Label showing how many accounts are connected"
                        ), accounts.count))
                            .font(.caption)
                            .foregroundColor(.green)
                        ...
                        if enabledCount > 1 {
                            Text(String(
                                localized: "settings.accounts.behavior.round-robin-auto-failover",
                    ...
                    Text(String(
                        ...
                        comment: "Empty-state message when there are no connected accounts"
                    ))
                        .font(.caption)
                        .foregroundColor(.secondary)
                        .padding(.leading, 28)
        ...
        .alert(String(
            localized: "settings.accounts.remove-confirmation.title",
            defaultValue: "Remove Account",
            comment: "Title for remove account confirmation prompt"
        ), isPresented: $showingRemoveConfirmation) {
            Button(String(
                localized: "settings.accounts.remove-confirmation.cancel",
                defaultValue: "Cancel",
                comment: "Cancel button title in remove account confirmation prompt"
            ), role: .cancel) {
                accountToRemove = nil
            }
            Button(String(
                localized: "settings.accounts.remove-confirmation.confirm",
                defaultValue: "Remove",
                comment: "Confirm button title in remove account confirmation prompt"
            ), role: .destructive) {
            ...
            if let account = accountToRemove {
                Text(String(format: String(
                    localized: "settings.accounts.remove-confirmation.message",
        ...
        HStack(spacing: 8) {
            ...
            if credential.isDisabled {
                Text(String(
                    ...
                    defaultValue: "(disabled)",
                    comment: "Status suffix shown for a disabled API key"
                ))
                    .font(.caption2)
                    .foregroundColor(.secondary)
            }
            if showDisableToggle {
                ...
                .help(!canDisable ? String(
                    ...
                    comment: "Validation message shown when disabling the last enabled API key"
                ) : "")
                .onHover { inside in
                    if canDisable {
                        if inside { NSCursor.pointingHand.push() } else { NSCursor.pop() }
...
struct CustomProviderRow: View {
    ...
    private var summaryText: String {
        if totalConfiguredKeyCount == 0 {
            return String(
                ...
                comment: "Empty-state message when no API keys are configured"
            )
        }
        if provider.inlineKeyCount > 0 && !credentials.isEmpty {
            return String(format: String(
                ...
                comment: "Summary line showing total API keys with breakdown from config and added locally"
        ...
        if provider.inlineKeyCount > 0 {
            return String(format: String(
                ...
                comment: "Summary text showing API key count sourced from config"
            ), totalConfiguredKeyCount)
        }
        return String(format: String(
            localized: "settings.api-keys.summary.added-here-count",
            defaultValue: "%d API key(s) added here",
            comment: "Summary text showing API key count added in the app"
        ), totalConfiguredKeyCount)
    }
    ...
    private var poolingStatusText: String? {
        ...
        return String(
            localized: "settings.api-keys.behavior.pooled-across-available-keys",
            defaultValue: "• Pooled across available keys",
            comment: "Bullet point describing key pooling behavior"
        )
    }

    private var endpointSummaryText: String {
        String(format: String(
            localized: "settings.provider.endpoint",
    ...
    private var modelSummaryText: String? {
        ...
        return String(format: String(
            localized: "settings.provider.models",
            defaultValue: "Models: %@",
            comment: "Label showing configured model aliases for provider"
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 4) {
            HStack {
                ...
                .help(isEnabled ? String(
                    localized: "settings.provider.action.disable-provider",
                    defaultValue: "Disable this provider",
                    comment: "Action title to disable a provider in settings"
                ) : String(
                    localized: "settings.provider.action.enable-provider",
                    defaultValue: "Enable this provider",
                    comment: "Action title to enable a provider in settings"
                ))
                ...
                if totalConfiguredKeyCount > 0 {
                    ...
                    if isExpanded {
                        VStack(alignment: .leading, spacing: 6) {
                            ...
                            if provider.inlineKeyCount > 0 {
                                Text(String(format: String(
                                    localized: "settings.api-keys.usage.inline-keys-from-config-path",
                                    defaultValue: "Using %d API key(s) from ~/.cli-proxy-api/config.yaml",
                                    comment: "Inline usage text showing API key count loaded from config file path"
        ...
        .alert(String(
            localized: "settings.api-keys.remove-confirmation.title",
            defaultValue: "Remove API Key",
            comment: "Title for remove API key confirmation prompt"
        ), isPresented: $showingRemoveConfirmation) {
            Button(String(
                localized: "settings.api-keys.remove-confirmation.cancel",
                defaultValue: "Cancel",
                comment: "Cancel button title in remove API key confirmation prompt"
            ), role: .cancel) {
                credentialToRemove = nil
            }
            Button(String(
                localized: "settings.api-keys.remove-confirmation.confirm",
                defaultValue: "Remove",
                comment: "Confirm button title in remove API key confirmation prompt"
            ), role: .destructive) {
            ...
            if let credential = credentialToRemove {
                Text(String(format: String(
                    localized: "settings.api-keys.remove-confirmation.message",
                    defaultValue: "Are you sure you want to remove %@ from %@?",
...
struct SettingsView: View {
    ...
    private var appVersion: String {
        if let version = Bundle.main.infoDictionary?["CFBundleShortVersionString"] as? String {
            return String(format: String(
                localized: "settings.app.version.label",
                defaultValue: "v%@",
                comment: "Version label shown in settings footer"
            ), "\(version)")
        }
        return ""
    }
    ...
    var body: some View {
            ...
            Form {
                Section {
                    HStack {
                        Text(String(
                            ...
                            comment: "Section title for server status in settings"
                        ))
                        Spacer()
                        Button(action: {
                            ...
                            HStack(spacing: 6) {
                                ...
                                Text(serverManager.isRunning ? String(
                                    ...
                                    comment: "Status value shown when local server is running"
                                ) : String(
                                    localized: "settings.server.status.stopped",
                                    defaultValue: "Stopped",
                                    comment: "Status value shown when local server is stopped"
                                ))
                            }
                        }
                        .buttonStyle(.plain)
                ...
                if let configErrorMessage = serverManager.configErrorMessage {
                    Section(String(
                        ...
                        comment: "Status label shown when server has a configuration error"
                    )) {
                ...
                Section {
                    ...
                    HStack {
                        ...
                        Spacer()
                        Button(String(
                            localized: "settings.auth-files.open-folder",
                ...
                Section(String(
                    ...
                    comment: "Section title for connected services in settings"
                )) {
                    ServiceRow(
                    ...
                    ServiceRow(
                        ...
                        customTitle: serverManager.vercelGatewayEnabled && !serverManager.vercelApiKey.isEmpty ? String(
                            ...
                            comment: "Service title for Claude Code via Vercel"
                        ) : nil,
                    ...
                    ServiceRow(
                        ...
                        helpText: String(
                            ...
                            comment: "Warning note for Gemini users about default project selection"
                        ),
                    ...
                    ServiceRow(
                        ...
                        helpText: String(
                            ...
                            comment: "Description text for Z.AI GLM service"
                        ),
                ...
                if !serverManager.customProviders.isEmpty {
                    Section(String(
                        ...
                        comment: "Section title for custom providers configuration"
                    )) {
            ...
            VStack(spacing: 4) {
                HStack(spacing: 4) {
                    Text(String(format: String(
                        ...
                        comment: "Credits introduction line including app version"
                    ), "\(appVersion)"))
                        .font(.caption)
                        .foregroundColor(.secondary)
                    ...
                    Text(String(
                        ...
                        comment: "License label in settings about section"
                    ))
                        .font(.caption)
                        .foregroundColor(.secondary)
                }
                ...
                HStack(spacing: 4) {
                    ...
                    Text(String(
                        ...
                        comment: "Footer legal text in settings about section"
                    ))
                        .font(.caption)
                        .foregroundColor(.secondary)
                }
        ...
        .sheet(isPresented: $showingQwenEmailPrompt) {
            VStack(spacing: 16) {
                Text(String(
                    localized: "settings.qwen-auth.email.title",
                    defaultValue: "Qwen Account Email",
                    comment: "Title for Qwen account email prompt"
                ))
                    .font(.headline)
                Text(String(
                    localized: "settings.qwen-auth.email.message",
                    defaultValue: "Enter your Qwen account email address",
                    comment: "Prompt message requesting Qwen account email"
                ))
                    .font(.caption)
                    .foregroundColor(.secondary)
                ...
                HStack(spacing: 12) {
                    Button(String(
                        localized: "settings.qwen-auth.email.cancel",
                        defaultValue: "Cancel",
                        comment: "Cancel button title in Qwen email prompt"
                    )) {
                        showingQwenEmailPrompt = false
                        qwenEmail = ""
                    }
                    Button(String(
                        localized: "settings.qwen-auth.email.continue",
                        defaultValue: "Continue",
                        comment: "Continue button title in Qwen email prompt"
                    )) {
                        showingQwenEmailPrompt = false
        ...
        .sheet(isPresented: $showingZaiApiKeyPrompt) {
            VStack(spacing: 16) {
                Text(String(
                    localized: "settings.zai-api-key.prompt.title",
                    defaultValue: "Z.AI API Key",
                    comment: "Title for Z.AI API key prompt"
                ))
                    .font(.headline)
                Text(String(
                    localized: "settings.zai-api-key.prompt.message",
                    defaultValue: "Enter your Z.AI API key from https://z.ai/manage-apikey/apikey-list",
                    comment: "Prompt message requesting Z.AI API key"
                ))
                    .font(.caption)
                    .foregroundColor(.secondary)
                ...
                HStack(spacing: 12) {
                    Button(String(
                        localized: "settings.zai-api-key.prompt.cancel",
                        defaultValue: "Cancel",
                        comment: "Cancel button title in Z.AI API key prompt"
                    )) {
                        showingZaiApiKeyPrompt = false
                        zaiApiKey = ""
                    }
                    Button(String(
                        localized: "settings.zai-api-key.prompt.add-key",
                        defaultValue: "Add Key",
                        comment: "Confirm button title to add Z.AI API key"
                    )) {
                        showingZaiApiKeyPrompt = false
        ...
        .sheet(item: $selectedCustomProvider, onDismiss: {
            ...
            VStack(spacing: 16) {
                Text(String(format: String(
                    localized: "settings.provider-api-key.prompt.title",
                    defaultValue: "%@ API Key",
                    comment: "Title for provider API key prompt"
                ), "\(provider.title)"))
                    .font(.headline)
                Text(String(format: String(
                    localized: "settings.provider-api-key.prompt.message",
                    defaultValue: "Enter an API key for %@",
                    comment: "Prompt message requesting API key for selected provider"
                ), "\(provider.title)"))
                    .font(.caption)
                    .foregroundColor(.secondary)
                ...
                HStack(spacing: 12) {
                    Button(String(
                        localized: "settings.provider-api-key.prompt.cancel",
                        defaultValue: "Cancel",
                        comment: "Cancel button title in provider API key prompt"
                    )) {
                        selectedCustomProvider = nil
                        customProviderApiKey = ""
                    }
                    Button(String(
                        localized: "settings.provider-api-key.prompt.add-key",
                        defaultValue: "Add Key",
                        comment: "Confirm button title to add provider API key"
                    )) {
        ...
        .alert(String(
            ...
            Button(String(
                localized: "settings.authentication-result.ok",
                defaultValue: "OK",
                comment: "Confirmation button title for authentication result alert"
            ), role: .cancel) { }
        } message: {
    ...
    private func toggleAccountDisabled(_ account: AuthAccount) {
        if authManager.toggleAccountDisabled(account) {
            ...
            authResultMessage = account.isDisabled
                ? String(format: String(
                    localized: "settings.account-toggle.result.enabled",
                    defaultValue: "✓ Enabled %@",
                    comment: "Success message when account is enabled"
                ), "\(account.displayName)")
                : String(format: String(
                    localized: "settings.account-toggle.result.disabled",
                    defaultValue: "✓ Disabled %@",
                    comment: "Success message when account is disabled"
                ), "\(account.displayName)")
            showingAuthResult = true
        } else {
            authResultSuccess = false
            authResultMessage = String(format: String(
                localized: "settings.account-toggle.result.update-failed",
                defaultValue: "Failed to update %@. Please try again.",
                comment: "Failure message when account enable or disable update fails"
            ), "\(account.displayName)")
            showingAuthResult = true
        }
    }
    ...
    private func successMessage(for serviceType: ServiceType) -> String {
        ...
        case .claude:
            return String(
                localized: "settings.authentication.claude-code.browser-opened-instructions",
                defaultValue: "🌐 Browser opened for Claude Code authentication.\n\nPlease complete the login in your browser.\n\nThe app will automatically detect your credentials.",
                comment: "Instructions shown when Claude Code browser authentication starts"
            )
        case .codex:
            return String(
                localized: "settings.authentication.codex.browser-opened-instructions",
                defaultValue: "🌐 Browser opened for Codex authentication.\n\nPlease complete the login in your browser.\n\nThe app will automatically detect your credentials.",
                comment: "Instructions shown when Codex browser authentication starts"
            )
        case .copilot:
            return String(
                localized: "settings.authentication.github-copilot.started-instructions",
                defaultValue: "🌐 GitHub Copilot authentication started!\n\nPlease visit github.com/login/device and enter the code shown.\n\nThe app will automatically detect your credentials.",
                comment: "Instructions shown when GitHub Copilot device authentication starts"
            )
        case .gemini:
            return String(
                localized: "settings.authentication.gemini.browser-opened-instructions",
                ...
                comment: "Instructions shown when Gemini browser authentication starts"
            )
        case .kimi:
            return String(
                localized: "settings.authentication.kimi.browser-opened-instructions",
                defaultValue: "🌐 Browser opened for Kimi authentication.\n\nPlease complete the login in your browser.\n\nThe app will automatically detect your Kimi account.",
                comment: "Instructions shown when Kimi browser authentication starts"
            )
        case .qwen:
            return String(
                localized: "settings.authentication.qwen.browser-opened-instructions",
                defaultValue: "🌐 Browser opened for Qwen authentication.\n\nPlease complete the login in your browser.",
                comment: "Instructions shown when Qwen browser authentication starts"
            )
        case .antigravity:
            return String(
                localized: "settings.authentication.antigravity.browser-opened-instructions",
                defaultValue: "🌐 Browser opened for Antigravity authentication.\n\nPlease complete the login in your browser.",
                comment: "Instructions shown when Antigravity browser authentication starts"
            )
        case .zai:
            return String(
                localized: "settings.zai-api-key.added-successfully-message",
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    case "API key saved successfully":
                        self.authResultMessage = String(format: String(
                            localized: "settings.provider-api-key.added-successfully-message",
                    ...
                    case String(
                        ...
                        self.authResultMessage = String(format: String(
                            ...
                            comment: "Message when provider API key already exists in config file"
                        ), "\(provider.title)")
                    case "API key already exists":
                        self.authResultMessage = String(format: String(
                            localized: "settings.provider-api-key.already-stored-message",
                            defaultValue: "✓ %@ already has this API key stored.",
                            comment: "Message when provider API key is already stored"
                        ), "\(provider.title)")
                    case "API key was already stored and has been re-enabled":
                        self.authResultMessage = String(format: String(
                            localized: "settings.provider-api-key.reenabled-message",
                            defaultValue: "✓ %@ already had this API key stored, and it has been re-enabled.",
                            comment: "Message indicating an existing provider API key was re-enabled"
                        ), "\(provider.title)")
                    default:
                        self.authResultMessage = output
                    }
    ...
    private func toggleCustomProviderCredential(provider: CustomProviderDefinition, credential: CustomProviderCredential) {
        if serverManager.toggleCustomProviderCredentialDisabled(credential) {
            ...
            authResultMessage = credential.isDisabled
                ? String(format: String(
                    localized: "settings.provider-credential.toggle-result.enabled",
                    defaultValue: "✓ Enabled %@ for %@",
                    comment: "Success message when a credential is enabled for a provider"
                ), "\(credential.label)", "\(provider.title)")
                : String(format: String(
                    localized: "settings.provider-credential.toggle-result.disabled",
            ...
            authResultSuccess = false
            authResultMessage = String(format: String(
                localized: "settings.provider-credential.toggle-result.failed",
                defaultValue: "Failed to update %@ for %@. Please try again.",
                comment: "Failure message when credential toggle update fails for a provider"
            ), "\(credential.label)", "\(provider.title)")
        }
        showingAuthResult = true
    }
    ...
    private func disconnectCustomProviderCredential(provider: CustomProviderDefinition, credential: CustomProviderCredential) {
        if serverManager.deleteCustomProviderCredential(credential) {
            authResultSuccess = true
            authResultMessage = String(format: String(
                localized: "settings.provider-credential.remove-result.success",
                defaultValue: "✓ Removed %@ from %@",
                comment: "Success message when a credential is removed from a provider"
            ), "\(credential.label)", "\(provider.title)")
        } else {
            authResultSuccess = false
            authResultMessage = String(format: String(
                localized: "settings.provider-credential.remove-result.failed",
                defaultValue: "Failed to remove %@ from %@",
                comment: "Failure message when removing a credential from a provider fails"
            ), "\(credential.label)", "\(provider.title)")
        }
        showingAuthResult = true
    }
    ...
    private func disconnectAccount(_ account: AuthAccount) {
        ...
        let cleanup = {
            if self.authManager.deleteAccount(account) {
                self.authResultSuccess = true
                self.authResultMessage = String(format: String(
                    localized: "settings.account.remove-result.success",
                    defaultValue: "✓ Removed %@ from %@",
                ...
                self.authResultMessage = String(
                    localized: "settings.account.remove-result.failed-title",
                    defaultValue: "Failed to remove account",
                    comment: "Failure title when removing an account fails"
                )
            }
            self.showingAuthResult = true
