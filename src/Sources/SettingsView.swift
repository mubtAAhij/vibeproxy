        ...
        VStack(alignment: .leading, spacing: 4) {
            ...
            HStack {
                ...
                if isAuthenticating {
                    ...
                    Button(String(localized: "settings.provider.add-account", defaultValue: "Add Account", comment: "Button to add account for provider")) {
                        onConnect()
                    }
                    .controlSize(.small)
                }
            ...
            if isEnabled {
                if !accounts.isEmpty {
                    // Collapsible summary
                    HStack(spacing: 4) {
                        Text(String(localized: "settings.provider.connected-accounts-count", defaultValue: "\(accounts.count) connected account\(accounts.count == 1 ? "" : "s")", comment: "Number of connected accounts"))
                            .font(.caption)
                            .foregroundColor(.green)

                        if accounts.count > 1 {
                            Text(String(localized: "settings.provider.round-robin-note", defaultValue: "• Round-robin w/ auto-failover", comment: "Note about round-robin with auto-failover"))
                                .font(.caption)
                                .foregroundColor(.secondary)
                        }
            ...
            Form {
                ...
                Section(String(localized: "settings.services-section", defaultValue: "Services", comment: "Section header for services")) {
                    ...
                    ServiceRow(
                        ...
                        customTitle: serverManager.vercelGatewayEnabled && !serverManager.vercelApiKey.isEmpty ? String(localized: "settings.claude.custom-title-vercel", defaultValue: "Claude Code (via Vercel)", comment: "Custom title when using Vercel gateway") : nil,
                    ...
                    ServiceRow(
                        ...
                        helpText: String(localized: "settings.gemini.help", defaultValue: "⚠️ Note: If you're an existing Gemini user with multiple projects, authentication will use your default project. Set your desired project as default in Google AI Studio before connecting.", comment: "Help text for Gemini provider"),
        ...
        serverManager.runAuthCommand(command) { success, output in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    let details = output.isEmpty ? String(localized: "settings.auth-result.no-output", defaultValue: "No output from authentication process", comment: "Default error when auth process produces no output") : output
                    self.authResultMessage = String(format: NSLocalizedString("settings.auth-result.failed", comment: "Authentication failed message"), details)
                    self.showingAuthResult = true
                }
            }
        }
        ...
        serverManager.runAuthCommand(.qwenLogin(email: email)) { success, output in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    self.authResultSuccess = false
                    let details = output.isEmpty ? String(localized: "settings.auth-result.qwen-no-output", defaultValue: "No output", comment: "Default error when Qwen auth produces no output") : output
                    self.authResultMessage = String(format: NSLocalizedString("settings.auth-result.qwen-failed", comment: "Qwen authentication failed message"), details)
                    self.showingAuthResult = true
                }
            }
        }
        ...
        serverManager.saveZaiApiKey(apiKey) { success, output in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    self.authResultSuccess = false
                    let details = output.isEmpty ? String(localized: "settings.zai-result.unknown-error", defaultValue: "Unknown error", comment: "Default error when Z.AI save produces no output") : output
                    self.authResultMessage = String(format: NSLocalizedString("settings.zai-result.save-failed", comment: "Z.AI API key save failed message"), details)
                    self.showingAuthResult = true
                }
            }
        }
    ...
    private func disconnectAccount(_ account: AuthAccount) {
        ...
        let cleanup = {
            if self.authManager.deleteAccount(account) {
                self.authResultSuccess = true
                self.authResultMessage = String(format: NSLocalizedString("settings.account.removed-success", comment: "Success message for account removal"), account.displayName, account.type.displayName)
            } else {
                self.authResultSuccess = false
