...
class ThinkingProxy {
    ...
    private func processRequest(data: Data, connection: NWConnection) {
        ...
        guard parts.count >= 3 else {
            sendError(to: connection, statusCode: 400, message: String(localized: "proxy.error.invalid-request-format", defaultValue: "Invalid request format", comment: "Error when HTTP request line format is malformed"))
            return
        }
    ...
    private func forwardToAmp(method: String, path: String, version: String, headers: [(String, String)], body: String, originalConnection: NWConnection) {
        ...
        targetConnection.stateUpdateHandler = { state in
            ...
            case .failed(let error):
                ...
                self.sendError(to: originalConnection, statusCode: 502, message: String(localized: "proxy.error.ampcode-connection", defaultValue: "Bad Gateway - Could not connect to ampcode.com", comment: "Error when connection to Amp server fails"))
                targetConnection.cancel()
    ...
    private func forwardToVercel(method: String, path: String, version: String, headers: [(String, String)], body: String, thinkingEnabled: Bool, originalConnection: NWConnection) {
        ...
        targetConnection.stateUpdateHandler = { [weak self] state in
            ...
            case .failed(let error):
                ...
                self.sendError(to: originalConnection, statusCode: 502, message: String(localized: "proxy.error.vercel-connection", defaultValue: "Bad Gateway - Could not connect to Vercel AI Gateway", comment: "Error when connection to Vercel AI Gateway fails"))
                targetConnection.cancel()
