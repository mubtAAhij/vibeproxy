...
class ThinkingProxy {
    ...
    private func processRequest(data: Data, connection: NWConnection) {
        guard let requestString = String(data: data, encoding: .utf8) else {
            sendError(to: connection, statusCode: 400, message: String(localized: "proxy.error.invalid-request", defaultValue: "Invalid request", comment: "Error when request data cannot be decoded"))
            return
        }
