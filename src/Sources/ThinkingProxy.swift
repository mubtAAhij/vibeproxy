...
class ThinkingProxy {
    ...
    private func processRequest(data: Data, connection: NWConnection) {
        guard let requestString = String(data: data, encoding: .utf8) else {
            sendError(to: connection, statusCode: 400, message: String(localized: "proxy.error.invalid-request", defaultValue: "Invalid request", bundle: .main, comment: "Error when request cannot be decoded"))
            return
        }
        ...
        guard let bodyStartRange = requestString.range(of: "\r\n\r\n") else {
            ...
            sendError(to: connection, statusCode: 400, message: String(localized: "proxy.error.invalid-request-no-body-separator", defaultValue: "Invalid request format - no body separator", bundle: .main, comment: "Error when HTTP request lacks body separator"))
            return
        }
    ...
    private func forwardRequest(method: String, path: String, version: String, headers: [(String, String)], body: String, thinkingEnabled: Bool = false, originalConnection: NWConnection, retryWithApiPrefix: Bool = false) {
        ...
        targetConnection.stateUpdateHandler = { state in
            ...
            case .failed(let error):
                ...
                self.sendError(to: originalConnection, statusCode: 502, message: String(localized: "proxy.error.bad-gateway", defaultValue: "Bad Gateway", bundle: .main, comment: "Generic bad gateway error"))
                targetConnection.cancel()
