...
class ThinkingProxy {
    ...
    private func processRequest(data: Data, connection: NWConnection) {
        guard let requestString = String(data: data, encoding: .utf8) else {
            sendError(to: connection, statusCode: 400, message: String(localized: "proxy.error.invalid-request", defaultValue: "Invalid request", bundle: .main, comment: "HTTP request parsing error"))
            return
        }
        ...
        guard let requestLine = lines.first else {
            sendError(to: connection, statusCode: 400, message: String(localized: "proxy.error.invalid-request-line", defaultValue: "Invalid request line", bundle: .main, comment: "HTTP request line parsing error"))
            return
        }
        ...
        guard parts.count >= 3 else {
            sendError(to: connection, statusCode: 400, message: String(localized: "proxy.error.invalid-request-format", defaultValue: "Invalid request format", bundle: .main, comment: "HTTP request format error"))
            return
        }
        ...
        guard let bodyStartRange = requestString.range(of: "\r\n\r\n") else {
            ...
            sendError(to: connection, statusCode: 400, message: String(localized: "proxy.error.no-body-separator", defaultValue: "Invalid request format - no body separator", bundle: .main, comment: "HTTP request body separator missing error"))
            return
        }
    ...
    private func forwardToAmp(method: String, path: String, version: String, headers: [(String, String)], body: String, originalConnection: NWConnection) {
        ...
        targetConnection.stateUpdateHandler = { state in
            ...
            case .failed(let error):
                ...
                self.sendError(to: originalConnection, statusCode: 502, message: String(localized: "proxy.error.ampcode-connection-failed", defaultValue: "Bad Gateway - Could not connect to ampcode.com", bundle: .main, comment: "Ampcode.com connection error"))
                targetConnection.cancel()
    ...
    private func forwardToVercel(method: String, path: String, version: String, headers: [(String, String)], body: String, thinkingEnabled: Bool, originalConnection: NWConnection) {
        ...
        targetConnection.stateUpdateHandler = { [weak self] state in
            ...
            case .failed(let error):
                ...
                self.sendError(to: originalConnection, statusCode: 502, message: String(localized: "proxy.error.vercel-connection-failed", defaultValue: "Bad Gateway - Could not connect to Vercel AI Gateway", bundle: .main, comment: "Vercel AI Gateway connection error"))
                targetConnection.cancel()
    ...
    private func forwardRequest(method: String, path: String, version: String, headers: [(String, String)], body: String, thinkingEnabled: Bool = false, originalConnection: NWConnection, retryWithApiPrefix: Bool = false) {
        ...
        guard let port = NWEndpoint.Port(rawValue: targetPort) else {
            ...
            sendError(to: originalConnection, statusCode: 500, message: String(localized: "proxy.error.internal-server-error", defaultValue: "Internal Server Error", bundle: .main, comment: "Internal server error"))
            return
        }
        ...
        targetConnection.stateUpdateHandler = { state in
            ...
            case .failed(let error):
                ...
                self.sendError(to: originalConnection, statusCode: 502, message: String(localized: "proxy.error.bad-gateway", defaultValue: "Bad Gateway", bundle: .main, comment: "Bad gateway error"))
                targetConnection.cancel()
