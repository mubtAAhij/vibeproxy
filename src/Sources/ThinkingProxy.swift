...
class ThinkingProxy {
    ...
    private func processRequest(data: Data, connection: NWConnection) {
        ...
        guard parts.count >= 3 else {
            sendError(to: connection, statusCode: 400, message: String(localized: "thinking-proxy.error.invalid-request-format", defaultValue: "Invalid request format", comment: "Error when HTTP request format is incorrect"))
            return
        }
        ...
        guard let bodyStartRange = requestString.range(of: "\r\n\r\n") else {
            ...
            sendError(to: connection, statusCode: 400, message: String(localized: "thinking-proxy.error.no-body-separator", defaultValue: "Invalid request format - no body separator", comment: "Error when HTTP request lacks body separator"))
            return
        }
    ...
    private func forwardToAmp(method: String, path: String, version: String, headers: [(String, String)], body: String, originalConnection: NWConnection) {
        ...
        targetConnection.stateUpdateHandler = { state in
            ...
            case .failed(let error):
                ...
                self.sendError(to: originalConnection, statusCode: 502, message: String(localized: "thinking-proxy.error.ampcode-connection-failed", defaultValue: "Bad Gateway - Could not connect to ampcode.com", comment: "Error when connection to ampcode.com fails"))
                targetConnection.cancel()
    ...
    private func forwardToVercel(method: String, path: String, version: String, headers: [(String, String)], body: String, thinkingEnabled: Bool, originalConnection: NWConnection) {
        ...
        targetConnection.stateUpdateHandler = { [weak self] state in
            ...
            case .failed(let error):
                ...
                self.sendError(to: originalConnection, statusCode: 502, message: String(localized: "thinking-proxy.error.vercel-connection-failed", defaultValue: "Bad Gateway - Could not connect to Vercel AI Gateway", comment: "Error when connection to Vercel AI Gateway fails"))
                targetConnection.cancel()
