...
class ThinkingProxy {
    ...
    private func processRequest(data: Data, connection: NWConnection) {
        guard let requestString = String(data: data, encoding: .utf8) else {
            sendError(to: connection, statusCode: 400, message: String(localized: "proxy.error.invalid-request", defaultValue: "Invalid request", comment: "Error when request data cannot be decoded as UTF-8"))
            return
        }
        ...
        guard let requestLine = lines.first else {
            sendError(to: connection, statusCode: 400, message: String(localized: "proxy.error.invalid-request-line", defaultValue: "Invalid request line", comment: "Error when HTTP request line is missing"))
            return
        }
        ...
        guard parts.count >= 3 else {
            sendError(to: connection, statusCode: 400, message: String(localized: "proxy.error.invalid-request-format", defaultValue: "Invalid request format", comment: "Error when HTTP request line does not have method, path, and version"))
            return
        }
        ...
        guard let bodyStartRange = requestString.range(of: "\r\n\r\n") else {
            ...
            sendError(to: connection, statusCode: 400, message: String(localized: "proxy.error.no-body-separator", defaultValue: "Invalid request format - no body separator", comment: "Error when HTTP request lacks the header/body separator"))
            return
        }
    ...
    private func forwardToAmp(method: String, path: String, version: String, headers: [(String, String)], body: String, originalConnection: NWConnection) {
        ...
        targetConnection.stateUpdateHandler = { state in
            ...
            case .failed(let error):
                ...
                self.sendError(to: originalConnection, statusCode: 502, message: String(localized: "proxy.error.ampcode-gateway", defaultValue: "Bad Gateway - Could not connect to ampcode.com", comment: "Error when proxy cannot connect to ampcode.com"))
                targetConnection.cancel()
    ...
    private func forwardToVercel(method: String, path: String, version: String, headers: [(String, String)], body: String, thinkingEnabled: Bool, originalConnection: NWConnection) {
        ...
        targetConnection.stateUpdateHandler = { [weak self] state in
            ...
            case .failed(let error):
                ...
                self.sendError(to: originalConnection, statusCode: 502, message: String(localized: "proxy.error.vercel-gateway", defaultValue: "Bad Gateway - Could not connect to Vercel AI Gateway", comment: "Error when proxy cannot connect to Vercel AI Gateway"))
                targetConnection.cancel()
    ...
    private func forwardRequest(method: String, path: String, version: String, headers: [(String, String)], body: String, thinkingEnabled: Bool = false, originalConnection: NWConnection, retryWithApiPrefix: Bool = false) {
        ...
        guard let port = NWEndpoint.Port(rawValue: targetPort) else {
            ...
            sendError(to: originalConnection, statusCode: 500, message: String(localized: "proxy.error.internal-server-error", defaultValue: "Internal Server Error", comment: "Generic internal server error message"))
            return
        }
        ...
        targetConnection.stateUpdateHandler = { state in
            ...
            case .failed(let error):
                ...
                self.sendError(to: originalConnection, statusCode: 502, message: String(localized: "proxy.error.bad-gateway", defaultValue: "Bad Gateway", comment: "Error when proxy cannot connect to target server"))
                targetConnection.cancel()
