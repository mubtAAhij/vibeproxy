...
class ThinkingProxy {
    ...
    private func processRequest(data: Data, connection: NWConnection) {
        guard let requestString = String(data: data, encoding: .utf8) else {
            sendError(to: connection, statusCode: 400, message: String(localized: "invalid_request", defaultValue: "Invalid request", comment: "HTTP error when request data cannot be decoded as UTF-8"))
            return
        }
        ...
        guard let requestLine = lines.first else {
            sendError(to: connection, statusCode: 400, message: String(localized: "invalid_request_line", defaultValue: "Invalid request line", comment: "HTTP error when request line is missing or malformed"))
            return
        }
        ...
        guard parts.count >= 3 else {
            sendError(to: connection, statusCode: 400, message: String(localized: "invalid_request_format", defaultValue: "Invalid request format", comment: "HTTP error when request line does not have required method, path, and version"))
            return
        }
        ...
        guard let bodyStartRange = requestString.range(of: "\r\n\r\n") else {
            ...
            sendError(to: connection, statusCode: 400, message: String(localized: "invalid_request_format_no_body_separator", defaultValue: "Invalid request format - no body separator", comment: "HTTP error when CRLF CRLF separator between headers and body is missing"))
            return
        }
    ...
    private func forwardToAmp(method: String, path: String, version: String, headers: [(String, String)], body: String, originalConnection: NWConnection) {
        ...
        targetConnection.stateUpdateHandler = { state in
            ...
            case .failed(let error):
                ...
                self.sendError(to: originalConnection, statusCode: 502, message: String(localized: "bad_gateway_ampcode_connection_failed", defaultValue: "Bad Gateway - Could not connect to ampcode.com", comment: "HTTP 502 error when proxy cannot establish connection to ampcode.com"))
                targetConnection.cancel()
    ...
    private func forwardToVercel(method: String, path: String, version: String, headers: [(String, String)], body: String, thinkingEnabled: Bool, originalConnection: NWConnection) {
        ...
        targetConnection.stateUpdateHandler = { [weak self] state in
            ...
            case .failed(let error):
                ...
                self.sendError(to: originalConnection, statusCode: 502, message: String(localized: "bad_gateway_vercel_connection_failed", defaultValue: "Bad Gateway - Could not connect to Vercel AI Gateway", comment: "HTTP 502 error when proxy cannot establish connection to Vercel AI Gateway"))
                targetConnection.cancel()
    ...
    private func forwardRequest(method: String, path: String, version: String, headers: [(String, String)], body: String, thinkingEnabled: Bool = false, originalConnection: NWConnection, retryWithApiPrefix: Bool = false) {
        ...
        guard let port = NWEndpoint.Port(rawValue: targetPort) else {
            ...
            sendError(to: originalConnection, statusCode: 500, message: String(localized: "internal_server_error", defaultValue: "Internal Server Error", comment: "HTTP 500 error for generic internal server errors"))
            return
        }
        ...
        targetConnection.stateUpdateHandler = { state in
            ...
            case .failed(let error):
                ...
                self.sendError(to: originalConnection, statusCode: 502, message: String(localized: "bad_gateway", defaultValue: "Bad Gateway", comment: "HTTP 502 error when proxy cannot connect to backend server"))
                targetConnection.cancel()
