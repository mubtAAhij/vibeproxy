...
class ThinkingProxy {
    ...
    private func processRequest(data: Data, connection: NWConnection) {
        ...
        guard parts.count >= 3 else {
            sendError(to: connection, statusCode: 400, message: String(localized: "proxy.error.invalid-request-format", defaultValue: "Invalid request format", comment: "HTTP error when request format is incorrect"))
            return
        }
    ...
    private func forwardToAmp(method: String, path: String, version: String, headers: [(String, String)], body: String, originalConnection: NWConnection) {
        ...
        targetConnection.stateUpdateHandler = { state in
            ...
            case .failed(let error):
                ...
                self.sendError(to: originalConnection, statusCode: 502, message: String(localized: "proxy.error.ampcode-unreachable", defaultValue: "Bad Gateway - Could not connect to ampcode.com", comment: "HTTP error when connection to ampcode.com fails"))
                targetConnection.cancel()
    ...
    private func forwardToVercel(method: String, path: String, version: String, headers: [(String, String)], body: String, thinkingEnabled: Bool, originalConnection: NWConnection) {
        ...
        targetConnection.stateUpdateHandler = { [weak self] state in
            ...
            case .failed(let error):
                ...
                self.sendError(to: originalConnection, statusCode: 502, message: String(localized: "proxy.error.vercel-unreachable", defaultValue: "Bad Gateway - Could not connect to Vercel AI Gateway", comment: "HTTP error when connection to Vercel AI Gateway fails"))
                targetConnection.cancel()
    ...
    private func forwardRequest(method: String, path: String, version: String, headers: [(String, String)], body: String, thinkingEnabled: Bool = false, originalConnection: NWConnection, retryWithApiPrefix: Bool = false) {
        ...
        guard let port = NWEndpoint.Port(rawValue: targetPort) else {
            ...
            sendError(to: originalConnection, statusCode: 500, message: String(localized: "proxy.error.internal-error", defaultValue: "Internal Server Error", comment: "HTTP 500 error message"))
            return
        }
