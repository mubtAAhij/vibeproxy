...
class ThinkingProxy {
    ...
    private func processRequest(data: Data, connection: NWConnection) {
        guard let requestString = String(data: data, encoding: .utf8) else {
            sendError(to: connection, statusCode: 400, message: String(localized: "thinking-proxy.error.invalid-request", defaultValue: "Invalid request", comment: "Error when request data cannot be decoded"))
            return
        }
        ...
        guard parts.count >= 3 else {
            sendError(to: connection, statusCode: 400, message: String(localized: "thinking-proxy.error.invalid-request-format", defaultValue: "Invalid request format", comment: "Error when HTTP request line is malformed"))
            return
        }
    ...
    private func forwardToAmp(method: String, path: String, version: String, headers: [(String, String)], body: String, originalConnection: NWConnection) {
        ...
        targetConnection.stateUpdateHandler = { state in
            ...
            case .failed(let error):
                ...
                self.sendError(to: originalConnection, statusCode: 502, message: String(localized: "thinking-proxy.error.ampcode-gateway", defaultValue: "Bad Gateway - Could not connect to ampcode.com", comment: "Error when proxy cannot connect to ampcode.com"))
                targetConnection.cancel()
    ...
    private func forwardToVercel(method: String, path: String, version: String, headers: [(String, String)], body: String, thinkingEnabled: Bool, originalConnection: NWConnection) {
        ...
        targetConnection.stateUpdateHandler = { [weak self] state in
            ...
            case .failed(let error):
                ...
                self.sendError(to: originalConnection, statusCode: 502, message: String(localized: "thinking-proxy.error.vercel-gateway", defaultValue: "Bad Gateway - Could not connect to Vercel AI Gateway", comment: "Error when proxy cannot connect to Vercel AI Gateway"))
                targetConnection.cancel()
