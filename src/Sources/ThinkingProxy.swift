...
class ThinkingProxy {
    ...
    private func processRequest(data: Data, connection: NWConnection) {
        ...
        guard parts.count >= 3 else {
            sendError(to: connection, statusCode: 400, message: String(localized: "thinking-proxy.error.invalid-request-format", defaultValue: "Invalid request format", comment: "Error when HTTP request line has incorrect format"))
            return
        }
        ...
        guard let bodyStartRange = requestString.range(of: "\r\n\r\n") else {
            ...
            sendError(to: connection, statusCode: 400, message: String(localized: "thinking-proxy.error.no-body-separator", defaultValue: "Invalid request format - no body separator", comment: "Error when HTTP headers and body separator is missing"))
            return
        }
    ...
    private func forwardToAmp(method: String, path: String, version: String, headers: [(String, String)], body: String, originalConnection: NWConnection) {
        ...
        targetConnection.stateUpdateHandler = { state in
            ...
            case .failed(let error):
                ...
                self.sendError(to: originalConnection, statusCode: 502, message: String(localized: "thinking-proxy.error.ampcode-gateway", defaultValue: "Bad Gateway - Could not connect to ampcode.com", comment: "Error when connection to ampcode.com fails"))
                targetConnection.cancel()
    ...
    private func forwardToVercel(method: String, path: String, version: String, headers: [(String, String)], body: String, thinkingEnabled: Bool, originalConnection: NWConnection) {
        ...
        targetConnection.stateUpdateHandler = { [weak self] state in
            ...
            case .failed(let error):
                ...
                self.sendError(to: originalConnection, statusCode: 502, message: String(localized: "thinking-proxy.error.vercel-gateway", defaultValue: "Bad Gateway - Could not connect to Vercel AI Gateway", comment: "Error when connection to Vercel AI Gateway fails"))
                targetConnection.cancel()
    ...
    private func forwardRequest(method: String, path: String, version: String, headers: [(String, String)], body: String, thinkingEnabled: Bool = false, originalConnection: NWConnection, retryWithApiPrefix: Bool = false) {
        ...
        guard let port = NWEndpoint.Port(rawValue: targetPort) else {
            ...
            sendError(to: originalConnection, statusCode: 500, message: String(localized: "thinking-proxy.error.internal", defaultValue: "Internal Server Error", comment: "Generic internal server error"))
            return
        }
        ...
        targetConnection.stateUpdateHandler = { state in
            ...
            case .failed(let error):
                ...
                self.sendError(to: originalConnection, statusCode: 502, message: String(localized: "thinking-proxy.error.bad-gateway", defaultValue: "Bad Gateway", comment: "Error when connection to target server fails"))
                targetConnection.cancel()
