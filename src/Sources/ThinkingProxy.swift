...
class ThinkingProxy {
    ...
    private func processRequest(data: Data, connection: NWConnection) {
        guard let requestString = String(data: data, encoding: .utf8) else {
            sendError(to: connection, statusCode: 400, message: String(localized: "thinking-proxy.error.invalid-request", defaultValue: "Invalid request", comment: "HTTP 400 error when request data cannot be decoded"))
            return
        }
        ...
        guard let requestLine = lines.first else {
            sendError(to: connection, statusCode: 400, message: String(localized: "thinking-proxy.error.invalid-request-line", defaultValue: "Invalid request line", comment: "HTTP 400 error when request line is missing"))
            return
        }
        ...
        guard parts.count >= 3 else {
            sendError(to: connection, statusCode: 400, message: String(localized: "thinking-proxy.error.invalid-request-format", defaultValue: "Invalid request format", comment: "HTTP 400 error when request line has incorrect number of parts"))
            return
        }
        ...
        guard let bodyStartRange = requestString.range(of: "\r\n\r\n") else {
            ...
            sendError(to: connection, statusCode: 400, message: String(localized: "thinking-proxy.error.no-body-separator", defaultValue: "Invalid request format - no body separator", comment: "HTTP 400 error when request lacks header-body separator"))
            return
        }
    ...
    private func forwardToAmp(method: String, path: String, version: String, headers: [(String, String)], body: String, originalConnection: NWConnection) {
        ...
        targetConnection.stateUpdateHandler = { state in
            ...
            case .failed(let error):
                ...
                self.sendError(to: originalConnection, statusCode: 502, message: String(localized: "thinking-proxy.error.ampcode-gateway", defaultValue: "Bad Gateway - Could not connect to ampcode.com", comment: "HTTP 502 error when connection to ampcode.com fails"))
                targetConnection.cancel()
    ...
    private func forwardToVercel(method: String, path: String, version: String, headers: [(String, String)], body: String, thinkingEnabled: Bool, originalConnection: NWConnection) {
        ...
        targetConnection.stateUpdateHandler = { [weak self] state in
            ...
            case .failed(let error):
                ...
                self.sendError(to: originalConnection, statusCode: 502, message: String(localized: "thinking-proxy.error.vercel-gateway", defaultValue: "Bad Gateway - Could not connect to Vercel AI Gateway", comment: "HTTP 502 error when connection to Vercel AI Gateway fails"))
                targetConnection.cancel()
    ...
    private func forwardRequest(method: String, path: String, version: String, headers: [(String, String)], body: String, thinkingEnabled: Bool = false, originalConnection: NWConnection, retryWithApiPrefix: Bool = false) {
        ...
        guard let port = NWEndpoint.Port(rawValue: targetPort) else {
            ...
            sendError(to: originalConnection, statusCode: 500, message: String(localized: "thinking-proxy.error.internal-server-error", defaultValue: "Internal Server Error", comment: "HTTP 500 error for internal server issues"))
            return
        }
