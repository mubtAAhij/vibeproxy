...
class ServerManager: ObservableObject {
    ...
    func runAuthCommand(_ command: AuthCommand, completion: @escaping (Bool, String) -> Void) {
        // Use bundled binary from app bundle
        guard let resourcePath = Bundle.main.resourcePath else {
            completion(false, String(localized: "server.error.resource-path-not-found", defaultValue: "Could not find resource path", bundle: .main, comment: "Error when app bundle resource path cannot be found"))
            return
        }
        ...
        do {
            ...
            DispatchQueue.global(qos: .userInitiated).asyncAfter(deadline: .now() + waitTime) {
                if authProcess.isRunning {
                    ...
                    if case .copilotLogin = command {
                        ...
                        if let codeRange = capture.text.range(of: "enter the code: "),
                           let endRange = capture.text[codeRange.upperBound...].range(of: "\n") {
                            ...
                            completion(true, String(format: String(localized: "auth.github-device-code-success", defaultValue: "🌐 Browser opened for GitHub authentication.\n\n📋 Code copied to clipboard:\n\n%@\n\nJust paste it in the browser!\n\nThe app will automatically detect when you're authenticated.", bundle: .main, comment: "Success message when GitHub device code is extracted and copied"), code))
                            return
                        ...
                        completion(true, String(localized: "auth.github-device-code-fallback", defaultValue: "🌐 Browser opened for GitHub authentication.\n\nCheck your terminal or the opened browser for the device code.\n\nThe app will automatically detect when you're authenticated.", bundle: .main, comment: "Fallback message when GitHub device code cannot be extracted"))
                        return
                    }

                    completion(true, String(localized: "auth.browser-opened-success", defaultValue: "🌐 Browser opened for authentication.\n\nPlease complete the login in your browser.\n\nThe app will automatically detect when you're authenticated.", bundle: .main, comment: "Generic auth success message when browser opens"))
                } else {
                    ...
                    if output.contains("Opening browser") || output.contains("Attempting to open URL") {
                        ...
                        let message = error.isEmpty ? (output.isEmpty ? String(localized: "auth.process-failed", defaultValue: "Authentication process failed unexpectedly", bundle: .main, comment: "Error when auth process fails without output") : output) : error
            ...
            completion(false, String(format: String(localized: "auth.failed-to-start", defaultValue: "Failed to start auth process: %@", bundle: .main, comment: "Error when auth process cannot start"), error.localizedDescription))
        }
    }
    ...
    func saveZaiApiKey(_ apiKey: String, completion: @escaping (Bool, String) -> Void) {
        ...
        do {
            ...
            completion(false, String(format: String(localized: "auth.failed-to-create-directory", defaultValue: "Failed to create auth directory: %@", bundle: .main, comment: "Error when auth directory cannot be created"), error.localizedDescription))
            return
        }
        ...
        do {
            ...
            completion(false, String(format: String(localized: "auth.failed-to-save-api-key", defaultValue: "Failed to save API key: %@", bundle: .main, comment: "Error when API key cannot be saved"), error.localizedDescription))
        }
    }
