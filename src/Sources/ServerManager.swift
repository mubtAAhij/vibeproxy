...
class ServerManager: ObservableObject {
    ...
    func runAuthCommand(_ command: AuthCommand, completion: @escaping (Bool, String) -> Void) {
        // Use bundled binary from app bundle
        guard let resourcePath = Bundle.main.resourcePath else {
            completion(false, String(localized: "auth.error.resource-path-not-found", defaultValue: "Could not find resource path", comment: "Error when app resource path cannot be located"))
            return
        }

        let bundledPath = (resourcePath as NSString).appendingPathComponent("cli-proxy-api-plus")
        guard FileManager.default.fileExists(atPath: bundledPath) else {
            completion(false, String(format: NSLocalizedString("auth.error.binary-not-found", comment: "Error when binary is missing at expected path"), bundledPath))
            return
        }
    ...
    func saveZaiApiKey(_ apiKey: String, completion: @escaping (Bool, String) -> Void) {
        ...
        do {
            ...
            completion(false, String(format: NSLocalizedString("auth.error.failed-to-save-api-key", comment: "Error when API key save fails"), error.localizedDescription))
        }
    }
