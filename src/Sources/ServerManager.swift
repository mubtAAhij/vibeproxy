...
class ServerManager: ObservableObject {
    ...
    func runAuthCommand(_ command: AuthCommand, completion: @escaping (Bool, String) -> Void) {
        ...
        do {
            ...
            DispatchQueue.global(qos: .userInitiated).asyncAfter(deadline: .now() + waitTime) {
                if authProcess.isRunning {
                    ...
                    if output.contains("Opening browser") || output.contains("Attempting to open URL") {
                        ...
                        let message = error.isEmpty ? (output.isEmpty ? String(localized: "auth.error.unexpected-failure", defaultValue: "Authentication process failed unexpectedly", comment: "Error when auth process fails with no output") : output) : error
            ...
            completion(false, String(format: String(localized: "auth.error.start-failed", defaultValue: "Failed to start auth process: %@", comment: "Error when auth process fails to start"), error.localizedDescription))
        }
    }
    ...
    func saveZaiApiKey(_ apiKey: String, completion: @escaping (Bool, String) -> Void) {
        ...
        do {
            ...
            completion(false, String(format: String(localized: "auth.zai.save-failed", defaultValue: "Failed to save API key: %@", comment: "Error when Z.AI API key save fails"), error.localizedDescription))
        }
    }
