...
class ServerManager: ObservableObject {
    ...
    func runAuthCommand(_ command: AuthCommand, completion: @escaping (Bool, String) -> Void) {
        // Use bundled binary from app bundle
        guard let resourcePath = Bundle.main.resourcePath else {
            completion(false, String(localized: "error.resource-path-not-found", defaultValue: "Could not find resource path", comment: "Error when app bundle resource path is unavailable"))
            return
        }

        let bundledPath = (resourcePath as NSString).appendingPathComponent("cli-proxy-api-plus")
        guard FileManager.default.fileExists(atPath: bundledPath) else {
            completion(false, String(format: String(localized: "error.binary-not-found", defaultValue: "Binary not found at %@", comment: "Error when cli-proxy-api-plus binary is missing at expected path"), bundledPath))
            return
        }
        ...
        do {
            ...
            DispatchQueue.global(qos: .userInitiated).asyncAfter(deadline: .now() + waitTime) {
                if authProcess.isRunning {
                    ...
                    if output.contains("Opening browser") || output.contains("Attempting to open URL") {
                        ...
                        let message = error.isEmpty ? (output.isEmpty ? String(localized: "auth.process-failed", defaultValue: "Authentication process failed unexpectedly", comment: "Error when authentication process fails without specific error") : output) : error
            ...
            completion(false, String(format: String(localized: "auth.process-start-failed", defaultValue: "Failed to start auth process: %@", comment: "Error when authentication process fails to launch"), error.localizedDescription))
        }
    }
    ...
    func saveZaiApiKey(_ apiKey: String, completion: @escaping (Bool, String) -> Void) {
        ...
        do {
            ...
            completion(false, String(format: String(localized: "auth.directory-creation-failed", defaultValue: "Failed to create auth directory: %@", comment: "Error when unable to create .cli-proxy-api directory"), error.localizedDescription))
            return
        }
        ...
        do {
            ...
            completion(true, String(localized: "auth.api-key-saved", defaultValue: "API key saved successfully", comment: "Success message after Z.AI API key is saved"))
        } catch {
            completion(false, String(format: String(localized: "auth.api-key-save-failed", defaultValue: "Failed to save API key: %@", comment: "Error when unable to save Z.AI API key file"), error.localizedDescription))
        }
    }
