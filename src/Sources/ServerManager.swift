...
class ServerManager: ObservableObject {
    ...
    private func providerConfigLockReason(_ providerKey: String, baseConfigRoot: [String: Any]?) -> String? {
        ...
        return String(format: String(
            localized: "server.manager.provider.oauth-excluded-models-disabled",
            defaultValue: "Disabled in config via oauth-excluded-models. Remove the '*' exclusion for %@ to enable it here.",
            comment: "Message explaining provider is disabled by oauth-excluded-models config"
        ), "\(oauthProviderKey)")
    }
    ...
    func setProviderEnabled(_ providerKey: String, enabled: Bool) {
        ...
        if enabled, let lockReason = providerConfigLockReason(providerKey) {
            addLog(String(format: String(
                localized: "server.manager.provider.remains-disabled",
                defaultValue: "⚠️ %@ remains disabled: %@",
                comment: "Message indicating provider remains disabled with reason"
            ...
            addLog(enabled ? String(format: String(
                ...
                comment: "Status message indicating provider was enabled"
            ), "\(providerKey)") : String(format: String(
                localized: "server.manager.provider.disabled",
                defaultValue: "⚠️ Disabled provider: %@",
                comment: "Status message indicating provider was disabled"
            ), "\(providerKey)"))
        }
        reloadCustomProviders()
        requestConfigUpdate()
    ...
    func start(completion: @escaping (Bool) -> Void) {
        ...
        guard FileManager.default.fileExists(atPath: bundledPath) else {
            addLog(String(format: String(
                ...
                comment: "Error message when bundled server binary is missing"
            ), "\(bundledPath)"))
            completion(false)
            return
        }
        ...
        guard !configPath.isEmpty && FileManager.default.fileExists(atPath: configPath) else {
            addLog(String(format: String(
                localized: "server.manager.error.active-config-path",
                defaultValue: "❌ Error: %@",
                comment: "Error message for active config path resolution failure with details"
            ), "\(configErrorMessage ?? String(
                localized: "server.manager.error.active-config-path.default-detail",
                defaultValue: "Could not resolve active config path",
                comment: "Fallback detail when active config path cannot be resolved"
            ))"))
            completion(false)
            return
        }
        ...
        errorPipe.fileHandleForReading.readabilityHandler = { [weak self] handle in
            ...
            if let output = String(data: data, encoding: .utf8), !output.isEmpty {
                self?.addLog(String(format: String(
                    ...
                    defaultValue: "⚠️ %@",
                    comment: "Warning output emitted by server process"
                ), "\(output)"))
            }
        }
        ...
        process?.terminationHandler = { [weak self] process in
            ...
            DispatchQueue.main.async {
                ...
                self?.addLog(String(format: String(
                    localized: "server.manager.status.stopped-with-code",
                    defaultValue: "Server stopped with code: %d",
                    comment: "Message indicating server stopped with termination code"
        ...
        do {
            ...
            addLog(String(format: String(
                localized: "server.manager.status.started-on-port",
                defaultValue: "✓ Server started on port %d",
                comment: "Status message when server starts successfully on port"
            ), port))
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + Timing.readinessCheckDelay) { [weak self] in
                ...
                if let process = self.process, process.isRunning {
                    ...
                    self.addLog(String(
                        localized: "server.manager.warning.exited-before-ready",
            ...
            addLog(String(format: String(
                localized: "server.manager.error.failed-to-start",
    ...
    func stop(completion: (() -> Void)? = nil) {
        ...
        addLog(String(format: String(
            localized: "server.manager.status.stopping-with-pid",
            defaultValue: "Stopping server (PID: %d)...",
            comment: "Status message while attempting graceful server stop"
        ), pid))
        processQueue.async { [weak self] in
            ...
            if process.isRunning {
                self.addLog(String(
                    localized: "server.manager.warning.force-killing",
            ...
            DispatchQueue.main.async {
                ...
                self.activeConfigPath = ""
                self.addLog(String(
                    localized: "server.manager.status.stopped",
                    defaultValue: "✓ Server stopped",
                    comment: "Status message after server has stopped"
                ))
                NotificationCenter.default.post(name: .serverStatusChanged, object: nil)
                completion?()
            }
    ...
    func runAuthCommand(_ command: AuthCommand, completion: @escaping (Bool, String) -> Void) {
        ...
        guard let resourcePath = Bundle.main.resourcePath else {
            completion(false, String(
                localized: "server.manager.error.resource-path-not-found.short",
                defaultValue: "Could not find resource path",
                comment: "Error text used when resource path resolution fails"
            ))
            return
        }
        ...
        guard FileManager.default.fileExists(atPath: bundledPath) else {
            completion(false, String(format: String(
                ...
                comment: "Error text when expected bundled binary path is missing"
            ), "\(bundledPath)"))
            return
        }
        ...
        guard !configPath.isEmpty else {
            completion(false, configErrorMessage ?? String(
                localized: "server.manager.error.config-path-not-resolved",
        ...
        do {
            ...
            try authProcess.run()
            addLog(String(format: String(
                localized: "server.manager.auth-process.started",
                defaultValue: "✓ Authentication process started (PID: %d) - browser should open shortly",
                comment: "Status message when authentication process starts and browser launch is expected"
                ...
                if authProcess.isRunning {
                    ...
                    if output.contains("Opening browser") || output.contains("Attempting to open URL") {
                        ...
                        let message = error.isEmpty ? (output.isEmpty ? String(
                            localized: "server.manager.auth-process.failed-unexpectedly",
            ...
            completion(false, String(format: String(
                localized: "server.manager.auth-process.failed-to-start",
                defaultValue: "Failed to start auth process: %@",
                comment: "Error message when auth process launch fails with detail"
            ), "\(error.localizedDescription)"))
        }
    }
    ...
    private func terminateActiveAuthProcessIfNeeded(reason: String) {
        ...
        if authProcess.isRunning {
            addLog(String(format: String(
                localized: "server.manager.auth-process.terminating-previous-before-retry",
                defaultValue: "⚠️ Terminating previous auth process (%d) before retry: %@",
                comment: "Warning when terminating previous auth process before retry"
        ...
        for pattern in patterns {
            ...
            do {
                ...
                for pid in pids {
                    ...
                    kill(pid, SIGKILL)
                    addLog(String(format: String(
                        localized: "server.manager.auth-listener.cleaned-up-stale-process",
                        defaultValue: "⚠️ Cleaned up stale auth listener process: %d",
                        comment: "Warning indicating stale auth listener process cleanup"
                    ), pid))
                }
            } catch {
        ...
        credentialMutationQueue.async { [weak self] in
            ...
            do {
                ...
                self.addLog(String(format: String(
                    ...
                    comment: "Success message when Z.AI API key is written to file"
                ), "\(filePath.lastPathComponent)"))
                self.refreshAuthBackedConfiguration()
                DispatchQueue.main.async {
                    completion(true, String(
                        localized: "server.manager.api-key.saved-successfully.zai",
                        defaultValue: "API key saved successfully",
                        comment: "Success message after saving Z.AI API key"
                    ))
                }
            } catch {
                DispatchQueue.main.async {
            ...
            do {
                ...
                guard let provider = customProviders.first(where: { $0.id == providerID }) else {
                    DispatchQueue.main.async {
                        completion(false, String(format: String(
                            ...
                            comment: "Error when custom provider identifier is missing from config"
                        ), "\(providerID)"))
                    }
                    return
                }
                ...
                if provider.inlineAPIKeys.contains(apiKey) {
                    self.addLog(String(format: String(
                        ...
                        comment: "Info message that custom provider API key already exists"
                    ), "\(providerID)"))
                    DispatchQueue.main.async {
                        completion(true, String(
                            localized: "server.manager.custom-provider.api-key-already-exists.short",
                            defaultValue: "API key already exists in config",
                            comment: "Short message that API key already exists in config"
                        ))
                    }
                    return
                }
                ...
                case .created(let record):
                    self.addLog(String(format: String(
                        ...
                        comment: "Success message after saving custom provider API key"
                    ), "\(record.providerID)"))
                case .alreadyPresent(let record):
                    self.addLog(String(format: String(
                        ...
                        comment: "Info message when custom provider key is already present"
                    ), "\(record.label)"))
                case .reenabled(let record):
                    self.addLog(String(format: String(
                        ...
                        comment: "Success message when custom provider key is re-enabled"
                    ), "\(record.label)"))
                }
                self.refreshAuthBackedConfiguration()
                DispatchQueue.main.async {
    ...
    func deleteCustomProviderCredential(_ credential: CustomProviderCredential) -> Bool {
        do {
            ...
            addLog(String(format: String(
                ...
                comment: "Success message after removing custom provider key"
            ), "\(credential.label)"))
            if deletedCount > 1 {
                addLog(String(format: String(
                    ...
                    comment: "Success message after removing duplicate credential files"
            ...
            addLog(
                credential.isDisabled
                    ? String(format: String(
                        ...
                        comment: "Success message when enabling custom provider key"
                    ), "\(credential.label)")
                    : String(format: String(
                        localized: "server.manager.custom-provider.key-disabled",
                        defaultValue: "⚠️ Disabled custom provider key: %@",
                        comment: "Status message when disabling custom provider key"
                    ), "\(credential.label)")
            )
            markObservedConfigInputsCurrent()
            reloadCustomProviders()
    ...
    private func resolveConfigPath(
        ...
        guard bundledConfigPath() != nil else {
            return .failure(ConfigResolutionFailure(message: String(
                localized: "server.manager.error.bundled-config-not-found",
        ...
        guard case .success(let baseConfig) = baseConfigResult else {
            ...
            return .failure(ConfigResolutionFailure(message: String(
                localized: "server.manager.error.base-configuration-load-failed",
            ...
            return .failure(
                ConfigResolutionFailure(
                    message: String(format: String(
                        localized: "server.manager.error.write-merged-config-failed",
    ...
    private func killOrphanedProcesses() {
        ...
        do {
            ...
            if checkTask.terminationStatus == 0 {
                ...
                if !pids.isEmpty {
                    addLog(String(format: String(
                        localized: "server.manager.warning.orphaned-processes-found",
                    ...
                    addLog(String(
                        localized: "server.manager.status.cleaned-up-orphaned-processes",
                        defaultValue: "✓ Cleaned up orphaned processes",
                        comment: "Status message confirming orphaned process cleanup"
                    ))
                }
            }
    ...
    private func loadBaseConfigRoot() -> Result<LoadedBaseConfig, ConfigResolutionFailure> {
        guard let bundledConfigPath = bundledConfigPath() else {
            return .failure(ConfigResolutionFailure(message: String(
                localized: "server.manager.error.bundled-config-not-found.app-bundle",
                defaultValue: "Could not locate the bundled config.yaml in the app bundle.",
                comment: "Error when bundled config.yaml is missing from app bundle"
            )))
        }
        ...
        guard case .success(let bundledRoot) = bundledRootResult else {
            ...
            return .failure(ConfigResolutionFailure(message: String(format: String(
                localized: "server.manager.error.bundled-config-load-failed",
                defaultValue: "Could not load the bundled config at %@.",
                comment: "Error when bundled configuration file cannot be loaded"
            ), "\(bundledConfigPath)")))
        }
        ...
        guard case .success(let userRoot) = userRootResult else {
            ...
            return .failure(ConfigResolutionFailure(message: String(format: String(
                localized: "server.manager.error.user-config-load-failed",
                defaultValue: "Could not load the user config at %@.",
                comment: "Error when user configuration file cannot be loaded"
            ), "\(userConfigPath)")))
        }
    ...
    private func loadYAMLDictionary(atPath path: String) -> Result<[String: Any], ConfigResolutionFailure> {
        do {
            ...
            guard let dictionary = ConfigComposer.stringKeyedDictionary(loaded) else {
                return .failure(ConfigResolutionFailure(message: String(format: String(
                    localized: "server.manager.error.config-root-must-be-mapping",
            ...
            return .failure(ConfigResolutionFailure(message: String(format: String(
                localized: "server.manager.error.yaml-parse-failed",
    ...
    private func validatedLoadedBaseConfig(
        ...
        guard validationErrors.isEmpty else {
            return .failure(
                ConfigResolutionFailure(
                    message: String(format: String(
                        localized: "server.manager.error.invalid-custom-provider-configuration",
                        defaultValue: "Invalid custom provider configuration. %@",
                        comment: "Validation error message for invalid custom provider configuration"
        ...
        let update = {
            ...
            if shouldLog {
                self.addLog(String(format: String(
                    ...
                    defaultValue: "❌ %@",
                    comment: "Generic error message prefixed with error icon"
                ), "\(message)"))
            }
        }
    ...
    private func finishConfigUpdateResolution(_ result: Result<String, ConfigResolutionFailure>) {
        ...
        if shouldRestart {
            ...
            hasPendingConfigUpdate = false
            addLog(String(
                localized: "server.manager.status.config-path-changed-restarting",
        ...
        if isRunning {
            addLog(String(
                localized: "server.manager.status.config-updated-hot-reload",
                defaultValue: "Config updated (hot reload)",
                comment: "Status message when config is reloaded without full restart"
            ))
        }
    }
