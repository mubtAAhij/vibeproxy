...
class ServerManager: ObservableObject {
    ...
    func runAuthCommand(_ command: AuthCommand, completion: @escaping (Bool, String) -> Void) {
        ...
        let bundledPath = (resourcePath as NSString).appendingPathComponent("cli-proxy-api-plus")
        guard FileManager.default.fileExists(atPath: bundledPath) else {
            completion(false, String(format: NSLocalizedString("server.error.binary-not-found", comment: "Error when CLI binary is missing"), bundledPath))
            return
        }
        ...
        do {
            ...
            DispatchQueue.global(qos: .userInitiated).asyncAfter(deadline: .now() + waitTime) {
                if authProcess.isRunning {
                    ...
                    if case .copilotLogin = command {
                        ...
                        if let codeRange = capture.text.range(of: "enter the code: "),
                           let endRange = capture.text[codeRange.upperBound...].range(of: "\n") {
                            ...
                            completion(true, String(format: NSLocalizedString("server.auth.github-code-copied", comment: "Success message with device code copied to clipboard"), code))
                            return
                        ...
                        completion(true, String(localized: "server.auth.github-no-code", defaultValue: "🌐 Browser opened for GitHub authentication.\n\nCheck your terminal or the opened browser for the device code.\n\nThe app will automatically detect when you're authenticated.", comment: "GitHub auth message when device code cannot be extracted"))
                        return
                    }
                    ...
                    if output.contains("Opening browser") || output.contains("Attempting to open URL") {
                        ...
                        let message = error.isEmpty ? (output.isEmpty ? String(localized: "server.auth.failed-unexpectedly", defaultValue: "Authentication process failed unexpectedly", comment: "Error when auth process fails without details") : output) : error
            ...
            completion(false, String(format: NSLocalizedString("server.auth.failed-to-start", comment: "Error when auth process cannot be started"), error.localizedDescription))
        }
    }
    ...
    func saveZaiApiKey(_ apiKey: String, completion: @escaping (Bool, String) -> Void) {
        ...
        do {
            ...
            completion(false, String(format: NSLocalizedString("server.error.create-auth-directory-failed", comment: "Error when auth directory cannot be created"), error.localizedDescription))
            return
        }
        ...
        do {
            ...
            completion(true, String(localized: "server.api-key.save-success", defaultValue: "API key saved successfully", comment: "Success message when API key is saved"))
        } catch {
            completion(false, String(format: NSLocalizedString("server.api-key.save-failed", comment: "Error when API key cannot be saved"), error.localizedDescription))
        }
    }
