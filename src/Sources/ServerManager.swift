...
class ServerManager: ObservableObject {
    ...
    func runAuthCommand(_ command: AuthCommand, completion: @escaping (Bool, String) -> Void) {
        ...
        let bundledPath = (resourcePath as NSString).appendingPathComponent("cli-proxy-api-plus")
        guard FileManager.default.fileExists(atPath: bundledPath) else {
            completion(false, String(format: NSLocalizedString("server.error.binary-not-found", comment: "Error when bundled binary is missing"), bundledPath))
            return
        }
                    ...
                    if case .copilotLogin = command {
                        ...
                        if let codeRange = capture.text.range(of: "enter the code: "),
                           let endRange = capture.text[codeRange.upperBound...].range(of: "\n") {
                            ...
                            completion(true, String(format: NSLocalizedString("auth.github.code-copied", comment: "Success message when GitHub auth code is copied to clipboard"), code))
                            return
                        ...
                        completion(true, String(localized: "auth.github.check-browser", defaultValue: "🌐 Browser opened for GitHub authentication.\n\nCheck your terminal or the opened browser for the device code.\n\nThe app will automatically detect when you're authenticated.", comment: "Message when GitHub auth is started but code extraction failed"))
                        return
                    }

                    completion(true, String(localized: "auth.browser-opened", defaultValue: "🌐 Browser opened for authentication.\n\nPlease complete the login in your browser.\n\nThe app will automatically detect when you're authenticated.", comment: "Generic message when authentication browser opens"))
                } else {
    ...
    func saveZaiApiKey(_ apiKey: String, completion: @escaping (Bool, String) -> Void) {
        ...
        do {
            ...
            completion(false, String(format: NSLocalizedString("auth.error.save-api-key-failed", comment: "Error when saving API key fails"), error.localizedDescription))
        }
    }
