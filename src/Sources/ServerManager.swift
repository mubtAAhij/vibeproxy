...
class ServerManager: ObservableObject {
    ...
    func runAuthCommand(_ command: AuthCommand, completion: @escaping (Bool, String) -> Void) {
        // Use bundled binary from app bundle
        guard let resourcePath = Bundle.main.resourcePath else {
            completion(false, String(localized: "server.error.no-resource-path", defaultValue: "Could not find resource path", comment: "Error when app bundle resource path is not found"))
            return
        }

        let bundledPath = (resourcePath as NSString).appendingPathComponent("cli-proxy-api-plus")
        guard FileManager.default.fileExists(atPath: bundledPath) else {
            completion(false, String(format: String(localized: "server.error.binary-not-found", defaultValue: "Binary not found at %@", comment: "Error when server binary is missing"), bundledPath))
            return
        }
        ...
        do {
            ...
            completion(false, String(format: String(localized: "auth.error.failed-to-start", defaultValue: "Failed to start auth process: %@", comment: "Error when auth process cannot start"), error.localizedDescription))
        }
    }
    ...
    func saveZaiApiKey(_ apiKey: String, completion: @escaping (Bool, String) -> Void) {
        ...
        do {
            ...
            completion(false, String(format: String(localized: "auth.zai.error.create-directory", defaultValue: "Failed to create auth directory: %@", comment: "Error when auth directory cannot be created"), error.localizedDescription))
            return
        }
        ...
        do {
            ...
            completion(true, String(localized: "auth.zai.success", defaultValue: "API key saved successfully", comment: "Success message when Z.AI API key is saved"))
        } catch {
            completion(false, String(format: String(localized: "auth.zai.error.save-failed", defaultValue: "Failed to save API key: %@", comment: "Error when Z.AI API key cannot be saved"), error.localizedDescription))
        }
    }
