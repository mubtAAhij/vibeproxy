...
class ServerManager: ObservableObject {
    ...
    func runAuthCommand(_ command: AuthCommand, completion: @escaping (Bool, String) -> Void) {
        // Use bundled binary from app bundle
        guard let resourcePath = Bundle.main.resourcePath else {
            completion(false, String(localized: "server.error.resource-path-not-found", defaultValue: "Could not find resource path", comment: "Error when app bundle resource path cannot be determined"))
            return
        }
        ...
        do {
            ...
            DispatchQueue.global(qos: .userInitiated).asyncAfter(deadline: .now() + waitTime) {
                if authProcess.isRunning {
                    ...
                    if case .copilotLogin = command {
                        ...
                        if let codeRange = capture.text.range(of: "enter the code: "),
                           let endRange = capture.text[codeRange.upperBound...].range(of: "\n") {
                            ...
                            completion(true, String(format: NSLocalizedString("auth.github.success-with-code", comment: "Success message when GitHub device code is extracted and copied"), code))
                            return
                    ...
                    if output.contains("Opening browser") || output.contains("Attempting to open URL") {
                        ...
                        let message = error.isEmpty ? (output.isEmpty ? String(localized: "auth.error.process-failed", defaultValue: "Authentication process failed unexpectedly", comment: "Error message when auth process fails without details") : output) : error
            ...
            completion(false, String(format: NSLocalizedString("auth.error.failed-to-start", comment: "Error message when auth process fails to start"), error.localizedDescription))
        }
    }
    ...
    func saveZaiApiKey(_ apiKey: String, completion: @escaping (Bool, String) -> Void) {
        ...
        do {
            ...
            completion(false, String(format: NSLocalizedString("auth.error.failed-to-create-directory", comment: "Error when auth directory creation fails"), error.localizedDescription))
            return
        }
        ...
        do {
            ...
            completion(false, String(format: NSLocalizedString("auth.error.failed-to-save-api-key", comment: "Error when saving API key fails"), error.localizedDescription))
        }
    }
