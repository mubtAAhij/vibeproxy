...
class ServerManager: ObservableObject {
    ...
    func runAuthCommand(_ command: AuthCommand, completion: @escaping (Bool, String) -> Void) {
        // Use bundled binary from app bundle
        guard let resourcePath = Bundle.main.resourcePath else {
            completion(false, String(localized: "error.resource-path-not-found", defaultValue: "Could not find resource path", comment: "Error when bundle resource path is unavailable"))
            return
        }

        let bundledPath = (resourcePath as NSString).appendingPathComponent("cli-proxy-api-plus")
        guard FileManager.default.fileExists(atPath: bundledPath) else {
            completion(false, String(format: NSLocalizedString("error.binary-not-found", comment: "Error when bundled binary is missing"), bundledPath))
            return
        }
        ...
        do {
            ...
            DispatchQueue.global(qos: .userInitiated).asyncAfter(deadline: .now() + waitTime) {
                if authProcess.isRunning {
                    ...
                    if case .copilotLogin = command {
                        ...
                        if let codeRange = capture.text.range(of: "enter the code: "),
                           let endRange = capture.text[codeRange.upperBound...].range(of: "\n") {
                            ...
                            completion(true, String(format: NSLocalizedString("auth.copilot.code-copied", comment: "GitHub auth success with code copied"), code))
                            return
                    ...
                    if output.contains("Opening browser") || output.contains("Attempting to open URL") {
                        ...
                        let message = error.isEmpty ? (output.isEmpty ? String(localized: "error.auth.failed-unexpectedly", defaultValue: "Authentication process failed unexpectedly", comment: "Generic auth failure message") : output) : error
            ...
            completion(false, String(format: NSLocalizedString("error.auth.failed-to-start", comment: "Error starting auth process"), error.localizedDescription))
        }
    }
    ...
    func saveZaiApiKey(_ apiKey: String, completion: @escaping (Bool, String) -> Void) {
        ...
        do {
            ...
            completion(false, String(format: NSLocalizedString("error.auth-dir.create-failed", comment: "Error creating auth directory"), error.localizedDescription))
            return
        }
        ...
        do {
            ...
            completion(true, String(localized: "zai.api-key-saved", defaultValue: "API key saved successfully", comment: "Success message for Z.AI key save"))
        } catch {
            completion(false, String(format: NSLocalizedString("error.zai.api-key-save-failed", comment: "Error saving Z.AI API key"), error.localizedDescription))
        }
    }
