...
class ServerManager: ObservableObject {
    ...
    func runAuthCommand(_ command: AuthCommand, completion: @escaping (Bool, String) -> Void) {
        // Use bundled binary from app bundle
        guard let resourcePath = Bundle.main.resourcePath else {
            completion(false, String(localized: "server-manager.error.resource-path-not-found", defaultValue: "Could not find resource path", bundle: .main, comment: "Error when app resource path cannot be located"))
            return
        }

        let bundledPath = (resourcePath as NSString).appendingPathComponent("cli-proxy-api-plus")
        guard FileManager.default.fileExists(atPath: bundledPath) else {
            completion(false, String(format: String(localized: "server-manager.error.binary-not-found", defaultValue: "Binary not found at %@", bundle: .main, comment: "Error when binary file is missing"), bundledPath))
            return
        }
            ...
            DispatchQueue.global(qos: .userInitiated).asyncAfter(deadline: .now() + waitTime) {
                if authProcess.isRunning {
                    ...
                    if case .copilotLogin = command {
                        ...
                        completion(true, String(localized: "server-manager.auth.github-browser-opened", defaultValue: "🌐 Browser opened for GitHub authentication.\n\nCheck your terminal or the opened browser for the device code.\n\nThe app will automatically detect when you're authenticated.", bundle: .main, comment: "Success message when browser opens for GitHub auth without code"))
                        return
                    }

                    completion(true, String(localized: "server-manager.auth.browser-opened", defaultValue: "🌐 Browser opened for authentication.\n\nPlease complete the login in your browser.\n\nThe app will automatically detect when you're authenticated.", bundle: .main, comment: "Success message when browser opens for authentication"))
                } else {
                    ...
                    if output.contains("Opening browser") || output.contains("Attempting to open URL") {
                        ...
                        let message = error.isEmpty ? (output.isEmpty ? String(localized: "server-manager.error.auth-failed-unexpectedly", defaultValue: "Authentication process failed unexpectedly", bundle: .main, comment: "Error when auth process fails with no error message") : output) : error
    ...
    func saveZaiApiKey(_ apiKey: String, completion: @escaping (Bool, String) -> Void) {
        ...
        do {
            ...
            completion(false, String(format: String(localized: "server-manager.error.auth-dir-failed", defaultValue: "Failed to create auth directory: %@", bundle: .main, comment: "Error when auth directory creation fails"), error.localizedDescription))
            return
        }
        ...
        do {
            ...
            completion(true, String(localized: "server-manager.auth.api-key-saved", defaultValue: "API key saved successfully", bundle: .main, comment: "Success message when API key is saved"))
        } catch {
