...
class ServerManager: ObservableObject {
    ...
    func runAuthCommand(_ command: AuthCommand, completion: @escaping (Bool, String) -> Void) {
        // Use bundled binary from app bundle
        guard let resourcePath = Bundle.main.resourcePath else {
            completion(false, String(localized: "error.resource-path-not-found", defaultValue: "Could not find resource path", comment: "Error when Bundle.main.resourcePath is nil"))
            return
        }

        let bundledPath = (resourcePath as NSString).appendingPathComponent("cli-proxy-api-plus")
        guard FileManager.default.fileExists(atPath: bundledPath) else {
            completion(false, String(format: String(localized: "error.binary-not-found", defaultValue: "Binary not found at %@", comment: "Error when cli-proxy-api-plus binary is missing"), bundledPath))
            return
        }
        ...
        do {
            ...
            DispatchQueue.global(qos: .userInitiated).asyncAfter(deadline: .now() + waitTime) {
                if authProcess.isRunning {
                    ...
                    if output.contains("Opening browser") || output.contains("Attempting to open URL") {
                        ...
                        let message = error.isEmpty ? (output.isEmpty ? String(localized: "error.auth-failed", defaultValue: "Authentication process failed unexpectedly", comment: "Error when auth process dies with no output") : output) : error
            ...
            completion(false, String(format: String(localized: "error.auth-start-failed", defaultValue: "Failed to start auth process: %@", comment: "Error when auth process fails to start"), error.localizedDescription))
        }
    }
    ...
    func saveZaiApiKey(_ apiKey: String, completion: @escaping (Bool, String) -> Void) {
        ...
        do {
            ...
            completion(false, String(format: String(localized: "error.auth-dir-creation-failed", defaultValue: "Failed to create auth directory: %@", comment: "Error when creating .cli-proxy-api directory fails"), error.localizedDescription))
            return
        }
        ...
        do {
            ...
            completion(false, String(format: String(localized: "error.api-key-save-failed", defaultValue: "Failed to save API key: %@", comment: "Error when saving Z.AI API key fails"), error.localizedDescription))
        }
    }
