...
class ServerManager: ObservableObject {
    ...
    func runAuthCommand(_ command: AuthCommand, completion: @escaping (Bool, String) -> Void) {
        ...
        let bundledPath = (resourcePath as NSString).appendingPathComponent("cli-proxy-api-plus")
        guard FileManager.default.fileExists(atPath: bundledPath) else {
            completion(false, String(format: String(localized: "server.error.binary-not-found", defaultValue: "Binary not found at %@", comment: "Error when CLI binary is missing from bundle"), bundledPath))
            return
        }
    ...
    func saveZaiApiKey(_ apiKey: String, completion: @escaping (Bool, String) -> Void) {
        ...
        do {
            ...
            completion(true, String(localized: "auth.apikey.saved", defaultValue: "API key saved successfully", comment: "Success message when API key is saved"))
        } catch {
            completion(false, String(format: String(localized: "auth.error.save-apikey", defaultValue: "Failed to save API key: %@", comment: "Error when API key save fails"), error.localizedDescription))
        }
    }
