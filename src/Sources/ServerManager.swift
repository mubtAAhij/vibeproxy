...
class ServerManager: ObservableObject {
    ...
    func runAuthCommand(_ command: AuthCommand, completion: @escaping (Bool, String) -> Void) {
        // Use bundled binary from app bundle
        guard let resourcePath = Bundle.main.resourcePath else {
            completion(false, String(localized: "server.error.no-resource-path", defaultValue: "Could not find resource path", bundle: .main, comment: "Error when app resource path is not found"))
            return
        }

        let bundledPath = (resourcePath as NSString).appendingPathComponent("cli-proxy-api-plus")
        guard FileManager.default.fileExists(atPath: bundledPath) else {
            completion(false, String(format: String(localized: "server.error.binary-not-found", defaultValue: "Binary not found at %@", bundle: .main, comment: "Error when server binary is not found at expected path"), bundledPath))
            return
        }
            ...
            DispatchQueue.global(qos: .userInitiated).asyncAfter(deadline: .now() + waitTime) {
                if authProcess.isRunning {
                    ...
                    if case .copilotLogin = command {
                        ...
                        completion(true, String(localized: "auth.copilot.browser-opened", defaultValue: "🌐 Browser opened for GitHub authentication.\n\nCheck your terminal or the opened browser for the device code.\n\nThe app will automatically detect when you're authenticated.", bundle: .main, comment: "Message shown when GitHub Copilot auth browser opens but code couldn't be extracted"))
                        return
                    }

                    completion(true, String(localized: "auth.browser-opened", defaultValue: "🌐 Browser opened for authentication.\n\nPlease complete the login in your browser.\n\nThe app will automatically detect when you're authenticated.", bundle: .main, comment: "Generic message shown when authentication browser opens"))
                } else {
                    ...
                    if output.contains("Opening browser") || output.contains("Attempting to open URL") {
                        ...
                        let message = error.isEmpty ? (output.isEmpty ? String(localized: "auth.error.process-failed", defaultValue: "Authentication process failed unexpectedly", bundle: .main, comment: "Error when auth process fails without specific error message") : output) : error
    ...
    func saveZaiApiKey(_ apiKey: String, completion: @escaping (Bool, String) -> Void) {
        ...
        do {
            ...
            completion(false, String(format: String(localized: "server.error.failed-to-create-auth-dir", defaultValue: "Failed to create auth directory: %@", bundle: .main, comment: "Error when authentication directory cannot be created"), error.localizedDescription))
            return
        }
        ...
        do {
            ...
            completion(true, String(localized: "server.api-key-saved", defaultValue: "API key saved successfully", bundle: .main, comment: "Success message when API key is saved"))
        } catch {
            completion(false, String(format: String(localized: "server.error.failed-to-save-api-key", defaultValue: "Failed to save API key: %@", bundle: .main, comment: "Error when API key cannot be saved"), error.localizedDescription))
        }
    }
