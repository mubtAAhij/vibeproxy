...
class ServerManager: ObservableObject {
    ...
    func runAuthCommand(_ command: AuthCommand, completion: @escaping (Bool, String) -> Void) {
        // Use bundled binary from app bundle
        guard let resourcePath = Bundle.main.resourcePath else {
            completion(false, String(localized: "server.error.no-resource-path", defaultValue: "Could not find resource path", bundle: .main, comment: "Error when app resource path cannot be found"))
            return
        }
            ...
            DispatchQueue.global(qos: .userInitiated).asyncAfter(deadline: .now() + waitTime) {
                if authProcess.isRunning {
                    ...
                    if case .copilotLogin = command {
                        ...
                        if let codeRange = capture.text.range(of: "enter the code: "),
                           let endRange = capture.text[codeRange.upperBound...].range(of: "\n") {
                            ...
                            completion(true, String(format: String(localized: "auth.github.code-copied", defaultValue: "🌐 Browser opened for GitHub authentication.\n\n📋 Code copied to clipboard:\n\n%@\n\nJust paste it in the browser!\n\nThe app will automatically detect when you're authenticated.", bundle: .main, comment: "Success message when GitHub device code is copied to clipboard"), code))
                            return
                        ...
                        completion(true, String(localized: "auth.github.browser-opened", defaultValue: "🌐 Browser opened for GitHub authentication.\n\nCheck your terminal or the opened browser for the device code.\n\nThe app will automatically detect when you're authenticated.", bundle: .main, comment: "Fallback message when GitHub auth browser opens but code cannot be extracted"))
                        return
                    }

                    completion(true, String(localized: "auth.browser-opened", defaultValue: "🌐 Browser opened for authentication.\n\nPlease complete the login in your browser.\n\nThe app will automatically detect when you're authenticated.", bundle: .main, comment: "Generic success message when authentication browser opens"))
                } else {
                    ...
                    if output.contains("Opening browser") || output.contains("Attempting to open URL") {
                        ...
                        let message = error.isEmpty ? (output.isEmpty ? String(localized: "auth.error.process-failed", defaultValue: "Authentication process failed unexpectedly", bundle: .main, comment: "Error when authentication process fails without specific error") : output) : error
    ...
    func saveZaiApiKey(_ apiKey: String, completion: @escaping (Bool, String) -> Void) {
        ...
        do {
            ...
            completion(true, String(localized: "auth.api-key-saved", defaultValue: "API key saved successfully", bundle: .main, comment: "Success message when API key is saved"))
        } catch {
            completion(false, String(format: String(localized: "auth.error.failed-to-save-api-key", defaultValue: "Failed to save API key: %@", bundle: .main, comment: "Error when API key cannot be saved"), error.localizedDescription))
        }
    }
