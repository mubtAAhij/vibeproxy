...
class ServerManager: ObservableObject {
    ...
    private func providerConfigLockReason(_ providerKey: String, baseConfigRoot: [String: Any]?) -> String? {
        ...
        return String(format: String(localized: "server-manager.provider-lock.disabled-in-config-oauth-excluded-models", defaultValue: "Disabled in config via oauth-excluded-models. Remove the '*' exclusion for %@ to enable it here.", comment: "Reason shown when provider is disabled by oauth-excluded-models config"), "\(oauthProviderKey)")
    }
    ...
    func setProviderEnabled(_ providerKey: String, enabled: Bool) {
        ...
        if enabled, let lockReason = providerConfigLockReason(providerKey) {
            addLog(String(format: String(localized: "server-manager.provider-status.remains-disabled", defaultValue: "⚠️ %@ remains disabled: %@", comment: "Status line when provider remains disabled with lock reason"), "\(providerKey)", "\(lockReason)"))
        } else {
            addLog(enabled ? String(format: String(localized: "server-manager.provider-status.enabled", defaultValue: "✓ Enabled provider: %@", comment: "Status line when provider is enabled"), "\(providerKey)") : String(format: String(localized: "server-manager.provider-status.disabled", defaultValue: "⚠️ Disabled provider: %@", comment: "Status line when provider is disabled"), "\(providerKey)"))
        }
        reloadCustomProviders()
        requestConfigUpdate()
    }
    ...
    func start(completion: @escaping (Bool) -> Void) {
        ...
        guard let resourcePath = Bundle.main.resourcePath else {
            addLog(String(localized: "server-manager.error.resource-path-not-found", defaultValue: "❌ Error: Could not find resource path", comment: "Error message when app resource path cannot be resolved"))
            completion(false)
            return
        }
        ...
        guard FileManager.default.fileExists(atPath: bundledPath) else {
            addLog(String(format: String(localized: "server-manager.error.binary-not-found-at-path", defaultValue: "❌ Error: cli-proxy-api-plus binary not found at %@", comment: "Error message when bundled cli-proxy-api-plus binary is missing"), "\(bundledPath)"))
            completion(false)
            return
        }
        ...
        guard !configPath.isEmpty && FileManager.default.fileExists(atPath: configPath) else {
            addLog(String(format: String(localized: "server-manager.error.config-resolution", defaultValue: "❌ Error: %@", comment: "Error message when resolving active config path fails"), "\(configErrorMessage ?? String(localized: "server-manager.error.active-config-path-unresolved", defaultValue: "Could not resolve active config path", comment: "Fallback config error detail when active config path cannot be resolved"))"))
            completion(false)
            return
        }
        ...
        errorPipe.fileHandleForReading.readabilityHandler = { [weak self] handle in
            let data = handle.availableData
            if let output = String(data: data, encoding: .utf8), !output.isEmpty {
                self?.addLog(String(format: String(localized: "server-manager.warning.output", defaultValue: "⚠️ %@", comment: "Warning line that prefixes server output"), "\(output)"))
            }
        }
        ...
        process?.terminationHandler = { [weak self] process in
            ...
            DispatchQueue.main.async {
                ...
                self?.addLog(String(format: String(localized: "server-manager.status.stopped-with-code", defaultValue: "Server stopped with code: %d", comment: "Status message when server process exits with termination code"), process.terminationStatus))
        ...
        do {
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + Timing.readinessCheckDelay) { [weak self] in
                ...
                if let process = self.process, process.isRunning {
                    ...
                    self.addLog(String(localized: "server-manager.warning.exited-before-ready", defaultValue: "⚠️ Server exited before becoming ready", comment: "Warning message when server exits before ready state"))
                    completion(false)
                }
            }
        } catch {
            addLog(String(format: String(localized: "server-manager.error.failed-to-start", defaultValue: "❌ Failed to start server: %@", comment: "Error message when server start fails with error details"), "\(error.localizedDescription)"))
            completion(false)
        }
    }
    ...
    func stop(completion: (() -> Void)? = nil) {
        ...
        addLog(String(format: String(localized: "server-manager.status.stopping-with-pid", defaultValue: "Stopping server (PID: %d)...", comment: "Status message when stopping server process by PID"), pid))
        processQueue.async { [weak self] in
            ...
            if process.isRunning {
                self.addLog(String(localized: "server-manager.warning.force-kill-server", defaultValue: "⚠️ Server didn't stop gracefully, force killing...", comment: "Warning message when server requires force kill"))
                kill(pid, SIGKILL)
            }
            ...
            DispatchQueue.main.async {
                ...
                self.isRunning = false
                self.activeConfigPath = ""
                self.addLog(String(localized: "server-manager.status.stopped", defaultValue: "✓ Server stopped", comment: "Status message when server stops successfully"))
                NotificationCenter.default.post(name: .serverStatusChanged, object: nil)
                completion?()
            }
        }
    ...
    func runAuthCommand(_ command: AuthCommand, completion: @escaping (Bool, String) -> Void) {
        ...
        // Use bundled binary from app bundle
        guard let resourcePath = Bundle.main.resourcePath else {
            completion(false, String(localized: "server-manager.error.resource-path-not-found-plain", defaultValue: "Could not find resource path", comment: "Plain error message when app resource path cannot be resolved"))
            return
        }
        
        let bundledPath = (resourcePath as NSString).appendingPathComponent("cli-proxy-api-plus")
        guard FileManager.default.fileExists(atPath: bundledPath) else {
            completion(false, String(format: String(localized: "server-manager.error.binary-not-found-at-path-plain", defaultValue: "Binary not found at %@", comment: "Plain error message when bundled binary is missing at path"), "\(bundledPath)"))
            return
        }
        ...
        guard !configPath.isEmpty else {
            completion(false, configErrorMessage ?? String(localized: "server-manager.error.config-path-unresolved", defaultValue: "Could not resolve config path", comment: "Error message when config path resolution fails"))
            return
        }
        ...
        do {
            ...
            try authProcess.run()
            addLog(String(format: String(localized: "server-manager.auth.status.process-started", defaultValue: "✓ Authentication process started (PID: %d) - browser should open shortly", comment: "Status message when authentication helper process starts"), authProcess.processIdentifier))
            ...
            DispatchQueue.global(qos: .userInitiated).asyncAfter(deadline: .now() + waitTime) {
                if authProcess.isRunning {
                    ...
                    if case .copilotLogin = command {
                        ...
                        if let codeRange = capture.text.range(of: "enter the code: "),
                           let endRange = capture.text[codeRange.upperBound...].range(of: "\n") {
                            ...
                            completion(true, String(format: String(localized: "server-manager.auth.github.browser-opened-code-copied", defaultValue: "🌐 Browser opened for GitHub authentication.\n\n📋 Code copied to clipboard:\n\n%@\n\nJust paste it in the browser!\n\nThe app will automatically detect when you're authenticated.", comment: "Instruction message when GitHub authentication starts and device code is copied"), "\(code)"))
                            return
                    ...
                    if output.contains("Opening browser") || output.contains("Attempting to open URL") {
                        ...
                        let message = error.isEmpty ? (output.isEmpty ? String(localized: "server-manager.auth.error.process-failed-unexpectedly", defaultValue: "Authentication process failed unexpectedly", comment: "Error message when authentication helper process fails unexpectedly") : output) : error
            ...
            completion(false, String(format: String(localized: "server-manager.auth.error.failed-to-start", defaultValue: "Failed to start auth process: %@", comment: "Error message when authentication process cannot be started"), "\(error.localizedDescription)"))
        }
    }
    ...
    private func terminateActiveAuthProcessIfNeeded(reason: String) {
        ...
        if authProcess.isRunning {
            addLog(String(format: String(localized: "server-manager.auth.warning.terminating-previous-process-before-retry", defaultValue: "⚠️ Terminating previous auth process (%d) before retry: %@", comment: "Warning message when previous auth process is terminated before retry"), authProcess.processIdentifier, "\(reason)"))
        ...
        for pattern in patterns {
            ...
            do {
                ...
                for pid in pids {
                    ...
                    kill(pid, SIGKILL)
                    addLog(String(format: String(localized: "server-manager.auth.warning.cleaned-stale-listener-process", defaultValue: "⚠️ Cleaned up stale auth listener process: %d", comment: "Warning message when stale auth listener process is cleaned up"), pid))
                }
            } catch {
    ...
    func saveZaiApiKey(_ apiKey: String, completion: @escaping (Bool, String) -> Void) {
        credentialMutationQueue.async { [weak self] in
            ...
            do {
                ...
                self.addLog(String(format: String(localized: "server-manager.zai-api-key.saved-to-file", defaultValue: "✓ Z.AI API key saved to %@", comment: "Success message when Z.AI API key is saved to file"), "\(filePath.lastPathComponent)"))
                self.refreshAuthBackedConfiguration()
                DispatchQueue.main.async {
                    completion(true, String(localized: "server-manager.api-key.saved-successfully", defaultValue: "API key saved successfully", comment: "Success title message after saving API key"))
                }
            } catch {
                DispatchQueue.main.async {
        ...
        credentialMutationQueue.async { [weak self] in
            ...
            do {
                ...
                guard let provider = customProviders.first(where: { $0.id == providerID }) else {
                    DispatchQueue.main.async {
                        completion(false, String(format: String(localized: "server-manager.custom-provider.not-defined-in-config", defaultValue: "Custom provider '%@' is not defined in config.yaml.", comment: "Error message when custom provider ID is missing from config.yaml"), "\(providerID)"))
                    }
                    return
                }
                ...
                if provider.inlineAPIKeys.contains(apiKey) {
                    ...
                    DispatchQueue.main.async {
                        completion(true, String(localized: "server-manager.api-key.already-exists-in-config", defaultValue: "API key already exists in config", comment: "Status title when API key already exists in config"))
                    }
                    return
                }
                ...
                case .created(let record):
                    self.addLog(String(format: String(localized: "server-manager.custom-provider.api-key-saved", defaultValue: "✓ Saved API key for custom provider: %@", comment: "Status message when custom provider API key is saved"), "\(record.providerID)"))
                case .alreadyPresent(let record):
                    self.addLog(String(format: String(localized: "server-manager.custom-provider.key-already-present", defaultValue: "✓ Custom provider key already present: %@", comment: "Status message when custom provider key is already stored"), "\(record.label)"))
                case .reenabled(let record):
                    self.addLog(String(format: String(localized: "server-manager.custom-provider.key-re-enabled", defaultValue: "✓ Re-enabled custom provider key: %@", comment: "Status message when existing custom provider key is re-enabled"), "\(record.label)"))
                }
                self.refreshAuthBackedConfiguration()
                DispatchQueue.main.async {
                    switch saveResult {
                    case .created:
                        completion(true, String(localized: "server-manager.api-key.saved-successfully", defaultValue: "API key saved successfully", comment: "Success title message after saving API key"))
                    case .alreadyPresent:
                        completion(true, String(localized: "server-manager.api-key.already-exists", defaultValue: "API key already exists", comment: "Status title when API key already exists"))
                    case .reenabled:
                        completion(true, String(localized: "server-manager.api-key.re-enabled", defaultValue: "API key was already stored and has been re-enabled", comment: "Status title when stored API key was re-enabled"))
                    }
                }
            } catch {
                DispatchQueue.main.async {
    ...
    func deleteCustomProviderCredential(_ credential: CustomProviderCredential) -> Bool {
        do {
            ...
            addLog(String(format: String(localized: "server-manager.custom-provider.key-removed", defaultValue: "✓ Removed custom provider key: %@", comment: "Status message when custom provider key is removed"), "\(credential.label)"))
            if deletedCount > 1 {
                addLog(String(format: String(localized: "server-manager.custom-provider.duplicate-credential-files-removed", defaultValue: "✓ Removed %d duplicate credential files for %@", comment: "Status message when duplicate credential files are removed for a provider"), deletedCount, "\(credential.providerID)"))
            }
            markObservedConfigInputsCurrent()
            reloadCustomProviders()
            requestConfigUpdate()
        ...
        do {
            ...
            addLog(
                credential.isDisabled
                    ? String(format: String(localized: "server-manager.custom-provider.key-enabled", defaultValue: "✓ Enabled custom provider key: %@", comment: "Status message when custom provider key is enabled"), "\(credential.label)")
                    : String(format: String(localized: "server-manager.custom-provider.key-disabled", defaultValue: "⚠️ Disabled custom provider key: %@", comment: "Status message when custom provider key is disabled"), "\(credential.label)")
            )
            markObservedConfigInputsCurrent()
            reloadCustomProviders()
            requestConfigUpdate()
        ...
        do {
            ...
            return .failure(
                ConfigResolutionFailure(
                    message: String(format: String(localized: "server-manager.config.error.write-merged-config-failed", defaultValue: "Failed to write merged config to %@: %@", comment: "Error when writing merged configuration file fails"), "\(mergedConfigPath.path)", "\(error.localizedDescription)")
                )
            )
        }
    }
        ...
        do {
            ...
            if checkTask.terminationStatus == 0 {
                ...
                if !pids.isEmpty {
                    ...
                    addLog(String(localized: "server-manager.process.status.orphaned-processes-cleaned", defaultValue: "✓ Cleaned up orphaned processes", comment: "Status message when orphaned server processes are cleaned up"))
                }
            }
    ...
    private func loadBaseConfigRoot() -> Result<LoadedBaseConfig, ConfigResolutionFailure> {
        ...
        guard case .success(let bundledRoot) = bundledRootResult else {
            ...
            return .failure(ConfigResolutionFailure(message: String(format: String(localized: "server-manager.config.error.bundled-config-load-failed", defaultValue: "Could not load the bundled config at %@.", comment: "Error when bundled config cannot be loaded from path"), "\(bundledConfigPath)")))
        }
        ...
        guard case .success(let userRoot) = userRootResult else {
            ...
            return .failure(ConfigResolutionFailure(message: String(format: String(localized: "server-manager.config.error.user-config-load-failed", defaultValue: "Could not load the user config at %@.", comment: "Error when user config cannot be loaded from path"), "\(userConfigPath)")))
        }
    ...
    private func loadYAMLDictionary(atPath path: String) -> Result<[String: Any], ConfigResolutionFailure> {
        do {
            ...
            guard let dictionary = ConfigComposer.stringKeyedDictionary(loaded) else {
                return .failure(ConfigResolutionFailure(message: String(format: String(localized: "server-manager.config.error.root-must-be-yaml-mapping", defaultValue: "Config at %@ must be a YAML mapping at the root.", comment: "Validation error when YAML config root is not a mapping"), "\(path)")))
            }
            return .success(dictionary)
        } catch {
            return .failure(ConfigResolutionFailure(message: String(format: String(localized: "server-manager.config.error.yaml-parse-failed", defaultValue: "Failed to parse YAML at %@: %@", comment: "Error when parsing YAML config fails"), "\(path)", "\(error.localizedDescription)")))
        }
    }
    ...
    private func validatedLoadedBaseConfig(
        ...
        guard validationErrors.isEmpty else {
            return .failure(
                ConfigResolutionFailure(
                    message: String(format: String(localized: "server-manager.custom-provider.error.invalid-configuration", defaultValue: "Invalid custom provider configuration. %@", comment: "Validation error message for invalid custom provider configuration"), "\(validationErrors.joined(separator: " "))")
                )
            )
        }
    ...
    private func publishConfigError(_ message: String) {
        let update = {
            ...
            if shouldLog {
                self.addLog(String(format: String(localized: "server-manager.error.prefixed-message", defaultValue: "❌ %@", comment: "Error line that prefixes a dynamic message"), "\(message)"))
            }
        }
    ...
    private func finishConfigUpdateResolution(_ result: Result<String, ConfigResolutionFailure>) {
        ...
        if shouldRestart {
            ...
            hasPendingConfigUpdate = false
            addLog(String(localized: "server-manager.config.status.path-changed-restarting", defaultValue: "Config path changed; restarting server", comment: "Status message when config path changes and server restarts"))
            stop { [weak self] in
