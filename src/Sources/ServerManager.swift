...
class ServerManager: ObservableObject {
    ...
    func runAuthCommand(_ command: AuthCommand, completion: @escaping (Bool, String) -> Void) {
        // Use bundled binary from app bundle
        guard let resourcePath = Bundle.main.resourcePath else {
            completion(false, String(localized: "server-manager.error.resource-path-not-found", defaultValue: "Could not find resource path", bundle: .main, comment: "Error when resource path cannot be located"))
            return
        }

        let bundledPath = (resourcePath as NSString).appendingPathComponent("cli-proxy-api-plus")
        guard FileManager.default.fileExists(atPath: bundledPath) else {
            completion(false, String(format: String(localized: "server-manager.error.binary-not-found", defaultValue: "Binary not found at %@", bundle: .main, comment: "Error when binary is not found at the expected path"), bundledPath))
            return
        }
        ...
        do {
            ...
            DispatchQueue.global(qos: .userInitiated).asyncAfter(deadline: .now() + waitTime) {
                if authProcess.isRunning {
                    ...
                    if case .copilotLogin = command {
                        ...
                        if let codeRange = capture.text.range(of: "enter the code: "),
                           let endRange = capture.text[codeRange.upperBound...].range(of: "\n") {
                            ...
                            completion(true, String(format: String(localized: "server-manager.auth.copilot-code-copied", defaultValue: "🌐 Browser opened for GitHub authentication.\n\n📋 Code copied to clipboard:\n\n%@\n\nJust paste it in the browser!\n\nThe app will automatically detect when you're authenticated.", bundle: .main, comment: "Message when GitHub Copilot device code is copied to clipboard"), code))
                            return
                        ...
                        completion(true, String(localized: "server-manager.auth.copilot-check-terminal", defaultValue: "🌐 Browser opened for GitHub authentication.\n\nCheck your terminal or the opened browser for the device code.\n\nThe app will automatically detect when you're authenticated.", bundle: .main, comment: "Fallback message when GitHub Copilot device code cannot be extracted"))
                        return
                    }

                    completion(true, String(localized: "server-manager.auth.browser-opened", defaultValue: "🌐 Browser opened for authentication.\n\nPlease complete the login in your browser.\n\nThe app will automatically detect when you're authenticated.", bundle: .main, comment: "Generic message when browser opens for authentication"))
                } else {
                    ...
                    if output.contains("Opening browser") || output.contains("Attempting to open URL") {
                        ...
                        let message = error.isEmpty ? (output.isEmpty ? String(localized: "server-manager.error.auth-failed-unexpectedly", defaultValue: "Authentication process failed unexpectedly", bundle: .main, comment: "Error when authentication process fails without output") : output) : error
            ...
            completion(false, String(format: String(localized: "server-manager.error.auth-start-failed", defaultValue: "Failed to start auth process: %@", bundle: .main, comment: "Error when authentication process fails to start"), error.localizedDescription))
        }
    }
    ...
    func saveZaiApiKey(_ apiKey: String, completion: @escaping (Bool, String) -> Void) {
        ...
        do {
            ...
            completion(false, String(format: String(localized: "server-manager.error.auth-directory-failed", defaultValue: "Failed to create auth directory: %@", bundle: .main, comment: "Error when auth directory creation fails"), error.localizedDescription))
            return
        }
        ...
        do {
            ...
            completion(true, String(localized: "server-manager.success.api-key-saved", defaultValue: "API key saved successfully", bundle: .main, comment: "Success message when API key is saved"))
        } catch {
            completion(false, String(format: String(localized: "server-manager.error.api-key-save-failed", defaultValue: "Failed to save API key: %@", bundle: .main, comment: "Error when API key save fails"), error.localizedDescription))
        }
    }
