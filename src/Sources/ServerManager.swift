...
class ServerManager: ObservableObject {
    ...
    func runAuthCommand(_ command: AuthCommand, completion: @escaping (Bool, String) -> Void) {
        // Use bundled binary from app bundle
        guard let resourcePath = Bundle.main.resourcePath else {
            completion(false, String(localized: "server.error.resource-path-not-found", defaultValue: "Could not find resource path", comment: "Error when app bundle resource path is unavailable"))
            return
        }

        let bundledPath = (resourcePath as NSString).appendingPathComponent("cli-proxy-api-plus")
        guard FileManager.default.fileExists(atPath: bundledPath) else {
            completion(false, String(format: String(localized: "server.error.binary-not-found", defaultValue: "Binary not found at %@", comment: "Error when bundled binary does not exist at expected path"), bundledPath))
            return
        }
        ...
        do {
            ...
            DispatchQueue.global(qos: .userInitiated).asyncAfter(deadline: .now() + waitTime) {
                if authProcess.isRunning {
                    ...
                    if case .copilotLogin = command {
                        ...
                        if let codeRange = capture.text.range(of: "enter the code: "),
                           let endRange = capture.text[codeRange.upperBound...].range(of: "\n") {
                            ...
                            completion(true, String(format: String(localized: "auth.github-copilot.device-code-copied", defaultValue: "🌐 Browser opened for GitHub authentication.\n\n📋 Code copied to clipboard:\n\n%@\n\nJust paste it in the browser!\n\nThe app will automatically detect when you're authenticated.", comment: "Success message when GitHub Copilot device code is copied to clipboard"), code))
                            return
                        ...
                        completion(true, String(localized: "auth.github-copilot.browser-opened", defaultValue: "🌐 Browser opened for GitHub authentication.\n\nCheck your terminal or the opened browser for the device code.\n\nThe app will automatically detect when you're authenticated.", comment: "Fallback message when GitHub Copilot auth browser opens but device code cannot be extracted"))
                        return
                    }

                    completion(true, String(localized: "auth.browser-opened", defaultValue: "🌐 Browser opened for authentication.\n\nPlease complete the login in your browser.\n\nThe app will automatically detect when you're authenticated.", comment: "Generic success message when authentication browser window opens"))
                } else {
                    ...
                    if output.contains("Opening browser") || output.contains("Attempting to open URL") {
                        ...
                        let message = error.isEmpty ? (output.isEmpty ? String(localized: "auth.error.process-failed", defaultValue: "Authentication process failed unexpectedly", comment: "Error when authentication process terminates without any output") : output) : error
            ...
            completion(false, String(format: String(localized: "auth.error.failed-to-start", defaultValue: "Failed to start auth process: %@", comment: "Error when authentication subprocess cannot be started"), error.localizedDescription))
        }
    }
    ...
    func saveZaiApiKey(_ apiKey: String, completion: @escaping (Bool, String) -> Void) {
        ...
        do {
            ...
            completion(false, String(format: String(localized: "auth.error.create-directory-failed", defaultValue: "Failed to create auth directory: %@", comment: "Error when creating .cli-proxy-api directory fails"), error.localizedDescription))
            return
        }
        ...
        do {
            ...
            completion(true, String(localized: "auth.api-key-saved", defaultValue: "API key saved successfully", comment: "Success message when Z.AI API key is saved"))
        } catch {
            completion(false, String(format: String(localized: "auth.error.save-api-key-failed", defaultValue: "Failed to save API key: %@", comment: "Error when writing Z.AI API key file fails"), error.localizedDescription))
        }
    }
