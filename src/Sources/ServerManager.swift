...
class ServerManager: ObservableObject {
    ...
    func runAuthCommand(_ command: AuthCommand, completion: @escaping (Bool, String) -> Void) {
        // Use bundled binary from app bundle
        guard let resourcePath = Bundle.main.resourcePath else {
            completion(false, String(localized: "server.error.resource-path-not-found", defaultValue: "Could not find resource path", bundle: .main, comment: "Error when app resource path cannot be found"))
            return
        }

        let bundledPath = (resourcePath as NSString).appendingPathComponent("cli-proxy-api-plus")
        guard FileManager.default.fileExists(atPath: bundledPath) else {
            completion(false, String(format: String(localized: "server.error.binary-not-found", defaultValue: "Binary not found at %@", bundle: .main, comment: "Error when binary is not found at expected path"), bundledPath))
            return
        }
        ...
        do {
            ...
            DispatchQueue.global(qos: .userInitiated).asyncAfter(deadline: .now() + waitTime) {
                if authProcess.isRunning {
                    ...
                    if case .copilotLogin = command {
                        ...
                        if let codeRange = capture.text.range(of: "enter the code: "),
                           let endRange = capture.text[codeRange.upperBound...].range(of: "\n") {
                            ...
                            completion(true, String(format: String(localized: "auth.copilot.code-copied", defaultValue: "🌐 Browser opened for GitHub authentication.\n\n📋 Code copied to clipboard:\n\n%@\n\nJust paste it in the browser!\n\nThe app will automatically detect when you're authenticated.", bundle: .main, comment: "Success message with device code for GitHub Copilot authentication"), code))
                            return
                        ...
                        completion(true, String(localized: "auth.copilot.code-not-extracted", defaultValue: "🌐 Browser opened for GitHub authentication.\n\nCheck your terminal or the opened browser for the device code.\n\nThe app will automatically detect when you're authenticated.", bundle: .main, comment: "Fallback message for GitHub Copilot authentication when device code cannot be extracted"))
                        return
                    }

                    completion(true, String(localized: "auth.generic.browser-opened", defaultValue: "🌐 Browser opened for authentication.\n\nPlease complete the login in your browser.\n\nThe app will automatically detect when you're authenticated.", bundle: .main, comment: "Generic success message when browser opens for authentication"))
                } else {
                    ...
                    if output.contains("Opening browser") || output.contains("Attempting to open URL") {
                        ...
                        let message = error.isEmpty ? (output.isEmpty ? String(localized: "auth.error.process-failed", defaultValue: "Authentication process failed unexpectedly", bundle: .main, comment: "Error when authentication process fails without specific error message") : output) : error
            ...
            completion(false, String(format: String(localized: "auth.error.failed-to-start", defaultValue: "Failed to start auth process: %@", bundle: .main, comment: "Error when authentication process fails to start"), error.localizedDescription))
        }
    }
    ...
    func saveZaiApiKey(_ apiKey: String, completion: @escaping (Bool, String) -> Void) {
        ...
        do {
            ...
            completion(false, String(format: String(localized: "auth.error.create-directory-failed", defaultValue: "Failed to create auth directory: %@", bundle: .main, comment: "Error when auth directory cannot be created"), error.localizedDescription))
            return
        }
        ...
        do {
            ...
            completion(true, String(localized: "auth.zai.save-success", defaultValue: "API key saved successfully", bundle: .main, comment: "Success message when Z.AI API key is saved"))
        } catch {
            completion(false, String(format: String(localized: "auth.zai.save-failed", defaultValue: "Failed to save API key: %@", bundle: .main, comment: "Error when Z.AI API key cannot be saved"), error.localizedDescription))
        }
    }
