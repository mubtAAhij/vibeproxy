...
class ServerManager: ObservableObject {
    ...
    func runAuthCommand(_ command: AuthCommand, completion: @escaping (Bool, String) -> Void) {
        // Use bundled binary from app bundle
        guard let resourcePath = Bundle.main.resourcePath else {
            completion(false, String(localized: "server-manager.error.resource-path-not-found", defaultValue: "Could not find resource path", comment: "Error when app resource path cannot be found"))
            return
        }

        let bundledPath = (resourcePath as NSString).appendingPathComponent("cli-proxy-api-plus")
        guard FileManager.default.fileExists(atPath: bundledPath) else {
            completion(false, String(format: NSLocalizedString("server-manager.error.binary-not-found", comment: "Error when binary file is not found at path"), bundledPath))
            return
        }
        ...
        do {
            ...
            DispatchQueue.global(qos: .userInitiated).asyncAfter(deadline: .now() + waitTime) {
                if authProcess.isRunning {
                    ...
                    if case .copilotLogin = command {
                        ...
                        if let codeRange = capture.text.range(of: "enter the code: "),
                           let endRange = capture.text[codeRange.upperBound...].range(of: "\n") {
                            ...
                            completion(true, String(format: NSLocalizedString("server-manager.auth.github-code-copied", comment: "Success message when GitHub auth code is copied to clipboard"), code))
                            return
                    ...
                    if output.contains("Opening browser") || output.contains("Attempting to open URL") {
                        ...
                        let message = error.isEmpty ? (output.isEmpty ? String(localized: "server-manager.error.auth-failed-unexpectedly", defaultValue: "Authentication process failed unexpectedly", comment: "Error when auth process fails without error message") : output) : error
            ...
            completion(false, String(format: NSLocalizedString("server-manager.error.auth-process-start-failed", comment: "Error when auth process fails to start"), error.localizedDescription))
        }
    }
    ...
    func saveZaiApiKey(_ apiKey: String, completion: @escaping (Bool, String) -> Void) {
        ...
        do {
            ...
            completion(false, String(format: NSLocalizedString("server-manager.error.auth-dir-create-failed", comment: "Error when auth directory creation fails"), error.localizedDescription))
            return
        }
        ...
        do {
            ...
            completion(true, String(localized: "server-manager.success.api-key-saved", defaultValue: "API key saved successfully", comment: "Success message when API key is saved"))
        } catch {
            completion(false, String(format: NSLocalizedString("server-manager.error.api-key-save-failed", comment: "Error when API key save fails"), error.localizedDescription))
        }
    }
