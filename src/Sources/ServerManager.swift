...
class ServerManager: ObservableObject {
    ...
    private func providerConfigLockReason(_ providerKey: String, baseConfigRoot: [String: Any]?) -> String? {
        ...
        return String(format: String(localized: "server-manager.oauth-provider.disabled-by-config", defaultValue: "Disabled in config via oauth-excluded-models. Remove the '*' exclusion for %@ to enable it here.", comment: "Message explaining OAuth provider remains disabled due to oauth-excluded-models config"), "\(oauthProviderKey)")
    }
    ...
    func setProviderEnabled(_ providerKey: String, enabled: Bool) {
        ...
        if enabled, let lockReason = providerConfigLockReason(providerKey) {
            addLog(String(format: String(localized: "server-manager.oauth-provider.remains-disabled", defaultValue: "⚠️ %@ remains disabled: %@", comment: "Status message when provider remains disabled with lock reason"), "\(providerKey)", "\(lockReason)"))
        } else {
            addLog(enabled ? String(format: String(localized: "server-manager.oauth-provider.enabled", defaultValue: "✓ Enabled provider: %@", comment: "Status message when provider is enabled"), "\(providerKey)") : String(format: String(localized: "server-manager.oauth-provider.disabled", defaultValue: "⚠️ Disabled provider: %@", comment: "Status message when provider is disabled"), "\(providerKey)"))
        }
        reloadCustomProviders()
        requestConfigUpdate()
    }
    ...
    func start(completion: @escaping (Bool) -> Void) {
        ...
        guard !configPath.isEmpty && FileManager.default.fileExists(atPath: configPath) else {
            addLog(String(format: String(localized: "server-manager.error.config-resolution-failed", defaultValue: "❌ Error: %@", comment: "Error message prefix for config resolution failure details"), "\(configErrorMessage ?? String(localized: "server-manager.error.active-config-path-not-resolved", defaultValue: "Could not resolve active config path", comment: "Fallback config error detail when active config path cannot be resolved"))"))
            completion(false)
            return
        }
        ...
        errorPipe.fileHandleForReading.readabilityHandler = { [weak self] handle in
            let data = handle.availableData
            if let output = String(data: data, encoding: .utf8), !output.isEmpty {
                self?.addLog(String(format: String(localized: "server-manager.warning.output", defaultValue: "⚠️ %@", comment: "Warning log line showing server output content"), "\(output)"))
            }
        }
        ...
        process?.terminationHandler = { [weak self] process in
            ...
            DispatchQueue.main.async {
                ...
                self?.addLog(String(format: String(localized: "server-manager.status.server-stopped-with-code", defaultValue: "Server stopped with code: %d", comment: "Status message when server process exits with termination code"), process.terminationStatus))
        ...
        do {
            ...
            addLog(String(format: String(localized: "server-manager.status.server-started-on-port", defaultValue: "✓ Server started on port %d", comment: "Status message when server starts successfully on a port"), port))
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + Timing.readinessCheckDelay) { [weak self] in
                ...
                if let process = self.process, process.isRunning {
                    ...
                    self.addLog(String(localized: "server-manager.warning.server-exited-before-ready", defaultValue: "⚠️ Server exited before becoming ready", comment: "Warning when server exits before readiness check completes"))
                    completion(false)
                }
            }
        } catch {
            addLog(String(format: String(localized: "server-manager.error.failed-to-start-server", defaultValue: "❌ Failed to start server: %@", comment: "Error message when server start throws an error"), "\(error.localizedDescription)"))
            completion(false)
        }
    }
    ...
    func stop(completion: (() -> Void)? = nil) {
        ...
        addLog(String(format: String(localized: "server-manager.status.stopping-server-with-pid", defaultValue: "Stopping server (PID: %d)...", comment: "Status message when stopping server process with PID"), pid))
        processQueue.async { [weak self] in
            ...
            if process.isRunning {
                self.addLog(String(localized: "server-manager.warning.server-force-kill", defaultValue: "⚠️ Server didn't stop gracefully, force killing...", comment: "Warning message when server requires force kill"))
                kill(pid, SIGKILL)
            }
            ...
            DispatchQueue.main.async {
                ...
                self.isRunning = false
                self.activeConfigPath = ""
                self.addLog(String(localized: "server-manager.status.server-stopped", defaultValue: "✓ Server stopped", comment: "Status message when server stops successfully"))
                NotificationCenter.default.post(name: .serverStatusChanged, object: nil)
                completion?()
            }
        }
    ...
    func runAuthCommand(_ command: AuthCommand, completion: @escaping (Bool, String) -> Void) {
        ...
        // Use bundled binary from app bundle
        guard let resourcePath = Bundle.main.resourcePath else {
            completion(false, String(localized: "server-manager.error.resource-path-not-found-plain", defaultValue: "Could not find resource path", comment: "Plain error text when resource path cannot be found"))
            return
        }
        
        let bundledPath = (resourcePath as NSString).appendingPathComponent("cli-proxy-api-plus")
        guard FileManager.default.fileExists(atPath: bundledPath) else {
            completion(false, String(format: String(localized: "server-manager.error.binary-not-found", defaultValue: "Binary not found at %@", comment: "Error text when binary path is missing"), "\(bundledPath)"))
            return
        }
        ...
        do {
            ...
            try authProcess.run()
            addLog(String(format: String(localized: "server-manager.auth-process.started", defaultValue: "✓ Authentication process started (PID: %d) - browser should open shortly", comment: "Status message when authentication process starts and browser is expected to open"), authProcess.processIdentifier))
            ...
            completion(false, String(format: String(localized: "server-manager.auth-process.start-failed", defaultValue: "Failed to start auth process: %@", comment: "Error message when starting authentication process fails"), "\(error.localizedDescription)"))
        }
    }
    ...
    private func terminateActiveAuthProcessIfNeeded(reason: String) {
        ...
        if authProcess.isRunning {
            addLog(String(format: String(localized: "server-manager.auth-process.terminating-previous-before-retry", defaultValue: "⚠️ Terminating previous auth process (%d) before retry: %@", comment: "Warning shown when terminating previous auth process before retry"), authProcess.processIdentifier, "\(reason)"))
        ...
        for pattern in patterns {
            ...
            do {
                ...
                for pid in pids {
                    ...
                    kill(pid, SIGKILL)
                    addLog(String(format: String(localized: "server-manager.auth-process.cleaned-stale-listener", defaultValue: "⚠️ Cleaned up stale auth listener process: %d", comment: "Warning shown when stale auth listener process is cleaned up"), pid))
                }
            } catch {
    ...
    func saveZaiApiKey(_ apiKey: String, completion: @escaping (Bool, String) -> Void) {
        credentialMutationQueue.async { [weak self] in
            ...
            do {
                ...
                self.addLog(String(format: String(localized: "server-manager.zai.api-key-saved-to-file", defaultValue: "✓ Z.AI API key saved to %@", comment: "Success message when Z.AI API key is saved to a file"), "\(filePath.lastPathComponent)"))
                self.refreshAuthBackedConfiguration()
                DispatchQueue.main.async {
                    completion(true, String(localized: "server-manager.api-key.saved-successfully", defaultValue: "API key saved successfully", comment: "Generic success message when API key is saved"))
                }
            } catch {
                DispatchQueue.main.async {
        ...
        credentialMutationQueue.async { [weak self] in
            ...
            do {
                ...
                guard let provider = customProviders.first(where: { $0.id == providerID }) else {
                    DispatchQueue.main.async {
                        completion(false, String(format: String(localized: "server-manager.custom-provider.not-defined-in-config", defaultValue: "Custom provider '%@' is not defined in config.yaml.", comment: "Error message when custom provider ID is missing from config"), "\(providerID)"))
                    }
                    return
                }
                ...
                if provider.inlineAPIKeys.contains(apiKey) {
                    ...
                    DispatchQueue.main.async {
                        completion(true, String(localized: "server-manager.api-key.already-exists-in-config", defaultValue: "API key already exists in config", comment: "Generic message when API key already exists in config"))
                    }
                    return
                }
                ...
                case .reenabled(let record):
                    self.addLog(String(format: String(localized: "server-manager.custom-provider.key-reenabled", defaultValue: "✓ Re-enabled custom provider key: %@", comment: "Message when disabled custom provider key is re-enabled"), "\(record.label)"))
                }
                self.refreshAuthBackedConfiguration()
                DispatchQueue.main.async {
                    switch saveResult {
                    case .created:
                        completion(true, String(localized: "server-manager.api-key.saved-successfully", defaultValue: "API key saved successfully", comment: "Generic success message when API key is saved"))
                    case .alreadyPresent:
                        completion(true, String(localized: "server-manager.api-key.already-exists", defaultValue: "API key already exists", comment: "Generic message when API key already exists"))
                    case .reenabled:
                        completion(true, String(localized: "server-manager.api-key.reenabled-existing", defaultValue: "API key was already stored and has been re-enabled", comment: "Message when stored API key is re-enabled"))
                    }
                }
            } catch {
                DispatchQueue.main.async {
    ...
    func deleteCustomProviderCredential(_ credential: CustomProviderCredential) -> Bool {
        do {
            ...
            addLog(String(format: String(localized: "server-manager.custom-provider.key-removed", defaultValue: "✓ Removed custom provider key: %@", comment: "Success message when custom provider key is removed"), "\(credential.label)"))
            if deletedCount > 1 {
                addLog(String(format: String(localized: "server-manager.custom-provider.duplicate-credential-files-removed", defaultValue: "✓ Removed %d duplicate credential files for %@", comment: "Message showing number of duplicate credential files removed for provider"), deletedCount, "\(credential.providerID)"))
            }
            markObservedConfigInputsCurrent()
            reloadCustomProviders()
            requestConfigUpdate()
        ...
        do {
            ...
            addLog(
                credential.isDisabled
                    ? String(format: String(localized: "server-manager.custom-provider.key-enabled", defaultValue: "✓ Enabled custom provider key: %@", comment: "Success message when custom provider key is enabled"), "\(credential.label)")
                    : String(format: String(localized: "server-manager.custom-provider.key-disabled", defaultValue: "⚠️ Disabled custom provider key: %@", comment: "Warning message when custom provider key is disabled"), "\(credential.label)")
            )
            markObservedConfigInputsCurrent()
            reloadCustomProviders()
            requestConfigUpdate()
    ...
    private func resolveConfigPath(
        ...
        guard bundledConfigPath() != nil else {
            return .failure(ConfigResolutionFailure(message: String(localized: "server-manager.error.bundled-config-not-found", defaultValue: "Could not locate the bundled config.yaml in the app bundle.", comment: "Error message when bundled config.yaml cannot be found in app bundle")))
        }
        ...
        guard case .success(let baseConfig) = baseConfigResult else {
            ...
            return .failure(ConfigResolutionFailure(message: String(localized: "server-manager.error.could-not-load-base-configuration", defaultValue: "Could not load the base configuration.", comment: "Error shown when base configuration cannot be loaded")))
        }
        ...
        do {
            ...
            return .failure(
                ConfigResolutionFailure(
                    message: String(format: String(localized: "server-manager.error.failed-to-write-merged-config", defaultValue: "Failed to write merged config to %@: %@", comment: "Error shown when writing merged config file fails"), "\(mergedConfigPath.path)", "\(error.localizedDescription)")
                )
            )
        }
    }
    ...
    private func killOrphanedProcesses() {
        ...
        do {
            ...
            if checkTask.terminationStatus == 0 {
                ...
                if !pids.isEmpty {
                    addLog(String(format: String(localized: "server-manager.warning.found-orphaned-processes", defaultValue: "⚠️ Found orphaned server process(es): %@", comment: "Warning shown when orphaned server process IDs are detected"), "\(pids.joined(separator: ", "))"))
                    ...
                    addLog(String(localized: "server-manager.status.cleaned-orphaned-processes", defaultValue: "✓ Cleaned up orphaned processes", comment: "Status message shown after orphaned processes are cleaned up"))
                }
            }
    ...
    private func loadBaseConfigRoot() -> Result<LoadedBaseConfig, ConfigResolutionFailure> {
        guard let bundledConfigPath = bundledConfigPath() else {
            return .failure(ConfigResolutionFailure(message: String(localized: "server-manager.error.bundled-config-not-found", defaultValue: "Could not locate the bundled config.yaml in the app bundle.", comment: "Error shown when bundled config.yaml cannot be found in app bundle")))
        }
        ...
        guard case .success(let bundledRoot) = bundledRootResult else {
            ...
            return .failure(ConfigResolutionFailure(message: String(format: String(localized: "server-manager.error.could-not-load-bundled-config-at-path", defaultValue: "Could not load the bundled config at %@.", comment: "Error shown when bundled config file cannot be loaded at path"), "\(bundledConfigPath)")))
        }
        ...
        guard case .success(let userRoot) = userRootResult else {
            ...
            return .failure(ConfigResolutionFailure(message: String(format: String(localized: "server-manager.error.could-not-load-user-config-at-path", defaultValue: "Could not load the user config at %@.", comment: "Error shown when user config file cannot be loaded at path"), "\(userConfigPath)")))
        }
    ...
    private func loadYAMLDictionary(atPath path: String) -> Result<[String: Any], ConfigResolutionFailure> {
        do {
            ...
            return .failure(ConfigResolutionFailure(message: String(format: String(localized: "server-manager.error.failed-to-parse-yaml-at-path", defaultValue: "Failed to parse YAML at %@: %@", comment: "Error shown when YAML parsing fails for config path"), "\(path)", "\(error.localizedDescription)")))
        }
    }
    ...
    private func validatedLoadedBaseConfig(
        ...
        guard validationErrors.isEmpty else {
            return .failure(
                ConfigResolutionFailure(
                    message: String(format: String(localized: "server-manager.error.invalid-custom-provider-configuration", defaultValue: "Invalid custom provider configuration. %@", comment: "Error shown when custom provider configuration validation fails"), "\(validationErrors.joined(separator: " "))")
                )
            )
        }
    ...
    private func publishConfigError(_ message: String) {
        let update = {
            ...
            if shouldLog {
                self.addLog(String(format: String(localized: "server-manager.error.prefixed-message", defaultValue: "❌ %@", comment: "Generic error output prefix with message content"), "\(message)"))
            }
        }
    ...
    private func finishConfigUpdateResolution(_ result: Result<String, ConfigResolutionFailure>) {
        ...
        if isRunning {
            addLog(String(localized: "server-manager.status.config-updated-hot-reload", defaultValue: "Config updated (hot reload)", comment: "Status message when config is updated by hot reload"))
        }
    }
