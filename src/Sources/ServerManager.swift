...
class ServerManager: ObservableObject {
                    ...
                    if case .copilotLogin = command {
                        ...
                        if let codeRange = capture.text.range(of: "enter the code: "),
                           let endRange = capture.text[codeRange.upperBound...].range(of: "\n") {
                            ...
                            completion(true, String(format: NSLocalizedString("auth.copilot.code-copied", comment: "Success message for Copilot auth with device code"), code))
                            return
    ...
    func saveZaiApiKey(_ apiKey: String, completion: @escaping (Bool, String) -> Void) {
        ...
        do {
            ...
            completion(true, String(localized: "zai.api-key-saved", defaultValue: "API key saved successfully", comment: "Success message for Z.AI API key save"))
        } catch {
            completion(false, String(format: NSLocalizedString("zai.error.save-failed", comment: "Error when Z.AI API key save fails"), error.localizedDescription))
        }
    }
