...
class ServerManager: ObservableObject {
    ...
    func runAuthCommand(_ command: AuthCommand, completion: @escaping (Bool, String) -> Void) {
        // Use bundled binary from app bundle
        guard let resourcePath = Bundle.main.resourcePath else {
            completion(false, String(localized: "server.error.no-resource-path", defaultValue: "Could not find resource path", comment: "Error when bundle resource path cannot be found"))
            return
        }

        let bundledPath = (resourcePath as NSString).appendingPathComponent("cli-proxy-api-plus")
        guard FileManager.default.fileExists(atPath: bundledPath) else {
            completion(false, String(format: String(localized: "server.error.binary-not-found", defaultValue: "Binary not found at %@", comment: "Error when binary is missing from expected path"), bundledPath))
            return
        }
            ...
            DispatchQueue.global(qos: .userInitiated).asyncAfter(deadline: .now() + waitTime) {
                if authProcess.isRunning {
                    ...
                    if output.contains("Opening browser") || output.contains("Attempting to open URL") {
                        ...
                        let message = error.isEmpty ? (output.isEmpty ? String(localized: "auth.error.unexpected-failure", defaultValue: "Authentication process failed unexpectedly", comment: "Auth process failed with no output") : output) : error
    ...
    func saveZaiApiKey(_ apiKey: String, completion: @escaping (Bool, String) -> Void) {
        ...
        do {
            ...
            completion(true, String(localized: "auth.zai.save-success", defaultValue: "API key saved successfully", comment: "Z.AI API key saved successfully"))
        } catch {
