...
class ServerManager: ObservableObject {
    ...
    private func providerConfigLockReason(_ providerKey: String, baseConfigRoot: [String: Any]?) -> String? {
        ...
        return String(format: String(localized: "server-manager.provider-toggle.disabled-in-config-oauth-excluded-models", defaultValue: "Disabled in config via oauth-excluded-models. Remove the '*' exclusion for %@ to enable it here.", comment: "Reason message when provider is disabled by oauth-excluded-models config"), "\(oauthProviderKey)")
    }
    ...
    func setProviderEnabled(_ providerKey: String, enabled: Bool) {
        ...
        if enabled, let lockReason = providerConfigLockReason(providerKey) {
            addLog(String(format: String(localized: "server-manager.provider-toggle.remains-disabled", defaultValue: "⚠️ %@ remains disabled: %@", comment: "Status message when provider remains disabled with lock reason"), "\(providerKey)", "\(lockReason)"))
        } else {
            addLog(enabled ? String(format: String(localized: "server-manager.provider-toggle.enabled", defaultValue: "✓ Enabled provider: %@", comment: "Status message when provider is enabled"), "\(providerKey)") : String(format: String(localized: "server-manager.provider-toggle.disabled", defaultValue: "⚠️ Disabled provider: %@", comment: "Status message when provider is disabled"), "\(providerKey)"))
        }
        reloadCustomProviders()
        requestConfigUpdate()
    }
    ...
    func start(completion: @escaping (Bool) -> Void) {
        ...
        guard let resourcePath = Bundle.main.resourcePath else {
            addLog(String(localized: "server-manager.start.error.resource-path-not-found", defaultValue: "❌ Error: Could not find resource path", comment: "Error message when resource path cannot be found"))
            completion(false)
            return
        }
        ...
        guard FileManager.default.fileExists(atPath: bundledPath) else {
            addLog(String(format: String(localized: "server-manager.start.error.binary-not-found-at-path", defaultValue: "❌ Error: cli-proxy-api-plus binary not found at %@", comment: "Error message when bundled binary is missing at expected path"), "\(bundledPath)"))
            completion(false)
            return
        }
        ...
        guard !configPath.isEmpty && FileManager.default.fileExists(atPath: configPath) else {
            addLog(String(format: String(localized: "server-manager.start.error.config-path-resolution", defaultValue: "❌ Error: %@", comment: "Error message when active config path resolution fails"), "\(configErrorMessage ?? String(localized: "server-manager.start.error.could-not-resolve-active-config-path", defaultValue: "Could not resolve active config path", comment: "Fallback error text when active config path cannot be resolved"))"))
            completion(false)
            return
        }
        ...
        errorPipe.fileHandleForReading.readabilityHandler = { [weak self] handle in
            let data = handle.availableData
            if let output = String(data: data, encoding: .utf8), !output.isEmpty {
                self?.addLog(String(format: String(localized: "server-manager.process.warning.output", defaultValue: "⚠️ %@", comment: "Warning prefix for process output line"), "\(output)"))
            }
        }
        ...
        process?.terminationHandler = { [weak self] process in
            ...
            DispatchQueue.main.async {
                ...
                self?.addLog(String(format: String(localized: "server-manager.process.stopped-with-code", defaultValue: "Server stopped with code: %d", comment: "Message reporting server process termination status code"), process.terminationStatus))
        ...
        do {
            ...
            addLog(String(format: String(localized: "server-manager.start.success.on-port", defaultValue: "✓ Server started on port %d", comment: "Success message when server starts on a port"), port))
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + Timing.readinessCheckDelay) { [weak self] in
                ...
                if let process = self.process, process.isRunning {
                    ...
                    self.addLog(String(localized: "server-manager.start.warning.exited-before-ready", defaultValue: "⚠️ Server exited before becoming ready", comment: "Warning message when server exits before readiness check completes"))
                    completion(false)
                }
            }
        } catch {
            addLog(String(format: String(localized: "server-manager.start.error.failed-to-start", defaultValue: "❌ Failed to start server: %@", comment: "Error message when server start throws an error"), "\(error.localizedDescription)"))
            completion(false)
        }
    }
    ...
    func stop(completion: (() -> Void)? = nil) {
        ...
        addLog(String(format: String(localized: "server-manager.stop.progress.stopping-with-pid", defaultValue: "Stopping server (PID: %d)...", comment: "Progress message while stopping server by PID"), pid))
        processQueue.async { [weak self] in
            ...
            if process.isRunning {
                self.addLog(String(localized: "server-manager.stop.warning.force-kill", defaultValue: "⚠️ Server didn't stop gracefully, force killing...", comment: "Warning message before force-killing server process"))
                kill(pid, SIGKILL)
            }
            ...
            DispatchQueue.main.async {
                ...
                self.isRunning = false
                self.activeConfigPath = ""
                self.addLog(String(localized: "server-manager.stop.success.stopped", defaultValue: "✓ Server stopped", comment: "Success message when server has stopped"))
                NotificationCenter.default.post(name: .serverStatusChanged, object: nil)
                completion?()
            }
        }
    ...
    func runAuthCommand(_ command: AuthCommand, completion: @escaping (Bool, String) -> Void) {
        ...
        // Use bundled binary from app bundle
        guard let resourcePath = Bundle.main.resourcePath else {
            completion(false, String(localized: "server-manager.check.error.resource-path-not-found", defaultValue: "Could not find resource path", comment: "Validation error when resource path is unavailable"))
            return
        }
        
        let bundledPath = (resourcePath as NSString).appendingPathComponent("cli-proxy-api-plus")
        guard FileManager.default.fileExists(atPath: bundledPath) else {
            completion(false, String(format: String(localized: "server-manager.check.error.binary-not-found-at-path", defaultValue: "Binary not found at %@", comment: "Validation error when binary is missing at expected bundled path"), "\(bundledPath)"))
            return
        }
        ...
        guard !configPath.isEmpty else {
            completion(false, configErrorMessage ?? String(localized: "server-manager.check.error.could-not-resolve-config-path", defaultValue: "Could not resolve config path", comment: "Validation error when config path cannot be resolved"))
            return
        }
        ...
        do {
            ...
            try authProcess.run()
            addLog(String(format: String(localized: "server-manager.authentication.started-with-pid", defaultValue: "✓ Authentication process started (PID: %d) - browser should open shortly", comment: "Status message when authentication process launches"), authProcess.processIdentifier))
            ...
            DispatchQueue.global(qos: .userInitiated).asyncAfter(deadline: .now() + waitTime) {
                if authProcess.isRunning {
                    ...
                    if case .copilotLogin = command {
                        ...
                        completion(true, String(localized: "server-manager.authentication.github.browser-opened-check-device-code", defaultValue: "🌐 Browser opened for GitHub authentication.\n\nCheck your terminal or the opened browser for the device code.\n\nThe app will automatically detect when you're authenticated.", comment: "Instruction message when GitHub authentication opens browser and user should check device code"))
                        return
                    }
                    
                    completion(true, String(localized: "server-manager.authentication.browser-opened.generic", defaultValue: "🌐 Browser opened for authentication.\n\nPlease complete the login in your browser.\n\nThe app will automatically detect when you're authenticated.", comment: "Generic instruction message when authentication browser flow starts"))
                } else {
                    ...
                    if output.contains("Opening browser") || output.contains("Attempting to open URL") {
                        ...
                        let message = error.isEmpty ? (output.isEmpty ? String(localized: "server-manager.authentication.failed-unexpectedly", defaultValue: "Authentication process failed unexpectedly", comment: "Error message when authentication process exits unexpectedly") : output) : error
            ...
            completion(false, String(format: String(localized: "server-manager.authentication.start-failed-with-error", defaultValue: "Failed to start auth process: %@", comment: "Error message when starting authentication process fails"), "\(error.localizedDescription)"))
        }
    }
    ...
    private func terminateActiveAuthProcessIfNeeded(reason: String) {
        ...
        if authProcess.isRunning {
            addLog(String(format: String(localized: "server-manager.authentication.retry.terminating-previous-process", defaultValue: "⚠️ Terminating previous auth process (%d) before retry: %@", comment: "Warning message when previous authentication process is terminated before retry"), authProcess.processIdentifier, "\(reason)"))
        ...
        for pattern in patterns {
            ...
            do {
                ...
                for pid in pids {
                    ...
                    kill(pid, SIGKILL)
                    addLog(String(format: String(localized: "server-manager.authentication.cleanup.stale-listener-process", defaultValue: "⚠️ Cleaned up stale auth listener process: %d", comment: "Warning message when stale authentication listener process is cleaned up"), pid))
                }
            } catch {
    ...
    func saveZaiApiKey(_ apiKey: String, completion: @escaping (Bool, String) -> Void) {
        credentialMutationQueue.async { [weak self] in
            ...
            do {
                ...
                self.addLog(String(format: String(localized: "server-manager.api-key.zai.saved-to-file", defaultValue: "✓ Z.AI API key saved to %@", comment: "Success message when Z.AI API key is saved to a file"), "\(filePath.lastPathComponent)"))
                self.refreshAuthBackedConfiguration()
                DispatchQueue.main.async {
                    completion(true, String(localized: "server-manager.api-key.saved-successfully", defaultValue: "API key saved successfully", comment: "Success title when API key save completes"))
                }
            } catch {
                DispatchQueue.main.async {
        ...
        credentialMutationQueue.async { [weak self] in
            ...
            do {
                ...
                guard let provider = customProviders.first(where: { $0.id == providerID }) else {
                    DispatchQueue.main.async {
                        completion(false, String(format: String(localized: "server-manager.custom-provider.not-defined-in-config", defaultValue: "Custom provider '%@' is not defined in config.yaml.", comment: "Error message when custom provider is missing from config file"), "\(providerID)"))
                    }
                    return
                }

                if provider.inlineAPIKeys.contains(apiKey) {
                    self.addLog(String(format: String(localized: "server-manager.custom-provider.api-key-already-in-config", defaultValue: "✓ API key for custom provider %@ already exists in config", comment: "Message when custom provider API key already exists in config"), "\(providerID)"))
                    DispatchQueue.main.async {
                        completion(true, String(localized: "server-manager.api-key.already-exists-in-config", defaultValue: "API key already exists in config", comment: "Title or summary when API key already exists in config"))
                    }
                    return
                }
                ...
                case .created(let record):
                    self.addLog(String(format: String(localized: "server-manager.custom-provider.api-key-saved", defaultValue: "✓ Saved API key for custom provider: %@", comment: "Success message when custom provider API key is saved"), "\(record.providerID)"))
                case .alreadyPresent(let record):
                    self.addLog(String(format: String(localized: "server-manager.custom-provider.key-already-present", defaultValue: "✓ Custom provider key already present: %@", comment: "Message when custom provider key is already present"), "\(record.label)"))
                case .reenabled(let record):
                    self.addLog(String(format: String(localized: "server-manager.custom-provider.key-reenabled", defaultValue: "✓ Re-enabled custom provider key: %@", comment: "Message when custom provider key is re-enabled"), "\(record.label)"))
                }
                self.refreshAuthBackedConfiguration()
                DispatchQueue.main.async {
                    switch saveResult {
                    case .created:
                        completion(true, String(localized: "server-manager.api-key.saved-successfully", defaultValue: "API key saved successfully", comment: "Success title when API key save completes"))
                    case .alreadyPresent:
                        completion(true, String(localized: "server-manager.api-key.already-exists", defaultValue: "API key already exists", comment: "Title when submitted API key already exists"))
                    case .reenabled:
                        completion(true, String(localized: "server-manager.api-key.reenabled", defaultValue: "API key was already stored and has been re-enabled", comment: "Message when existing API key is re-enabled"))
                    }
                }
            } catch {
                DispatchQueue.main.async {
    ...
    func deleteCustomProviderCredential(_ credential: CustomProviderCredential) -> Bool {
        do {
            ...
            addLog(String(format: String(localized: "server-manager.custom-provider.key-removed", defaultValue: "✓ Removed custom provider key: %@", comment: "Success message when removing a custom provider key"), "\(credential.label)"))
            if deletedCount > 1 {
                addLog(String(format: String(localized: "server-manager.custom-provider.duplicate-credential-files-removed", defaultValue: "✓ Removed %d duplicate credential files for %@", comment: "Success message when duplicate credential files are removed for a provider"), deletedCount, "\(credential.providerID)"))
            }
            markObservedConfigInputsCurrent()
            reloadCustomProviders()
            requestConfigUpdate()
        ...
        do {
            ...
            addLog(
                credential.isDisabled
                    ? String(format: String(localized: "server-manager.custom-provider.key-enabled", defaultValue: "✓ Enabled custom provider key: %@", comment: "Success message when enabling a custom provider key"), "\(credential.label)")
                    : String(format: String(localized: "server-manager.custom-provider.key-disabled", defaultValue: "⚠️ Disabled custom provider key: %@", comment: "Warning message when disabling a custom provider key"), "\(credential.label)")
            )
            markObservedConfigInputsCurrent()
            reloadCustomProviders()
            requestConfigUpdate()
    ...
    private func resolveConfigPath(
        ...
        guard case .success(let baseConfig) = baseConfigResult else {
            ...
            return .failure(ConfigResolutionFailure(message: String(localized: "server-manager.config.error.base-config-load-failed", defaultValue: "Could not load the base configuration.", comment: "Error when base configuration cannot be loaded")))
        }
        ...
        do {
            ...
            return .failure(
                ConfigResolutionFailure(
                    message: String(format: String(localized: "server-manager.config.error.write-merged-config-failed", defaultValue: "Failed to write merged config to %@: %@", comment: "Error when writing merged config file fails"), "\(mergedConfigPath.path)", "\(error.localizedDescription)")
                )
            )
        }
    }
    ...
    private func killOrphanedProcesses() {
        ...
        do {
            ...
            if checkTask.terminationStatus == 0 {
                ...
                if !pids.isEmpty {
                    addLog(String(format: String(localized: "server-manager.orphan-processes.found", defaultValue: "⚠️ Found orphaned server process(es): %@", comment: "Warning listing orphaned server process IDs"), "\(pids.joined(separator: ", "))"))
                    ...
                    addLog(String(localized: "server-manager.orphan-processes.cleaned-up", defaultValue: "✓ Cleaned up orphaned processes", comment: "Success message after cleaning orphaned server processes"))
                }
            }
    ...
    private func loadBaseConfigRoot() -> Result<LoadedBaseConfig, ConfigResolutionFailure> {
        ...
        guard case .success(let bundledRoot) = bundledRootResult else {
            ...
            return .failure(ConfigResolutionFailure(message: String(format: String(localized: "server-manager.config.error.bundled-config-load-failed-at-path", defaultValue: "Could not load the bundled config at %@.", comment: "Error when bundled config file cannot be loaded from path"), "\(bundledConfigPath)")))
        }
        ...
        guard case .success(let userRoot) = userRootResult else {
            ...
            return .failure(ConfigResolutionFailure(message: String(format: String(localized: "server-manager.config.error.user-config-load-failed-at-path", defaultValue: "Could not load the user config at %@.", comment: "Error when user config file cannot be loaded from path"), "\(userConfigPath)")))
        }
    ...
    private func loadYAMLDictionary(atPath path: String) -> Result<[String: Any], ConfigResolutionFailure> {
        do {
            ...
            return .failure(ConfigResolutionFailure(message: String(format: String(localized: "server-manager.config.error.yaml-parse-failed-at-path", defaultValue: "Failed to parse YAML at %@: %@", comment: "Error when YAML parsing fails for a config path"), "\(path)", "\(error.localizedDescription)")))
        }
    }
    ...
    private func validatedLoadedBaseConfig(
        ...
        guard validationErrors.isEmpty else {
            return .failure(
                ConfigResolutionFailure(
                    message: String(format: String(localized: "server-manager.custom-provider.error.invalid-configuration", defaultValue: "Invalid custom provider configuration. %@", comment: "Validation error summary for invalid custom provider configuration"), "\(validationErrors.joined(separator: " "))")
                )
            )
        }
    ...
    private func publishConfigError(_ message: String) {
        let update = {
            ...
            if shouldLog {
                self.addLog(String(format: String(localized: "server-manager.error.prefixed-message", defaultValue: "❌ %@", comment: "Generic error message prefixed with error symbol"), "\(message)"))
            }
        }
    ...
    private func finishConfigUpdateResolution(_ result: Result<String, ConfigResolutionFailure>) {
        ...
        if shouldRestart {
            ...
            hasPendingConfigUpdate = false
            addLog(String(localized: "server-manager.config.path-changed.restarting-server", defaultValue: "Config path changed; restarting server", comment: "Status message when config path change triggers server restart"))
            stop { [weak self] in
        ...
        if isRunning {
            addLog(String(localized: "server-manager.config.updated.hot-reload", defaultValue: "Config updated (hot reload)", comment: "Status message when config hot reload completes"))
        }
    }
