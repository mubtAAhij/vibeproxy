...
class ServerManager: ObservableObject {
    ...
    func runAuthCommand(_ command: AuthCommand, completion: @escaping (Bool, String) -> Void) {
        // Use bundled binary from app bundle
        guard let resourcePath = Bundle.main.resourcePath else {
            completion(false, String(localized: "could_not_find_resource_path", defaultValue: "Could not find resource path", comment: "Error when app bundle resource path cannot be located"))
            return
        }

        let bundledPath = (resourcePath as NSString).appendingPathComponent("cli-proxy-api-plus")
        guard FileManager.default.fileExists(atPath: bundledPath) else {
            completion(false, String(format: String(localized: "binary_not_found_at_path", defaultValue: "Binary not found at %@", comment: "Error when cli-proxy-api-plus binary is missing from bundle"), bundledPath))
            return
        }
        ...
        do {
            ...
            DispatchQueue.global(qos: .userInitiated).asyncAfter(deadline: .now() + waitTime) {
                if authProcess.isRunning {
                    ...
                    if case .copilotLogin = command {
                        ...
                        if let codeRange = capture.text.range(of: "enter the code: "),
                           let endRange = capture.text[codeRange.upperBound...].range(of: "\n") {
                            ...
                            completion(true, String(format: String(localized: "github_auth_browser_opened_code_copied", defaultValue: "🌐 Browser opened for GitHub authentication.\n\n📋 Code copied to clipboard:\n\n%@\n\nJust paste it in the browser!\n\nThe app will automatically detect when you're authenticated.", comment: "Success message when GitHub Copilot auth opens browser and copies device code"), code))
                            return
                        ...
                        completion(true, String(localized: "github_auth_browser_opened_check_terminal", defaultValue: "🌐 Browser opened for GitHub authentication.\n\nCheck your terminal or the opened browser for the device code.\n\nThe app will automatically detect when you're authenticated.", comment: "Fallback message when GitHub Copilot auth opens browser but device code cannot be extracted"))
                        return
                    }

                    completion(true, String(localized: "auth_browser_opened_complete_login", defaultValue: "🌐 Browser opened for authentication.\n\nPlease complete the login in your browser.\n\nThe app will automatically detect when you're authenticated.", comment: "Generic success message when authentication process opens browser"))
                } else {
                    ...
                    if output.contains("Opening browser") || output.contains("Attempting to open URL") {
                        ...
                        let message = error.isEmpty ? (output.isEmpty ? String(localized: "authentication_process_failed_unexpectedly", defaultValue: "Authentication process failed unexpectedly", comment: "Generic error when auth process terminates with no output") : output) : error
            ...
            completion(false, String(format: String(localized: "failed_to_start_auth_process", defaultValue: "Failed to start auth process: %@", comment: "Error when authentication process cannot be launched"), error.localizedDescription))
        }
    }
    ...
    func saveZaiApiKey(_ apiKey: String, completion: @escaping (Bool, String) -> Void) {
        ...
        do {
            ...
            completion(false, String(format: String(localized: "failed_to_create_auth_directory", defaultValue: "Failed to create auth directory: %@", comment: "Error when .cli-proxy-api directory cannot be created"), error.localizedDescription))
            return
        }
        ...
        do {
            ...
            completion(true, String(localized: "api_key_saved_successfully", defaultValue: "API key saved successfully", comment: "Success message when Z.AI API key is saved to disk"))
        } catch {
            completion(false, String(format: String(localized: "failed_to_save_api_key", defaultValue: "Failed to save API key: %@", comment: "Error when Z.AI API key file cannot be written"), error.localizedDescription))
        }
    }
