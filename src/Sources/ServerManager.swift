...
class ServerManager: ObservableObject {
    ...
    func runAuthCommand(_ command: AuthCommand, completion: @escaping (Bool, String) -> Void) {
        // Use bundled binary from app bundle
        guard let resourcePath = Bundle.main.resourcePath else {
            completion(false, String(localized: "server-manager.error.resource-path-not-found", defaultValue: "Could not find resource path", bundle: .main, comment: "Error when app resource path is unavailable"))
            return
        }

        let bundledPath = (resourcePath as NSString).appendingPathComponent("cli-proxy-api-plus")
        guard FileManager.default.fileExists(atPath: bundledPath) else {
            completion(false, String(format: String(localized: "server-manager.error.binary-not-found", defaultValue: "Binary not found at %@", bundle: .main, comment: "Error when cli-proxy-api-plus binary is missing"), bundledPath))
            return
        }
        ...
        do {
            ...
            DispatchQueue.global(qos: .userInitiated).asyncAfter(deadline: .now() + waitTime) {
                if authProcess.isRunning {
                    ...
                    if output.contains("Opening browser") || output.contains("Attempting to open URL") {
                        ...
                        let message = error.isEmpty ? (output.isEmpty ? String(localized: "server-manager.error.auth-process-failed", defaultValue: "Authentication process failed unexpectedly", bundle: .main, comment: "Error when auth process fails without details") : output) : error
            ...
            completion(false, String(format: String(localized: "server-manager.error.auth-start-failed", defaultValue: "Failed to start auth process: %@", bundle: .main, comment: "Error when auth process fails to start"), error.localizedDescription))
        }
    }
    ...
    func saveZaiApiKey(_ apiKey: String, completion: @escaping (Bool, String) -> Void) {
        ...
        do {
            ...
            completion(false, String(format: String(localized: "server-manager.error.auth-dir-creation-failed", defaultValue: "Failed to create auth directory: %@", bundle: .main, comment: "Error when auth directory cannot be created"), error.localizedDescription))
            return
        }
        ...
        do {
            ...
            completion(false, String(format: String(localized: "server-manager.error.api-key-save-failed", defaultValue: "Failed to save API key: %@", bundle: .main, comment: "Error when API key cannot be saved"), error.localizedDescription))
        }
    }
