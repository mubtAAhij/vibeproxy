...
class ServerManager: ObservableObject {
    ...
    func runAuthCommand(_ command: AuthCommand, completion: @escaping (Bool, String) -> Void) {
        // Use bundled binary from app bundle
        guard let resourcePath = Bundle.main.resourcePath else {
            completion(false, String(localized: "server.error.resource-path-not-found", defaultValue: "Could not find resource path", comment: "Error when app bundle resource path cannot be located"))
            return
        }

        let bundledPath = (resourcePath as NSString).appendingPathComponent("cli-proxy-api-plus")
        guard FileManager.default.fileExists(atPath: bundledPath) else {
            completion(false, String(format: String(localized: "server.error.binary-not-found", defaultValue: "Binary not found at %@", comment: "Error when CLI binary is missing from expected path"), bundledPath))
            return
        }
        ...
        do {
            ...
            DispatchQueue.global(qos: .userInitiated).asyncAfter(deadline: .now() + waitTime) {
                if authProcess.isRunning {
                    ...
                    if output.contains("Opening browser") || output.contains("Attempting to open URL") {
                        ...
                        let message = error.isEmpty ? (output.isEmpty ? String(localized: "auth.error.process-failed", defaultValue: "Authentication process failed unexpectedly", comment: "Error when authentication process exits without clear error message") : output) : error
            ...
            completion(false, String(format: String(localized: "auth.error.start-failed", defaultValue: "Failed to start auth process: %@", comment: "Error when authentication process cannot be started"), error.localizedDescription))
        }
    }
    ...
    func saveZaiApiKey(_ apiKey: String, completion: @escaping (Bool, String) -> Void) {
        ...
        do {
            ...
            completion(false, String(format: String(localized: "auth.error.directory-create-failed", defaultValue: "Failed to create auth directory: %@", comment: "Error when authentication directory cannot be created"), error.localizedDescription))
            return
        }
        ...
        do {
            ...
            completion(true, String(localized: "zai.api-key.save-success", defaultValue: "API key saved successfully", comment: "Success message when Z.AI API key is saved"))
        } catch {
            completion(false, String(format: String(localized: "zai.api-key.save-failed", defaultValue: "Failed to save API key: %@", comment: "Error when Z.AI API key cannot be saved"), error.localizedDescription))
        }
    }
