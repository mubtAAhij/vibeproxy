...
class ServerManager: ObservableObject {
    ...
    func runAuthCommand(_ command: AuthCommand, completion: @escaping (Bool, String) -> Void) {
        // Use bundled binary from app bundle
        guard let resourcePath = Bundle.main.resourcePath else {
            completion(false, String(localized: "server-manager.error.no-resource-path", defaultValue: "Could not find resource path", comment: "Error when bundle resource path cannot be found"))
            return
        }

        let bundledPath = (resourcePath as NSString).appendingPathComponent("cli-proxy-api-plus")
        guard FileManager.default.fileExists(atPath: bundledPath) else {
            completion(false, String(format: NSLocalizedString("server-manager.error.binary-not-found", comment: "Error when binary is missing at specified path"), bundledPath))
            return
        }
        ...
        do {
            ...
            DispatchQueue.global(qos: .userInitiated).asyncAfter(deadline: .now() + waitTime) {
                if authProcess.isRunning {
                    ...
                    if case .copilotLogin = command {
                        ...
                        if let codeRange = capture.text.range(of: "enter the code: "),
                           let endRange = capture.text[codeRange.upperBound...].range(of: "\n") {
                            ...
                            completion(true, String(format: NSLocalizedString("server-manager.auth.github-code-copied", comment: "Success message when GitHub device code is extracted and copied"), code))
                            return
                        ...
                        completion(true, String(localized: "server-manager.auth.github-no-code", defaultValue: "🌐 Browser opened for GitHub authentication.\n\nCheck your terminal or the opened browser for the device code.\n\nThe app will automatically detect when you're authenticated.", comment: "Success message for GitHub auth when device code cannot be extracted"))
                        return
                    }
                    ...
                    if output.contains("Opening browser") || output.contains("Attempting to open URL") {
                        ...
                        let message = error.isEmpty ? (output.isEmpty ? String(localized: "server-manager.error.auth-failed", defaultValue: "Authentication process failed unexpectedly", comment: "Error when auth process fails without specific error message") : output) : error
            ...
            completion(false, String(format: NSLocalizedString("server-manager.error.auth-start-failed", comment: "Error when auth process fails to start"), error.localizedDescription))
        }
    }
    ...
    func saveZaiApiKey(_ apiKey: String, completion: @escaping (Bool, String) -> Void) {
        ...
        do {
            ...
            completion(false, String(format: NSLocalizedString("server-manager.error.auth-dir-failed", comment: "Error when auth directory cannot be created"), error.localizedDescription))
            return
        }
        ...
        do {
            ...
            completion(false, String(format: NSLocalizedString("server-manager.error.api-key-save-failed", comment: "Error when API key cannot be saved"), error.localizedDescription))
        }
    }
