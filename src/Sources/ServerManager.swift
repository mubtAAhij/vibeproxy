...
class ServerManager: ObservableObject {
    ...
    func runAuthCommand(_ command: AuthCommand, completion: @escaping (Bool, String) -> Void) {
        ...
        let bundledPath = (resourcePath as NSString).appendingPathComponent("cli-proxy-api-plus")
        guard FileManager.default.fileExists(atPath: bundledPath) else {
            completion(false, String(format: String(localized: "server.error.binary-not-found", defaultValue: "Binary not found at %@", comment: "Error when cli-proxy-api-plus binary is missing"), bundledPath))
            return
        }
        ...
        do {
            ...
            DispatchQueue.global(qos: .userInitiated).asyncAfter(deadline: .now() + waitTime) {
                if authProcess.isRunning {
                    ...
                    if case .copilotLogin = command {
                        ...
                        if let codeRange = capture.text.range(of: "enter the code: "),
                           let endRange = capture.text[codeRange.upperBound...].range(of: "\n") {
                            ...
                            completion(true, String(format: String(localized: "auth.copilot.browser-opened-with-code", defaultValue: "🌐 Browser opened for GitHub authentication.\n\n📋 Code copied to clipboard:\n\n%@\n\nJust paste it in the browser!\n\nThe app will automatically detect when you're authenticated.", comment: "Success message when GitHub Copilot auth browser opens with device code"), code))
                            return
                        ...
                        completion(true, String(localized: "auth.copilot.browser-opened-check-terminal", defaultValue: "🌐 Browser opened for GitHub authentication.\n\nCheck your terminal or the opened browser for the device code.\n\nThe app will automatically detect when you're authenticated.", comment: "Success message when GitHub Copilot auth browser opens but device code not extracted"))
                        return
                    }

                    completion(true, String(localized: "auth.generic.browser-opened", defaultValue: "🌐 Browser opened for authentication.\n\nPlease complete the login in your browser.\n\nThe app will automatically detect when you're authenticated.", comment: "Generic success message when OAuth browser opens"))
                } else {
                    ...
                    if output.contains("Opening browser") || output.contains("Attempting to open URL") {
                        ...
                        let message = error.isEmpty ? (output.isEmpty ? String(localized: "auth.error.process-failed-unexpectedly", defaultValue: "Authentication process failed unexpectedly", comment: "Error when auth process fails with no output") : output) : error
            ...
            completion(false, String(format: String(localized: "auth.error.failed-to-start", defaultValue: "Failed to start auth process: %@", comment: "Error when auth process fails to start"), error.localizedDescription))
        }
    }
    ...
    func saveZaiApiKey(_ apiKey: String, completion: @escaping (Bool, String) -> Void) {
        ...
        do {
            ...
            completion(false, String(format: String(localized: "auth.error.failed-to-create-directory", defaultValue: "Failed to create auth directory: %@", comment: "Error when auth directory creation fails"), error.localizedDescription))
            return
        }
        ...
        do {
            ...
            completion(true, String(localized: "auth.zai.api-key-saved", defaultValue: "API key saved successfully", comment: "Success message when Z.AI API key is saved"))
        } catch {
            completion(false, String(format: String(localized: "auth.error.failed-to-save-api-key", defaultValue: "Failed to save API key: %@", comment: "Error when Z.AI API key save fails"), error.localizedDescription))
        }
    }
