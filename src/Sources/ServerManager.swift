...
class ServerManager: ObservableObject {
    ...
    func runAuthCommand(_ command: AuthCommand, completion: @escaping (Bool, String) -> Void) {
        ...
        let bundledPath = (resourcePath as NSString).appendingPathComponent("cli-proxy-api-plus")
        guard FileManager.default.fileExists(atPath: bundledPath) else {
            completion(false, String(format: NSLocalizedString("server.error.binary-not-found", comment: "Error when binary is not found at expected path"), bundledPath))
            return
        }
        ...
        do {
            ...
            DispatchQueue.global(qos: .userInitiated).asyncAfter(deadline: .now() + waitTime) {
                if authProcess.isRunning {
                    ...
                    if case .copilotLogin = command {
                        ...
                        if let codeRange = capture.text.range(of: "enter the code: "),
                           let endRange = capture.text[codeRange.upperBound...].range(of: "\n") {
                            ...
                            completion(true, String(format: NSLocalizedString("auth.copilot.success-with-code", comment: "Success message with device code for GitHub authentication"), code))
                            return
                    ...
                    if output.contains("Opening browser") || output.contains("Attempting to open URL") {
                        ...
                        let message = error.isEmpty ? (output.isEmpty ? String(localized: "auth.error.unexpected-failure", defaultValue: "Authentication process failed unexpectedly", comment: "Error when authentication process fails unexpectedly") : output) : error
            ...
            completion(false, String(format: NSLocalizedString("auth.error.failed-to-start", comment: "Error when authentication process fails to start"), error.localizedDescription))
        }
    }
    ...
    func saveZaiApiKey(_ apiKey: String, completion: @escaping (Bool, String) -> Void) {
        ...
        do {
            ...
            completion(false, String(format: NSLocalizedString("auth.error.failed-to-create-directory", comment: "Error when creating auth directory fails"), error.localizedDescription))
            return
        }
        ...
        do {
            ...
            completion(false, String(format: NSLocalizedString("auth.zai.save-failed", comment: "Error when saving Z.AI API key fails"), error.localizedDescription))
        }
    }
