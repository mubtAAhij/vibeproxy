...
class ServerManager: ObservableObject {
    ...
    func runAuthCommand(_ command: AuthCommand, completion: @escaping (Bool, String) -> Void) {
        // Use bundled binary from app bundle
        guard let resourcePath = Bundle.main.resourcePath else {
            completion(false, String(localized: "server.error.resource-path", defaultValue: "Could not find resource path", comment: "Error when app resource path cannot be determined"))
            return
        }
            ...
            DispatchQueue.global(qos: .userInitiated).asyncAfter(deadline: .now() + waitTime) {
                if authProcess.isRunning {
                    ...
                    if case .copilotLogin = command {
                        ...
                        if let codeRange = capture.text.range(of: "enter the code: "),
                           let endRange = capture.text[codeRange.upperBound...].range(of: "\n") {
                            ...
                            completion(true, String(format: NSLocalizedString("auth.github.code-copied", comment: "GitHub auth success with device code copied"), code))
                            return
                    ...
                    if output.contains("Opening browser") || output.contains("Attempting to open URL") {
                        ...
                        let message = error.isEmpty ? (output.isEmpty ? String(localized: "auth.error.unexpected", defaultValue: "Authentication process failed unexpectedly", comment: "Error when auth process fails without output") : output) : error
    ...
    func saveZaiApiKey(_ apiKey: String, completion: @escaping (Bool, String) -> Void) {
        ...
        do {
            ...
            completion(false, String(format: NSLocalizedString("auth.error.create-directory", comment: "Error when auth directory creation fails"), error.localizedDescription))
            return
        }
