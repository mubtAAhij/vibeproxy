    ...
    @objc func updateMenuBarStatus() {
        ...
        if let button = statusItem.button {
            ...
            if let icon = IconCatalog.shared.image(named: iconName, resizedTo: NSSize(width: 18, height: 18), template: true) {
                ...
                let fallback = NSImage(systemSymbolName: fallbackSymbol, accessibilityDescription: serverManager.isRunning ? String(localized: "accessibility.server.running", defaultValue: "Running", comment: "Accessibility description for running server") : String(localized: "accessibility.server.stopped", defaultValue: "Stopped", comment: "Accessibility description for stopped server"))
                fallback?.isTemplate = true
                button.image = fallback
