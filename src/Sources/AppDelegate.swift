...
class AppDelegate: NSObject, NSApplicationDelegate, NSWindowDelegate, UNUserNotificationCenterDelegate {
    ...
    func setupMenuBar() {
        ...
        // Server Status
        menu.addItem(NSMenuItem(title: String(localized: "menubar.server-stopped", defaultValue: "Server: Stopped", comment: "Menu bar item showing server is stopped"), action: nil, keyEquivalent: ""))
    ...
    @objc func updateMenuBarStatus() {
        ...
        if let serverStatus = menu.item(at: 0) {
            serverStatus.title = serverManager.isRunning ? String(format: NSLocalizedString("menubar.server-running", comment: "Menu bar item showing server is running with port"), thinkingProxy.proxyPort) : String(localized: "menubar.server-stopped-status", defaultValue: "Server: Stopped", comment: "Menu bar status when server is stopped")
        }
