...
class AppDelegate: NSObject, NSApplicationDelegate, NSWindowDelegate, UNUserNotificationCenterDelegate {
    ...
    func setupMenuBar() {
        ...
        // Server Status
        menu.addItem(NSMenuItem(title: String(localized: "menu.status.stopped", defaultValue: "Server: Stopped", comment: "Menu bar status when server is stopped"), action: nil, keyEquivalent: ""))
    ...
    @objc func updateMenuBarStatus() {
        ...
        if let serverStatus = menu.item(at: 0) {
            serverStatus.title = serverManager.isRunning ? String(format: NSLocalizedString("menu.status.running", comment: "Menu bar status when server is running"), thinkingProxy.proxyPort) : String(localized: "menu.status.stopped", defaultValue: "Server: Stopped", comment: "Menu bar status when server is stopped")
        }
        ...
        if let button = statusItem.button {
            ...
            if let icon = IconCatalog.shared.image(named: iconName, resizedTo: NSSize(width: 18, height: 18), template: true) {
                ...
                let runningDesc = String(localized: "accessibility.running", defaultValue: "Running", comment: "Accessibility description when running")
                let stoppedDesc = String(localized: "accessibility.stopped", defaultValue: "Stopped", comment: "Accessibility description when stopped")
