...
class AppDelegate: NSObject, NSApplicationDelegate, NSWindowDelegate, UNUserNotificationCenterDelegate {
    ...
    private func setupMainMenu() {
        ...
        appMenu.addItem(NSMenuItem(title: String(localized: "menu.quit-vibeproxy", defaultValue: "Quit VibeProxy", comment: "Quit menu item in app menu"), action: #selector(NSApplication.terminate(_:)), keyEquivalent: "q"))
    ...
    func setupMenuBar() {
        ...
        // Check for Updates
        let checkForUpdatesItem = NSMenuItem(title: String(localized: "menubar.check-updates", defaultValue: "Check for Updates...", comment: "Menu item to check for software updates"), action: #selector(SPUStandardUpdaterController.checkForUpdates(_:)), keyEquivalent: "u")
    ...
    @objc func updateMenuBarStatus() {
        ...
        if let button = statusItem.button {
            ...
            if let icon = IconCatalog.shared.image(named: iconName, resizedTo: NSSize(width: 18, height: 18), template: true) {
                ...
                let fallback = NSImage(systemSymbolName: fallbackSymbol, accessibilityDescription: serverManager.isRunning ? String(localized: "icon.running", defaultValue: "Running", comment: "Icon accessibility description for running state") : String(localized: "icon.stopped", defaultValue: "Stopped", comment: "Icon accessibility description for stopped state"))
                fallback?.isTemplate = true
                button.image = fallback
