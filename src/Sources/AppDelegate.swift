    ...
    @objc func updateMenuBarStatus() {
        ...
        if let button = statusItem.button {
            ...
            if let icon = IconCatalog.shared.image(named: iconName, resizedTo: NSSize(width: 18, height: 18), template: true) {
                ...
                let fallback = NSImage(systemSymbolName: fallbackSymbol, accessibilityDescription: serverManager.isRunning ? String(localized: "menubar.accessibility.running", defaultValue: "Running", comment: "Accessibility description for running status icon") : String(localized: "menubar.accessibility.stopped", defaultValue: "Stopped", comment: "Accessibility description for stopped status icon"))
                fallback?.isTemplate = true
                button.image = fallback
