...
class AppDelegate: NSObject, NSApplicationDelegate, NSWindowDelegate, UNUserNotificationCenterDelegate {
    ...
    private func setupMainMenu() {
        ...
        let appMenu = NSMenu()
        appMenu.addItem(NSMenuItem(title: String(localized: "menu.app.about", defaultValue: "About VibeProxy", comment: "About menu item"), action: #selector(NSApplication.orderFrontStandardAboutPanel(_:)), keyEquivalent: ""))
        appMenu.addItem(NSMenuItem.separator())
        appMenu.addItem(NSMenuItem(title: String(localized: "menu.app.quit", defaultValue: "Quit VibeProxy", comment: "Quit menu item"), action: #selector(NSApplication.terminate(_:)), keyEquivalent: "q"))
        appMenuItem.submenu = appMenu
        mainMenu.addItem(appMenuItem)

        // Edit menu (for Cmd+C/V/X/A to work)
        let editMenuItem = NSMenuItem()
        let editMenu = NSMenu(title: String(localized: "menu.edit.title", defaultValue: "Edit", comment: "Edit menu title"))
    ...
    func setupMenuBar() {
        ...
        // Server Status
        menu.addItem(NSMenuItem(title: String(localized: "menubar.status.stopped", defaultValue: "Server: Stopped", comment: "Menu bar status when server is stopped"), action: nil, keyEquivalent: ""))
        menu.addItem(NSMenuItem.separator())

        // Main Actions
        menu.addItem(NSMenuItem(title: String(localized: "menubar.action.open-settings", defaultValue: "Open Settings", comment: "Menu item to open settings"), action: #selector(openSettings), keyEquivalent: "s"))
        menu.addItem(NSMenuItem.separator())

        // Server Control
        let startStopItem = NSMenuItem(title: String(localized: "menubar.action.start-server", defaultValue: "Start Server", comment: "Menu item to start server"), action: #selector(toggleServer), keyEquivalent: "")
        ...
        // Copy URL
        let copyURLItem = NSMenuItem(title: String(localized: "menubar.action.copy-url", defaultValue: "Copy Server URL", comment: "Menu item to copy server URL"), action: #selector(copyServerURL), keyEquivalent: "c")
        ...
        // Check for Updates
        let checkForUpdatesItem = NSMenuItem(title: String(localized: "menubar.action.check-updates", defaultValue: "Check for Updates...", comment: "Menu item to check for updates"), action: #selector(SPUStandardUpdaterController.checkForUpdates(_:)), keyEquivalent: "u")
        ...
        // Quit
        menu.addItem(NSMenuItem(title: String(localized: "menubar.action.quit", defaultValue: "Quit", comment: "Menu item to quit app"), action: #selector(quit), keyEquivalent: "q"))
    ...
    private func pollForProxyReadiness(attempts: Int, maxAttempts: Int, intervalMs: Int) {
        ...
        if attempts >= maxAttempts {
            DispatchQueue.main.async { [weak self] in
                ...
                self?.thinkingProxy.stop()
                self?.showNotification(title: String(localized: "notification.server-failed.title", defaultValue: "Server Failed", comment: "Notification title when server fails"), body: String(localized: "notification.server-failed.proxy", defaultValue: "Could not start thinking proxy on port 8317 (timeout)", comment: "Notification body when thinking proxy fails to start"))
            }
            return
        }
    ...
    @objc func updateMenuBarStatus() {
        ...
        if let serverStatus = menu.item(at: 0) {
            serverStatus.title = serverManager.isRunning ? String(format: String(localized: "menubar.status.running", defaultValue: "Server: Running (port %d)", comment: "Menu bar status when server is running"), thinkingProxy.proxyPort) : String(localized: "menubar.status.stopped", defaultValue: "Server: Stopped", comment: "Menu bar status when server is stopped")
        }

        // Update button states
        if let startStopItem = menu.item(withTag: 100) {
            startStopItem.title = serverManager.isRunning ? String(localized: "menubar.action.stop-server", defaultValue: "Stop Server", comment: "Menu item to stop server") : String(localized: "menubar.action.start-server", defaultValue: "Start Server", comment: "Menu item to start server")
        }
