...
class AppDelegate: NSObject, NSApplicationDelegate, NSWindowDelegate, UNUserNotificationCenterDelegate {
    ...
    func setupMenuBar() {
        ...
        // Main Actions
        menu.addItem(NSMenuItem(title: String(localized: "menubar.open-settings", defaultValue: "Open Settings", comment: "Menu bar open settings item"), action: #selector(openSettings), keyEquivalent: "s"))
        menu.addItem(NSMenuItem.separator())

        // Server Control
        let startStopItem = NSMenuItem(title: String(localized: "menubar.start-server", defaultValue: "Start Server", comment: "Menu bar start server item"), action: #selector(toggleServer), keyEquivalent: "")
        ...
        // Copy URL
        let copyURLItem = NSMenuItem(title: String(localized: "menubar.copy-server-url", defaultValue: "Copy Server URL", comment: "Menu bar copy URL item"), action: #selector(copyServerURL), keyEquivalent: "c")
        ...
        // Check for Updates
        let checkForUpdatesItem = NSMenuItem(title: String(localized: "menubar.check-for-updates", defaultValue: "Check for Updates...", comment: "Menu bar check for updates item"), action: #selector(SPUStandardUpdaterController.checkForUpdates(_:)), keyEquivalent: "u")
        ...
        // Quit
        menu.addItem(NSMenuItem(title: String(localized: "menubar.quit", defaultValue: "Quit", comment: "Menu bar quit item"), action: #selector(quit), keyEquivalent: "q"))
    ...
    @objc func copyServerURL() {
        ...
        showNotification(title: String(localized: "notification.copied.title", defaultValue: "Copied", comment: "Notification title when URL is copied"), body: String(localized: "notification.copied.body", defaultValue: "Server URL copied to clipboard", comment: "Notification body when URL is copied"))
    }

    @objc func updateMenuBarStatus() {
        // Update status items
        if let serverStatus = menu.item(at: 0) {
            serverStatus.title = serverManager.isRunning ? String(format: NSLocalizedString("menubar.server-running-with-port", comment: "Menu bar server running status with port"), thinkingProxy.proxyPort) : String(localized: "menubar.server-stopped", defaultValue: "Server: Stopped", comment: "Menu bar server stopped status")
        }

        // Update button states
        if let startStopItem = menu.item(withTag: 100) {
            startStopItem.title = serverManager.isRunning ? String(localized: "menubar.stop-server", defaultValue: "Stop Server", comment: "Menu bar stop server item") : String(localized: "menubar.start-server", defaultValue: "Start Server", comment: "Menu bar start server item")
        }
