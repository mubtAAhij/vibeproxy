...
class AppDelegate: NSObject, NSApplicationDelegate, NSWindowDelegate, UNUserNotificationCenterDelegate {
    ...
    func setupMenuBar() {
        ...
        // Check for Updates
        let checkForUpdatesItem = NSMenuItem(title: String(localized: "menubar.check-for-updates", defaultValue: "Check for Updates...", comment: "Menu item to check for updates"), action: #selector(SPUStandardUpdaterController.checkForUpdates(_:)), keyEquivalent: "u")
    ...
    @objc func updateMenuBarStatus() {
        ...
        if let serverStatus = menu.item(at: 0) {
            serverStatus.title = serverManager.isRunning ? String(format: NSLocalizedString("menubar.server-running-port", comment: "Menu bar status when server is running"), thinkingProxy.proxyPort) : String(localized: "menubar.server-stopped-status", defaultValue: "Server: Stopped", comment: "Menu bar status when server is stopped")
        }

        // Update button states
        if let startStopItem = menu.item(withTag: 100) {
            startStopItem.title = serverManager.isRunning ? String(localized: "menubar.stop-server", defaultValue: "Stop Server", comment: "Menu item to stop server") : String(localized: "menubar.start-server-action", defaultValue: "Start Server", comment: "Menu item to start server")
        }
