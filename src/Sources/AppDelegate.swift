...
class AppDelegate: NSObject, NSApplicationDelegate, NSWindowDelegate, UNUserNotificationCenterDelegate {
    ...
    private func setupMainMenu() {
        ...
        let appMenu = NSMenu()
        appMenu.addItem(NSMenuItem(title: String(localized: "menu.about-vibeproxy", defaultValue: "About VibeProxy", comment: "About menu item"), action: #selector(NSApplication.orderFrontStandardAboutPanel(_:)), keyEquivalent: ""))
    ...
    @objc func updateMenuBarStatus() {
        ...
        if let button = statusItem.button {
            ...
            if let icon = IconCatalog.shared.image(named: iconName, resizedTo: NSSize(width: 18, height: 18), template: true) {
                ...
                let fallback = NSImage(systemSymbolName: fallbackSymbol, accessibilityDescription: serverManager.isRunning ? String(localized: "menubar.accessibility.running", defaultValue: "Running", comment: "Accessibility description for running icon") : String(localized: "menubar.accessibility.stopped", defaultValue: "Stopped", comment: "Accessibility description for stopped icon"))
                fallback?.isTemplate = true
                button.image = fallback
