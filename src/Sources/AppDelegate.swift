...
class AppDelegate: NSObject, NSApplicationDelegate, NSWindowDelegate, UNUserNotificationCenterDelegate {
    ...
    @objc func updateMenuBarStatus() {
        ...
        if let serverStatus = menu.item(at: 0) {
            serverStatus.title = serverManager.isRunning ? String(format: NSLocalizedString("menubar.server-running", comment: "Menu bar status when server is running with port"), thinkingProxy.proxyPort) : String(localized: "menubar.server-stopped-status", defaultValue: "Server: Stopped", comment: "Menu bar status when server is stopped")
        }

        // Update button states
        if let startStopItem = menu.item(withTag: 100) {
            startStopItem.title = serverManager.isRunning ? String(localized: "menubar.stop-server", defaultValue: "Stop Server", comment: "Menu item to stop server") : String(localized: "menubar.start-server-item", defaultValue: "Start Server", comment: "Menu item to start server")
        }
