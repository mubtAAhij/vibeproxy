...
class AppDelegate: NSObject, NSApplicationDelegate, NSWindowDelegate, UNUserNotificationCenterDelegate {
    ...
    private func setupMainMenu() {
        ...
        appMenu.addItem(NSMenuItem(title: String(localized: "app.menu.quit-vibeproxy", defaultValue: "Quit VibeProxy", comment: "Application menu item title to quit app"), action: #selector(NSApplication.terminate(_:)), keyEquivalent: "q"))
        appMenuItem.submenu = appMenu
        mainMenu.addItem(appMenuItem)
        
        // Edit menu (for Cmd+C/V/X/A to work)
        let editMenuItem = NSMenuItem()
        let editMenu = NSMenu(title: String(localized: "app.menu.edit", defaultValue: "Edit", comment: "Main edit menu title"))
    ...
    func setupMenuBar() {
        ...
        // Server Status
        menu.addItem(NSMenuItem(title: String(localized: "app.status-menu.server-stopped", defaultValue: "Server: Stopped", comment: "Status menu title when server is stopped"), action: nil, keyEquivalent: ""))
        menu.addItem(NSMenuItem.separator())

        // Main Actions
        menu.addItem(NSMenuItem(title: String(localized: "app.status-menu.open-settings", defaultValue: "Open Settings", comment: "Status menu item title to open settings window"), action: #selector(openSettings), keyEquivalent: "s"))
        menu.addItem(NSMenuItem.separator())

        // Server Control
        let startStopItem = NSMenuItem(title: String(localized: "app.status-menu.start-server", defaultValue: "Start Server", comment: "Status menu item title to start backend server"), action: #selector(toggleServer), keyEquivalent: "")
        ...
        // Check for Updates
        let checkForUpdatesItem = NSMenuItem(title: String(localized: "app.status-menu.check-for-updates", defaultValue: "Check for Updates...", comment: "Status menu item title to check for application updates"), action: #selector(SPUStandardUpdaterController.checkForUpdates(_:)), keyEquivalent: "u")
        ...
        // Quit
        menu.addItem(NSMenuItem(title: String(localized: "app.status-menu.quit", defaultValue: "Quit", comment: "Status menu item title to quit the application"), action: #selector(quit), keyEquivalent: "q"))
    ...
    private func pollForProxyReadiness(attempts: Int, maxAttempts: Int, intervalMs: Int) {
            ...
            serverManager.start { [weak self] success in
                DispatchQueue.main.async {
                    if success {
                        ...
                        self?.showNotification(title: String(localized: "app.notification.server-started.title", defaultValue: "Server Started", comment: "Notification title shown when server starts successfully"), body: String(localized: "app.notification.server-started.message", defaultValue: "VibeProxy is now running", comment: "Notification message shown when server starts successfully"))
                    } else {
                        // Backend failed - stop the proxy to keep state consistent
                        self?.thinkingProxy.stop()
                        self?.showNotification(title: String(localized: "app.notification.server-failed.title", defaultValue: "Server Failed", comment: "Notification title shown when server fails to start"), body: String(localized: "app.notification.server-failed.message", defaultValue: "Could not start backend server on port 8318", comment: "Notification message shown when backend server fails to start on default port"))
                    }
                }
            }
            return
        ...
        if attempts >= maxAttempts {
            DispatchQueue.main.async { [weak self] in
                ...
                self?.thinkingProxy.stop()
                self?.showNotification(title: String(localized: "app.notification.server-failed.thinking-proxy.title", defaultValue: "Server Failed", comment: "Notification title shown when thinking proxy fails to start"), body: String(localized: "app.notification.server-failed.thinking-proxy.timeout-message", defaultValue: "Could not start thinking proxy on port 8317 (timeout)", comment: "Notification message shown when thinking proxy startup times out"))
            }
            return
        }
    ...
    @objc func copyServerURL() {
        ...
        showNotification(title: String(localized: "app.notification.clipboard.copied.title", defaultValue: "Copied", comment: "Notification title shown when clipboard copy succeeds"), body: String(localized: "app.notification.clipboard.server-url-copied.message", defaultValue: "Server URL copied to clipboard", comment: "Notification message shown when server URL is copied"))
    }
    ...
    @objc func updateMenuBarStatus() {
        ...
        if let serverStatus = menu.item(at: 0) {
            serverStatus.title = serverManager.isRunning ? String(format: String(localized: "app.status-menu.server-running-port", defaultValue: "Server: Running (port %d)", comment: "Status menu title when server is running with active port"), thinkingProxy.proxyPort) : String(localized: "app.status-menu.server-stopped", defaultValue: "Server: Stopped", comment: "Status menu title when server is stopped")
        }

        // Update button states
        if let startStopItem = menu.item(withTag: 100) {
            startStopItem.title = serverManager.isRunning ? String(localized: "app.status-menu.stop-server", defaultValue: "Stop Server", comment: "Status menu item title to stop backend server") : String(localized: "app.status-menu.start-server", defaultValue: "Start Server", comment: "Status menu item title to start backend server")
        }
