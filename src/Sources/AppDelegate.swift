...
class AppDelegate: NSObject, NSApplicationDelegate, NSWindowDelegate, UNUserNotificationCenterDelegate {
    ...
    func setupMenuBar() {
        ...
        // Server Status
        menu.addItem(NSMenuItem(title: String(localized: "menubar.server-stopped", defaultValue: "Server: Stopped", comment: "Menu bar server stopped status"), action: nil, keyEquivalent: ""))
    ...
    private func pollForProxyReadiness(attempts: Int, maxAttempts: Int, intervalMs: Int) {
            ...
            serverManager.start { [weak self] success in
                DispatchQueue.main.async {
                    if success {
                        ...
                        self?.showNotification(title: String(localized: "notification.server-started.title", defaultValue: "Server Started", comment: "Server started notification title"), body: String(localized: "notification.server-started.body", defaultValue: "VibeProxy is now running", comment: "Server started notification body"))
                    } else {
                        // Backend failed - stop the proxy to keep state consistent
                        self?.thinkingProxy.stop()
                        self?.showNotification(title: String(localized: "notification.server-failed.title", defaultValue: "Server Failed", comment: "Server failed notification title"), body: String(localized: "notification.server-failed.backend", defaultValue: "Could not start backend server on port 8318", comment: "Backend server start failure"))
                    }
                }
            }
            return
        ...
        if attempts >= maxAttempts {
            DispatchQueue.main.async { [weak self] in
                ...
                self?.thinkingProxy.stop()
                self?.showNotification(title: String(localized: "notification.server-failed.title", defaultValue: "Server Failed", comment: "Server failed notification title"), body: String(localized: "notification.server-failed.proxy", defaultValue: "Could not start thinking proxy on port 8317 (timeout)", comment: "Thinking proxy start failure"))
            }
            return
        }
    ...
    @objc func copyServerURL() {
        ...
        showNotification(title: String(localized: "notification.copied.title", defaultValue: "Copied", comment: "Copied notification title"), body: String(localized: "notification.copied.body", defaultValue: "Server URL copied to clipboard", comment: "Server URL copied notification"))
    }

    @objc func updateMenuBarStatus() {
        // Update status items
        if let serverStatus = menu.item(at: 0) {
            serverStatus.title = serverManager.isRunning ? String(format: NSLocalizedString("menubar.server-running", comment: "Menu bar server running status with port"), thinkingProxy.proxyPort) : String(localized: "menubar.server-stopped", defaultValue: "Server: Stopped", comment: "Menu bar server stopped status")
        }
        ...
        if let button = statusItem.button {
            ...
            if let icon = IconCatalog.shared.image(named: iconName, resizedTo: NSSize(width: 18, height: 18), template: true) {
                ...
                let fallback = NSImage(systemSymbolName: fallbackSymbol, accessibilityDescription: serverManager.isRunning ? String(localized: "menubar.icon.running", defaultValue: "Running", comment: "Running icon accessibility description") : String(localized: "menubar.icon.stopped", defaultValue: "Stopped", comment: "Stopped icon accessibility description"))
                fallback?.isTemplate = true
                button.image = fallback
