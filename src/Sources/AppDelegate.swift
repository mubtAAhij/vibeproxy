...
class AppDelegate: NSObject, NSApplicationDelegate, NSWindowDelegate, UNUserNotificationCenterDelegate {
    ...
    private func setupMainMenu() {
        ...
        appMenu.addItem(NSMenuItem(title: String(localized: "menu.quit-vibeproxy", defaultValue: "Quit VibeProxy", comment: "Quit menu item with app name"), action: #selector(NSApplication.terminate(_:)), keyEquivalent: "q"))
    ...
    func setupMenuBar() {
        ...
        // Copy URL
        let copyURLItem = NSMenuItem(title: String(localized: "menubar.copy-server-url", defaultValue: "Copy Server URL", comment: "Menu item to copy server URL to clipboard"), action: #selector(copyServerURL), keyEquivalent: "c")
        ...
        // Quit
        menu.addItem(NSMenuItem(title: String(localized: "menubar.quit", defaultValue: "Quit", comment: "Menu item to quit the app"), action: #selector(quit), keyEquivalent: "q"))
    ...
    private func pollForProxyReadiness(attempts: Int, maxAttempts: Int, intervalMs: Int) {
        ...
        if attempts >= maxAttempts {
            DispatchQueue.main.async { [weak self] in
                ...
                self?.thinkingProxy.stop()
                self?.showNotification(title: String(localized: "notification.server-failed.title", defaultValue: "Server Failed", comment: "Notification title when server fails to start"), body: String(localized: "notification.proxy-timeout.body", defaultValue: "Could not start thinking proxy on port 8317 (timeout)", comment: "Notification body when thinking proxy times out"))
            }
            return
        }
    ...
    @objc func copyServerURL() {
        ...
        showNotification(title: String(localized: "notification.copied.title", defaultValue: "Copied", comment: "Notification title when URL is copied"), body: String(localized: "notification.url-copied.body", defaultValue: "Server URL copied to clipboard", comment: "Notification body when server URL is copied to clipboard"))
    }
    ...
    @objc func updateMenuBarStatus() {
        ...
        if let button = statusItem.button {
            ...
            if let icon = IconCatalog.shared.image(named: iconName, resizedTo: NSSize(width: 18, height: 18), template: true) {
                ...
                let fallback = NSImage(systemSymbolName: fallbackSymbol, accessibilityDescription: serverManager.isRunning ? String(localized: "menubar.accessibility.running", defaultValue: "Running", comment: "Accessibility description when server is running") : String(localized: "menubar.accessibility.stopped", defaultValue: "Stopped", comment: "Accessibility description when server is stopped"))
                fallback?.isTemplate = true
                button.image = fallback
