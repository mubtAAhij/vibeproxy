...
class AppDelegate: NSObject, NSApplicationDelegate, NSWindowDelegate, UNUserNotificationCenterDelegate {
    ...
    private func setupMainMenu() {
        ...
        let appMenu = NSMenu()
        appMenu.addItem(NSMenuItem(title: String(localized: "menu.about", defaultValue: "About VibeProxy", comment: "Menu item to show About panel"), action: #selector(NSApplication.orderFrontStandardAboutPanel(_:)), keyEquivalent: ""))
        appMenu.addItem(NSMenuItem.separator())
        appMenu.addItem(NSMenuItem(title: String(localized: "menu.quit-app", defaultValue: "Quit VibeProxy", comment: "Menu item to quit application"), action: #selector(NSApplication.terminate(_:)), keyEquivalent: "q"))
        ...
        editMenu.addItem(NSMenuItem(title: String(localized: "menu.edit.undo", defaultValue: "Undo", comment: "Menu item to undo last action"), action: Selector(("undo:")), keyEquivalent: "z"))
        editMenu.addItem(NSMenuItem(title: String(localized: "menu.edit.redo", defaultValue: "Redo", comment: "Menu item to redo last undone action"), action: Selector(("redo:")), keyEquivalent: "Z"))
        editMenu.addItem(NSMenuItem.separator())
        editMenu.addItem(NSMenuItem(title: String(localized: "menu.edit.cut", defaultValue: "Cut", comment: "Menu item to cut selected text"), action: #selector(NSText.cut(_:)), keyEquivalent: "x"))
        editMenu.addItem(NSMenuItem(title: String(localized: "menu.edit.copy", defaultValue: "Copy", comment: "Menu item to copy selected text"), action: #selector(NSText.copy(_:)), keyEquivalent: "c"))
        editMenu.addItem(NSMenuItem(title: String(localized: "menu.edit.paste", defaultValue: "Paste", comment: "Menu item to paste from clipboard"), action: #selector(NSText.paste(_:)), keyEquivalent: "v"))
        editMenu.addItem(NSMenuItem(title: String(localized: "menu.edit.select-all", defaultValue: "Select All", comment: "Menu item to select all text"), action: #selector(NSText.selectAll(_:)), keyEquivalent: "a"))
    ...
    func setupMenuBar() {
        ...
        // Check for Updates
        let checkForUpdatesItem = NSMenuItem(title: String(localized: "menubar.check-updates", defaultValue: "Check for Updates...", comment: "Menu item to check for application updates"), action: #selector(SPUStandardUpdaterController.checkForUpdates(_:)), keyEquivalent: "u")
    ...
    private func pollForProxyReadiness(attempts: Int, maxAttempts: Int, intervalMs: Int) {
        ...
        if attempts >= maxAttempts {
            DispatchQueue.main.async { [weak self] in
                ...
                self?.thinkingProxy.stop()
                self?.showNotification(title: String(localized: "notification.server-failed.title", defaultValue: "Server Failed", comment: "Notification title when server fails to start"), body: String(localized: "notification.server-failed.proxy-timeout", defaultValue: "Could not start thinking proxy on port 8317 (timeout)", comment: "Notification message when thinking proxy fails to start due to timeout"))
            }
            return
        }
    ...
    @objc func copyServerURL() {
        ...
        showNotification(title: String(localized: "notification.copied.title", defaultValue: "Copied", comment: "Notification title when URL is copied"), body: String(localized: "notification.copied.message", defaultValue: "Server URL copied to clipboard", comment: "Notification message when server URL is copied to clipboard"))
    }

    @objc func updateMenuBarStatus() {
        // Update status items
        if let serverStatus = menu.item(at: 0) {
            serverStatus.title = serverManager.isRunning ? String(format: String(localized: "menubar.server-running", defaultValue: "Server: Running (port %d)", comment: "Menu bar status when server is running with port number"), thinkingProxy.proxyPort) : String(localized: "menubar.server-stopped", defaultValue: "Server: Stopped", comment: "Menu bar status when server is stopped")
        }
        ...
        if let button = statusItem.button {
            ...
            if let icon = IconCatalog.shared.image(named: iconName, resizedTo: NSSize(width: 18, height: 18), template: true) {
                ...
                let fallback = NSImage(systemSymbolName: fallbackSymbol, accessibilityDescription: serverManager.isRunning ? String(localized: "menubar.icon.running", defaultValue: "Running", comment: "Accessibility description for running server icon") : String(localized: "menubar.icon.stopped", defaultValue: "Stopped", comment: "Accessibility description for stopped server icon"))
                fallback?.isTemplate = true
                button.image = fallback
