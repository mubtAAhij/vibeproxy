...
class AppDelegate: NSObject, NSApplicationDelegate, NSWindowDelegate, UNUserNotificationCenterDelegate {
    ...
    private func setupMainMenu() {
        ...
        appMenu.addItem(NSMenuItem(title: String(localized: "menu.quit-vibeproxy", defaultValue: "Quit VibeProxy", comment: "Quit VibeProxy menu item"), action: #selector(NSApplication.terminate(_:)), keyEquivalent: "q"))
    ...
    func setupMenuBar() {
        ...
        // Main Actions
        menu.addItem(NSMenuItem(title: String(localized: "menu.open-settings", defaultValue: "Open Settings", comment: "Menu item to open settings window"), action: #selector(openSettings), keyEquivalent: "s"))
        menu.addItem(NSMenuItem.separator())

        // Server Control
        let startStopItem = NSMenuItem(title: String(localized: "menu.start-server", defaultValue: "Start Server", comment: "Menu item to start the server"), action: #selector(toggleServer), keyEquivalent: "")
        ...
        // Copy URL
        let copyURLItem = NSMenuItem(title: String(localized: "menu.copy-server-url", defaultValue: "Copy Server URL", comment: "Menu item to copy server URL to clipboard"), action: #selector(copyServerURL), keyEquivalent: "c")
        ...
        // Check for Updates
        let checkForUpdatesItem = NSMenuItem(title: String(localized: "menu.check-updates", defaultValue: "Check for Updates...", comment: "Menu item to check for app updates"), action: #selector(SPUStandardUpdaterController.checkForUpdates(_:)), keyEquivalent: "u")
        ...
        // Quit
        menu.addItem(NSMenuItem(title: String(localized: "menu.quit", defaultValue: "Quit", comment: "Menu item to quit the app"), action: #selector(quit), keyEquivalent: "q"))
    ...
    @objc func updateMenuBarStatus() {
        ...
        if let serverStatus = menu.item(at: 0) {
            serverStatus.title = serverManager.isRunning ? String(format: String(localized: "menu.server-running-port", defaultValue: "Server: Running (port %d)", comment: "Menu bar status when server is running with port number"), thinkingProxy.proxyPort) : String(localized: "menu.server-stopped", defaultValue: "Server: Stopped", comment: "Menu bar status when server is stopped")
        }

        // Update button states
        if let startStopItem = menu.item(withTag: 100) {
            startStopItem.title = serverManager.isRunning ? String(localized: "menu.stop-server", defaultValue: "Stop Server", comment: "Menu item to stop the server") : String(localized: "menu.start-server", defaultValue: "Start Server", comment: "Menu item to start the server")
        }
        ...
        if let button = statusItem.button {
            ...
            if let icon = IconCatalog.shared.image(named: iconName, resizedTo: NSSize(width: 18, height: 18), template: true) {
                ...
                let fallback = NSImage(systemSymbolName: fallbackSymbol, accessibilityDescription: serverManager.isRunning ? String(localized: "accessibility.running", defaultValue: "Running", comment: "Accessibility description when server is running") : String(localized: "accessibility.stopped", defaultValue: "Stopped", comment: "Accessibility description when server is stopped"))
                fallback?.isTemplate = true
                button.image = fallback
