...
class AppDelegate: NSObject, NSApplicationDelegate, NSWindowDelegate, UNUserNotificationCenterDelegate {
    ...
    private func setupMainMenu() {
        ...
        appMenu.addItem(NSMenuItem(title: String(localized: "menu.quit-app", defaultValue: "Quit VibeProxy", comment: "Quit menu item in app menu"), action: #selector(NSApplication.terminate(_:)), keyEquivalent: "q"))
    ...
    func setupMenuBar() {
        ...
        // Main Actions
        menu.addItem(NSMenuItem(title: String(localized: "menubar.open-settings", defaultValue: "Open Settings", comment: "Menu bar item to open settings"), action: #selector(openSettings), keyEquivalent: "s"))
        menu.addItem(NSMenuItem.separator())
        ...
        // Check for Updates
        let checkForUpdatesItem = NSMenuItem(title: String(localized: "menubar.check-for-updates", defaultValue: "Check for Updates...", comment: "Menu bar item to check for updates"), action: #selector(SPUStandardUpdaterController.checkForUpdates(_:)), keyEquivalent: "u")
        ...
        // Quit
        menu.addItem(NSMenuItem(title: String(localized: "menubar.quit", defaultValue: "Quit", comment: "Menu bar item to quit the app"), action: #selector(quit), keyEquivalent: "q"))
    ...
    private func pollForProxyReadiness(attempts: Int, maxAttempts: Int, intervalMs: Int) {
        ...
        if attempts >= maxAttempts {
            DispatchQueue.main.async { [weak self] in
                ...
                self?.thinkingProxy.stop()
                self?.showNotification(title: String(localized: "notification.server-failed.title", defaultValue: "Server Failed", comment: "Notification title when server fails to start"), body: String(localized: "notification.server-failed-proxy-timeout.body", defaultValue: "Could not start thinking proxy on port 8317 (timeout)", comment: "Notification body when thinking proxy fails to start due to timeout"))
            }
            return
        }
