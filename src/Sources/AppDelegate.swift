...
class AppDelegate: NSObject, NSApplicationDelegate, NSWindowDelegate, UNUserNotificationCenterDelegate {
    ...
    func setupMenuBar() {
        ...
        // Server Status
        menu.addItem(NSMenuItem(title: String(localized: "menubar.server-stopped", defaultValue: "Server: Stopped", comment: "Server stopped status in menu bar"), action: nil, keyEquivalent: ""))
    ...
    private func pollForProxyReadiness(attempts: Int, maxAttempts: Int, intervalMs: Int) {
        ...
        if attempts >= maxAttempts {
            DispatchQueue.main.async { [weak self] in
                ...
                self?.thinkingProxy.stop()
                self?.showNotification(title: String(localized: "notification.server-failed.title", defaultValue: "Server Failed", comment: "Notification title when server fails"), body: String(localized: "notification.server-failed.proxy-body", defaultValue: "Could not start thinking proxy on port 8317 (timeout)", comment: "Notification body when proxy fails to start"))
            }
            return
        }
    ...
    @objc func updateMenuBarStatus() {
        ...
        if let serverStatus = menu.item(at: 0) {
            serverStatus.title = serverManager.isRunning ? String(format: NSLocalizedString("menubar.server-running", comment: "Server running status with port"), thinkingProxy.proxyPort) : String(localized: "menubar.server-stopped-status", defaultValue: "Server: Stopped", comment: "Server stopped status")
        }

        // Update button states
        if let startStopItem = menu.item(withTag: 100) {
            startStopItem.title = serverManager.isRunning ? String(localized: "menubar.stop-server", defaultValue: "Stop Server", comment: "Stop server menu item") : String(localized: "menubar.start-server-action", defaultValue: "Start Server", comment: "Start server action")
        }
        ...
        if let button = statusItem.button {
            ...
            if let icon = IconCatalog.shared.image(named: iconName, resizedTo: NSSize(width: 18, height: 18), template: true) {
                ...
                let runningDesc = String(localized: "menubar.icon.running", defaultValue: "Running", comment: "Accessibility description for running icon")
                let stoppedDesc = String(localized: "menubar.icon.stopped", defaultValue: "Stopped", comment: "Accessibility description for stopped icon")
