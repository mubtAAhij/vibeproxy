...
class AppDelegate: NSObject, NSApplicationDelegate, NSWindowDelegate, UNUserNotificationCenterDelegate {
    ...
    private func setupMainMenu() {
        ...
        let editMenu = NSMenu(title: String(localized: "menu.edit.title", defaultValue: "Edit", comment: "Main Edit menu title"))
        editMenu.addItem(NSMenuItem(title: String(localized: "menu.edit.undo", defaultValue: "Undo", comment: "Edit menu item for Undo action"), action: Selector(("undo:")), keyEquivalent: "z"))
        editMenu.addItem(NSMenuItem(title: String(localized: "menu.edit.redo", defaultValue: "Redo", comment: "Edit menu item for Redo action"), action: Selector(("redo:")), keyEquivalent: "Z"))
        editMenu.addItem(NSMenuItem.separator())
        editMenu.addItem(NSMenuItem(title: String(localized: "menu.edit.cut", defaultValue: "Cut", comment: "Edit menu item for Cut action"), action: #selector(NSText.cut(_:)), keyEquivalent: "x"))
        editMenu.addItem(NSMenuItem(title: String(localized: "menu.edit.copy", defaultValue: "Copy", comment: "Edit menu item for Copy action"), action: #selector(NSText.copy(_:)), keyEquivalent: "c"))
        editMenu.addItem(NSMenuItem(title: String(localized: "menu.edit.paste", defaultValue: "Paste", comment: "Edit menu item for Paste action"), action: #selector(NSText.paste(_:)), keyEquivalent: "v"))
        editMenu.addItem(NSMenuItem(title: String(localized: "menu.edit.select-all", defaultValue: "Select All", comment: "Edit menu item for Select All action"), action: #selector(NSText.selectAll(_:)), keyEquivalent: "a"))
    ...
    func setupMenuBar() {
        ...
        // Server Status
        menu.addItem(NSMenuItem(title: String(localized: "status-menu.server.stopped", defaultValue: "Server: Stopped", comment: "Status menu label when server is stopped"), action: nil, keyEquivalent: ""))
        menu.addItem(NSMenuItem.separator())

        // Main Actions
        menu.addItem(NSMenuItem(title: String(localized: "status-menu.open-settings", defaultValue: "Open Settings", comment: "Status menu item to open settings window"), action: #selector(openSettings), keyEquivalent: "s"))
        menu.addItem(NSMenuItem.separator())

        // Server Control
        let startStopItem = NSMenuItem(title: String(localized: "status-menu.start-server", defaultValue: "Start Server", comment: "Status menu item to start backend server"), action: #selector(toggleServer), keyEquivalent: "")
        ...
        // Copy URL
        let copyURLItem = NSMenuItem(title: String(localized: "status-menu.copy-server-url", defaultValue: "Copy Server URL", comment: "Status menu item to copy server URL"), action: #selector(copyServerURL), keyEquivalent: "c")
        copyURLItem.isEnabled = false
        copyURLItem.tag = 102
        menu.addItem(copyURLItem)

        // Open Dashboard
        let dashboardItem = NSMenuItem(title: String(localized: "status-menu.open-dashboard", defaultValue: "Open Dashboard", comment: "Status menu item to open dashboard"), action: #selector(openDashboard), keyEquivalent: "d")
        ...
        // Check for Updates
        let checkForUpdatesItem = NSMenuItem(title: String(localized: "status-menu.check-for-updates", defaultValue: "Check for Updates...", comment: "Status menu item to check for updates"), action: #selector(SPUStandardUpdaterController.checkForUpdates(_:)), keyEquivalent: "u")
        ...
        // Quit
        menu.addItem(NSMenuItem(title: String(localized: "status-menu.quit", defaultValue: "Quit", comment: "Status menu item to quit application"), action: #selector(quit), keyEquivalent: "q"))
    ...
    private func pollForProxyReadiness(attempts: Int, maxAttempts: Int, intervalMs: Int) {
        ...
        if attempts >= maxAttempts {
            DispatchQueue.main.async { [weak self] in
                ...
                self?.thinkingProxy.stop()
                self?.showNotification(title: String(localized: "notification.server-failed.title", defaultValue: "Server Failed", comment: "Notification title shown when server startup fails"), body: String(localized: "notification.server-failed.thinking-proxy-timeout-body", defaultValue: "Could not start thinking proxy on port 8317 (timeout)", comment: "Notification body shown when thinking proxy times out on startup"))
            }
            return
        }
    ...
    @objc func copyServerURL() {
        ...
        showNotification(title: String(localized: "notification.clipboard.copied.title", defaultValue: "Copied", comment: "Notification title shown when content is copied to clipboard"), body: String(localized: "notification.clipboard.server-url-copied.body", defaultValue: "Server URL copied to clipboard", comment: "Notification body shown when server URL is copied"))
    }
    ...
    @objc func updateMenuBarStatus() {
        ...
        if let serverStatus = menu.item(at: 0) {
            serverStatus.title = serverManager.isRunning ? String(format: String(localized: "status-menu.server.running-port", defaultValue: "Server: Running (port %d)", comment: "Status menu label when server is running on a port"), thinkingProxy.proxyPort) : String(localized: "status-menu.server.stopped", defaultValue: "Server: Stopped", comment: "Status menu label when server is stopped")
        }

        // Update button states
        if let startStopItem = menu.item(withTag: 100) {
            startStopItem.title = serverManager.isRunning ? String(localized: "status-menu.stop-server", defaultValue: "Stop Server", comment: "Status menu item to stop backend server") : String(localized: "status-menu.start-server", defaultValue: "Start Server", comment: "Status menu item to start backend server")
        }
