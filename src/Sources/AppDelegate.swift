...
class AppDelegate: NSObject, NSApplicationDelegate, NSWindowDelegate, UNUserNotificationCenterDelegate {
    ...
    func setupMenuBar() {
        ...
        // Server Status
        menu.addItem(NSMenuItem(title: String(localized: "menubar.server-stopped", defaultValue: "Server: Stopped", comment: "Menu item showing server is stopped"), action: nil, keyEquivalent: ""))
    ...
    @objc func copyServerURL() {
        ...
        showNotification(title: String(localized: "notification.copied.title", defaultValue: "Copied", comment: "Notification title when URL is copied"), body: String(localized: "notification.copied.body", defaultValue: "Server URL copied to clipboard", comment: "Notification body when server URL is copied to clipboard"))
    }

    @objc func updateMenuBarStatus() {
        // Update status items
        if let serverStatus = menu.item(at: 0) {
            serverStatus.title = serverManager.isRunning ? String(format: NSLocalizedString("menubar.server-running", comment: "Menu item showing server is running with port number"), thinkingProxy.proxyPort) : String(localized: "menubar.server-stopped-status", defaultValue: "Server: Stopped", comment: "Menu item showing server is stopped in status area")
        }

        // Update button states
        if let startStopItem = menu.item(withTag: 100) {
            startStopItem.title = serverManager.isRunning ? String(localized: "menubar.stop-server", defaultValue: "Stop Server", comment: "Menu item to stop the server") : String(localized: "menubar.start-server-action", defaultValue: "Start Server", comment: "Menu item to start the server in action area")
        }
        ...
        if let button = statusItem.button {
            ...
            if let icon = IconCatalog.shared.image(named: iconName, resizedTo: NSSize(width: 18, height: 18), template: true) {
                ...
                let fallback = NSImage(systemSymbolName: fallbackSymbol, accessibilityDescription: serverManager.isRunning ? String(localized: "menubar.icon.running", defaultValue: "Running", comment: "Accessibility description for running status icon") : String(localized: "menubar.icon.stopped", defaultValue: "Stopped", comment: "Accessibility description for stopped status icon"))
                fallback?.isTemplate = true
                button.image = fallback
