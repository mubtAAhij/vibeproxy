...
class AppDelegate: NSObject, NSApplicationDelegate, NSWindowDelegate, UNUserNotificationCenterDelegate {
    ...
    private func setupMainMenu() {
        ...
        appMenu.addItem(NSMenuItem(title: String(localized: "menu.quit-full", defaultValue: "Quit VibeProxy", comment: "Quit menu item with app name"), action: #selector(NSApplication.terminate(_:)), keyEquivalent: "q"))
    ...
    func setupMenuBar() {
        ...
        // Copy URL
        let copyURLItem = NSMenuItem(title: String(localized: "menubar.copy-url", defaultValue: "Copy Server URL", comment: "Menu bar copy server URL item"), action: #selector(copyServerURL), keyEquivalent: "c")
        ...
        // Check for Updates
        let checkForUpdatesItem = NSMenuItem(title: String(localized: "menubar.check-updates", defaultValue: "Check for Updates...", comment: "Menu bar check for updates item"), action: #selector(SPUStandardUpdaterController.checkForUpdates(_:)), keyEquivalent: "u")
    ...
    @objc func updateMenuBarStatus() {
        ...
        if let serverStatus = menu.item(at: 0) {
            serverStatus.title = serverManager.isRunning ? String(format: String(localized: "menubar.server-running", defaultValue: "Server: Running (port %d)", comment: "Menu bar server running status with port"), thinkingProxy.proxyPort) : String(localized: "menubar.server-stopped", defaultValue: "Server: Stopped", comment: "Menu bar server stopped status")
        }
