...
class AppDelegate: NSObject, NSApplicationDelegate, NSWindowDelegate, UNUserNotificationCenterDelegate {
    ...
    private func pollForProxyReadiness(attempts: Int, maxAttempts: Int, intervalMs: Int) {
            ...
            serverManager.start { [weak self] success in
                DispatchQueue.main.async {
                    if success {
                        ...
                        self?.showNotification(title: String(localized: "notification.server-failed.title", defaultValue: "Server Failed", comment: "Server failed notification title"), body: String(localized: "notification.server-failed.backend", defaultValue: "Could not start backend server on port 8318", comment: "Backend server failed to start"))
                    }
                }
            }
            return
        ...
        if attempts >= maxAttempts {
            DispatchQueue.main.async { [weak self] in
                ...
                self?.thinkingProxy.stop()
                self?.showNotification(title: String(localized: "notification.server-failed.title", defaultValue: "Server Failed", comment: "Server failed notification title"), body: String(localized: "notification.server-failed.proxy-timeout", defaultValue: "Could not start thinking proxy on port 8317 (timeout)", comment: "Thinking proxy failed to start (timeout)"))
            }
            return
        }
    ...
    @objc func copyServerURL() {
        ...
        showNotification(title: String(localized: "notification.copied.title", defaultValue: "Copied", comment: "Notification title when URL copied"), body: String(localized: "notification.copied.body", defaultValue: "Server URL copied to clipboard", comment: "Notification body when URL copied"))
    }

    @objc func updateMenuBarStatus() {
        // Update status items
        if let serverStatus = menu.item(at: 0) {
            serverStatus.title = serverManager.isRunning ? String(format: String(localized: "menubar.server-running", defaultValue: "Server: Running (port %d)", comment: "Server running status with port"), thinkingProxy.proxyPort) : String(localized: "menubar.server-stopped", defaultValue: "Server: Stopped", comment: "Server stopped status")
        }
