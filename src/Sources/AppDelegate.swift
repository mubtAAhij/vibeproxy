...
class AppDelegate: NSObject, NSApplicationDelegate, NSWindowDelegate, UNUserNotificationCenterDelegate {
    ...
    private func setupMainMenu() {
        ...
        appMenu.addItem(NSMenuItem(title: String(localized: "menu.quit-app", defaultValue: "Quit VibeProxy", comment: "Quit menu item"), action: #selector(NSApplication.terminate(_:)), keyEquivalent: "q"))
    ...
    func setupMenuBar() {
        ...
        // Copy URL
        let copyURLItem = NSMenuItem(title: String(localized: "menubar.copy-server-url", defaultValue: "Copy Server URL", comment: "Menu item to copy server URL"), action: #selector(copyServerURL), keyEquivalent: "c")
    ...
    private func pollForProxyReadiness(attempts: Int, maxAttempts: Int, intervalMs: Int) {
        ...
        if attempts >= maxAttempts {
            DispatchQueue.main.async { [weak self] in
                ...
                self?.thinkingProxy.stop()
                self?.showNotification(title: String(localized: "notification.server-failed.title", defaultValue: "Server Failed", comment: "Notification title when server fails"), body: String(localized: "notification.server-failed.proxy-timeout-body", defaultValue: "Could not start thinking proxy on port 8317 (timeout)", comment: "Notification body when thinking proxy times out"))
            }
            return
        }
    ...
    @objc func updateMenuBarStatus() {
        ...
        if let button = statusItem.button {
            ...
            if let icon = IconCatalog.shared.image(named: iconName, resizedTo: NSSize(width: 18, height: 18), template: true) {
                ...
                let fallback = NSImage(systemSymbolName: fallbackSymbol, accessibilityDescription: serverManager.isRunning ? String(localized: "menubar.accessibility.running", defaultValue: "Running", comment: "Accessibility description when running") : String(localized: "menubar.accessibility.stopped", defaultValue: "Stopped", comment: "Accessibility description when stopped"))
                fallback?.isTemplate = true
                button.image = fallback
