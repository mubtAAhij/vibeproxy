    ...
    private func pollForProxyReadiness(attempts: Int, maxAttempts: Int, intervalMs: Int) {
        ...
        if attempts >= maxAttempts {
            DispatchQueue.main.async { [weak self] in
                ...
                self?.thinkingProxy.stop()
                self?.showNotification(title: String(localized: "notification.server-failed-title", defaultValue: "Server Failed", comment: "Notification title when server fails to start"), body: String(localized: "notification.proxy-failed", defaultValue: "Could not start thinking proxy on port 8317 (timeout)", comment: "Notification body when thinking proxy fails"))
            }
            return
        }
