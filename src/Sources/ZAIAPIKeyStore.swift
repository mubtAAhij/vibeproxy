            ...
            do {
                ...
                throw ZAIAPIKeyStoreError.failedToCreateDirectory(
                    String(format: String(
                        ...
                        comment: "Error when creating auth directory for API key storage"
            ...
            do {
                ...
                throw ZAIAPIKeyStoreError.failedToSerializeKey(
                    String(format: String(
                        ...
                        comment: "Error when serializing Z.AI API key JSON payload"
                    ...
                    issues.append(
                        ZAIAPIKeyLoadIssue(
                            ...
                            message: String(format: String(
                                ...
                                comment: "Unexpected error while loading API key file"
    ...
    private func loadActiveAPIKey(at filePath: URL) throws -> String? {
        ...
        do {
            ...
            throw ZAIAPIKeyStoreError.invalidKeyJSON(
                String(format: String(
                    localized: "zai-api-key-store.error.invalid-json-in-api-key-file",
                    defaultValue: "Z.AI API key file at %@ contains invalid JSON: %@",
                    comment: "Error when API key file JSON is malformed"
                ), "\(filePath.path)", "\(error.localizedDescription)")
            )
        }
        ...
        guard let json = ConfigComposer.stringKeyedDictionary(jsonObject) else {
            throw ZAIAPIKeyStoreError.malformedKey(
                String(format: String(
                    localized: "zai-api-key-store.error-api-key-file-must-be-json-object",
                    defaultValue: "Z.AI API key file at %@ must contain a JSON object.",
                    comment: "Error when API key file root JSON type is not an object"
                ), "\(filePath.path)")
            )
        }
        guard (json["type"] as? String) == Self.authType else {
            throw ZAIAPIKeyStoreError.malformedKey(
                String(format: String(
                    localized: "zai-api-key-store.error.unexpected-api-key-file-type",
                    defaultValue: "Z.AI API key file at %@ has an unexpected type.",
                    comment: "Error when parsed API key value has an unexpected type"
                ), "\(filePath.path)")
            )
        }
        guard let apiKey = json["api_key"] as? String, !apiKey.isEmpty else {
            throw ZAIAPIKeyStoreError.malformedKey(
                String(format: String(
                    localized: "zai-api-key-store.error.missing-api-key-field",
                    defaultValue: "Z.AI API key file at %@ is missing an api_key.",
                    comment: "Error when api_key field is missing from API key file"
                ), "\(filePath.path)")
            )
        }
