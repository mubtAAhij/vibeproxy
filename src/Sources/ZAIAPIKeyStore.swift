        ...
        try queue.sync {
            do {
                ...
                throw ZAIAPIKeyStoreError.failedToCreateDirectory(
                    String(format: String(
                        localized: "zai-api-key-store.errors.create-auth-directory-failed",
                        defaultValue: "Failed to create auth directory at %@: %@",
                        comment: "Error when creating authentication directory fails"
            ...
            do {
                ...
                throw ZAIAPIKeyStoreError.failedToSerializeKey(
                    String(format: String(
                        localized: "zai-api-key-store.errors.serialize-api-key-failed",
                        defaultValue: "Failed to serialize Z.AI API key: %@",
                        comment: "Error when serializing stored Z.AI API key fails"
            ...
            do {
                ...
                throw ZAIAPIKeyStoreError.failedToWriteKey(
                    String(format: String(
                        localized: "zai-api-key-store.errors.write-api-key-file-failed",
                        defaultValue: "Failed to write Z.AI API key file at %@: %@",
                        comment: "Error when writing Z.AI API key file fails"
                    ...
                    issues.append(
                        ZAIAPIKeyLoadIssue(
                            ...
                            message: String(format: String(
                                localized: "zai-api-key-store.errors.unexpected-load-error",
                                defaultValue: "Unexpected error while loading %@: %@",
                                comment: "Unexpected error while loading API key data from disk"
    ...
    private func loadActiveAPIKey(at filePath: URL) throws -> String? {
        ...
        do {
            ...
            throw ZAIAPIKeyStoreError.failedToReadKey(
                String(format: String(
                    localized: "zai-api-key-store.errors.read-api-key-file-failed",
                    defaultValue: "Failed to read Z.AI API key file at %@: %@",
                    comment: "Error when reading Z.AI API key file fails"
                ), "\(filePath.path)", "\(error.localizedDescription)")
            )
        }
        ...
        do {
            ...
            throw ZAIAPIKeyStoreError.invalidKeyJSON(
                String(format: String(
                    localized: "zai-api-key-store.errors.invalid-json-in-api-key-file",
                    defaultValue: "Z.AI API key file at %@ contains invalid JSON: %@",
                    comment: "Error when API key file content is not valid JSON"
                ), "\(filePath.path)", "\(error.localizedDescription)")
            )
        }
        ...
        guard let json = ConfigComposer.stringKeyedDictionary(jsonObject) else {
            throw ZAIAPIKeyStoreError.malformedKey(
                String(format: String(
                    localized: "zai-api-key-store.errors.api-key-file-must-be-json-object",
                    defaultValue: "Z.AI API key file at %@ must contain a JSON object.",
                    comment: "Validation error when API key file JSON root is not an object"
                ), "\(filePath.path)")
            )
        }
        guard (json["type"] as? String) == Self.authType else {
            throw ZAIAPIKeyStoreError.malformedKey(
                String(format: String(
                    localized: "zai-api-key-store.errors.api-key-file-unexpected-type",
                    defaultValue: "Z.AI API key file at %@ has an unexpected type.",
                    comment: "Validation error when api_key value has unexpected type"
                ), "\(filePath.path)")
            )
        }
        guard let apiKey = json["api_key"] as? String, !apiKey.isEmpty else {
            throw ZAIAPIKeyStoreError.malformedKey(
                String(format: String(
                    localized: "zai-api-key-store.errors.api-key-file-missing-api-key",
                    defaultValue: "Z.AI API key file at %@ is missing an api_key.",
                    comment: "Validation error when api_key field is missing in API key file"
                ), "\(filePath.path)")
            )
        }
